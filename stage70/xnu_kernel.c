...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage70.gicDistributorBase, PE_state_stage70.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage70_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage70 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage70_loader_preflight_run(args)) {
