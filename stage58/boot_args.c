...
void build_boot_args(struct boot_args *args, void *dt, uint32_t dt_len)
{
    ...
    args->virtBase = 0;                  /* Stage58 boot args are still identity-based before MMU handoff. */
    args->physBase = STAGE58_BASE;
    args->memSize = RAM_CONSOLE_BASE - RAM_PHYS_BASE;
    args->topOfKernelData = (uint32_t)(uintptr_t)__stage58_image_end;

    args->machineType = MACHINE_TYPE_MSM8974;
    args->deviceTreeP = dt;
    args->deviceTreeLength = dt_len;

    const char cmd[] = "debug=0x144 serial=0x1 mi4ios6.stage=58 msm8974=cancro xnu-obj-subset xnu-bootstrap-contract xnu-pmap-bootstrap-contract pmap-table-dryrun pmap-ref-only xnu-link-proof no-xnu-build no-xnu-exec no-persist-write";
