...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage58.gicDistributorBase, PE_state_stage58.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage58_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage58 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage58_loader_preflight_run(args)) {
