...
mkdir -p $REPO_ROOT/out/stage85

./xnu_workspace_validate.sh $REPO_ROOT/out/stage85
./xnu_compile_graph_scan.py $REPO_ROOT/out/stage85
./xnu_object_subset_compile.sh $REPO_ROOT/out/stage85
./xnu_link_proof.sh $REPO_ROOT/out/stage85
...
"$PYTHON" "$MACHO_FIXTURE_GEN" \
  ...
  --bin-output $REPO_ROOT/out/stage85/stage85_fixture.macho \
  --symbol-prefix stage85 \
  --metadata-file $REPO_ROOT/out/stage85/xnu-link-macho-metadata.txt
...
LDFLAGS=(
  ...
  -Wl,-T,linker.ld
  -Wl,--build-id=none
  -Wl,-Map,$REPO_ROOT/out/stage85/stage85.map
)
...
for src in "${SOURCES[@]}"; do
  obj="$REPO_ROOT/out/stage85/${src%.*}.o"
  OBJECTS+=("$obj")
  $CC "${CFLAGS[@]}" -c "$src" -o "$obj"
done

$CC "${CFLAGS[@]}" "${LDFLAGS[@]}" "${OBJECTS[@]}" -o $REPO_ROOT/out/stage85/stage85.elf
$OBJCOPY -O binary $REPO_ROOT/out/stage85/stage85.elf $REPO_ROOT/out/stage85/stage85.bin
$OBJDUMP -d $REPO_ROOT/out/stage85/stage85.elf > $REPO_ROOT/out/stage85/stage85.disasm
$NM -n $REPO_ROOT/out/stage85/stage85.elf > $REPO_ROOT/out/stage85/stage85.symbols
$SIZE $REPO_ROOT/out/stage85/stage85.elf > $REPO_ROOT/out/stage85/stage85.size

: > $REPO_ROOT/out/stage85/empty-ramdisk

$MKBOOTIMG \
  --kernel $REPO_ROOT/out/stage85/stage85.bin \
  --ramdisk $REPO_ROOT/out/stage85/empty-ramdisk \
  --base 0x00000000 \
...
if [[ -f "$QCDT_DT" ]]; then
  "$QCDT_PACKER" \
    ...
    --pagesize 2048 \
    --cmdline 'stage85 mi4ios6=stage85 c-runtime apple-dt macho-fixture load-plan materialize highva-dryrun safe-table stage-owned-tables ttbr0-roundtrip recovery-table cache-bits-preserved xnu-entry-stub xnu-early-init early-pmap-platform xnu-pe-init-false xnu-postpe cpu-topo bootcpu rtclock xnu-armvm live-pmap ttbr-live tlb-live pmap-restore prevm-pexpert dtinit-facts peid-machine xnu-bs-contract xnu-pmap-bs-contract xnu-pmap-table-contract xnu-pmap-page-contract xnu-pmap-attr-contract xnu-pmap-mw-contract xnu-pmap-trans-contract xnu-pexpert-hook xnu-iokit-platform xnu-iokit-match-contract xnu-iokit-regsvc-contract xnu-iokit-provider xnu-iokit-catalog xnu-iokit-propinh xnu-iokit-regtop pexpert-hook-ready iokit-platform-scaffold iokit-match-local iokit-regsvc-local iokit-provider-local iokit-catalog-local iokit-propinh-local iokit-regtop-local irq-timer-hook pmap-bootstrap-ref public-xnu-workspace public-xnu-compile-graph public-xnu-platform-graph public-xnu-object-subset xnu-controlled-link xnu-bounded-pe-gen xnu-arm-pe-bootargs xnu-arm-consistent-debug stage85-xnu-link-proof inert-macho-fixture cancro-target arm-init-stub no-full-xnu-build no-pub-start no-pub-arm-init no-pub-thread no-pub-cpuboot no-pub-rtclock no-pub-pmap no-pub-pexpert no-pub-peinit no-pub-dtinit no-pub-peid no-pub-armvm no-iokit-runtime-exec no-macho-exec no-cache-change no-persist-write no-external-mutation' \
    --output $REPO_ROOT/out/stage85/stage85-qcdt.img
else
  echo "warning: QCDT_DT not found: $QCDT_DT; skipping stage85-qcdt.img" >&2
fi

sha256sum $REPO_ROOT/out/stage85/stage85_fixture.macho $REPO_ROOT/out/stage85/stage85.elf $REPO_ROOT/out/stage85/stage85.bin $REPO_ROOT/out/stage85/stage85.img \
  $(if [[ -f $REPO_ROOT/out/stage85/stage85-qcdt.img ]]; then printf '%s' $REPO_ROOT/out/stage85/stage85-qcdt.img; fi) \
  > $REPO_ROOT/out/stage85/SHA256SUMS.txt

ls -l $REPO_ROOT/out/stage85/stage85.elf $REPO_ROOT/out/stage85/stage85.bin $REPO_ROOT/out/stage85/stage85.img \
  $(if [[ -f $REPO_ROOT/out/stage85/stage85-qcdt.img ]]; then printf '%s' $REPO_ROOT/out/stage85/stage85-qcdt.img; fi)
cat $REPO_ROOT/out/stage85/stage85.size
cat $REPO_ROOT/out/stage85/SHA256SUMS.txt
