...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage86.gicDistributorBase, PE_state_stage86.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage86_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage84 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage86_loader_preflight_run(args)) {
