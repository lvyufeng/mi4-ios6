...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage79.gicDistributorBase, PE_state_stage79.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage79_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage79 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage79_loader_preflight_run(args)) {
