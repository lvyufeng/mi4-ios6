...
struct gic_state_snapshot GIC_state_stage69;
volatile uint32_t stage69_irq_count;
volatile uint32_t stage69_last_iar;
volatile uint32_t stage69_last_irq_id;
volatile uint32_t stage69_sgi0_count;
volatile uint32_t stage69_spurious_irq_count;
volatile uint32_t stage69_timer_irq_count;
volatile uint32_t stage69_last_timer_irq_id;
volatile uint32_t stage69_last_timer_ctl;
volatile uint32_t stage69_other_irq_count;
volatile uint32_t stage69_sgi_selftest_passed;
volatile uint32_t stage69_sgi_irq_count_observed;
volatile uint32_t stage69_sgi_sgi0_count_observed;
volatile uint32_t stage69_sgi_last_irq_id_observed;
volatile uint32_t stage69_timer_selftest_passed;
volatile uint32_t stage69_timer_irq_count_observed;
volatile uint32_t stage69_timer_timer_count_observed;
volatile uint32_t stage69_timer_last_irq_id_observed;
volatile uint32_t stage69_timer_last_ctl_observed;
...
{
    memset(&GIC_state_stage69, 0, sizeof(GIC_state_stage69));
    GIC_state_stage69.distBase = dist_base;
    GIC_state_stage69.cpuBase = cpu_base;

    xnu_log_puts("gic snapshot begin (read-only)\n");

    xnu_log_puts("gic pre read GICD_CTLR\n");
    GIC_state_stage69.distCtlr = mmio_read32(dist_base + GICD_CTLR);
    xnu_log_puts("gic pre read GICD_TYPER\n");
    GIC_state_stage69.distTyper = mmio_read32(dist_base + GICD_TYPER);
    xnu_log_puts("gic pre read GICD_IIDR\n");
    GIC_state_stage69.distIidr = mmio_read32(dist_base + GICD_IIDR);

    GIC_state_stage69.irqCount = ((GIC_state_stage69.distTyper & 0x1fu) + 1u) * 32u;
    GIC_state_stage69.cpuInterfaceCount = ((GIC_state_stage69.distTyper >> 5) & 0x7u) + 1u;

    xnu_log_puts("gic pre read GICD_ISENABLER0\n");
    GIC_state_stage69.isenabler0 = mmio_read32(dist_base + GICD_ISENABLER0);
    xnu_log_puts("gic pre read GICD_ISPENDR0\n");
    GIC_state_stage69.ispendr0 = mmio_read32(dist_base + GICD_ISPENDR0);
    xnu_log_puts("gic pre read GICD_IPRIORITY0\n");
    GIC_state_stage69.priority0 = mmio_read32(dist_base + GICD_IPRIORITY0);
    xnu_log_puts("gic pre read GICD_ITARGETS0\n");
    GIC_state_stage69.targets0 = mmio_read32(dist_base + GICD_ITARGETS0);

    xnu_log_puts("gic pre read GICC_CTLR\n");
    GIC_state_stage69.cpuCtlr = mmio_read32(cpu_base + GICC_CTLR);
    xnu_log_puts("gic pre read GICC_PMR\n");
    GIC_state_stage69.cpuPmr = mmio_read32(cpu_base + GICC_PMR);
    xnu_log_puts("gic pre read GICC_BPR\n");
    GIC_state_stage69.cpuBpr = mmio_read32(cpu_base + GICC_BPR);
    xnu_log_puts("gic pre read GICC_IIDR\n");
    GIC_state_stage69.cpuIidr = mmio_read32(cpu_base + GICC_IIDR);
...
void gic_log_snapshot(void)
{
    ...
    xnu_log_kv32("gic_distBase", GIC_state_stage69.distBase);
    xnu_log_kv32("gic_cpuBase", GIC_state_stage69.cpuBase);
    xnu_log_kv32("gic_distCtlr", GIC_state_stage69.distCtlr);
    xnu_log_kv32("gic_distTyper", GIC_state_stage69.distTyper);
    xnu_log_kv32("gic_distIidr", GIC_state_stage69.distIidr);
    xnu_log_kv32("gic_cpuIidr", GIC_state_stage69.cpuIidr);
    xnu_log_kv32("gic_cpuCtlr", GIC_state_stage69.cpuCtlr);
    xnu_log_kv32("gic_cpuPmr", GIC_state_stage69.cpuPmr);
    xnu_log_kv32("gic_cpuBpr", GIC_state_stage69.cpuBpr);
    xnu_log_kv32("gic_irqCount", GIC_state_stage69.irqCount);
    xnu_log_kv32("gic_cpuInterfaceCount", GIC_state_stage69.cpuInterfaceCount);
    xnu_log_kv32("gic_isenabler0", GIC_state_stage69.isenabler0);
    xnu_log_kv32("gic_ispendr0", GIC_state_stage69.ispendr0);
    xnu_log_kv32("gic_priority0", GIC_state_stage69.priority0);
    xnu_log_kv32("gic_targets0", GIC_state_stage69.targets0);
...
{
    int ok = 1;
    ok &= (GIC_state_stage69.distBase == 0xf9000000u);
    ok &= (GIC_state_stage69.cpuBase == 0xf9002000u);
    ok &= (GIC_state_stage69.irqCount >= 288u);
    ok &= (GIC_state_stage69.cpuInterfaceCount == 4u);
    ok &= (GIC_state_stage69.distIidr != 0u);
    ok &= (GIC_state_stage69.cpuIidr != 0u);
...
{
    const uint32_t cpu_base = GIC_state_stage69.cpuBase;
    const uint32_t iar = mmio_read32(cpu_base + GICC_IAR);
    const uint32_t intid = iar & GICC_IAR_INTID_MASK;
    const uint32_t count = stage69_irq_count + 1u;

    stage69_irq_count = count;
    stage69_last_iar = iar;
    stage69_last_irq_id = intid;

    if (intid == GIC_SGI0_ID) {
        stage69_sgi0_count++;
    } else if (intid == GIC_TIMER_PPI0_ID || intid == GIC_TIMER_PPI1_ID) {
        uint32_t ctl = read_cntp_ctl();
        stage69_timer_irq_count++;
        stage69_last_timer_irq_id = intid;
        stage69_last_timer_ctl = ctl;
        generic_timer_shutdown();
    } else if (intid == GICC_SPURIOUS_ID) {
        stage69_spurious_irq_count++;
    } else {
        stage69_other_irq_count++;
    }

    if (count <= 8u) {
        log_puts("MI4IOS6_STAGE69 irq handler iar=");
        log_hex32(iar);
        log_puts(" id=");
        log_hex32(intid);
        log_puts(" count=");
        ...
        log_hex32(stage69_timer_irq_count);
        log_puts("\n");
    }
...
{
    stage69_irq_count = 0;
    stage69_last_iar = 0xffffffffu;
    stage69_last_irq_id = 0xffffffffu;
    stage69_sgi0_count = 0;
    stage69_spurious_irq_count = 0;
    stage69_timer_irq_count = 0;
    stage69_last_timer_irq_id = 0xffffffffu;
    stage69_last_timer_ctl = 0xffffffffu;
    stage69_other_irq_count = 0;
}

int gic_sgi_selftest(void)
{
    const uint32_t dist_base = GIC_state_stage69.distBase;
    const uint32_t cpu_base = GIC_state_stage69.cpuBase;
    uint64_t start;
    ...
    while (stage69_sgi0_count == 0u && timebase_elapsed_us(start, timebase_ticks()) < 20000u) {
    ...
    xnu_log_kv32("gic_sgi_irq_count", stage69_irq_count);
    xnu_log_kv32("gic_sgi_sgi0_count", stage69_sgi0_count);
    xnu_log_kv32("gic_sgi_spurious_count", stage69_spurious_irq_count);
    xnu_log_kv32("gic_sgi_last_iar", stage69_last_iar);
    xnu_log_kv32("gic_sgi_last_irq_id", stage69_last_irq_id);

    stage69_sgi_irq_count_observed = stage69_irq_count;
    stage69_sgi_sgi0_count_observed = stage69_sgi0_count;
    stage69_sgi_last_irq_id_observed = stage69_last_irq_id;

    if (stage69_sgi0_count > 0u) {
        stage69_sgi_selftest_passed = 1u;
        xnu_log_puts("gic SGI selftest ok\n");
        return 1;
    }

    stage69_sgi_selftest_passed = 0u;
...
int gic_timer_selftest(void)
{
    ...
    enable_irq_delivery();
    start = timebase_ticks();
    while (stage69_timer_irq_count == 0u && timebase_elapsed_us(start, timebase_ticks()) < 50000u) {
        __asm__ volatile ("nop" ::: "memory");
    }
    disable_irq_delivery();
    generic_timer_shutdown();
    ...
    xnu_log_kv32("gic_timer_irq_count", stage69_irq_count);
    xnu_log_kv32("gic_timer_timer_count", stage69_timer_irq_count);
    xnu_log_kv32("gic_timer_other_count", stage69_other_irq_count);
    xnu_log_kv32("gic_timer_spurious_count", stage69_spurious_irq_count);
    xnu_log_kv32("gic_timer_last_iar", stage69_last_iar);
    xnu_log_kv32("gic_timer_last_irq_id", stage69_last_irq_id);
    xnu_log_kv32("gic_timer_last_timer_id", stage69_last_timer_irq_id);
    xnu_log_kv32("gic_timer_last_timer_ctl", stage69_last_timer_ctl);
    ...
    stage69_timer_irq_count_observed = stage69_irq_count;
    stage69_timer_timer_count_observed = stage69_timer_irq_count;
    stage69_timer_last_irq_id_observed = stage69_last_timer_irq_id;
    stage69_timer_last_ctl_observed = stage69_last_timer_ctl;

    if (stage69_timer_irq_count > 0u &&
        (stage69_last_timer_irq_id == GIC_TIMER_PPI0_ID || stage69_last_timer_irq_id == GIC_TIMER_PPI1_ID)) {
        stage69_timer_selftest_passed = 1u;
        xnu_log_puts("gic timer selftest ok\n");
        return 1;
    }

    stage69_timer_selftest_passed = 0u;
