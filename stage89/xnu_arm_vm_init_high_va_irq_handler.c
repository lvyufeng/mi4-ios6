 ...
 * of the existing stage89_vectors table and verifying a timer IRQ is delivered and
 * handled correctly.
 *
 * Stage85 proved:
...
extern uint8_t stage89_vectors[];  /* From vectors.S */
extern volatile uint32_t stage89_irq_count;
extern volatile uint32_t stage89_timer_irq_count;

static struct stage89_xnu_arm_vm_init_high_va_irq_handler_result g_result;

static uint32_t stage89_xnu_arm_vm_init_high_va_irq_handler_checksum(
    volatile const struct stage89_xnu_arm_vm_init_high_va_irq_handler_result *r)
{
	volatile const uint32_t *words = (volatile const uint32_t *)r;
	uint32_t count = (uint32_t)(offsetof(struct stage89_xnu_arm_vm_init_high_va_irq_handler_result, checksum) / sizeof(uint32_t));
	uint32_t chk = 0;
...
static void stage89_xnu_arm_vm_init_high_va_irq_handler_log(
    volatile const struct stage89_xnu_arm_vm_init_high_va_irq_handler_result *r)
{
	xnu_log_puts("stage89_xnu_arm_vm_init_high_va_irq_handler result:\n");
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_version", r->version);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_size", r->size);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_status", r->status);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_required_mask", r->required_mask);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_satisfied_mask", r->satisfied_mask);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_failure_mask", r->failure_mask);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_full_pmap_status", r->full_pmap_status);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_vectors_phys", r->vectors_phys);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_vectors_high_va", r->vectors_high_va);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_original_vbar", r->original_vbar);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_high_va_vbar_set", r->high_va_vbar_set);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_irq_count_before", r->irq_count_before);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_timer_irq_count_before", r->timer_irq_count_before);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_irq_window_opened", r->irq_window_opened);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_irq_count_after", r->irq_count_after);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_timer_irq_count_after", r->timer_irq_count_after);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_irq_delivered", r->irq_delivered);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_vbar_restored", r->vbar_restored);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_restored_vbar", r->restored_vbar);
	xnu_log_kv32("stage89_xnu_arm_vm_init_high_va_irq_handler_checksum", r->checksum);
}

int stage89_xnu_arm_vm_init_high_va_irq_handler_run(
    struct boot_args *args,
    struct stage89_xnu_entry_stub_result *entry_result)
{
	struct stage89_xnu_arm_vm_init_high_va_irq_handler_result *r = &g_result;
	const struct stage89_xnu_arm_vm_init_full_pmap_result *pmap_result;
	uint32_t vectors_phys = (uint32_t)(uintptr_t)&stage89_vectors;
	...
	/* Initialize result */
	memset(r, 0, sizeof(*r));
	r->version = STAGE89_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_VERSION;
	r->size = sizeof(*r);
	r->required_mask = STAGE89_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_REQUIRED_MASK;

	/* Check full pmap prerequisite (Stage86's full pmap install) */
	pmap_result = stage89_xnu_arm_vm_init_full_pmap_result();
	if (!pmap_result || pmap_result->status != STAGE89_STATUS_OK) {
		r->failure_mask |= STAGE89_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_FAIL_FULL_PMAP;
		goto finish;
	}
	r->full_pmap_status = pmap_result->status;
	r->satisfied_mask |= STAGE89_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_FULL_PMAP_OK;
	...
	r->satisfied_mask |= STAGE89_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_VECTORS_ADDR_COMPUTED;
	...
	irq_count_before = stage89_irq_count;
	timer_irq_count_before = stage89_timer_irq_count;
	 ...
	 * The GIC distributor base is at 0xf9000000 (from GIC_state_stage89 in gic.c). */
	...
	hvir_enable_irqs();
	timeout = 100000;
	while (stage89_timer_irq_count == timer_irq_count_before && timeout > 0) {
		timeout--;
	}
	hvir_disable_irqs();

	/* Check if timer fired with low-VA VBAR */
	if (stage89_timer_irq_count > timer_irq_count_before) {
		...
		r->satisfied_mask |= STAGE89_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_HIGH_VA_VBAR_SET;
		...
		r->satisfied_mask |= STAGE89_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_IRQ_WINDOW_OPENED;
	...
	irq_count_after = stage89_irq_count;
	timer_irq_count_after = stage89_timer_irq_count;
	...
	if (timer_irq_count_after > timer_irq_count_before) {
		r->irq_delivered = 1;
		r->satisfied_mask |= STAGE89_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_IRQ_DELIVERED;
	} else {
		r->failure_mask |= STAGE89_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_FAIL_IRQ_NOT_DELIVERED;
	}
	...
	r->satisfied_mask |= STAGE89_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_VBAR_RESTORED;

finish:
	/* Compute final status */
	if (r->satisfied_mask == r->required_mask && r->failure_mask == 0) {
		r->status = STAGE89_STATUS_OK;
	} else {
		r->status = STAGE89_STATUS_FAIL(r->failure_mask);
	}

	/* Compute checksum */
	r->checksum = stage89_xnu_arm_vm_init_high_va_irq_handler_checksum(r);

	/* Log result */
	stage89_xnu_arm_vm_init_high_va_irq_handler_log(r);

	return r->status == STAGE89_STATUS_OK ? 0 : -1;
}

const struct stage89_xnu_arm_vm_init_high_va_irq_handler_result *
stage89_xnu_arm_vm_init_high_va_irq_handler_result(void)
{
	return &g_result;
}
