...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage89.gicDistributorBase, PE_state_stage89.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage89_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage84 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage89_loader_preflight_run(args)) {
