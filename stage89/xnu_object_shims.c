...
#define STAGE89_XNU_SHIM_KALLOC_BYTES   4096u
#define STAGE89_XNU_CONSISTENT_DEBUG_WINDOW_BYTES 0x4000u

void *memset(void *dst, int c, size_t n);

static boot_args g_stage89_xnu_public_arm_boot_args = {
...
PE_state_t PE_state = {
    ...
    .video = {0},
    .deviceTreeHead = NULL,
    .bootArgs = &g_stage89_xnu_public_arm_boot_args,
};

static uint8_t g_stage89_xnu_shim_kalloc[STAGE89_XNU_SHIM_KALLOC_BYTES];
static vm_size_t g_stage89_xnu_shim_kalloc_used;
static uint8_t g_stage89_xnu_consistent_debug_window[STAGE89_XNU_CONSISTENT_DEBUG_WINDOW_BYTES] __attribute__((aligned(8)));
...
void *kalloc(vm_size_t size)
{
    ...
    if (aligned_size == 0u || aligned_size > (STAGE89_XNU_SHIM_KALLOC_BYTES - g_stage89_xnu_shim_kalloc_used)) {
        return NULL;
    }

    ptr = &g_stage89_xnu_shim_kalloc[g_stage89_xnu_shim_kalloc_used];
    g_stage89_xnu_shim_kalloc_used += aligned_size;
    memset(ptr, 0, size);
    return ptr;
}
...
vm_map_address_t ml_map_high_window(vm_offset_t phys_addr, vm_size_t len)
{
    ...
    if (len == 0u || len > sizeof(g_stage89_xnu_consistent_debug_window)) {
        return 0u;
    }
    return (vm_map_address_t)(uintptr_t)g_stage89_xnu_consistent_debug_window;
}
