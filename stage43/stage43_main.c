...
static void build_stage43_apple_dt(struct apple_dt_builder *b)
{
    ...
    apple_dt_prop_str(b, "compatible", "qcom,msm8974-xnu-stage43");
    apple_dt_prop_str(b, "model", "Xiaomi Mi 4 cancro Stage43");
    ...
    apple_dt_prop_str(b, "boot-args", "debug=0x144 serial=0x1 mi4ios6.stage=43 msm8974=cancro startup-entry=selftest macho-loader-probe xnu-preflight");
...
{
    log_puts("MI4IOS6_STAGE43 handoff -> test_kernel_entry(boot_args*)\n");

    if (!args) {
        log_puts("MI4IOS6_STAGE43 handoff bad: null args\n");
        return 0;
    }
    ...
    if (args->Revision != BOOT_ARGS_REVISION || args->Version != BOOT_ARGS_VERSION) {
        log_puts("MI4IOS6_STAGE43 handoff bad: rev/version\n");
        return 0;
    }
    if (!args->deviceTreeP || !args->deviceTreeLength) {
        log_puts("MI4IOS6_STAGE43 handoff bad: no dt\n");
        return 0;
    }

    if (!apple_dt_selftest_and_log(args->deviceTreeP, args->deviceTreeLength)) {
        log_puts("MI4IOS6_STAGE43 handoff bad: dt selftest\n");
        return 0;
    }

    log_puts("MI4IOS6_STAGE43 handoff ok: boot_args + fuller apple_dt validated\n");
    return 1;
}
...
void platform_reboot(void)
{
    ...
    *restart_reason = RESTART_NORMAL;
    ...
    log_puts("MI4IOS6_STAGE43 attempting MSM8974 PS_HOLD reset\n");
    *ps_hold = 0;
    __asm__ volatile ("dsb sy" ::: "memory");
...
void stage43_main(void)
{
    struct apple_dt_builder b;
    uint32_t dt_len;

    log_init();
    log_puts("MI4IOS6_STAGE43 v1 entered; C runtime active; ram_console live\n");
    log_kv32("stage43_image_end", (uint32_t)(uintptr_t)__stage43_image_end);

    build_stage43_apple_dt(&b);
    dt_len = apple_dt_finish(&b);
    log_kv32("built_apple_dt_len", dt_len);

    if (!dt_len) {
        log_puts("MI4IOS6_STAGE43 apple_dt build failed\n");
        platform_reboot();
    }

    build_boot_args(&g_boot_args, g_apple_dt, dt_len);
    log_puts("MI4IOS6_STAGE43 boot_args built (rev2); fuller apple_dt attached\n");

    log_puts("MI4IOS6_STAGE43 vector base installed at ");
    log_hex32((uint32_t)(uintptr_t)stage43_vectors);
    log_puts("\n");

    log_puts("MI4IOS6_STAGE43 boot-wrapper handoff -> kernel_entry(boot_args*)\n");
    if (kernel_entry(&g_boot_args)) {
        log_puts("MI4IOS6_STAGE43 kernel_entry returned success\n");
    } else {
        log_puts("MI4IOS6_STAGE43 kernel_entry returned failure\n");
        platform_reboot();
    }
