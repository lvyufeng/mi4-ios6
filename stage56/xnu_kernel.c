...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage56.gicDistributorBase, PE_state_stage56.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage56_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage56 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage56_loader_preflight_run(args)) {
