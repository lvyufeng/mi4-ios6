...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage74.gicDistributorBase, PE_state_stage74.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage74_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage74 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage74_loader_preflight_run(args)) {
