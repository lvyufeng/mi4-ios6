...
- **Status**: `loader_status=0x84000001` (OK)
- **IRQ window status**: `stage84_xnu_arm_vm_init_irq_window_status=0x84000001` (OK)
...
C handler (`stage84_irq_handler_c`):
...
Artifacts:
- `out/stage84/stage84.elf` (507KB)
- `out/stage84/stage84.bin` (454KB)
- `out/stage84/stage84-qcdt.img` (2.9MB, includes QCDT device tree)

## Non-Persistent Hardware Boot

```bash
sudo adb -s 4a2fe00b reboot bootloader
sudo fastboot -s 4a2fe00b boot out/stage84/stage84-qcdt.img
sudo adb -s 4a2fe00b wait-for-device
sudo adb -s 4a2fe00b exec-out 'cat /proc/last_kmsg' > /tmp/stage84-last_kmsg.txt
```

Expected pass markers:
```
stage84_xnu_arm_vm_init_irq_window_status=0x84000001
stage84_xnu_arm_vm_init_irq_window_satisfied_mask=0x00003fff
stage84_xnu_arm_vm_init_irq_window_failure_mask=0x00000000
...
- Hardware validation: `docs/experiment-87-stage84-sgi-irq-timer.md` (to be created)
