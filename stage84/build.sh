...
./xnu_workspace_validate.sh ../out/stage84
./xnu_compile_graph_scan.py ../out/stage84
./xnu_object_subset_compile.sh ../out/stage84
./xnu_link_proof.sh ../out/stage84
...
"$PYTHON" "$MACHO_FIXTURE_GEN" \
  ...
  --bin-output ../out/stage84/stage84_fixture.macho \
  --symbol-prefix stage84 \
  --metadata-file ../out/stage84/xnu-link-macho-metadata.txt
...
LDFLAGS=(
  ...
  -Wl,-T,linker.ld
  -Wl,--build-id=none
  -Wl,-Map,../out/stage84/stage84.map
)
...
for src in "${SOURCES[@]}"; do
  obj="../out/stage84/${src%.*}.o"
  OBJECTS+=("$obj")
  $CC "${CFLAGS[@]}" -c "$src" -o "$obj"
done

$CC "${CFLAGS[@]}" "${LDFLAGS[@]}" "${OBJECTS[@]}" -o ../out/stage84/stage84.elf
$OBJCOPY -O binary ../out/stage84/stage84.elf ../out/stage84/stage84.bin
$OBJDUMP -d ../out/stage84/stage84.elf > ../out/stage84/stage84.disasm
$NM -n ../out/stage84/stage84.elf > ../out/stage84/stage84.symbols
$SIZE ../out/stage84/stage84.elf > ../out/stage84/stage84.size

: > ../out/stage84/empty-ramdisk

$MKBOOTIMG \
  --kernel ../out/stage84/stage84.bin \
  --ramdisk ../out/stage84/empty-ramdisk \
  --base 0x00000000 \
...
if [[ -f "$QCDT_DT" ]]; then
  "$QCDT_PACKER" \
    ...
    --pagesize 2048 \
    --cmdline 'stage84 mi4ios6=stage84 c-runtime apple-dt macho-fixture load-plan materialize highva-dryrun safe-table stage-owned-tables ttbr0-roundtrip recovery-table cache-bits-preserved xnu-entry-stub xnu-early-init early-pmap-platform xnu-pe-init-false xnu-postpe cpu-topo bootcpu rtclock xnu-armvm live-pmap ttbr-live tlb-live pmap-restore prevm-pexpert dtinit-facts peid-machine xnu-bs-contract xnu-pmap-bs-contract xnu-pmap-table-contract xnu-pmap-page-contract xnu-pmap-attr-contract xnu-pmap-mw-contract xnu-pmap-trans-contract xnu-pexpert-hook xnu-iokit-platform xnu-iokit-match-contract xnu-iokit-regsvc-contract xnu-iokit-provider xnu-iokit-catalog xnu-iokit-propinh xnu-iokit-regtop pexpert-hook-ready iokit-platform-scaffold iokit-match-local iokit-regsvc-local iokit-provider-local iokit-catalog-local iokit-propinh-local iokit-regtop-local irq-timer-hook pmap-bootstrap-ref public-xnu-workspace public-xnu-compile-graph public-xnu-platform-graph public-xnu-object-subset xnu-controlled-link xnu-bounded-pe-gen xnu-arm-pe-bootargs xnu-arm-consistent-debug stage84-xnu-link-proof inert-macho-fixture cancro-target arm-init-stub no-full-xnu-build no-pub-start no-pub-arm-init no-pub-thread no-pub-cpuboot no-pub-rtclock no-pub-pmap no-pub-pexpert no-pub-peinit no-pub-dtinit no-pub-peid no-pub-armvm no-iokit-runtime-exec no-macho-exec no-cache-change no-persist-write no-external-mutation' \
    --output ../out/stage84/stage84-qcdt.img
else
  echo "warning: QCDT_DT not found: $QCDT_DT; skipping stage84-qcdt.img" >&2
fi

sha256sum ../out/stage84/stage84_fixture.macho ../out/stage84/stage84.elf ../out/stage84/stage84.bin ../out/stage84/stage84.img \
  $(if [[ -f ../out/stage84/stage84-qcdt.img ]]; then printf '%s' ../out/stage84/stage84-qcdt.img; fi) \
  > ../out/stage84/SHA256SUMS.txt

ls -l ../out/stage84/stage84.elf ../out/stage84/stage84.bin ../out/stage84/stage84.img \
  $(if [[ -f ../out/stage84/stage84-qcdt.img ]]; then printf '%s' ../out/stage84/stage84-qcdt.img; fi)
cat ../out/stage84/stage84.size
cat ../out/stage84/SHA256SUMS.txt
