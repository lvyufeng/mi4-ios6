...
#define STAGE44_PAGE_SIZE 4096u

struct stage44_macho_header_32 {
    uint32_t magic;
    uint32_t cputype;
    uint32_t cpusubtype;
    uint32_t filetype;
...
struct stage44_macho_load_command {
    uint32_t cmd;
    uint32_t cmdsize;
};

struct stage44_macho_segment_command_32 {
    uint32_t cmd;
    uint32_t cmdsize;
    char segname[16];
    uint32_t vmaddr;
...
struct stage44_macho_section_32 {
    char sectname[16];
    char segname[16];
    uint32_t addr;
    uint32_t size;
...
struct stage44_macho_symtab_command {
    uint32_t cmd;
    uint32_t cmdsize;
    uint32_t symoff;
    uint32_t nsyms;
...
static struct stage44_loader_preflight stage44_loader_preflight_block;
...
static uint32_t segment_mask_for_name(const char segname[16])
{
    if (fixed16_eq(segname, "__TEXT")) {
        return STAGE44_MACHO_SEG_TEXT;
    }
    if (fixed16_eq(segname, "__DATA")) {
        return STAGE44_MACHO_SEG_DATA;
    }
    if (fixed16_eq(segname, "__LINKEDIT")) {
        return STAGE44_MACHO_SEG_LINKEDIT;
    }
    if (fixed16_eq(segname, "__KLD")) {
        return STAGE44_MACHO_SEG_KLD;
    }
    if (fixed16_eq(segname, "__LAST")) {
        return STAGE44_MACHO_SEG_LAST;
    }
    if (fixed16_eq(segname, "__PRELINK_TEXT")) {
        return STAGE44_MACHO_SEG_PRELINK_TEXT;
    }
    if (fixed16_eq(segname, "__PRELINK_INFO")) {
        return STAGE44_MACHO_SEG_PRELINK_INFO;
    }
    if (fixed16_eq(segname, "__PRELINK_STATE")) {
        return STAGE44_MACHO_SEG_PRELINK_STATE;
    }
    if (fixed16_eq(segname, "__PRELINK")) {
        return STAGE44_MACHO_SEG_PRELINK;
    }

    return 0;
}

static void record_key_segment(struct stage44_macho_probe_result *result,
                               const struct stage44_macho_segment_command_32 *segment,
                               uint32_t seg_bit)
{
    if (seg_bit == STAGE44_MACHO_SEG_TEXT) {
    ...
    } else if (seg_bit == STAGE44_MACHO_SEG_DATA) {
    ...
    } else if (seg_bit == STAGE44_MACHO_SEG_LINKEDIT) {
    ...
    } else if (seg_bit == STAGE44_MACHO_SEG_PRELINK_TEXT) {
    ...
    } else if (seg_bit == STAGE44_MACHO_SEG_PRELINK_INFO) {
    ...
    } else if (seg_bit == STAGE44_MACHO_SEG_PRELINK_STATE) {
...
static void add_load_plan_entry(struct stage44_macho_probe_result *result,
                                const struct stage44_macho_segment_command_32 *segment,
                                uint32_t seg_bit)
{
    struct stage44_macho_load_plan *plan = &result->load_plan;
    struct stage44_macho_load_plan_entry *entry;

    if (segment->vmsize == 0u) {
        return;
    }
    if (plan->entry_count >= STAGE44_MACHO_LOAD_PLAN_MAX) {
        result->failure_mask |= STAGE44_MACHO_FAIL_LOAD_PLAN_FULL;
        return;
    }
    ...
    if (segment->filesize != 0u) {
        entry->flags |= STAGE44_MACHO_LOAD_PLAN_FILE_BACKED;
    }
    if ((seg_bit & STAGE44_MACHO_SEG_PRELINK_MASK) != 0u) {
        entry->flags |= STAGE44_MACHO_LOAD_PLAN_PRELINK;
    }
    if (segment->vmsize > segment->filesize) {
        entry->flags |= STAGE44_MACHO_LOAD_PLAN_ZERO_FILL;
    }
    plan->entry_count++;
}

static void record_section_match(struct stage44_macho_probe_result *result,
                                 const struct stage44_macho_section_32 *section)
{
    uint32_t section_bit = 0;

    if (fixed16_eq(section->segname, "__TEXT") && fixed16_eq(section->sectname, "__text")) {
        section_bit = STAGE44_MACHO_SECT_TEXT_TEXT;
    } else if (fixed16_eq(section->segname, "__DATA") && fixed16_eq(section->sectname, "__const")) {
        section_bit = STAGE44_MACHO_SECT_DATA_CONST;
        ...
        section_bit = STAGE44_MACHO_SECT_PRELINK_TEXT_TEXT;
        ...
        section_bit = STAGE44_MACHO_SECT_PRELINK_INFO_INFO;
                           ...
                           const struct stage44_macho_section_32 *section)
{
    ...
    if (add_overflow_u32(section->addr, section->size, &section_end) ||
        ...
        result->failure_mask |= STAGE44_MACHO_FAIL_SECTION_BOUNDS;
    }
...
static void record_segment(struct stage44_macho_probe_result *result,
                           const struct stage44_macho_segment_command_32 *segment)
{
    uint32_t vm_end = 0;
    uint32_t file_end = 0;
...
static void parse_segment_sections(struct stage44_macho_probe_result *result,
                                   const uint8_t *bytes,
                                   ...
                                   const struct stage44_macho_segment_command_32 *segment)
{
    ...
    if (mul_overflow_u32(segment->nsects, sizeof(struct stage44_macho_section_32), &section_bytes) ||
    ...
    for (uint32_t i = 0; i < segment->nsects; i++) {
        const struct stage44_macho_section_32 *section =
            (const struct stage44_macho_section_32 *)(const void *)(bytes + section_offset);
        record_section(result, segment, section);
        section_offset += sizeof(*section);
    }
}

static void finalize_load_plan(struct stage44_macho_probe_result *result)
{
    struct stage44_macho_load_plan *plan = &result->load_plan;
    ...
    plan->version = STAGE44_MACHO_LOAD_PLAN_VERSION;
    ...
    if (plan->entry_count == 0u || result->min_vmaddr == 0u || result->max_vmaddr <= result->min_vmaddr) {
        result->failure_mask |= STAGE44_MACHO_FAIL_LOAD_PLAN;
        return;
    }

    for (uint32_t i = 0; i < plan->entry_count; i++) {
        struct stage44_macho_load_plan_entry *entry = &plan->entries[i];
        uint32_t vm_delta;
        ...
        if (add_overflow_u32(RAM_PHYS_BASE, vm_delta, &entry->physaddr) ||
            align_up_checked_u32(entry->vmsize, STAGE44_PAGE_SIZE, &aligned_size) ||
            add_overflow_u32(entry->physaddr, aligned_size, &entry_end)) {
            result->failure_mask |= STAGE44_MACHO_FAIL_PHYS_RANGE;
            continue;
        }
    ...
    if ((result->failure_mask & (STAGE44_MACHO_FAIL_LOAD_PLAN | STAGE44_MACHO_FAIL_PHYS_RANGE |
                                 STAGE44_MACHO_FAIL_LOAD_PLAN_FULL)) == 0u) {
        plan->status = STAGE44_STATUS_OK;
        result->load_plan_status = STAGE44_STATUS_OK;
        result->validation_mask |= STAGE44_MACHO_VALID_LOAD_PLAN;
    } else {
        plan->status = STAGE44_STATUS_BASE | result->failure_mask;
...
static void log_load_plan_entry(const char *prefix_vmaddr, const char *prefix_vmsize,
                                ...
                                const struct stage44_macho_load_plan_entry *entry)
{
...
static void log_load_plan_entries(const struct stage44_macho_probe_result *result)
{
    const struct stage44_macho_load_plan *plan = &result->load_plan;
                        ...
                        struct stage44_macho_probe_result *result)
{
    ...
    const struct stage44_macho_header_32 *header;
    ...
    result->version = STAGE44_MACHO_PROBE_VERSION;
    result->artifact_base = (uint32_t)(uintptr_t)artifact;
    result->artifact_size = artifact_size;
    result->segment_required_mask = STAGE44_MACHO_SEG_REQUIRED;
    result->section_required_mask = STAGE44_MACHO_SECT_REQUIRED;
    result->entry_kind_mask = STAGE44_MACHO_ENTRY_NOT_EXECUTED;
    result->status = STAGE44_STATUS_BASE;
    result->load_plan.version = STAGE44_MACHO_LOAD_PLAN_VERSION;

    xnu_log_puts("Stage44 Mach-O probe begin\n");
    xnu_log_kv32("macho_artifact_base", result->artifact_base);
    xnu_log_kv32("macho_artifact_size", result->artifact_size);

    if (!artifact) {
        result->failure_mask |= STAGE44_MACHO_FAIL_NULL;
        goto done;
    }
    if (artifact_size < sizeof(*header)) {
        result->failure_mask |= STAGE44_MACHO_FAIL_SIZE;
        goto done;
    }

    header = (const struct stage44_macho_header_32 *)artifact;
    ...
    if (header->magic == STAGE44_MACHO_MAGIC) {
        result->validation_mask |= STAGE44_MACHO_VALID_MAGIC | STAGE44_MACHO_VALID_ENDIAN;
    } else if (header->magic == STAGE44_MACHO_CIGAM) {
        result->validation_mask |= STAGE44_MACHO_VALID_MAGIC;
        result->failure_mask |= STAGE44_MACHO_FAIL_ENDIAN;
    } else {
        result->failure_mask |= STAGE44_MACHO_FAIL_MAGIC;
    }

    if (header->cputype == STAGE44_MACHO_CPU_TYPE_ARM) {
        result->validation_mask |= STAGE44_MACHO_VALID_CPU;
    } else {
        result->failure_mask |= STAGE44_MACHO_FAIL_CPU;
    }

    if (header->cpusubtype == STAGE44_MACHO_CPU_SUBTYPE_ARM_ALL ||
        header->cpusubtype == STAGE44_MACHO_CPU_SUBTYPE_ARM_V7) {
        result->validation_mask |= STAGE44_MACHO_VALID_SUBTYPE;
    } else {
        result->failure_mask |= STAGE44_MACHO_FAIL_SUBTYPE;
    }

    if (header->filetype == STAGE44_MACHO_FILETYPE_EXECUTE ||
        header->filetype == STAGE44_MACHO_FILETYPE_PRELOAD) {
        result->validation_mask |= STAGE44_MACHO_VALID_FILETYPE;
    } else {
        result->failure_mask |= STAGE44_MACHO_FAIL_FILETYPE;
    }

    if (add_overflow_u32(commands_start, header->sizeofcmds, &commands_end) ||
        commands_end > artifact_size || (header->sizeofcmds & 3u) != 0u) {
        result->failure_mask |= STAGE44_MACHO_FAIL_COMMAND_BOUNDS;
    } else {
        result->validation_mask |= STAGE44_MACHO_VALID_COMMAND_BOUNDS;
    ...
    for (uint32_t i = 0; i < header->ncmds && offset < commands_end; i++) {
        const struct stage44_macho_load_command *load_command;
        ...
        load_command = (const struct stage44_macho_load_command *)(const void *)(bytes + offset);
        if (load_command->cmdsize < sizeof(*load_command) || (load_command->cmdsize & 3u) != 0u ||
            ...
            result->failure_mask |= STAGE44_MACHO_FAIL_COMMAND_SIZE;
            break;
        }

        result->load_command_count_seen++;
        if (load_command->cmd == STAGE44_MACHO_LC_SEGMENT) {
            if (load_command->cmdsize < sizeof(struct stage44_macho_segment_command_32)) {
                result->failure_mask |= STAGE44_MACHO_FAIL_COMMAND_SIZE;
            } else {
                const struct stage44_macho_segment_command_32 *segment =
                    (const struct stage44_macho_segment_command_32 *)(const void *)(bytes + offset);
                result->load_command_seen_mask |= STAGE44_MACHO_CMD_SEGMENT;
                record_segment(result, segment);
                parse_segment_sections(result, bytes, offset, segment);
            }
        } else if (load_command->cmd == STAGE44_MACHO_LC_SYMTAB) {
            result->load_command_seen_mask |= STAGE44_MACHO_CMD_SYMTAB;
            if (load_command->cmdsize >= sizeof(struct stage44_macho_symtab_command)) {
                const struct stage44_macho_symtab_command *symtab =
                    (const struct stage44_macho_symtab_command *)(const void *)(bytes + offset);
                uint32_t str_end = 0;
                if (add_overflow_u32(symtab->stroff, symtab->strsize, &str_end) || str_end > artifact_size) {
                    result->failure_mask |= STAGE44_MACHO_FAIL_SEGMENT_EXTENT;
                }
            }
        } else if (load_command->cmd == STAGE44_MACHO_LC_UNIXTHREAD) {
            result->load_command_seen_mask |= STAGE44_MACHO_CMD_UNIXTHREAD;
            result->entry_kind_mask |= STAGE44_MACHO_ENTRY_UNIXTHREAD;
        ...
        } else if (load_command->cmd == STAGE44_MACHO_LC_MAIN) {
            result->load_command_seen_mask |= STAGE44_MACHO_CMD_MAIN;
            result->entry_kind_mask |= STAGE44_MACHO_ENTRY_MAIN;
        } else {
            result->load_command_seen_mask |= STAGE44_MACHO_CMD_UNKNOWN;
...
after_commands:
    if ((result->segment_seen_mask & STAGE44_MACHO_SEG_REQUIRED) == STAGE44_MACHO_SEG_REQUIRED) {
        result->validation_mask |= STAGE44_MACHO_VALID_REQUIRED_SEGMENTS;
    } else {
        result->failure_mask |= STAGE44_MACHO_FAIL_REQUIRED_SEGMENTS;
    }
    if ((result->section_seen_mask & STAGE44_MACHO_SECT_REQUIRED) == STAGE44_MACHO_SECT_REQUIRED) {
        result->validation_mask |= STAGE44_MACHO_VALID_REQUIRED_SECTIONS;
    } else {
        result->failure_mask |= STAGE44_MACHO_FAIL_REQUIRED_SECTIONS;
    }
    if ((result->failure_mask & STAGE44_MACHO_FAIL_SEGMENT_EXTENT) == 0u) {
        result->validation_mask |= STAGE44_MACHO_VALID_FILE_EXTENTS;
    }
    if ((result->failure_mask & STAGE44_MACHO_FAIL_SECTION_BOUNDS) == 0u) {
        result->validation_mask |= STAGE44_MACHO_VALID_SECTION_BOUNDS;
    }
    if ((result->prelink_segment_seen_mask | result->prelink_section_seen_mask) != 0u) {
        result->validation_mask |= STAGE44_MACHO_VALID_PRELINK_REPORT;
    }
    if ((result->entry_kind_mask & (STAGE44_MACHO_ENTRY_UNIXTHREAD | STAGE44_MACHO_ENTRY_MAIN)) != 0u) {
        result->validation_mask |= STAGE44_MACHO_VALID_ENTRY_NOT_EXECUTED;
    } else {
        result->failure_mask |= STAGE44_MACHO_FAIL_NO_ENTRY_METADATA;
    }
    ...
    if ((result->validation_mask & STAGE44_MACHO_VALID_REQUIRED) == STAGE44_MACHO_VALID_REQUIRED &&
        result->failure_mask == 0u) {
        result->status = STAGE44_STATUS_OK;
        xnu_log_puts("Stage44 Mach-O probe ok; entry metadata observed but not executed\n");
        return 1;
    }

    result->status = STAGE44_STATUS_BASE | result->failure_mask;
    xnu_log_puts("Stage44 Mach-O probe failed\n");
    return 0;
}

static uint32_t stage44_loader_preflight_checksum(volatile const struct stage44_loader_preflight *preflight)
{
    volatile const uint32_t *words = (volatile const uint32_t *)(uintptr_t)preflight;
    uint32_t count = (uint32_t)(offsetof(struct stage44_loader_preflight, checksum) / sizeof(uint32_t));
    uint32_t checksum = 0;
...
static uint32_t stage44_dt_semantic_mask(struct boot_args *args)
{
    uint32_t mask = 0;
    uint32_t len = 0;
...
{
    struct stage44_macho_probe_result probe;
    struct stage44_loader_preflight *preflight = &stage44_loader_preflight_block;
    uint32_t workspace_limit;
    uint32_t loaded_end_for_workspace;

    xnu_log_puts("Stage44 Mach-O/XNU loader preflight begin\n");
    memset(preflight, 0, sizeof(*preflight));
    preflight->version = STAGE44_LOADER_PREFLIGHT_VERSION;
    preflight->size = sizeof(*preflight);
    preflight->xnu_baseline_tag = STAGE44_XNU_BASELINE_2050_22_13;
    preflight->xnu_baseline_commit = STAGE44_XNU_BASELINE_COMMIT_CC8A9B0C;
    preflight->xnu_master_version = STAGE44_XNU_BASELINE_MASTER_12_3_0;
    preflight->xnu_arm_reference = STAGE44_XNU_ARM_REFERENCE_4570_1_46;

    if (stage44_macho_probe(stage44_embedded_macho, stage44_embedded_macho_size, &probe)) {
        preflight->satisfied_mask |= STAGE44_LOADER_SAT_MACHO;
    }
    ...
    preflight->proposed_topOfKernelData = align_up_u32(loaded_end_for_workspace, STAGE44_XNU_TTE_ALIGNMENT);
    preflight->proposed_ttep_workspace_base = preflight->proposed_topOfKernelData;
    preflight->proposed_ttep_workspace_size = STAGE44_XNU_TTE_WORKSPACE_BYTES;
    ...
    preflight->proposed_workspace_alignment = preflight->proposed_topOfKernelData & (STAGE44_XNU_TTE_ALIGNMENT - 1u);
    ...
    preflight->apple_dt_semantic_mask = stage44_dt_semantic_mask(args);
    preflight->pexpert_gap_mask = STAGE44_PLATFORM_GAP_PEXPERT_IMPL;
    preflight->platform_gap_mask = STAGE44_PLATFORM_GAP_REQUIRED_RECORDED;
    preflight->interrupt_ready_mask = STAGE44_LOADER_IRQ_READY_REQUIRED;
    preflight->safety_mask = STAGE44_LOADER_SAFETY_REQUIRED;

    if (preflight->xnu_baseline_tag == STAGE44_XNU_BASELINE_2050_22_13 &&
        preflight->xnu_baseline_commit == STAGE44_XNU_BASELINE_COMMIT_CC8A9B0C &&
        preflight->xnu_master_version == STAGE44_XNU_BASELINE_MASTER_12_3_0 &&
        preflight->xnu_arm_reference == STAGE44_XNU_ARM_REFERENCE_4570_1_46) {
        preflight->satisfied_mask |= STAGE44_LOADER_SAT_BASELINE;
    }
    if (preflight->boot_args_rev_ver == 0x00020002u &&
        ...
        preflight->satisfied_mask |= STAGE44_LOADER_SAT_BOOT_ARGS;
    }
    if (preflight->proposed_workspace_range_ok == 1u) {
        preflight->satisfied_mask |= STAGE44_LOADER_SAT_WORKSPACE;
    }
    if (preflight->apple_dt_semantic_mask == STAGE44_DT_READY_REQUIRED) {
        preflight->satisfied_mask |= STAGE44_LOADER_SAT_DT;
    }
    if (preflight->platform_gap_mask == STAGE44_PLATFORM_GAP_REQUIRED_RECORDED) {
        preflight->satisfied_mask |= STAGE44_LOADER_SAT_PLATFORM_GAPS;
    }
    if (preflight->interrupt_ready_mask == STAGE44_LOADER_IRQ_READY_REQUIRED) {
        preflight->satisfied_mask |= STAGE44_LOADER_SAT_INTERRUPTS;
    }
    if (preflight->safety_mask == STAGE44_LOADER_SAFETY_REQUIRED) {
        preflight->satisfied_mask |= STAGE44_LOADER_SAT_SAFETY;
    }

    preflight->readiness_mask = preflight->satisfied_mask;
    preflight->failure_mask = probe.failure_mask;
    if (preflight->satisfied_mask != STAGE44_LOADER_SAT_REQUIRED) {
        preflight->failure_mask |= 0x80000000u;
    }
    preflight->checksum = stage44_loader_preflight_checksum(preflight);
    preflight->status = (preflight->satisfied_mask == STAGE44_LOADER_SAT_REQUIRED && preflight->failure_mask == 0u &&
        preflight->checksum == stage44_loader_preflight_checksum(preflight)) ? STAGE44_STATUS_OK : (STAGE44_STATUS_BASE | preflight->failure_mask);
    ...
    xnu_log_puts("Stage44 XNU handoff disabled: no Mach-O entry executed, no TTBR switch, caches unchanged\n");

    if (preflight->status == STAGE44_STATUS_OK) {
        xnu_log_puts("Stage44 Mach-O/XNU loader preflight ok\n");
        return 1;
    }

    xnu_log_puts("Stage44 Mach-O/XNU loader preflight failed\n");
    return 0;
}
