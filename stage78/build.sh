...
./xnu_workspace_validate.sh ../out/stage78
./xnu_compile_graph_scan.py ../out/stage78
./xnu_object_subset_compile.sh ../out/stage78
./xnu_link_proof.sh ../out/stage78
...
"$PYTHON" "$MACHO_FIXTURE_GEN" \
  ...
  --bin-output ../out/stage78/stage78_fixture.macho \
  --symbol-prefix stage78 \
  --metadata-file ../out/stage78/xnu-link-macho-metadata.txt
...
LDFLAGS=(
  ...
  -Wl,-T,linker.ld
  -Wl,--build-id=none
  -Wl,-Map,../out/stage78/stage78.map
)
...
for src in "${SOURCES[@]}"; do
  obj="../out/stage78/${src%.*}.o"
  OBJECTS+=("$obj")
  $CC "${CFLAGS[@]}" -c "$src" -o "$obj"
done

$CC "${CFLAGS[@]}" "${LDFLAGS[@]}" "${OBJECTS[@]}" -o ../out/stage78/stage78.elf
$OBJCOPY -O binary ../out/stage78/stage78.elf ../out/stage78/stage78.bin
$OBJDUMP -d ../out/stage78/stage78.elf > ../out/stage78/stage78.disasm
$NM -n ../out/stage78/stage78.elf > ../out/stage78/stage78.symbols
$SIZE ../out/stage78/stage78.elf > ../out/stage78/stage78.size

: > ../out/stage78/empty-ramdisk

$MKBOOTIMG \
  --kernel ../out/stage78/stage78.bin \
  --ramdisk ../out/stage78/empty-ramdisk \
  --base 0x00000000 \
  ...
  --pagesize 2048 \
  --cmdline 'stage78 mi4ios6=stage78 c-runtime apple-dt macho-fixture load-plan materialize highva-dryrun safe-table stage-owned-tables ttbr0-roundtrip recovery-table cache-bits-preserved xnu-entry-stub xnu-early-init early-pmap-platform stage-owned-early-init xnu-bs-contract xnu-pmap-bs-contract xnu-pmap-table-contract xnu-pmap-page-contract xnu-pmap-attr-contract xnu-pmap-mw-contract xnu-pmap-trans-contract xnu-pexpert-hook xnu-iokit-platform xnu-iokit-match-contract xnu-iokit-regsvc-contract xnu-iokit-provider xnu-iokit-catalog xnu-iokit-propinh xnu-iokit-regtop xnu-iokit-attachstart xnu-iokit-lifecycle xnu-iokit-pn-open pexpert-hook-ready iokit-platform-scaffold iokit-match-local iokit-regsvc-local iokit-provider-local iokit-catalog-local iokit-propinh-local iokit-regtop-local iokit-attachstart-local lifecycle-regsvc-local pn-open-local irq-timer-hook pmap-table-local pmap-page-local pmap-attr-local pmap-mw-local pmap-transition-local pmap-bootstrap-ref public-xnu-workspace public-xnu-compile-graph public-xnu-platform-graph public-xnu-object-subset xnu-controlled-link xnu-bounded-pe-gen xnu-arm-pe-bootargs xnu-arm-consistent-debug stage78-xnu-link-proof inert-macho-fixture cancro-target arm-init-stub no-full-xnu-build no-pub-start no-pub-arm-init no-platform-runtime-exec no-iokit-runtime-exec no-pub-pmap no-pub-pexpert no-macho-exec no-phys-write no-tte-write no-pmap-write no-live-pmap-install no-tlb-invalidate no-cache-change no-persist-write no-external-mutation' \
  --header_version 0 \
  --output ../out/stage78/stage78.img
...
if [[ -f "$QCDT_DT" ]]; then
  "$QCDT_PACKER" \
    ...
    --pagesize 2048 \
    --cmdline 'stage78 mi4ios6=stage78 c-runtime apple-dt macho-fixture load-plan materialize highva-dryrun safe-table stage-owned-tables ttbr0-roundtrip recovery-table cache-bits-preserved xnu-entry-stub xnu-early-init early-pmap-platform stage-owned-early-init xnu-bs-contract xnu-pmap-bs-contract xnu-pmap-table-contract xnu-pmap-page-contract xnu-pmap-attr-contract xnu-pmap-mw-contract xnu-pmap-trans-contract xnu-pexpert-hook xnu-iokit-platform xnu-iokit-match-contract xnu-iokit-regsvc-contract xnu-iokit-provider xnu-iokit-catalog xnu-iokit-propinh xnu-iokit-regtop xnu-iokit-attachstart xnu-iokit-lifecycle xnu-iokit-pn-open pexpert-hook-ready iokit-platform-scaffold iokit-match-local iokit-regsvc-local iokit-provider-local iokit-catalog-local iokit-propinh-local iokit-regtop-local iokit-attachstart-local lifecycle-regsvc-local pn-open-local irq-timer-hook pmap-table-local pmap-page-local pmap-attr-local pmap-mw-local pmap-transition-local pmap-bootstrap-ref public-xnu-workspace public-xnu-compile-graph public-xnu-platform-graph public-xnu-object-subset xnu-controlled-link xnu-bounded-pe-gen xnu-arm-pe-bootargs xnu-arm-consistent-debug stage78-xnu-link-proof inert-macho-fixture cancro-target arm-init-stub no-full-xnu-build no-pub-start no-pub-arm-init no-platform-runtime-exec no-iokit-runtime-exec no-pub-pmap no-pub-pexpert no-macho-exec no-phys-write no-tte-write no-pmap-write no-live-pmap-install no-tlb-invalidate no-cache-change no-persist-write no-external-mutation' \
    --output ../out/stage78/stage78-qcdt.img
else
  echo "warning: QCDT_DT not found: $QCDT_DT; skipping stage78-qcdt.img" >&2
fi

sha256sum ../out/stage78/stage78_fixture.macho ../out/stage78/stage78.elf ../out/stage78/stage78.bin ../out/stage78/stage78.img \
  $(if [[ -f ../out/stage78/stage78-qcdt.img ]]; then printf '%s' ../out/stage78/stage78-qcdt.img; fi) \
  > ../out/stage78/SHA256SUMS.txt

ls -l ../out/stage78/stage78.elf ../out/stage78/stage78.bin ../out/stage78/stage78.img \
  $(if [[ -f ../out/stage78/stage78-qcdt.img ]]; then printf '%s' ../out/stage78/stage78-qcdt.img; fi)
cat ../out/stage78/stage78.size
cat ../out/stage78/SHA256SUMS.txt
