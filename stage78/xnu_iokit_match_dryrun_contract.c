...
static struct stage78_xnu_iokit_match_dryrun_contract g_stage78_xnu_iokit_match_dryrun_contract;

static uint32_t stage78_iokit_match_dryrun_checksum(
    volatile const struct stage78_xnu_iokit_match_dryrun_contract *contract)
{
    volatile const uint32_t *words = (volatile const uint32_t *)(uintptr_t)contract;
    uint32_t count = (uint32_t)(offsetof(struct stage78_xnu_iokit_match_dryrun_contract, checksum) /
                                sizeof(uint32_t));
...
static void stage78_iokit_match_dryrun_log(const struct stage78_xnu_iokit_match_dryrun_contract *contract)
{
    xnu_log_kv32("stage78_xnu_iokit_match_dryrun_contract_status", contract->status);
    xnu_log_kv32("stage78_xnu_iokit_match_dryrun_contract_required_mask", contract->required_mask);
    xnu_log_kv32("stage78_xnu_iokit_match_dryrun_contract_satisfied_mask", contract->satisfied_mask);
    xnu_log_kv32("stage78_xnu_iokit_match_dryrun_contract_failure_mask", contract->failure_mask);
    xnu_log_kv32("stage78_xnu_iokit_match_dryrun_contract_checksum", contract->checksum);
    xnu_log_kv32("stage78_xnu_iokit_match_provider_node_props", contract->provider_prop_count);
    xnu_log_kv32("stage78_xnu_iokit_match_driver_node_props", contract->driver_prop_count);
    xnu_log_kv32("stage78_xnu_iokit_match_provider_class_match", contract->provider_class_match);
    xnu_log_kv32("stage78_xnu_iokit_match_provider_path_match", contract->provider_path_match);
    xnu_log_kv32("stage78_xnu_iokit_match_category_match", contract->match_category_match);
    xnu_log_kv32("stage78_xnu_iokit_match_compatible_match", contract->compatible_match);
    xnu_log_kv32("stage78_xnu_iokit_match_driver_probe_score", contract->driver_probe_score);
    xnu_log_kv32("stage78_xnu_iokit_match_dryrun_match_count", contract->dryrun_match_count);
    xnu_log_kv32("stage78_xnu_iokit_match_attach_deferred", contract->attach_deferred);
    xnu_log_kv32("stage78_xnu_iokit_match_start_deferred", contract->start_deferred);
    xnu_log_kv32("stage78_xnu_iokit_match_iokit_reference_mask", contract->iokit_reference_mask);
    xnu_log_kv32("stage78_xnu_iokit_match_iokit_runtime_blocked_mask", contract->iokit_runtime_blocked_mask);
    xnu_log_kv32("stage78_xnu_iokit_match_public_compile_count", contract->iokit_public_compile_count);
    xnu_log_kv32("stage78_xnu_iokit_match_public_link_count", contract->iokit_public_link_count);
    xnu_log_kv32("stage78_xnu_iokit_match_no_iokit_runtime_exec", contract->no_iokit_runtime_exec);
    xnu_log_kv32("stage78_xnu_iokit_match_no_platform_driver_exec", contract->no_platform_driver_exec);
    xnu_log_kv32("stage78_xnu_iokit_match_no_live_pmap_tables_installed", contract->no_live_pmap_tables_installed);
    xnu_log_kv32("stage78_xnu_iokit_match_proposed_workspace_written", contract->proposed_workspace_written);
    xnu_log_kv32("stage78_xnu_iokit_match_pmap_ttbr_written", contract->pmap_ttbr_written);
    xnu_log_kv32("stage78_xnu_iokit_match_pmap_tlbs_invalidated", contract->pmap_tlbs_invalidated);
    xnu_log_kv32("stage78_xnu_iokit_match_caches_changed", contract->caches_changed);
    xnu_log_kv32("stage78_xnu_iokit_match_persistent_write_attempted", contract->persistent_write_attempted);
    xnu_log_kv32("stage78_xnu_iokit_match_platform_gap_mask", contract->proposed_platform_gap_mask);
}

int stage78_xnu_iokit_match_dryrun_contract_selftest(const struct stage78_loader_preflight *preflight)
{
    struct stage78_xnu_iokit_match_dryrun_contract *contract = &g_stage78_xnu_iokit_match_dryrun_contract;
    ...
    contract->version = STAGE78_XNU_IOKIT_MATCH_DRYRUN_CONTRACT_VERSION;
    contract->size = sizeof(*contract);
    contract->status = STAGE78_STATUS_BASE;
    contract->required_mask = STAGE78_XNU_IOKIT_MATCH_DRYRUN_REQUIRED_MASK;

    if (!preflight || !args || !root) {
        contract->failure_mask = STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_SOURCE |
            STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_SAFETY_BOUNDARY;
        contract->checksum = stage78_iokit_match_dryrun_checksum(contract);
        stage78_iokit_match_dryrun_log(contract);
        return 0;
    }
    ...
    source_ok = (contract->source_scaffold_status == STAGE78_STATUS_OK &&
    ...
    if (source_ok) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_SOURCE_SCAFFOLD;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_SOURCE;
    }

    if (contract->apple_dt_semantic_mask == STAGE78_DT_READY_REQUIRED) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_APPLE_DT;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_DT;
    }
    ...
    if (provider_ok) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_PROVIDER_NODE;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_PROVIDER;
    }
    ...
    if (driver_ok) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_DRIVER_NODE;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_DRIVER;
    }

    if (contract->provider_class_match == 1u && contract->provider_path_match == 1u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_PROVIDER_CLASS_MATCH;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_MATCH;
    }
    if (contract->match_category_match == 1u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_MATCH_CATEGORY;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_MATCH;
    }
    if (contract->compatible_match == 1u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_COMPATIBLE_MATCH;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_MATCH;
    }

    if (contract->driver_gic_base == STAGE78_XNU_IOKIT_PLATFORM_EXPECTED_GIC_DIST_BASE &&
        contract->driver_timer_base == STAGE78_XNU_IOKIT_PLATFORM_EXPECTED_TIMER_BASE &&
        contract->driver_timebase_frequency == STAGE78_XNU_IOKIT_PLATFORM_EXPECTED_TIMEBASE_FREQ &&
        contract->driver_cpu_count == STAGE78_XNU_IOKIT_PLATFORM_EXPECTED_CPU_COUNT) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_MSM8974_FACTS;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_DRIVER;
    }
    ...
    if (contract->iokit_reference_mask == STAGE78_XNU_IOKIT_REFERENCE_REQUIRED &&
        ...
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_IOKIT_REFERENCE;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_PUBLIC_BOUNDARY;
    }
    ...
    if (match_ok) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_SERVICE_MATCH_DRYRUN;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_MATCH;
    }

    if (contract->driver_probe_score == STAGE78_XNU_IOKIT_MATCH_EXPECTED_PROBE_SCORE) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_PROBE_SCORE;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_MATCH;
    }

    if (contract->attach_deferred == 1u && contract->start_deferred == 1u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_ATTACH_START_DEFERRED;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_SAFETY_BOUNDARY;
    }
    ...
    if (public_boundary_ok) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_PUBLIC_BOUNDARY |
            STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_NO_IOKIT_RUNTIME_EXEC |
            STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_NO_PLATFORM_DRIVER_EXEC |
            STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_NO_PUBLIC_XNU_EXEC;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_PUBLIC_BOUNDARY;
    }

    if (contract->no_public_pmap_exec == 1u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_NO_PUBLIC_PMAP_EXEC;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_PMAP_BOUNDARY;
    }
    ...
    if (contract->no_live_pmap_tables_installed == 0u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_NO_LIVE_PMAP_INSTALL;
    }
    if (contract->proposed_workspace_written == 0u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_NO_PROPOSED_PMAP_WRITE;
    }
    if (contract->pmap_ttbr_written == 0u && contract->pmap_ttbcr_written == 0u &&
        contract->pmap_dacr_written == 0u && contract->pmap_sctlr_written == 0u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_NO_PMAP_CONTROL_WRITE;
    }
    if (contract->pmap_tlbs_invalidated == 0u && contract->caches_changed == 0u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_NO_TLB_CACHE_CHANGE;
    }
    if (pmap_boundary_ok && contract->source_pmap_transition_status == STAGE78_STATUS_OK &&
        contract->source_pmap_transition_failure_mask == 0u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_PMAP_BOUNDARY;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_PMAP_BOUNDARY;
    }

    if (contract->xnu_start_executed == 0u && contract->generated_macho_executed == 0u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_NO_XNU_MACHO_EXEC;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_SAFETY_BOUNDARY;
    }
    if (contract->persistent_write_attempted == 0u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_NO_PERSIST_WRITE;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_SAFETY_BOUNDARY;
    }
    ...
    if (contract->proposed_platform_gap_mask == (STAGE78_PLATFORM_GAP_REQUIRED_RECORDED & ~STAGE78_PLATFORM_GAP_IOKIT_STACK) &&
        ...
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_PLATFORM_ROLLUP;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_ROLLUP;
    }

    if (contract->local_only == 1u && contract->fail_closed == 1u) {
        contract->satisfied_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_SAT_FAIL_CLOSED;
    } else {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_SAFETY_BOUNDARY;
    }

    contract->checksum = stage78_iokit_match_dryrun_checksum(contract);
    if (contract->checksum != stage78_iokit_match_dryrun_checksum(contract)) {
        contract->failure_mask |= STAGE78_XNU_IOKIT_MATCH_DRYRUN_FAIL_CHECKSUM;
    }
...
{
    return &g_stage78_xnu_iokit_match_dryrun_contract;
}
