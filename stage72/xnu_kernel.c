...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage72.gicDistributorBase, PE_state_stage72.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage72_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage72 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage72_loader_preflight_run(args)) {
