...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage61.gicDistributorBase, PE_state_stage61.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage61_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage61 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage61_loader_preflight_run(args)) {
