...
OUT_DIR=${1:-../out/stage75}
OBJ_DIR="$OUT_DIR/xnu-objects"
mkdir -p "$OBJ_DIR"

CC=${STAGE75_XNU_OBJ_CC:-arm-none-eabi-gcc}
OBJDUMP=${STAGE75_XNU_OBJ_OBJDUMP:-arm-none-eabi-objdump}
NM=${STAGE75_XNU_OBJ_NM:-arm-none-eabi-nm}
SIZE=${STAGE75_XNU_OBJ_SIZE:-arm-none-eabi-size}
...
printf 'Stage75 public-XNU pexpert/platform compile graph gated object subset compile\n' >> "$STATUS_TXT"
...
graph_ok=0
if [[ -f "$GRAPH_HEADER" ]] &&
   grep -q 'STAGE75_XNU_GRAPH_HOST_STATUS' "$GRAPH_HEADER" &&
   grep -q '0x75000001u' "$GRAPH_HEADER" &&
   grep -q 'STAGE75_XNU_GRAPH_HOST_PE_GEN_ALLOWED' "$GRAPH_HEADER" &&
   grep -q 'STAGE75_XNU_GRAPH_HOST_ARM_BOOTARGS_ALLOWED' "$GRAPH_HEADER" &&
   grep -q 'STAGE75_XNU_GRAPH_HOST_ARM_CONSISTENT_DEBUG_ALLOWED' "$GRAPH_HEADER" &&
   grep -q 'STAGE75_XNU_GRAPH_HOST_DUPLICATE_SYMBOL_COUNT 0x00000000u' "$GRAPH_HEADER"; then
  graph_ok=1
...
add_satisfied 0x00008000  # outputs under ignored out/stage75
...
printf 'stage75_xnu_object_subset_status=0x%08x\n' "$status" >> "$STATUS_TXT"
printf 'stage75_xnu_object_subset_required_mask=0x%08x\n' "$required" >> "$STATUS_TXT"
printf 'stage75_xnu_object_subset_satisfied_mask=0x%08x\n' "$satisfied" >> "$STATUS_TXT"
printf 'stage75_xnu_object_subset_failure_mask=0x%08x\n' "$failure" >> "$STATUS_TXT"
printf 'stage75_xnu_object_source_mask=0x%08x\n' 0x0000001f >> "$STATUS_TXT"
printf 'stage75_xnu_object_shim_mask=0x%08x\n' 0x000003ff >> "$STATUS_TXT"
printf 'stage75_xnu_object_count=0x%08x\n' "$object_count" >> "$STATUS_TXT"
printf 'stage75_xnu_object_public_2050_count=0x00000003\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_public_arm_pexpert_count=0x00000002\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_stage_owned_shim_count=0x00000001\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_duplicate_symbol_count=0x%08x\n' "$duplicate_count" >> "$STATUS_TXT"
printf 'stage75_xnu_object_pe_state_abi_shim_ready=0x00000001\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_consistent_debug_abi_shim_ready=0x00000001\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_device_tree_bytes=0x%08x\n' "$device_tree_bytes" >> "$STATUS_TXT"
printf 'stage75_xnu_object_bootargs_bytes=0x%08x\n' "$bootargs_bytes" >> "$STATUS_TXT"
printf 'stage75_xnu_object_pe_gen_bytes=0x%08x\n' "$pe_gen_bytes" >> "$STATUS_TXT"
printf 'stage75_xnu_object_arm_pe_bootargs_bytes=0x%08x\n' "$arm_pe_bootargs_bytes" >> "$STATUS_TXT"
printf 'stage75_xnu_object_arm_pe_consistent_debug_bytes=0x%08x\n' "$arm_pe_consistent_debug_bytes" >> "$STATUS_TXT"
printf 'stage75_xnu_object_device_tree_sha32=%s\n' "$device_tree_sha32" >> "$STATUS_TXT"
printf 'stage75_xnu_object_bootargs_sha32=%s\n' "$bootargs_sha32" >> "$STATUS_TXT"
printf 'stage75_xnu_object_pe_gen_sha32=%s\n' "$pe_gen_sha32" >> "$STATUS_TXT"
printf 'stage75_xnu_object_arm_pe_bootargs_sha32=%s\n' "$arm_pe_bootargs_sha32" >> "$STATUS_TXT"
printf 'stage75_xnu_object_arm_pe_consistent_debug_sha32=%s\n' "$arm_pe_consistent_debug_sha32" >> "$STATUS_TXT"
printf 'stage75_xnu_object_public_only=0x00000001\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_no_full_xnu_build=0x00000001\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_no_macho_link=0x00000001\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_no_public_xnu_exec=0x00000001\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_no_platform_runtime_exec=0x00000001\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_no_external_mutation=0x00000001\n' >> "$STATUS_TXT"
printf 'stage75_xnu_object_outputs_ignored=0x00000001\n' >> "$STATUS_TXT"
...
#ifndef MI4IOS6_STAGE75_XNU_OBJECT_SUBSET_GENERATED_H
#define MI4IOS6_STAGE75_XNU_OBJECT_SUBSET_GENERATED_H

#define STAGE75_XNU_OBJ_HOST_STATUS                 0x$(printf '%08x' "$status")u
#define STAGE75_XNU_OBJ_HOST_REQUIRED_MASK          0x07ffffffu
#define STAGE75_XNU_OBJ_HOST_SATISFIED_MASK         0x$(printf '%08x' "$satisfied")u
#define STAGE75_XNU_OBJ_HOST_FAILURE_MASK           0x$(printf '%08x' "$failure")u
#define STAGE75_XNU_OBJ_HOST_SOURCE_MASK            0x0000001fu
#define STAGE75_XNU_OBJ_HOST_SHIM_MASK              0x000003ffu
#define STAGE75_XNU_OBJ_HOST_OBJECT_COUNT           0x$(printf '%08x' "$object_count")u
#define STAGE75_XNU_OBJ_HOST_PUBLIC_2050_OBJECT_COUNT 0x00000003u
#define STAGE75_XNU_OBJ_HOST_PUBLIC_ARM_PEXPERT_OBJECT_COUNT 0x00000002u
#define STAGE75_XNU_OBJ_HOST_STAGE_OWNED_SHIM_OBJECT_COUNT 0x00000001u
#define STAGE75_XNU_OBJ_HOST_DUPLICATE_SYMBOL_COUNT 0x$(printf '%08x' "$duplicate_count")u
#define STAGE75_XNU_OBJ_HOST_PE_STATE_ABI_SHIM_READY 1u
#define STAGE75_XNU_OBJ_HOST_CONSISTENT_DEBUG_ABI_SHIM_READY 1u
#define STAGE75_XNU_OBJ_HOST_BASELINE_COMMIT32      0xcc8a9b0cu
#define STAGE75_XNU_OBJ_HOST_MASTER_VERSION_12_3_0  0x000c0300u
#define STAGE75_XNU_OBJ_HOST_DEVICE_TREE_BYTES      0x$(printf '%08x' "$device_tree_bytes")u
#define STAGE75_XNU_OBJ_HOST_BOOTARGS_BYTES         0x$(printf '%08x' "$bootargs_bytes")u
#define STAGE75_XNU_OBJ_HOST_PE_GEN_BYTES           0x$(printf '%08x' "$pe_gen_bytes")u
#define STAGE75_XNU_OBJ_HOST_ARM_PE_BOOTARGS_BYTES  0x$(printf '%08x' "$arm_pe_bootargs_bytes")u
#define STAGE75_XNU_OBJ_HOST_ARM_PE_CONSISTENT_DEBUG_BYTES 0x$(printf '%08x' "$arm_pe_consistent_debug_bytes")u
#define STAGE75_XNU_OBJ_HOST_DEVICE_TREE_SHA32      ${device_tree_sha32}u
#define STAGE75_XNU_OBJ_HOST_BOOTARGS_SHA32         ${bootargs_sha32}u
#define STAGE75_XNU_OBJ_HOST_PE_GEN_SHA32           ${pe_gen_sha32}u
#define STAGE75_XNU_OBJ_HOST_ARM_PE_BOOTARGS_SHA32  ${arm_pe_bootargs_sha32}u
#define STAGE75_XNU_OBJ_HOST_ARM_PE_CONSISTENT_DEBUG_SHA32 ${arm_pe_consistent_debug_sha32}u
#define STAGE75_XNU_OBJ_HOST_PUBLIC_2050_BASELINE   1u
#define STAGE75_XNU_OBJ_HOST_PUBLIC_ONLY            1u
#define STAGE75_XNU_OBJ_HOST_NO_FULL_XNU_BUILD      1u
#define STAGE75_XNU_OBJ_HOST_NO_MACHO_LINK          1u
#define STAGE75_XNU_OBJ_HOST_NO_PUBLIC_XNU_EXEC     1u
#define STAGE75_XNU_OBJ_HOST_NO_PLATFORM_RUNTIME_EXEC 1u
#define STAGE75_XNU_OBJ_HOST_NO_EXTERNAL_MUTATION   1u
#define STAGE75_XNU_OBJ_HOST_OUTPUTS_IGNORED        1u
#define STAGE75_XNU_OBJ_HOST_FAIL_CLOSED            1u
