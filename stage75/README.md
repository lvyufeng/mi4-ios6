# Stage75 — XNU IOKit client-open / provider-claim / close-readiness dry-run contract
...
- the Stage75 provider-callback / client-notification readiness dry-run contract,
- the Stage75 provider-notification / interest / delivery-readiness dry-run contract,
- the Stage75 lifecycle/register-service readiness dry-run contract,
- the Stage75 attach/start readiness dry-run contract,
- the Stage75 registry-plane / IODeviceTree topology dry-run contract,
- the Stage75 property-inheritance / registry-entry dry-run contract,
- the Stage75 catalog/property/personality dry-run contract,
- the Stage75 provider-plane dry-run contract,
- the Stage75 broader IOKit registry/service dry-run contract,
- the Stage75 IOKit/platform scaffold contract,
- the Stage75 local IOKit match dry-run contract,
- the Stage75-renumbered pexpert hook readiness contract,
- the retained Stage75 pmap transition dry-run contract,
...
The Apple flattened device tree keeps the Stage66/Stage67 provider/service/candidate nodes and the Stage68/Stage69/Stage70/Stage71/Stage72/Stage73/Stage74 catalog/personality/registry-entry/topology/attach-start/lifecycle/provider-notification/provider-callback nodes:
...
Stage75 does not add new root nodes. The root child count remains 19. It extends the four selected personality nodes from 83 to 100 properties with local client-open / provider-claim / client-close facts:
...
Because the 32-bit loader satisfied mask was already fully consumed, Stage75 does not add a new top-level loader satisfied bit. Instead, the client-open / provider-claim / close-readiness dry-run contract is copied into loader preflight and logged as a roll-up. Final success still requires:
...
`stage75/xnu_compile_graph_scan.py` classifies twenty-three public candidates. It allows only the five bounded pexpert sources in the public object subset, records three public IOKit files as reference-only, and blocks public IOKit runtime execution.

Local graph/object/link validation reported:

```text
stage75_xnu_compile_graph_status=0x75000001
stage75_xnu_compile_graph_required_mask=0x7fffffff
...
stage75_xnu_compile_graph_iokit_reference_mask=0x00000007
stage75_xnu_compile_graph_iokit_runtime_blocked_mask=0x00000007
