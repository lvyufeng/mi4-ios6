...
int apple_dt_selftest_and_log(const void *dt, uint32_t len)
{
    ...
    if (len < sizeof(struct dt_node_hdr)) {
        log_puts("MI4IOS6_STAGE34 apple_dt too small\n");
        return 0;
    }
    ...
    if (rh->nProperties == 0 || rh->nChildren < 5) {
        log_puts("MI4IOS6_STAGE34 apple_dt root invalid\n");
        return 0;
    }

    const uint8_t *after = skip_node(root, end);
    if (after != end) {
        log_puts("MI4IOS6_STAGE34 apple_dt walk length mismatch\n");
        return 0;
    }
    ...
    if (ok) {
        log_puts("MI4IOS6_STAGE34 apple_dt selftest ok\n");
    } else {
        log_puts("MI4IOS6_STAGE34 apple_dt selftest failed\n");
    }
