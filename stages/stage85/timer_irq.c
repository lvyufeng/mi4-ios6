...
static struct stage85_timer_irq_state {
	uint32_t cntfrq;
...
void stage85_timer_irq_init(uint32_t interval_us)
{
	uint32_t cntfrq;
	uint32_t ticks;
	uint32_t ctl;
	...
	g_timer_irq_state.init_status = STAGE85_STATUS_OK;
}

void stage85_timer_irq_clear(void)
{
	/* Re-arm timer for next interval */
	write_cntv_tval(g_timer_irq_state.ticks_requested);
}

void stage85_timer_irq_disable(void)
{
	uint32_t ctl = 0;  /* ENABLE=0, IMASK=0 */
	write_cntv_ctl(ctl);
}

const struct stage85_timer_irq_state *stage85_timer_irq_state(void)
{
