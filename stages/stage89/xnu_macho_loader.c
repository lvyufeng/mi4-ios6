...
static struct stage89_xnu_macho_loader_result g_result;
...
static uint32_t stage89_xnu_macho_loader_checksum(
    volatile const struct stage89_xnu_macho_loader_result *r)
{
	volatile const uint32_t *words = (volatile const uint32_t *)r;
	uint32_t count = (uint32_t)(offsetof(struct stage89_xnu_macho_loader_result, checksum) / sizeof(uint32_t));
	uint32_t chk = 0;
...
/* Helper: log result */
static void stage89_xnu_macho_loader_log(
    volatile const struct stage89_xnu_macho_loader_result *r)
{
	xnu_log_puts("stage89_xnu_macho_loader result:\n");
	xnu_log_kv32("stage89_xnu_macho_loader_version", r->version);
	xnu_log_kv32("stage89_xnu_macho_loader_status", r->status);
	xnu_log_kv32("stage89_xnu_macho_loader_macho_base", r->macho_base);
	xnu_log_kv32("stage89_xnu_macho_loader_macho_size", r->macho_size);
	xnu_log_kv32("stage89_xnu_macho_loader_text_segment_va", r->text_segment_va);
	xnu_log_kv32("stage89_xnu_macho_loader_text_segment_size", r->text_segment_size);
	xnu_log_kv32("stage89_xnu_macho_loader_data_segment_va", r->data_segment_va);
	xnu_log_kv32("stage89_xnu_macho_loader_data_segment_size", r->data_segment_size);
	xnu_log_kv32("stage89_xnu_macho_loader_linkedit_segment_va", r->linkedit_segment_va);
	xnu_log_kv32("stage89_xnu_macho_loader_segments_loaded", r->segments_loaded);
	xnu_log_kv32("stage89_xnu_macho_loader_entry_point_offset", r->entry_point_offset);
	xnu_log_kv32("stage89_xnu_macho_loader_xnu_entry_va", r->xnu_entry_va);
	xnu_log_kv32("stage89_xnu_macho_loader_ready_for_handoff", r->ready_for_handoff);
	xnu_log_kv32("stage89_xnu_macho_loader_checksum", r->checksum);
}
...
static int load_segment(struct segment_command *seg, uint8_t *macho_base,
                        struct stage89_xnu_macho_loader_result *r)
{
...
int stage89_xnu_macho_loader_run(
    struct boot_args *args,
    struct stage89_xnu_entry_stub_result *entry_result)
{
	struct stage89_xnu_macho_loader_result *r = &g_result;
	...
	/* Initialize result */
	memset(r, 0, sizeof(*r));
	r->version = STAGE89_XNU_MACHO_LOADER_VERSION;
	r->size = sizeof(*r);
	r->required_mask = STAGE89_XNU_MACHO_LOADER_REQUIRED_MASK;
	...
	macho_base = (uint8_t *)stage89_embedded_macho;
	r->macho_base = (uint32_t)(uintptr_t)macho_base;
	r->macho_size = 4096;  /* Fixture is approximately 4KB */

	xnu_log_puts("stage89_xnu_macho_loader: loading Mach-O kernel\n");
	...
	if (mh->magic != 0xfeedface) {
		...
		r->failure_mask |= STAGE89_XNU_MACHO_LOADER_FAIL_INVALID_MACHO;
		goto finish;
	}

	r->satisfied_mask |= STAGE89_XNU_MACHO_LOADER_SAT_MACHO_PARSED;
	...
	if (!found_text) {
		...
		r->failure_mask |= STAGE89_XNU_MACHO_LOADER_FAIL_NO_TEXT;
		goto finish;
	}

	r->satisfied_mask |= STAGE89_XNU_MACHO_LOADER_SAT_SEGMENTS_LOADED;
	...
	r->satisfied_mask |= STAGE89_XNU_MACHO_LOADER_SAT_ENTRY_RESOLVED;

	/* Prepare for handoff */
	r->boot_args_ptr = (uint32_t)(uintptr_t)args;
	r->ready_for_handoff = 1;
	r->satisfied_mask |= STAGE89_XNU_MACHO_LOADER_SAT_READY_FOR_HANDOFF;
	...
	xnu_log_puts("stage89_xnu_macho_loader: ready for handoff (NOT jumping yet)\n");
...
finish:
	...
	/* Compute checksum */
	r->checksum = stage89_xnu_macho_loader_checksum(r);

	/* Log result */
	stage89_xnu_macho_loader_log(r);

	return r->status == STAGE89_STATUS_OK ? 0 : -1;
}

const struct stage89_xnu_macho_loader_result *
stage89_xnu_macho_loader_result(void)
{
	return &g_result;
}
