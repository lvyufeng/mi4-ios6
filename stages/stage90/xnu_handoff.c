    ...
    struct stage90_xnu_entry_stub_result *entry_result)
{
	...
	/* Validate code at the Stage-owned high-VA target (read first 4 instructions). */
	...
	/* THE JUMP - IRQs are enabled (arm opened them). This target should not return. */
	target_start = (handoff_entry_t)(uintptr_t)r->stage_target_high_va;
	r->jumped = 1;
	target_start(args);

	/* If we reach here, the target returned (shouldn't happen). */
