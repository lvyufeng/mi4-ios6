...
Ten files were deleted from this directory in `experiment-120`, and the reason is worth stating
before the history below, because that history is written as though they were missing:
...
`build_xnu_arm_layer.sh` reported **0 of 32** after the deletion until the generated roots were
added to its include line, which is the same fact from the other side: those headers were load
bearing, they just were not this directory's to supply.
