...
case "$(value_of STAGE90_XNU_ENTRY)" in
  ...
  *)
    ...
    echo "                 same panic returned, with the device recovering on its own. The next"
    echo "                 arm has to make the two stacks different memory at the same time -"
    echo "                 __wrap_Idle_load_context is the hook (experiment 518 section 5)."
    echo "                 The image this preflight is being run against is 32a513bc... (518's"
    echo "                 arm: istack separation on, the exit's PoC flush off), whose run is"
    echo "                 already in the log; out/stage90/stage90-qcdt.img says which."
    echo
