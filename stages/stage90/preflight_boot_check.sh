...
case "$(value_of STAGE90_XNU_ENTRY)" in
  ...
  *)
    ...
    echo "                 **That arm is built (521) and this is the run of it.** Four things"
    echo "                 to read in the log, and the first is the verdict: (1) no"
    echo "                 xnu_live_sleh_storm growth past its last pre-521 value and no"
    echo "                 'sleh_abort' panic at all - the idle exit retires its own"
    echo "                 epilogue; (2) xnu_live_slot_post_calls >= 1, which says the exit"
    echo "                 *returned* through the wrapper (520's was 0 - its pass died inside"
    echo "                 the call), with the post reading showing {fp, lr} where 520's"
    echo "                 pre reading showed the deadline; (3) xnu_live_slot_pre_m4/m8 are"
    echo "                 the slot's own words now rather than entry_slot_note's saved"
    echo "                 registers (520 read 0x8047c974 and 0x80553520 there - its own"
    echo "                 frame); (4) xnu_live_slot_ab_* is no longer all zeros: the abort"
    echo "                 site publishes every abort up to 64 and the guard's window is the"
    echo "                 kernel map's own, so the ordinary aborts on 0xc8xxxxxx stacks are"
    echo "                 read rather than refused. A run that dies the same way as 520's"
    echo "                 means the flush is not sufficient and the next step is the L2"
    echo "                 geometry (FlushPoC_Dcache is two loops - check both ran) or a"
    echo "                 write-back the exit does itself between the push and the pop."
    echo "                 The image this preflight is being"
    echo "                 run against is"
