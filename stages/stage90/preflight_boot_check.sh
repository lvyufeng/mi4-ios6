...
case "$(value_of STAGE90_XNU_ENTRY)" in
  ...
  *)
    ...
    echo "                 Read two things: xnu_live_idlestack_sp inside"
    echo "                 [stage90_idle_stack, +16384) with xnu_live_idlestack_inwin = 0 (the"
    echo "                 arm worked), and whether the run still ends in 'sleh_abort at"
    echo "                 interrupt context' (the collision) or in the diagnosed 'prefetch abort"
    echo "                 in kernel mode' (something else writes the idle code's saved return"
    echo "                 address). The image this preflight is being run against is"
    echo "                 40bf8a0b... (519's arm: the idle thread's own stack on, istack"
    echo "                 separation off, the exit's PoC flush off);"
    echo "                 out/stage90/stage90-qcdt.img says which, and this gate prints it."
    echo
