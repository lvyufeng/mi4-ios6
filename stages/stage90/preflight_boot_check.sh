...
case "$(value_of STAGE90_XNU_ENTRY)" in
  ...
  *)
    ...
    echo "                 A candidate mechanism exists (experiment 518, host-side only): the"
    echo "                 interrupt handler's stack starts at the top of the interrupt stack,"
    echo "                 which is where the boot and the idle loop are running, so its spills"
