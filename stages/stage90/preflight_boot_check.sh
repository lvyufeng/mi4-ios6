...
case "$(value_of STAGE90_XNU_ENTRY)" in
  ...
  *)
    ...
    echo "                 WATCHED, not reasoned about. So this image is still the arm to"
    echo "                 run and a re-run re-reads the same two verdicts; the next one"
    echo "                 (520) publishes the two words of the exit's saved {fp, lr} slot,"
    echo "                 before and after its call and at the abort, plus"
    echo "                 __wrap_ml_get_timebase's own readings and rtcPop. The image this"
    echo "                 preflight is being run against is"
