...
the difference between a list and a plan. Four outcomes, and they need different work:
  ...
  * **in the manifest but never compiled** — the build's `failed.txt` is not enough to decide this.
    It holds only the files that *failed*, and `build_xnu_arm_kernel.sh` **skips every `.s` and
    `.cpp`**, so an assembly file that cannot assemble is neither compiled nor failed and looks
    "fine" to anything keyed on that list. A first version of this tool made exactly that mistake
    and reported 30 boot-path symbols as "defined by a compiled file - should not be a stub",
    including `machine_routines_asm.s`, which does not assemble. The test here is **whether an
    object exists for the file**, which is the same question the build answers.
        ...
        for p in (os.path.join(XNU, comp),):
            for root, _d, files in os.walk(p):
                if any(x in root for x in ("/i386", "/x86_64", "/arm64")):
                    continue
                for name in files:
                    if not name.endswith((".c", ".s", ".S", ".cpp")):
                        continue
