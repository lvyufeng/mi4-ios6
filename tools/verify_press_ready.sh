    ...
    'the window is reachable EXACTLY ONCE in this image.'*)
      ...
      case $seam in
        poc)
          ...
          conseq="this press's log carries the seam pair, and on THIS arm the pair reads the operation's own cells (run_and_capture.sh:2069-2088) - and the EXPECTED row is STALE LINE, WRITTEN OUT, i.e. a1 == this log's own rtcpre_pop: the clean writes the whole dirty line out, and the line's word at that offset is already measured to be rtcpre_pop (the 2026-09-24 run read it cacheably where the seam read the pushed lr). CLEAN LINE (a1 == b1) is the falsifier - the operation inert with SCTLR.C clear - and CHANGED (a1 neither) the least likely of the three. **Whether the boot gets PAST the pop is a SEPARATE question the pair does not settle**: the line was invalidated and the words restored, but four instructions sit between the re-enable and the pop (0x8004632c-0x80046338) and two of them are cacheable memory accesses, so 571's reading - the invalidate does not survive that distance - competes with the reading that it does. The frontier reading beside it is slot_post_calls, absent in every capture since 520" ;;
        measure)
