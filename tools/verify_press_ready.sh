...
if [[ ! -x $ARM_CHECK ]]; then
  ...
  if [[ $wst == 1 ]]; then
    entry_arm="$entry_arm - **and this is NOT any of those arms either: it is 692's STORAGE arm** (STAGE90_XNU_STORAGE_PROBE=1), which runs that same operation and the same ending, and whose new act is **a device and not a site**: \`__wrap_platform_cache_idle_exit\` calls \`entry_storage_probe()\` on its first return, before the clock's block, and the probe installs **one** 1 MB section for the eMMC controller (\`0xf9824000 >> 20 == 0xf9824900 >> 20 == 0xF98\`, 531's two windows inside one section) and then reads six of its registers"
    entry_conseq+=" **THIS ARM READS A BLOCK THE IMAGE HAS NEVER TOUCHED, SO ITS ANSWER IS A READING AND ITS HAZARD IS A BUS WAIT.** The reads are gated and the gate is published: \`xnu_live_storage_gate\` is \`BIT(0)\` of SDCC1's CBCR (\`0xFC4004C4\`, in a megabyte \`0xFC4\` **and 692 WAS PRESSED, so this is now measured rather than inferred: that megabyte is NOT mapped.** \`xnu_live_sleh_far_frame=0xfc4004c0\` with \`fsr_frame=0x5\` (a section translation fault), \`pc=0x8000d0c4\` = \`entry_storage_probe+0xcc\` = \`ldr r1,[r3,#1216]\` with \`r3=0xfc400000\` (the BCR read, the probe's third line), and the panic's own \`r0=0x80487fc8\` is the pointer to the string \`xnu_live_storage_bcr\`. **The parenthetical here used to read \"in a megabyte \`entry_epilogue\`'s own PS_HOLD store proves mapped\", and that proof took a STORE inside a code path for evidence an ADDRESS is mapped** - the class of \`mi4-a-device-address-can-be-right-and-undereferenceable\`, whose citation is 532, the experiment this arm's plan came from. It was contradicted by the previous arm's own parked log, unjoined until 692: 690's fatal abort is \`pc=0x8047b488\` = \`entry_seam_end_run+0xc\`, the \`str r3,[r2,#0x65c]\` with \`r2=0x0fa00000\` (RESTART_REASON), \`far_frame=0x0fa0065c\` - **the forced ending faulted on its FIRST store** and never reached the \`dsb\`, the PS_HOLD store or the \`wfe\`, so no arm has been observed to complete a PS_HOLD store. **So read this arm's press as the FIRST LINE of the probe and nothing after it**: the five keys \`_calls=1\`, \`_live_state=1\`, \`_section=0x00000f98\`, \`_hc_mem_section=0x00000f98\` and \`_windows_share_section=1\` are real readings (both windows really are in one 1 MB section, so 532 section 6's step 2 is one install), and every cell below - \`_gate\`, \`_map\`, \`_loads\`, \`_writes\`, \`_mode_bit\`, \`_sw_rst\` - is UNREAD on it) and it is read **before** the section is installed, so the three cells are told apart by two keys and not by one - \`_map\` (the install's own return: 0 means one of the mapper's four refusals fired and **no register was then read**, \`_slot_before\`/\`_desc\`/\`_l1\`/\`_l1_moved\` say which), \`_gated_out=1\` with \`_loads=0\` (the section is installed and the block was deliberately not touched), and \`_loads=6\` with \`_writes=0\` (the controller answered and **nothing was written to it**). **\`xnu_live_storage_writes=0\` is the arm's own safety reading and not a comment**: the vendor's own probe opens with three writes (\`CORE_HC_MODE <- 0\`, \`CORE_SW_RST\`, \`HC_MODE_EN\`) and 531 section 8 adds a fourth reason not to - writing 0 to \`POWER_CONTROL 0x29\` **is** a bus-off request - so this arm's answer is bought with loads alone. **The two pre-registered cells are the bits 531 section 6 left open**: \`xnu_live_storage_mode_bit\` (\`CORE_HC_MODE & HC_MODE_EN\`) reads 1 if the block the bootloader handed over is already in SDHCI mode - the mode sequence is then a *re-do* - and 0 if it is still ahead of us, and \`xnu_live_storage_sw_rst\` (\`CORE_POWER & CORE_SW_RST\`) reads 1 if something is holding the core in reset, which would make the version word's answer meaningless rather than absent. A \`_mci_version\` of 0 or 0xffffffff is *the block did not answer*, which is a reading of the same arm and not a failed run. **The negative cell is a NON-RETURN (exit 2) and it is this arm's own hazard, named here rather than discovered**: a load from a block whose clock is off is a bus wait nothing ends, the run does not come back, and it owes a power press - which is why the gate read exists and why the exit-2 cell means \`the gate said the branch was on and the block still did not answer\`, not \`the mapping failed\` (a mapping failure with a live gate is a translation fault and comes back with a log - **and 692 measured WHICH address faults first: the probe reads the GATE before it installs anything, so the first fault is the gate's megabyte (\`0xfc4004c0\`) and not the controller's \`0xf9824000\`. This cell named the wrong site while getting the shape right.** THE NEXT ARM'S FIRST CHANGE IS ONE MORE MAPPING: \`entry_mmio_section(0xfc400000, 0xfc400000, ...)\` before the gate is read, at a different L1 index (\`0xFC4\`) from the \`0xF98\` install, so 532 section 3.2's occupied-slot refusal cannot fire and the two calls cannot collide - and the same question then applies to the reset path's own two stores (\`0x0fa0065c\`, \`0xfc4ab000\`)**). **The ending is 690's clock and it did not move**, so \`xnu_live_post_end_calls\` is still this arm's own ending cell and every consequence above about the elapsed pair holds unchanged; the probe's records precede it because the call is placed before the clock's block."
  elif [[ $wse == 1 ]]; then
