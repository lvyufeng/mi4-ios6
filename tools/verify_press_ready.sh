...
if [[ ! -x $ARM_CHECK ]]; then
  ...
  # the rung's own value, the values this file has narration for are `1`, `2` and `3`, and a rung it has
  # none for is refused **by the row** (the narration must quote the rung the record carries) instead of
  # being answered by falling into another arm's sentence.
  if [[ $wst =~ ^[1-9][0-9]*$ ]]; then
    ...
    if [[ $wst == 2 || $wst == 3 || $wst == 4 || $wst == 5 ]]; then
  ...
  if [[ $vline == 'the window is reachable EXACTLY ONCE in this image.'* && -z $sub_arm ]]; then
    if [[ -n $wst && $wst != 0 && $arm != *"STAGE90_XNU_STORAGE_PROBE=$wst"* ]]; then
      ...
      nar_miss=$(printf '%s\n' "$nar_tok" | while read -r _t; do
                   [[ -n $_t ]] || continue
                   printf '%s\n' "$nar_glob" | grep -qx -- "$_t" && continue
