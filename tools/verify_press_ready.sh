...
# **A RUNG'S CONSEQUENCE PARAGRAPH IS A STATEMENT AND NOT A BRANCH, AND THAT IS THE 2026-09-26 REPAIR.**
# Every one of them was written as `if [[ $wst == N ]]; then entry_conseq+="..." fi`, and each new rung's
# paragraph was anchored on the previous rung's - so the rung-10 (ff753a23), rung-11 (1c4d0d50) and
# rung-14 (87fda503) paragraphs were written INSIDE the `sw_on STAGE90_HW_WATCHDOG_SELFTEST` branch,
# where no storage arm can reach them: **three consecutive rungs narrated no consequence at all, and the
# narration said nothing about the absence** - the shape `[[mi4-silence-is-a-reading-only-if-success-is-
# silent]]` names, one level up from a key. Nothing caught it because every other check this row makes is
# about the ARM's NAME, which was right in all three cases and still right, and because a paragraph that
# never runs looks exactly like a paragraph that had nothing to add.
#
# `rung_para N "..."` is ONE statement carrying the rung's value ONCE - the guard and the record are the
# same occurrence, so the two cannot drift apart the way a value in a comment and a branch beside it can -
# and every call sits at the storage ladder's own level, where a new paragraph cannot inherit a nesting
# by being anchored on its neighbour. **`RUNG_HIT` is what makes placement a READING**: the row below
# parses this file for the rungs that have a paragraph and refuses when the rung the arm carries is one
# of them and no call recorded it, which is what a misplaced paragraph looks like from outside.
RUNG_HIT=()
rung_para() {
  [[ $wst == $1 ]] || return 0
  RUNG_HIT+=("$1")
...
if [[ ! -x $ARM_CHECK ]]; then
  ...
  if [[ $wst =~ ^[1-9][0-9]*$ ]]; then
    ...
    if [[ $wst == 2 ]]; then
      ...
      entry_arm="$entry_arm - **and this is NOT any of those arms either: it is 721's STORAGE arm, THE FIRST RUNG IN THIS PROJECT THAT ADDRESSES THE CARD RATHER THAN THE CONTROLLER** (STAGE90_XNU_STORAGE_PROBE=11, resolved set \`${SET:-<unresolved>}\`), which is 718's arm PLUS ONE ACT AND NO VALUE: the same \`st_pwr_wait\` with the same bound ($wpt_txt), the same \`st_power_set()\`, the same clock set, the same vendor mode sequence and the same read-only probe - and then **\`sdhci_send_command\` (\`sdhci.c:1076-1155\`) ported for the driver's own FIRST TWO DATA-LESS COMMANDS**. **The commands are the driver's own and so are their arguments and their order**: \`mmc_go_idle\` (\`mmc_ops.c:96-127\`) is \`MMC_GO_IDLE_STATE\` with argument 0 and \`MMC_RSP_NONE\`, and \`mmc_attach_mmc\` (\`mmc.c:1913-1923\`) opens with \`mmc_send_op_cond(host, 0, &ocr)\` - argument 0, the driver's own single-pass probe form (\`mmc_ops.c:139-141\`) - with \`MMC_RSP_R3\` = \`MMC_RSP_PRESENT\` (\`core.h:54\`), a 48-bit response with no CRC and no opcode check; \`mmc.c:1356-1359\` is where one function does them back to back (\"\`mmc_go_idle\` is needed for eMMC that are asleep\"). The two words written to \`SDHCI_COMMAND 0x0E\` are therefore **\`0x0000\`** and **\`0x0102\`** (\`sdhci.h:57\`'s \`MAKE_CMD\` over \`sdhci.c:1130-1140\`'s decode), and what is NOT ported is \`mmc_rescan_try_freq\`'s sequence: between its \`mmc_go_idle\` and the MMC attach are CMD8 and the SDIO and SD attach attempts (\`core.c:3104-3111\`), three probes an eMMC answers with timeouts by design. **THE ONE SUBSTITUTION IS RUNG 9'S, AND HERE IT IS ALSO A SAFETY RULE**: the driver's completion arrives through \`sdhci_irq\`, and this image enables no SDHCI interrupt, so the block becomes a BOUNDED poll of \`SDHCI_INT_STATUS\`'s own \`SDHCI_INT_RESPONSE\` bit - because the block's \`hc_irq\` is **SPI 123 -> intid 155, a line this image hands to nobody**, and a delivery would reach the dispatcher as \`_irq_other_count\` and END THE RUN (\`entry_irq.c:549-556\`, which is exactly what 709's rung-7 press measured on the \`pwr_irq\` line before rung 8 owned it). **THE THREE GATES, EACH OF WHICH REFUSES RATHER THAN PROCEEDS**: (a) \`INT_ENABLE 0x34\` and \`SIGNAL_ENABLE 0x38\` must BOTH read zero before anything is written - they are the only thing that can let the block raise that line, so they are the gate and not a comment (\`_cmd_enabled_out\`, \`_cmd_refused\`, \`_cmd_sent=0\`); (b) \`PRESENT_STATE\`'s \`SDHCI_CMD_INHIBIT\` must clear inside the driver's own 10 ms (\`sdhci.c:1084\`, \`:1096\`) - issuing a command into a controller still holding one is the one act here that could put a SECOND transaction on the bus (\`_cmdN_inhibit_timeout\`); and (c) \`INT_STATUS\`'s command bits are READ, WRITTEN STRAIGHT BACK (write-1-to-clear) and READ AGAIN - and this is what makes the poll a reading at all, because the bit LATCHES: without the clear, CMD1's poll would be satisfied by CMD0's own completion and the arm would report an answer nobody read (\`_cmdN_stale\`, \`_cmdN_clear_after\`, and the body order \`clear -> ARGUMENT -> COMMAND\` is asserted by the build). **THE CELL THIS PRESS IS FOR IS \`_cmd1_resp\`** - the R3 response word out of \`RESPONSE 0x10\`, which \`sdhci_finish_command\` reads only when the command asked for one (\`sdhci.c:1169-1175\`), i.e. NEVER on this ladder before this arm. It is pre-registered as a SHAPE and not a value, because the OCR is the card's to state: bit 30 set with a non-zero voltage window in bits 23:15 is the form Linux logs for an eMMC that has answered, and \`0x40ff8080\` / \`0xc0ff8080\` are the two reference words. \`_cmd0_resp\` does not exist and \`_cmd0_rsp_present\` reads 0 - CMD0 asked for no response, and that absence is the control for CMD1's. **THE ONE BIT THAT SEPARATES THE TWO FAILURES IS IN \`_cmd1_err\`**: \`0x00010000\` (\`SDHCI_INT_TIMEOUT\`) is \"the command went out and the card did not answer\" - a finding about the CARD, the first this line has ever had - while any other bit is about the controller. **AND THE WRITE SET IS THE SAFETY CONTRACT**: this rung adds THREE STORES PER CALL and nothing else - \`INT_STATUS 0x30\` (the write-1-to-clear), \`ARGUMENT 0x08\` (\`sdhci.c:1119\`) and \`COMMAND 0x0E\` (\`:1153\`) - all in \`hc_mem\`, all inside \`st_send_command\`'s own \`noinline\` body, which the build asserts offset by offset and width by width; \`TRANSFER_MODE 0x0C\` is NOT written (\`sdhci_set_transfer_mode\` returns on its first line for a data-less command, \`sdhci.c:985-986\`), no data-path register is touched, \`POWER_CONTROL 0x29\` stays out of the linked image (\`hc_mem\` is still \`47 44 44 41\`), the GCC and \`core_mem\` store sets are rung 6's unchanged, and **no byte of the medium can change**: CMD0 and CMD1 are \`bc\`/\`bcr\` commands with no data phase, so the controller has no transfer to run. The command path is TWO symbols whose bodies are checked as a pair - \`st_send_command\` holds every device WRITE, and \`st_cmd_path\` is asserted to make NONE (its accesses are \`INT_ENABLE\`, \`SIGNAL_ENABLE\` and \`PRESENT_STATE\`, reads only) - and \`st_cmd_path\` calls it exactly TWICE. The pre-registration is \`docs/experiments/experiment-721-the-rung-12-pre-registration-the-drivers-first-command-and-the-cards-first-answer.md\`, whose section 5 cell table carries the expected values the paragraph after this one reads against"
    elif [[ $wst == 12 ]]; then
      entry_arm="$entry_arm - **and this is NOT any of those arms either: it is 724's STORAGE arm, THE FIRST RUNG IN THIS PROJECT THAT READS THE REGISTER STATE AT THE INSTANT OF THE COMMAND AND THE COMMAND'S OWN RETURN PATH** (STAGE90_XNU_STORAGE_PROBE=12, resolved set \`${SET:-<unresolved>}\`), which is 721's arm - the same \`st_pwr_wait\` with the same bound ($wpt_txt), the same \`st_power_set()\`, the same clock set and the same vendor mode sequence - PLUS **ONE READ-ONLY BODY AND THE COMMAND FUNCTION'S OWN RETURN PATH - AND NO NEW STORE ANYWHERE**, which is the whole safety argument of this rung: the \`hc_mem\` write set this ladder has asserted since 701 is UNCHANGED at \`47 44 44 41\`, \`st_send_command\`'s three stores are rung 11's in rung 11's order, and the new census is asserted by the build to make NONE. **THE CENSUS IS THE RUNG'S ANSWER TO ITS OWN PREDECESSOR'S DEFECT.** 723 section 3 read \`_reg_power_control\` = 0x00 as \"POWER_CONTROL 0x29 reads 0x00, this ladder has never written that byte\" - and both cells it cited are rung 3's census (\`entry_storage.c:630\`) and rung 4's reset tail (\`:783\`), BOTH READ BEFORE RUNG 7 RUNS; the pair that describes the register now is \`_pwr_before\` 0x00 -> \`_pwr_wrote\` 0x0b -> \`_pwr_after\` **0x0b**, so the store lands and holds, and the write set 723 quoted as proof of that store's unreachability CONTAINS \`0x29\` (corrected in 723 as a dated section 5, and recorded as m732/m733). So \`st_cmd_census\` - \`noinline\`, \`noclone\`, eleven locals, **ten device reads and no store** - re-takes at the command's OWN MOMENT what earlier rungs read at theirs: \`POWER_CONTROL 0x29\` as a BYTE, \`CLOCK_CONTROL 0x2C\` as a HALFWORD with its three bits published separately, \`PRESENT_STATE 0x24\`, \`INT_ENABLE 0x34\` and \`SIGNAL_ENABLE 0x38\`, and the GCC's \`SDCC1_BCR 0x04C0\`, apps branch \`0x04C4\`, AHB branch \`0x04C8\` and the apps root's \`CMD_RCGR 0x04D0\`/\`CFG_RCGR 0x04D4\` (the two RCG words read RAW and also decoded into \`_cmd2_rcg_root_en\`/\`_root_status\`/\`_update\`/\`_src\`/\`_div\`/\`_mnd_mode\`). **Its one refusal is new and it is 531 section 8 read in the safe direction**: \`POWER_CONTROL\`'s bus-power bit must still be SET, or the command is not issued at all (\`_cmd2_ok\`, \`_cmd2_refused_power\`) - a block that is not driving the bus must not be sent a command, and the arm refuses rather than writes. **THE RETURN PATH IS THE SECOND HALF AND IT IS WHAT MAKES THIS A MEASUREMENT RATHER THAN ANOTHER ATTEMPT.** Rung 11 polled \`SDHCI_INT_STATUS\`'s \`SDHCI_INT_RESPONSE\` bit alone and read zero for 1.2 s, which cannot distinguish four states; this rung reads, inside \`st_send_command\` and in the same three-store write set, (1) **\`COMMAND 0x0E\` READ BACK** as a halfword immediately after the store (\`_cmdN_word_read\`) - the block's own copy of the word, which the driver never needs because it has an interrupt and this arm does not; (2) **\`PRESENT_STATE\`'s \`SDHCI_CMD_INHIBIT\` read immediately after the store** (\`_cmdN_inhibit_after\`) and then **sampled over the first 1024 poll iterations only** (\`_cmdN_inhibit_seen\`, \`_cmdN_inhibit_last\`) - one extra load per sample, which is why it is bounded to the start of the transmission where the bit rises and falls; **\`_seen = 0\` is \"the block never started the command\", \`_seen > 0\` with the bit still set is \"in flight and stuck\", and \`_seen > 0\` with the command complete is \"it ran\"**; (3) **the FIRST NON-ZERO \`INT_STATUS\` OF ANY KIND** (\`_cmdN_status_any\`, \`_cmdN_any_polls\`), beside rung 11's narrower poll so that if the block latched something other than \`RESPONSE\` this run says WHICH BIT and at which poll instead of repeating \"nothing latched\"; and (4) **\`RESPONSE 0x10\` read UNCONDITIONALLY** (\`_cmdN_resp_read\` = 1) with \`_cmdN_rsp_present\` still carrying the driver's own condition (\`sdhci.c:1169-1175\`), so that a zero response is a READING rather than the ABSENCE OF A READ, which is what rung 11's \`_cmd0_resp\` was. \`_cmdN_status_after\`, \`_complete\`, \`_err\`, \`_polls\`, \`_ticks\` and \`_timeout\` keep their rung-11 meanings, and the poll's break condition now includes the driver's own second arm (\`sdhci_cmd_irq\`, \`sdhci.c:2867-2876\`, finishes the command on any \`SDHCI_INT_CMD_MASK\` bit). The three gates, the two commands, their two words (\`0x0000\` and \`0x0102\`) and the gate between them are rung 11's UNCHANGED. **THE SEAM CONSTANT MOVED A THIRD TIME AND THE BUILD REFUSED IT**: the rung's ~3.9 KB pushed the entry group past the next page boundary, so the exit's \`bl FlushPoU_Dcache\` is now at \`0x800492d8\` returning to \`0x800492dc\` (\`0x800482dc\` on 708's arm), and \`run_and_capture.sh\`'s \`EXIT_POP_LR_LITERAL\` moved with it. The pre-registration is \`docs/experiments/experiment-724-the-rung-13-pre-registration-the-instant-of-the-command-and-the-return-path.md\`, whose section 5 cell table carries the expected values the paragraph after this one reads against"
    elif [[ $wst == 13 ]]; then
      entry_arm="$entry_arm - **and this is NOT any of those arms either: it is 729's STORAGE arm, THE FIRST RUNG IN THIS PROJECT THAT ASKS THE CONTROLLER WHERE IT REPORTS A COMPLETION AND THE FIRST THAT WRITES AN INTERRUPT-ENABLE REGISTER** (STAGE90_XNU_STORAGE_PROBE=13, resolved set \`${SET:-<unresolved>}\`), which is 724's arm PLUS ONE BODY AND TWO STORES AND NO NEW COMMAND: the same \`st_pwr_wait\` with the same bound ($wpt_txt), the same \`st_power_set()\`, the same clock set, the same vendor mode sequence, the same \`st_cmd_census\` with its one power-bit refusal, and **the same two commands** (CMD0 word \`0x0000\`, then CMD1 word \`0x0102\`) with rung 11's three gates and rung 11's gate between them - all of them rung 11's and rung 12's UNCHANGED. **THE QUESTION IS RUNG 13's OWN AND THIS ARM IS THE ONE IT NAMED.** 724 section 2 listed five hypotheses for an \`INT_STATUS\` that read zero for 1.2 s under a command the block had plainly taken; 726's press refuted all five (the block HOLDS the driver's word, \`_cmd0_inhibit_after=1\`; the command was IN FLIGHT on the CMD line, \`_cmd0_inhibit_seen=0x21b\`; the inhibit RELEASED, \`_cmd0_inhibit_last\` bit 0 clear; the bus was POWERED and the clock root ENABLED at that same instant, from the census) and left a sixth that the first five did not contain: **this controller runs a command to completion and does not set its interrupt-status register.** The SDHCI normal-interrupt path is \`INT_STATUS -> INT_ENABLE -> SIGNAL_ENABLE -> the line\`, so a block built as *the status bit latches only if its enable is set* produces exactly rung 13's log. This arm looks for the completion in the two OTHER status registers the block has and in the word it still holds, and then ASKS FOR IT. **IT RUNS AT THE ONE POINT IN \`st_cmd_path\` THIS MACHINE HAS EVER REACHED, AND THE PLACEMENT IS A READING AND NOT A PLACE ON THE PAGE**: the body is called immediately after CMD0's publishes and BEFORE the gate between the commands, because rung 13's log says \`_cmd_gated=1\` - the gate FIRED, \`st_cmd_path\` returned before CMD1 - and a block placed after the commands would sit on a path no press has taken and buy a log with no \`_int_*\` key in it at all (m720's shape, an absent key with one of its three producers guaranteed by construction). It is called UNCONDITIONALLY, and a reader qualifies \`_int_status_after\` against \`_cmd0_sent\` in the same log rather than trusting an \`if\`. **WHAT IT DOES, IN ORDER, IS THE WHOLE OF THE ARM**: (1) the elsewhere-reads - \`SLOT_INT_STATUS 0xFC\` as a HALFWORD, \`CORE_PWRCTL_STATUS 0xDC\`, \`COMMAND 0x0E\` read back as a HALFWORD, \`PRESENT_STATE 0x24\` - a completion recorded in any of them is a completion rung 11 was reading past; (2) the gate's own state BEFORE it is written (\`_int_enable_before\`, \`_int_sig_enable\`) and the status BASELINE with the enables clear (\`_int_status_before\`); (3) **the one-bit probe**: \`INT_ENABLE 0x34 <- 0x00000001\` (\`SDHCI_INT_RESPONSE\`) with **\`SIGNAL_ENABLE 0x38\` left at ZERO**, then \`INT_STATUS 0x30\` read immediately (\`_int_status_after\`, the rung's answer), then \`INT_ENABLE\` read BACK (\`_int_enable_held\`); and (4) **the restore, published**: \`INT_ENABLE <- 0\` and read back (\`_int_enable_readback\`, with \`_int_enable_restored\` naming the value written). **\`_int_enable_held\` IS NOT DECORATION AND ITS ABSENCE WOULD MAKE THE HEADLINE CELL AMBIGUOUS**: a zero \`_int_status_after\` has two producers - the block's answer (*the enable is not the mask*) and a store that never took (*a software defect*) - and only the block's own copy of the enable separates them, which is the same reason rung 12 added \`_cmdN_word_read\` beside \`_cmdN_word\`. **THE WRITE SET IS THE SAFETY CONTRACT AND IT IS TWO STORES, BOTH TO THE SAME OFFSET**: \`INT_ENABLE 0x34\`, the second of them the restore. No new command, no \`ARGUMENT 0x08\`, no \`COMMAND 0x0E\` write, no \`RESPONSE 0x10\` write, no data-path register, no \`POWER_CONTROL 0x29\` (it is not touched at ALL by this body), no RCG word, no GCC word, no \`core_mem\` write, and no byte of the medium - and \`SIGNAL_ENABLE 0x38\` is READ and NEVER WRITTEN, which is the half of the pair this arm does not touch and the reason the hypothesis is testable without putting the line in play. **THE PRE-REGISTERED FAILURE MODE IS A DIAGNOSIS AND NOT A LOST DEVICE**: the block's \`hc_irq\` is **SPI 123 -> intid 155**, a line this image hands to nobody, and if the block does NOT AND the two enables the line rises and the run ends at the dispatcher as \`_irq_other_count=1\` with \`_irq_other_iar=155\` - **an ending this image already reads and survives** (709's press ended exactly that way on intid 170 and the device came back) and which the payload's own armed watchdog bounds. So on this arm an exit-2/exit-3 cell is not the expected outcome, it is the answer to *are the two enables ANDed*; and \`_int_status_after\` non-zero is the other answer - the completion was there to be seen all along and the enable was the mask. **THE OTHER OUTCOME IS A CELL AND NOT A FAILURE**: \`_int_status_after = 0\` with \`_int_enable_held = 1\` says the enable is NOT the mask and the remaining candidates are the vendor's second path through \`core_mem\` (\`_int_pwrctl_status\`) or the line itself. **AND TWO SENTENCES IN 726 SECTION 3 AND 728 SECTION 2 ARE FALSE, WHICH THIS PARAGRAPH CORRECTS RATHER THAN REPEATS**: both write that \`SLOT_INT_STATUS 0xFC\` is a register *this ladder has never read at all*. It is read - rung 3's \`st_standard_census\` reads it as a halfword and 726's OWN log carries the value, \`xnu_live_storage_reg_slot_int_status=0x00000000\` - and \`CORE_PWRCTL_STATUS 0xDC\` is read by rung 2's mode sequence and rung 8's handler (\`_pwr_irq_status32=0x02\` in the same log). The true statement is stronger and is the one this rung needed: **neither has ever been read at a moment when there was a completion to report**, and rung 3's and rung 8's readings are before-values in the same log as the new cells, so the pair is a comparison rather than a claim. **THE SEAM CONSTANT DID NOT MOVE THIS TIME, AND 728 SECTION 5 PREDICTED IT WOULD**: the rung's entry-side code landed inside the entry group's alignment slack, so the entry bin is \`5552764\` bytes - BYTE-IDENTICAL in size to the pressed rung-13 arm - and the exit's \`bl FlushPoU_Dcache\` is STILL at \`0x800492d8\` returning to \`0x800492dc\` (\`platform_cache_idle_enter\` still \`0x80049238\`), which 724's paragraph's *moved a third time* is not contradicted by: that sentence is about 724's arm and the value it names is this arm's value too. What moved is everything from the new body's address up to \`arm_init\`, and nothing above it. **WHAT THE BUILD ASSERTS ABOUT THIS BODY, AND IT IS MORE THAN THE TWO RUNGS BELOW ASSERT**: the device accesses in program order and distinct, the exact \`<address>:<mnemonic>\` set (\`f98249fc:ldrh\` and \`f982490e:ldrh\` are the two offsets no 32-bit access may reach, and the widths are checked HERE because this body is a \`noinline\` static emitted BEFORE \`entry_storage_probe\` and the width-vs-offset census reads \`[sym_addr entry_storage_probe, next_global)\`), the store list, and **the COUNTS** - \`INT_STATUS 0x30\` read exactly TWICE (the baseline and the answer) and \`INT_ENABLE 0x34\` read exactly THREE times and written exactly TWICE - which rungs 11 and 12 only echo, because a set cannot see a dropped read and these two multiplicities ARE this rung's experiment. The pre-registration is \`docs/experiments/experiment-728-the-rung-14-pre-registration-where-this-controller-reports-a-completion.md\`"
    elif [[ $wst == 14 ]]; then
      # **732: THE FIRST RUNG THAT WRITES AN INTERRUPT-ENABLE REGISTER WHILE A COMMAND IS IN FLIGHT -
      # and not the first to write one at all, which is the rung above this branch.** 729's arm ASKED
      # the controller where it reports a completion, read `INT_STATUS 0x30` as zero over 5,088,256
      # polls, and then wrote `INT_ENABLE 0x34` to 0x00000001 in the body that runs AFTER CMD0 had
      # already completed; 730's press of it measured the pair `_int_status_before = 0x00000000` ->
      # `_int_status_after = 0x00000001` with that one store between them. So the completion was in the
      # block all along, `INT_ENABLE` was the mask, and `_cmd_gated = 1` was the MASKED POLL's answer
      # rather than the block's - but 730's store came too late for the poll it was supposed to un-mask,
      # which is exactly what this rung moves.
      #
      # **AND THIS ARM'S FIRST BUILD GOT THE PLACE WRONG, WHICH IS WHY THIS PARAGRAPH NAMES IT.** The
      # window stood after CMD0 instead of before it, so the poll that decides `c0.complete` (it is
      # INSIDE `st_send_command`) ran masked, the between-commands gate would have refused, and the
      # arm's own promised cell `_cmd_gated = 0` was unreachable - a press spent re-measuring the rung
      # below. It was refuted from 730's own log with no device action, and the build now asserts the
      # order: it splits `st_cmd_path` at its first `bl <st_send_command>` and requires the enable's
      # store in the half before it and no device write but `PRESENT_STATE` in the half after.
      entry_arm="$entry_arm - **and this is NOT any of those arms either: it is 732's STORAGE arm, THE FIRST RUNG IN THIS PROJECT THAT PUTS A COMMAND ON THE BUS WITH THE INTERRUPT ENABLE STANDING, AND THE FIRST THAT WRITES \`INT_ENABLE 0x34\` BEFORE THE COMMAND RATHER THAN AFTER IT** (STAGE90_XNU_STORAGE_PROBE=14, resolved set \`${SET:-<unresolved>}\`), which is 729's arm's whole act - the same two commands with the driver's own arguments and order, the same three gates, the same census, the same \`st_int_report\` body - **plus ONE 32-bit store to \`INT_ENABLE 0x34\` (\`SDHCI_INT_RESPONSE\`, \`0x00000001\`) IMMEDIATELY BEFORE CMD0 IS PUT ON THE BUS, AND ITS RESTORE ON ONE UNCONDITIONAL LINE IMMEDIATELY AFTER CMD0'S PUBLISHES**, with \`SIGNAL_ENABLE 0x38\` still ZERO. **THE WINDOW IS ONE ENTRANCE AND ONE EXIT, BOTH UNCONDITIONAL**, and every branch of the body - the census's refusal and the register half of the gate above the window, the between-commands gate below it - stands outside them, so the enabled interval is exactly CMD0's own send and its poll. **The reason is 730's press and not a preference.** Gate 1 is NARROWED rather than removed: \`SIGNAL_ENABLE 0x38\` must still read ZERO and \`INT_ENABLE 0x34\`'s only permitted bit is RESPONSE, both checked BEFORE anything is written, and the refusal publishes WHICH of the two fired (\`_cmd_gate_kind\`: 1 = \`SIGNAL_ENABLE\` non-zero, 2 = a bit of \`INT_ENABLE\` other than RESPONSE) instead of reporting a boolean. **THE CELL THAT SAYS THE RUNG WORKED IS \`_cmd_gated = 0\` WITH \`_cmd1_*\` KEYS PRESENT, AND THE CELL BESIDE IT IS \`_cmd0_complete = 1\`** - the first command this line drives to a completion, the first CMD1 it ever issues, and the pair that says the enable was the mask; \`_cmd0_status_any\` beside \`_cmd0_any_polls\` then says WHEN the bit appeared (the first sample is a bit the block was already holding, a later one is a block that latched it while the poll ran). **THE PREDICTED VALUES COME OFF 730'S OWN LOG**: \`_ena_wrote\` = 0x00000001, \`_ena_held\` = **0x00008001** and \`_ena_readback\` = **0x00008000**, because 730's press measured its own readback of a 0x00000001 store as 0x00008001 and of a 0x00000000 store as 0x00008000 - bit 15 (\`SDHCI_INT_ERROR\`) is set by a write to 0x34 and is not cleared by one, so this rung's restore is a restore of BIT 0 and its own readback is the cell that says so; \`_ena_restore_seq\` = 1 names the one exit, \`_ena_wrote_back\` = 0 is the value written, and \`_ena_status_post\` is \`INT_STATUS 0x30\` read after the disable (**the cell that separates a status the enable GATES from a status the enable only makes VISIBLE**). **THREE READS 697 AND 730 LEFT OWED COME WITH IT**: \`HOST_VERSION 0xFE\` as a HALFWORD (\`_ena_host_version\` - the offset no 32-bit access may reach, and \`0xFE\` is not 4-aligned), \`INT_STATUS 0x30\` WITH THE ENABLE SET AND **NO COMMAND IN FLIGHT** (\`_ena_status_pre\`, expected 0 here where 730's same read - taken after a command had completed and been cleared - was 1), and that same register after the restore. **AND THE WRITE SET IS THE SAFETY CONTRACT**: this rung adds ONE \`str\` to \`hc_mem\` and nothing else - \`f9824934:str\` occurs exactly TWICE in the linked image, once in \`st_cmd_path\` (the enable, and the build's clause holds that window to exactly one store AND to its place before the first \`bl\` to \`st_send_command\`) and once in the \`noinline\` \`st_cmd_enable_restore\` (the restore, whose call count the build holds to exactly one) - and there is no new command, no \`POWER_CONTROL 0x29\`, no GCC word, no \`core_mem\` word and no byte of the medium. THE WAIT RUNG 9 INTRODUCED IS INHERITED UNCHANGED, AND ITS BOUND IS PART OF THIS ARM'S NAME: $wpt_txt"
    elif [[ $wst == 15 ]]; then
      entry_arm="$entry_arm - **and this is NOT any of those arms either: it is 734's STORAGE arm, THE FIRST RUNG IN THIS LADDER THAT PUTS THE ONE-BIT ENABLE ON A BLOCK THAT HAS NEVER CARRIED A COMMAND - and it is a DISCRIMINATION rather than a new act** (STAGE90_XNU_STORAGE_PROBE=15, resolved set \`${SET:-<unresolved>}\`), which is 732's arm's whole act - the same two commands, the same three gates, the same census and the same bodies, with the enable standing from before CMD0 and closed on the one unconditional line after its publishes - **plus \`st_quiet_enable_probe\`, a body of its own called from \`entry_storage_probe\` immediately BEFORE \`st_cmd_path()\` and after rung 9's wait**, so it runs at the one moment this ladder has never used: the block is in SDHCI mode, powered and clocked, and **no command has been put on its bus in this image's life**. It reads \`INT_STATUS 0x30\`, \`INT_ENABLE 0x34\` and \`SIGNAL_ENABLE 0x38\` and publishes all three, writes \`INT_ENABLE <- (read | SDHCI_INT_RESPONSE)\`, reads \`INT_STATUS\` IMMEDIATELY after that store (\`_quiet_status_after\`, this rung's answer), reads the enable back (\`_quiet_held\`), writes it back to its before-value, then reads \`INT_STATUS\` again (\`_quiet_status_post\`) and the enable again (\`_quiet_readback\`). \`SIGNAL_ENABLE\` is READ and NEVER WRITTEN, which is the one thing 730's press measured is safe; **TWO stores, both to \`0x34\`, the second the undo of the first**; and no new command, no \`POWER_CONTROL 0x29\`, no GCC word, no \`core_mem\` word and no byte of the medium. **THE REASON IS 733's OWN FAILED PREDICTION**: 733 measured \`_int_status_before = 0x00000000\` -> \`_int_status_after = 0x00000001\` across exactly one store to \`0x34\`, while \`_ena_status_post = 0\` and \`_int_status_before = 0\` both read the register CLEAR - the same 0 -> 1 transition 730 read as *the enable revealed a completion*, on a register just read as clear twice. So that pair does not separate **(A)** a write to \`0x34\` that makes \`0x30\`'s RESPONSE bit READ 1 from **(B)** a completion the block was holding and re-latched when the enable returned - 726's sixth hypothesis, which 733 did NOT put down. **(B) needs a completion to have happened, and this arm runs where none ever has**: (B) predicts \`_quiet_status_after = 0\` and (A) predicts \`1\`, so **either reading is an answer**. The counter-evidence that keeps (A) from being assumed is in 733's own log - with the enable standing from before CMD0's send, CMD0's poll read \`0\` for its first 538 samples (\`_cmd0_any_polls = 0x21b\` against \`_cmd0_inhibit_seen = 0x219\`) - and \`_quiet_status_after\` beside \`_cmd0_any_polls\` is the pair that settles it. The build holds the body to its own shape: \`INT_ENABLE\` read three times and written exactly twice, \`INT_STATUS\` read three times, \`SIGNAL_ENABLE\` read and never written, every store in that body at \`f9824934\`, its single call site asserted, and **its call ordered BEFORE \`st_cmd_path\`'s own call in \`entry_storage_probe\`'s disassembly** - the clause that refuses the arm placed after the command path, i.e. one that re-measured the rung below while publishing this rung's cell names. The pre-registration is \`docs/experiments/experiment-734-the-rung-16-pre-registration-the-quiet-block.md\` **AND THE WAIT RUNG 9 INTRODUCED IS INHERITED UNCHANGED HERE, WITH ITS BOUND A PART OF THIS ARM'S NAME: $wpt_txt**"
    elif [[ $wst == 16 ]]; then
      # **737: THE FIRST RUNG WHOSE COMMAND CARRIES A 136-BIT RESPONSE, AND THE FIRST THAT PUTS A
      # RUNG-SHAPED REASON IN THE BUILD INSTEAD OF IN A SENTENCE.** 736's press is what decides the
      # shape: rung 15's own store leaves bit 15 set in `INT_ENABLE 0x34`, no write clears it, and
      # `st_cmd_path`'s gate then read `0x00008000` and refused the whole command path (`_cmd_gate_kind
      # = 2`, `_cmd_sent = 0`). A rung that wants the command path to RUN cannot carry a write to `0x34`
      # above the gate, so **`st_quiet_enable_probe` is compiled for the value 15 and no other** and the
      # build refuses its presence at this rung - the exclusion is asserted, not remembered.
      entry_arm="$entry_arm - **and this is NOT any of those arms either: it is 737's STORAGE arm, THE FIRST RUNG IN THIS PROJECT WHOSE COMMAND CARRIES A 136-BIT RESPONSE AND THE FIRST THAT READS IT THE WAY THE DRIVER READS IT** (STAGE90_XNU_STORAGE_PROBE=16, resolved set \`${SET:-<unresolved>}\`). It is 732's arm's whole act - the same two commands, the same three gates, the same census and the same bodies, with the enable standing from before CMD0 and closed on the one unconditional line after its publishes - **plus \`st_all_send_cid\`, a body of its own inside \`st_cmd_path\` BELOW its between-commands gate**, which opens the SAME one-bit window (\`INT_ENABLE 0x34 <- read | SDHCI_INT_RESPONSE\`, \`SIGNAL_ENABLE 0x38\` still never written), issues \`mmc_all_send_cid\`'s CMD2 (opcode 2, argument 0, \`MMC_RSP_R2\` = PRESENT|136|CRC, so the word \`0x0209\` - \`SDHCI_CMD_RESP_LONG 0x01 | SDHCI_CMD_CRC 0x08\` in this tree's vendor header, which orders those constants the other way from upstream Linux), and reads the 136-bit response THE DRIVER'S OWN WAY - \`resp[i] = readl(RESPONSE + (3-i)*4) << 8\` with \`readb(RESPONSE + (3-i)*4 - 1)\` ORed in except for the last word, which is \`sdhci.c:1164-1172\`'s shift its own comment calls *CRC is stripped*. It closes the window on ONE unconditional line and publishes its restore. **TWO stores, both to \`0x34\`, the second the undo of the first, and the FIRST of them stands before CMD2 is put on the bus** - because the poll that has to see CMD2's completion runs INSIDE \`st_send_command\`, which is 732's first build and 733's press: an enable written after the command measures a masked block. **THE GATE IS THE DRIVER'S OWN AND IT IS READ OFF CMD1's RESPONSE, NOT OFF A STATUS BIT**: \`c1.sent\` set and \`c1.resp & MMC_CARD_BUSY\` clear, because 733's press measured \`_cmd1_complete = 0\` with \`_cmd1_resp = 0x40ff8080\` - the card answered and nothing latched - so a status-bit gate would refuse on every arm this ladder can build. **WHY THE ENABLE HAS TO BE STANDING IS ALSO 733's MEASUREMENT**: CMD0 ran inside the enabled window and completed (\`_cmd0_complete = 1\`), CMD1 ran with it closed and was answered without latching anything (\`_cmd1_complete = 0\` over 5,088,000 polls), so on this controller a command's status bit is latched only while its enable stands. **The whole new surface is the read**: four 32-bit words at \`0x10/0x14/0x18/0x1c\` and three BYTE reads at \`0x13/0x17/0x1b\`, published twice - as \`_cid_raw0\`..\`_cid_raw3\` and as \`_cid_resp0\`..\`_cid_resp3\` - so a reader can see whether the shift moved anything at all, with \`_cid_resp3\` the one word that has no byte read in it. The other cells are the ladder's own: \`_cid_gated\` with \`_cid_gated_reason\` (1 = CMD1 was never sent, 2 = it was sent and the card was still busy - **published on BOTH paths, so a refusal is a reading and not an absent block**), \`_cid_calls\`, \`_cid_sent\`, \`_cid_word\` (the block's own stored copy of the \`0x0209\` above), \`_cid_word_read\` (the block's own copy of that word), \`_cid_complete\`, \`_cid_err\`, \`_cid_timeout\`, \`_cid_status_any\` beside \`_cid_any_polls\`, \`_cid_inhibit_seen\`/\`_cid_inhibit_last\` (which separate NEVER STARTED from RAN from IN FLIGHT), \`_cid_polls\`/\`_cid_ticks\`, \`_cid_clear_after\`, and the window's own four: \`_cid_ena_wrote\`, \`_cid_ena_held\` (\`0x00008001\` is the expected reading - bit 15 rides along with any write to \`0x34\`), \`_cid_wrote_back\` and \`_cid_readback\` (\`0x00008000\`: the restore is a PARTIAL one). \`_cid_sig_enable\` is read INSIDE the window and must be zero, which is the conjunction that would raise this block's SPI 123 -> intid 155, a line nobody here owns. The build holds the body to its own shape: **twelve device accesses and no more** (the enable written twice and read twice, \`SIGNAL_ENABLE\` READ AND NEVER WRITTEN, \`INT_STATUS\` once after the restore, \`PRESENT_STATE\` once at the end, the four words and the three bytes), an EMPTY image side, its single call site, **its call ordered AFTER \`st_cmd_path\`'s last \`bl <st_send_command>\`** (the clause that refuses the arm placed above the gate, which is 732's first build and 736's press), **and the absence of \`st_quiet_enable_probe\` from the image and from the probe's call sites**, which is 736's measurement made structural. The three words this ladder puts on \`SDHCI_COMMAND 0x0E\` - \`0x0000\` for CMD0, \`0x0102\` for CMD1 and \`0x0209\` for CMD2 - are now \`_Static_assert\`ed at compile time in the source, because the build's \`st_send_command\` clause asserts that body's ACCESSES and never the value it stores. The pre-registration is \`docs/experiments/experiment-737-the-rung-17-pre-registration-the-first-136-bit-response.md\` **AND THE WAIT RUNG 9 INTRODUCED IS INHERITED UNCHANGED HERE, WITH ITS BOUND A PART OF THIS ARM'S NAME: $wpt_txt**"
    elif [[ $wst == 17 ]]; then
      # **739: THE LADDER'S FIRST RUNG WHOSE NEW ACT IS A READ AT A MOMENT NO RUNG HAS READ AT, AND
      # THE FIRST WHOSE QUESTION IS ABOUT THE LADDER'S OWN EVIDENCE RATHER THAN ABOUT THE BLOCK.**
      # 738's press is what it comes from and it is a doubt, not a new act: the first 136-bit response
      # read this ladder ever made returned `_cid_resp0 = 0x40ff8080` - bit-for-bit `_cmd1_resp` in the
      # same log, with the four raw words holding that value one byte further along - with no completion
      # latched and no `CMD_INHIBIT` ever seen. So `RESPONSE 0x10` read before ANY command has been on
      # the bus is the one reading that separates *the card wrote it* from *the block's own default for
      # a command that was never taken*, and it is READ-ONLY: no store anywhere, the class of rung 3's
      # census. That is also why it may sit above `st_cmd_path`'s gate where rung 15's store could not -
      # 736 spent a press measuring the cost of a store there.
      entry_arm="$entry_arm - **and this is NOT any of those arms either: it is 739's STORAGE arm, THE FIRST RUNG IN THIS LADDER WHOSE NEW ACT IS A READ AND NOTHING ELSE, AND THE FIRST WHOSE QUESTION IS ABOUT THIS LADDER'S OWN EVIDENCE** (STAGE90_XNU_STORAGE_PROBE=17, resolved set \`${SET:-<unresolved>}\`). It is 737's arm's whole act - the same two commands on the driver's own arguments and order, the same three gates, the same census, the same bodies, and CMD2 carried inside the one-bit enable window with \`SIGNAL_ENABLE 0x38\` never written - **plus \`st_resp_before\`, a READ-ONLY body of its own called from \`entry_storage_probe\` immediately BEFORE \`st_cmd_path()\` and after rung 9's wait**, which is the one moment this ladder has never read \`RESPONSE 0x10\` at: the block is in SDHCI mode, powered, clocked and quiesced, and **no command has ever been put on its bus in this image's life** (the only two bodies in the image that touch that register at any earlier rung read it AFTER a command - \`st_send_command\` and \`st_all_send_cid\` - and rung 3's own census of ten registers does not read it at all). It reads the four words at \`+0x1c/+0x18/+0x14/+0x10\` in the driver's own order (word 3 first) and the three BYTE fills the driver takes below them at \`0x1b/0x17/0x13\`, publishing FOUR RAW WORDS AND FOUR SHIFTED ONES through ONE derivation - the same arithmetic \`st_all_send_cid\` uses after CMD2, so the pre-command words and the post-CMD2 words are comparable one for one in a single log. **\`_rb_resp_zero\` is the arm's answer**, derived in the image from the four raw words: \`1\` says the register was EMPTY before any command, so CMD1's \`0x40ff8080\` was put there by CMD1 and 733's reading survives; \`0\` says the register already held a value with no command in the block's past, and if the four shifted words match the post-CMD2 ones then **the ladder's one piece of evidence that a card exists is retired** and the push from CMD1 must be re-derived from a signal other than \`RESPONSE\`. The body also reads \`PRESENT_STATE 0x24\` (whole, and its \`SDHCI_CMD_INHIBIT\` bit published as \`_rb_inhibit\` - the before-value for 738's discriminator, since \`_cmd0_inhibit_seen = 0x219\` while \`_cmd1_inhibit_seen\` and \`_cid_inhibit_seen\` are both ZERO), \`COMMAND 0x0e\` as a HALFWORD (\`_rb_cmd_word\`, which must read \`0x0000\` if no command has ever carried), and the three interrupt registers \`0x30/0x34/0x38\` - **read and never written, \`SIGNAL_ENABLE\` included, at every rung**. **THE WRITE SET IS EMPTY AND THAT IS THE SAFETY ARGUMENT**: the build's own clause prints this body's store set as EMPTY and refuses any store in it, before its set and count clauses, because **a body that stores nothing cannot move a register \`st_cmd_path\`'s gate reads** - and 736's press is the measurement of what a store above that gate costs (rung 15's write left bit 15 set in \`INT_ENABLE 0x34\`, the gate read \`0x00008000\`, and no command went on the bus). There is no new command class, no data path, no \`POWER_CONTROL 0x29\`, no GCC word, no \`core_mem\` word and no byte of the medium. The build holds the body to twelve device accesses, EVERY ONE OF THEM A READ AND EACH ADDRESS EXACTLY ONCE, in the driver's own program order, with an EMPTY image side, one call site, **and that call ordered BEFORE \`st_cmd_path\`'s own call in \`entry_storage_probe\`'s disassembly** - the clause that refuses the arm placed after the command path, where a non-zero register is a fact about a block that has just answered three commands and \`_rb_resp_zero\` could not mean what its name says. **AND RUNG 15'S BODY IS EXCLUDED HERE FOR 736's REASON, ASSERTED ON THE SYMBOL AND ON THE CALL SITE**: \`st_quiet_enable_probe\` is compiled for the value 15 and no other, so a log from this arm carrying any \`_quiet_*\` key is a log from an arm that is not this one. The pre-registration is \`docs/experiments/experiment-739-the-rung-18-pre-registration-the-response-register-before-any-command.md\` **AND THE WAIT RUNG 9 INTRODUCED IS INHERITED UNCHANGED HERE, WITH ITS BOUND A PART OF THIS ARM'S NAME: $wpt_txt**"
    elif [[ $wst == 18 ]]; then
      # **741: THE LADDER'S FIRST COMMAND WITH A NON-ZERO ARGUMENT AND ITS FIRST `INDEX` BIT, AND THE
      # FIRST WHOSE ANSWER IS A PAIR OF READINGS AT TWO TIMES RATHER THAN A SINGLE CELL.** 740's press
      # is what it comes from and it is an ANSWER rather than a doubt: `_rb_resp_zero = 1` - the
      # response register was EMPTY before any command, so CMD1's `0x40ff8080` was put there by CMD1,
      # 733's reading survives, and 738's doubt is retired. So the driver's own continuation is what is
      # left to walk, and its next statement is `mmc.c:1409`'s `mmc_set_relative_addr`.
      entry_arm="$entry_arm - **and this is NOT any of those arms either: it is 741's STORAGE arm, THE FIRST RUNG IN THIS LADDER WHOSE COMMAND CARRIES A NON-ZERO ARGUMENT, THE FIRST THAT PUTS AN \`INDEX\` BIT IN THE COMMAND WORD, AND THE FIRST WHOSE ANSWER IS A PAIR OF READINGS AT TWO TIMES** (STAGE90_XNU_STORAGE_PROBE=18, resolved set \`${SET:-<unresolved>}\`). It is 739's arm's whole act - the same two commands on the driver's own arguments and order, the same three gates, the same census, the same bodies INCLUDING rung 18's read-only \`st_resp_before\` before any command, and CMD2 carried inside the one-bit enable window with \`SIGNAL_ENABLE 0x38\` never written - **plus \`st_set_relative_addr\`, a body of its own called from \`st_cmd_path\` AFTER rung 17's CMD2 publishes and entered only when CMD2 was SENT**, which is \`mmc.c:1409\`'s \`mmc_set_relative_addr(card)\` transcribed from \`mmc_ops.c:194-210\`: opcode \`MMC_SET_RELATIVE_ADDR\` 3, argument \`card->rca << 16\` with \`card->rca = 1\` assigned eight lines earlier at \`mmc.c:1400\`, so **the argument is \`0x00010000\` - the first NON-ZERO argument this ladder has ever put on the bus** - and flags \`MMC_RSP_R1 | MMC_CMD_AC\` = \`PRESENT|CRC|OPCODE\`, which decodes under THIS TREE'S VENDOR ORDERING to \`RESP_SHORT 0x02 | CRC 0x08 | INDEX 0x10\` and the word **\`0x031A\`**: the first word here that is not \`opcode << 8 | small_flags\`, because an R1 response carries the opcode back and that is why \`MMC_RSP_OPCODE\` is in the flags. It opens the SAME one-bit window rung 17 opens around CMD2 (\`INT_ENABLE 0x34 <- read | SDHCI_INT_RESPONSE\`, on the value the gate read rather than a constant, because 733's press measured that a command's status bit is latched only while its enable stands), and closes it on ONE unconditional line immediately after the publishes, with \`SIGNAL_ENABLE 0x38\` still READ AND NEVER WRITTEN at any rung. **AND ITS OWN NEW CELL IS A PAIR, WHICH IS WHAT RUNG 18's METHOD MAKES POSSIBLE**: \`RESPONSE 0x10\` is read through the DRIVER'S OWN WORD-0 DERIVATION - \`(readl(RESPONSE + 0x1C) << 8) | readb(RESPONSE + 0x1B)\`, the same arithmetic rung 17 uses after CMD2 and rung 18 uses before any command, so the three are ONE derivation at three times and not three that agree - immediately BEFORE the window opens and again immediately AFTER the command's publishes, so \`_rca_resp_pre\` and \`_rca_resp_post\` are one arithmetic taken twice and **\`_rca_resp_moved\` is this rung's answer**: 1 says CMD3 moved the register, 0 says it did not. **\`_rca_resp_is_arg\` names the alternative producer explicitly** - 1 iff the word after CMD3 equals the ARGUMENT \`0x00010000\`, so a register echoing what was written to \`ARGUMENT 0x08\` is a reading this arm publishes rather than one a reader has to think of - and the three R1 fields are published beside the raw word as \`_rca_state\` (\`(resp >> 9) & 0xF\`, \`CURRENT_STATE\`), \`_rca_ready\` (bit 8) and \`_rca_illegal\` (bit 22), with \`_rca_resp\` beside them so no reader has to take a decode on trust: **CMD1's answer \`0x40ff8080\` has an OCR's shape and an R1 CARD STATUS does not, so a word that comes back as \`0x40ff8080\` again is the register not answering CMD3 at all**. **THE INHIBIT TRIPLET IS PUBLISHED FOR CMD3 AS IT IS FOR CMD0, CMD1 AND CMD2** (\`_rca_inhibit_after\`, \`_rca_inhibit_seen\`, \`_rca_inhibit_last\`, the same three cells rung 12 added), so ONE log holds four commands' own inhibit readings side by side - which is 740 section 6's next question made answerable rather than asked, since CMD0's \`_cmd0_inhibit_seen\` was 0x219 while CMD1's and CMD2's were both ZERO. **A \`_rca_gated = 1\` IS A COMPLETE ANSWER AND NOT A LOST PRESS**: CMD2 is the gate and it is the value \`st_all_send_cid\` now RETURNS - the same field it publishes as \`_cid_sent\` in the same log - so the gate and the cell are one reading with two consumers and cannot disagree. **AND IT ADDS NO NEW REGISTER CLASS**: the same \`INT_ENABLE 0x34\` window rung 17 opens and restores, \`RESPONSE 0x10\` read through the word at \`0x1C\` and the byte at \`0x1B\`, \`SIGNAL_ENABLE 0x38\` read and NEVER written, no data-path register, no \`POWER_CONTROL 0x29\`, no GCC word, no \`core_mem\` word and no byte of the medium - and the four commands are the driver's own CMD0, CMD1, CMD2 and CMD3 in \`mmc_attach_mmc\`'s order. The build holds the body to its own shape: the symbol present and sized, its device access set and counts, **its stores exactly the two to \`0x34\` that are the window and its undo**, its empty image side, its FOUR command call sites (\`st_send_command\` called twice from inside it and once from each of rungs 17 and 18's bodies), **its single call site, and that call ordered AFTER \`st_all_send_cid\` and AFTER the last \`st_send_command\`** - the clause that refuses the arm whose CMD3 is placed above the command that must precede it. The three constants this rung transcribes - opcode 3, the R1 flag set and the argument \`0x00010000\` - are \`_Static_assert\`ed at compile time in the source (the word \`0x031A\` and \`1u << 16\`), because the build's \`st_send_command\` clause asserts that body's ACCESSES and never the value it stores; the run's own \`_rca_op\`, \`_rca_flags\` and \`_rca_arg\` cells then say which of them reached the bus. The pre-registration is \`docs/experiments/experiment-741-the-rung-19-pre-registration-the-drivers-own-cmd3.md\` **AND THE WAIT RUNG 9 INTRODUCED IS INHERITED UNCHANGED HERE, WITH ITS BOUND A PART OF THIS ARM'S NAME: $wpt_txt**"
    elif [[ $wst == 19 ]]; then
      # **743: ONE CONSTANT MOVED, AND THIS RUNG'S CELL IS A CONTRAST.** 742's press answered the rung
      # below in three parts - CMD3 was TAKEN (the word and the argument read back out of the block),
      # STARTED (`CMD_INHIBIT` bit 0 held for every sample of the poll) and NEVER FINISHED (no
      # completion, no error, no timeout, nothing latched) - and the ONE command this ladder has ever
      # driven to a completion is CMD0, the one that asks for NOTHING BACK. So this rung is the same
      # command with the response demand removed, which is one constant and not a plan.
      entry_arm="$entry_arm - **and this is NOT any of those arms either: it is 743's STORAGE arm, AND IT IS THE SAME COMMAND AS THE RUNG BELOW WITH ONE CONSTANT MOVED - THE RESPONSE DEMAND REMOVED** (STAGE90_XNU_STORAGE_PROBE=19, resolved set \`${SET:-<unresolved>}\`). It is 742's arm's whole act: opcode 3, the argument \`0x00010000\`, rung 17's one-bit \`INT_ENABLE 0x34\` window and its single unconditional restore, \`SIGNAL_ENABLE 0x38\` READ AND NEVER WRITTEN, no data path, no \`POWER_CONTROL 0x29\`, no GCC word, no \`core_mem\` word, no byte of the medium - and THE SAME GATE, \`st_all_send_cid\`'s returned \`cid_sent\`, so the two rungs' logs are comparable at the same place in the same driver order. **WHAT MOVED IS ONE FLAG WORD**: \`MMC_RSP_R1\` becomes \`MMC_RSP_NONE\` (a literal zero, the ABSENCE of \`MMC_RSP_PRESENT\`), and because every other term \`sdhci_cmd_to_flags\` adds is conditioned on it, the command word goes \`0x031A\` to \`0x0300\` and the ladder's FIRST \`INDEX\` bit does not survive. **AND THE ARM'S OWN CELL IS A CONTRAST AGAINST THE TWO COMMANDS IN LOGS THE PROJECT ALREADY HAS**: read this command's inhibit triplet and completion beside CMD0's (inhibit bit seen in 537 of its 1024 samples, and the command COMPLETED) and beside 742's CMD3 (the bit set in ALL 1024 samples, and no completion) - **that is the one comparison the ladder could not make until this rung existed**, and this file's own log carries neither of those rows. RUNG 19's BODY IS NOT IN THIS IMAGE - \`st_set_relative_addr\` is absent, asserted on the symbol - because the two bodies carry the same opcode and the same argument with a different flag word, and an image with both would put two response-demand variants of one command on the same bus in one boot, making each reading unattributable to its own flag word. **The three R1 decode cells rung 19 publishes are ABSENT here by design**: this command asks for no response, so \`rsp_present\` is zero and the register holds the previous command's word, and decoding that as this command's answer is the stale-word trap in its cheapest form. **AND THE WAIT RUNG 9 INTRODUCED IS INHERITED UNCHANGED HERE, WITH ITS BOUND A PART OF THIS ARM'S NAME: $wpt_txt**"
      entry_conseq+=" **FOUR CELLS ARE THIS RUNG'S OWN, AND THEY ARE THE WHOLE ANSWER.** \`_nrsp_complete\` with \`_nrsp_status_any\` says whether a command that asks for nothing back ever FINISHES: \`_nrsp_complete = 1\` with a non-zero \`_nrsp_status_any\` means the response DEMAND was the stall, and the subject becomes the response path; \`_nrsp_inhibit_seen\` greater than zero beside \`_nrsp_complete = 0\` means the block accepted and started this command and never finished it either, which EXONERATES the demand and moves the census to the registers 726 section 3 named and no rung has read with a completion to report (\`SLOT_INT_STATUS 0xFC\`, \`CORE_PWRCTL_STATUS 0xDC\`); \`_nrsp_inhibit_seen = 0\` with \`_nrsp_word_read\` reading the arm's own word means the block never started it at all. And \`_nrsp_resp_post\` is the register read a THIRD time, through the same driver derivation as \`_nrsp_resp_pre\` and the command object's own word: 742 measured that register going from CMD1's word to ZERO across its CMD3, so a value here that is still CMD1's \`0x40ff8080\` CONFIRMS that the clearing was tied to the response demand - while a zero means the register is cleared by something else and 742 section 1's reading is REFUTED. **The window's cells read as every rung since 13's do**: \`_nrsp_ena_held\` = \`0x8001\` for a store of \`0x00000001\` and \`_nrsp_readback\` = \`0x8000\` for the PARTIAL restore, with \`_nrsp_sig_enable\` reading ZERO and never written. **The failure mode is a diagnosis and not a lost device**: the 1.2 s bound is inside \`st_send_command\`, and a delivery on the block's own line ends the run at the dispatcher, an ending this image already reads and survives (709 ended that way on intid 170)."
    ...
    if [[ $wst == 2 || $wst == 3 || $wst == 4 || $wst == 5 || $wst == 6 || $wst == 7 || $wst == 8 || $wst == 9 || $wst == 10 || $wst == 11 ]]; then
  ...
  if [[ $vline == 'the window is reachable EXACTLY ONCE in this image.'* && -z $sub_arm ]]; then
    ...
    # (3) **AND A RUNG THIS FILE HAS A CONSEQUENCE PARAGRAPH FOR MUST HAVE SAID IT.** The set is read
    #     out of THIS FILE - the `rung_para N` call sites, comments skipped - so a new rung's paragraph
    #     joins this check by being written and no list here can be outgrown by the ladder; and it is
    #     read against `RUNG_HIT`, which is what those calls recorded as they ran. **Measured 2026-09-26
    #     while arming the rung-16 arm**: the rung-10 (ff753a23), rung-11 (1c4d0d50) and rung-14
    #     (87fda503) paragraphs had each been written INSIDE the `sw_on STAGE90_HW_WATCHDOG_SELFTEST`
    #     branch, where no storage arm reaches them, so three consecutive rungs printed a narration with
    #     no reading of their own in it - and this row said `ok`, because every other check it makes is
    #     about the arm's NAME, which was right in all three. A rung with NO paragraph of its own (2, 12,
    #     13) is not required to have one: this asks only that a paragraph that EXISTS be one that RAN.
    #     An empty parse on a rung >= 3 is the extractor failing and not the file having nothing to say -
    #     m728's shape - so it refuses rather than passing on a set it never read.
    if [[ -z $nar_bad ]]; then
      if [[ ! -r $SELF ]]; then
        nar_bad="this row cannot read its own file ($SELF), so whether the rung's consequence paragraph ran is not a reading it can make - and a rung whose reading is unknown is not one this row can name"
      else
        _r_paras=$(awk '/^[[:space:]]*#/ { next }
                        { n = split($0, a, "rung_para ")
                          for (i = 2; i <= n; i++) { v = a[i]
                            if (v ~ /^[0-9]+/) { sub(/[^0-9].*/, "", v); print v } } }' "$SELF" | LC_ALL=C sort)
        _r_dups=$(printf '%s\n' "$_r_paras" | LC_ALL=C uniq -d | grep -c '[0-9]')
        _r_has=$(printf '%s\n' "$_r_paras" | awk -v w="$wst" '$1 == w { n++ } END { print n + 0 }')
        _r_ran=$(printf '%s\n' "${RUNG_HIT[@]:-}" | awk -v w="$wst" '$1 == w { n++ } END { print n + 0 }')
        if [[ -z ${_r_paras//$'\n'/} && $wst =~ ^[0-9]+$ ]] && (( wst >= 3 )); then
          nar_bad="this row read no rung_para call site out of $SELF at all, and every rung from 3 on has a consequence paragraph of its own in that file - so the parse failed, and the narration above is not known to carry this rung's reading"
        elif (( _r_dups > 0 )); then
          nar_bad="$_r_dups rung value(s) are claimed by more than one rung_para call site, so such a rung's paragraph is appended twice: $(printf '%s' "$_r_paras" | LC_ALL=C uniq -d | tr '\n' ' ')"
        elif (( _r_has > 0 && _r_ran == 0 )); then
          nar_bad="the entry record's STAGE90_XNU_STORAGE_PROBE=$wst has a consequence paragraph of its own in this file (rung_para $wst) and NOTHING APPENDED IT: the call is inside a branch this arm does not run, so the reading that rung exists for is missing from the narration above while this row's other checks - all of them about the arm's NAME - are right about it. Measured on the rung-16 arm 2026-09-26, against rungs 10, 11 and 14. **AND THE LIMIT IS NAMED HERE RATHER THAN LEFT TO BE INFERRED, because the neighbouring shape is NOT refused**: a rung with NO \`rung_para\` call site at all passes this row, and that is deliberate - rungs 12 and 13 carry no paragraph of their own and a rule \`every rung must have one\` would be red on master, which is a check nobody reads. So this row refuses a paragraph that was WRITTEN and cannot print, and it does not refuse one that was never written; the step that adds a rung writes its own paragraph, and m744's subject is a claim that named a check which did not exist - not this one, whose limit is stated."
        fi
        # **(4) AND A NARRATION LINE THAT BUILDS A DOUBLE-QUOTED STRING MUST ESCAPE ITS BACKTICKS.** Every
        #     `rung_para N "…"`, `entry_arm="$entry_arm …"` and `entry_conseq+="…"` line is a
        #     double-quoted shell string, so an unescaped backtick in one is a COMMAND SUBSTITUTION: the
        #     names it wraps are executed, their output replaces them, and the sentence the operator
        #     reads prints with a hole where the register it is about belongs - while this row's other
        #     checks all pass, because the NAMES that survive are the names it verifies.
        #     **Measured 2026-09-26, arming the rung-17 arm**: the 736 addendum inside the rung-14
        #     paragraph had five such names, and the press-time print read
        #     `line 829: _cmd_gate_kind: command not found` five times while the row said `ok`. That is
        #     `mi4-measurement-defects`'s m730 - *a backtick inside a double-quoted refusal message is a
        #     command substitution, so the refusal fires and its explanation has a hole* - and it was
        #     reintroduced by the step that wrote a sentence about a refusal.
        #     **The scan is restricted to those three line shapes on purpose**: a backtick inside a
        #     SINGLE-quoted string (this file's own extractor patterns, line ~905) is not a substitution
        #     and must not be refused, so a file-wide scan would be a check with false positives.
        _r_tick=$(awk '
                    { t = $0
                      if (t !~ /^[[:space:]]*(rung_para|entry_arm=|entry_conseq\+=)/) next
                      s = t
                      while (match(s, /`/)) {
                        if (substr(s, RSTART - 1, 1) != "\\") { printf "%d ", FNR; break }
                        s = substr(s, RSTART + 1)
                      }
                    }' "$SELF")
        if [[ -z $nar_bad && -n ${_r_tick// /} ]]; then
          nar_bad="the narration is BUILT on line(s) [${_r_tick% }] with an UNESCAPED backtick, so the text the operator reads is not the text those lines contain: each such backtick is a command substitution, the name it wraps is executed, and the sentence prints with a hole where the name belongs. The other checks in this row cannot see it - they verify the names that SURVIVE. Escape it as \\\`name\\\` (m730)"
        fi
      fi
    fi
