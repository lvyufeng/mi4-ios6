#!/usr/bin/env python3
"""
Check the things 487's census of the *second plane* depends on, before the device is asked.
...
expert. The prediction is `IOService` for the children and no attached services, on the ground that
no `IODTPlatformExpert` subclass exists in this image for `configure` to run in - and a run that
answers `IOPlatformDevice`, or a non-empty service plane, falsifies the prediction rather than
failing this check. What this file guarantees is that the run is *able* to answer: both planes are
read, both counts are published, every child is named and classed, and every key has a writer.
"""
...
REPORT_KEYS = (
    ...
    "xnu_entry_svc_none", "xnu_entry_svc_named", "xnu_entry_svc_matched",
...
CLAIMS = (claim_planes, claim_machinery, claim_shape, claim_caps, claim_sites, claim_keys,
          claim_prediction, claim_wrapper)
