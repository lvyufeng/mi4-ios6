...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage65.gicDistributorBase, PE_state_stage65.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage65_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage65 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage65_loader_preflight_run(args)) {
