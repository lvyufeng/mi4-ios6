...
CC=${STAGE67_XNU_LINK_CC:-arm-none-eabi-gcc}
NM=${STAGE67_XNU_LINK_NM:-arm-none-eabi-nm}
OBJDUMP=${STAGE67_XNU_LINK_OBJDUMP:-arm-none-eabi-objdump}
READELF=${STAGE67_XNU_LINK_READELF:-arm-none-eabi-readelf}
SIZE=${STAGE67_XNU_LINK_SIZE:-arm-none-eabi-size}
...
LINK_ELF="$LINK_DIR/stage67-xnu-link.elf"
MAP_TXT="$LINK_DIR/stage67-xnu-link.map"
SYMBOLS_TXT="$LINK_DIR/stage67-xnu-link.symbols"
UNDEFINED_TXT="$LINK_DIR/stage67-xnu-link.undefined"
DISASM_TXT="$LINK_DIR/stage67-xnu-link.disasm"
SIZE_TXT="$LINK_DIR/stage67-xnu-link.size"
SECTIONS_TXT="$LINK_DIR/stage67-xnu-link.sections"
READELF_TXT="$LINK_DIR/stage67-xnu-link.readelf"
...
printf 'Stage67 controlled public-XNU compile-graph ARM ELF link proof\n' >> "$STATUS_TXT"
...
expected = {
    'stage67_xnu_link_noentry', 'DTInit', 'DTLookupEntry', 'DTGetProperty',
...
add_satisfied 0x00004000  # outputs under ignored out/stage67
...
printf 'stage67_xnu_link_status=0x%08x\n' "$status" >> "$STATUS_TXT"
printf 'stage67_xnu_link_required_mask=0x%08x\n' "$required" >> "$STATUS_TXT"
printf 'stage67_xnu_link_satisfied_mask=0x%08x\n' "$satisfied" >> "$STATUS_TXT"
printf 'stage67_xnu_link_failure_mask=0x%08x\n' "$failure" >> "$STATUS_TXT"
printf 'stage67_xnu_link_object_count=0x%08x\n' "$linked_object_count" >> "$STATUS_TXT"
printf 'stage67_xnu_link_support_object_count=0x%08x\n' "$support_object_count" >> "$STATUS_TXT"
printf 'stage67_xnu_link_undefined_symbol_count=0x%08x\n' "$undefined_count" >> "$STATUS_TXT"
printf 'stage67_xnu_link_global_symbol_count=0x%08x\n' "$global_count" >> "$STATUS_TXT"
printf 'stage67_xnu_link_elf_bytes=0x%08x\n' "$elf_bytes" >> "$STATUS_TXT"
printf 'stage67_xnu_link_elf_sha32=%s\n' "$elf_sha32" >> "$STATUS_TXT"
printf 'stage67_xnu_link_text_addr=0x%08x\n' "$text_addr" >> "$STATUS_TXT"
printf 'stage67_xnu_link_text_size=0x%08x\n' "$text_size" >> "$STATUS_TXT"
printf 'stage67_xnu_link_data_addr=0x%08x\n' "$data_addr" >> "$STATUS_TXT"
printf 'stage67_xnu_link_data_size=0x%08x\n' "$data_size" >> "$STATUS_TXT"
printf 'stage67_xnu_link_bss_addr=0x%08x\n' "$bss_addr" >> "$STATUS_TXT"
printf 'stage67_xnu_link_bss_size=0x%08x\n' "$bss_size" >> "$STATUS_TXT"
printf 'stage67_xnu_link_public_only=0x00000001\n' >> "$STATUS_TXT"
printf 'stage67_xnu_link_no_full_xnu_build=0x00000001\n' >> "$STATUS_TXT"
printf 'stage67_xnu_link_no_public_xnu_exec=0x00000001\n' >> "$STATUS_TXT"
printf 'stage67_xnu_link_no_macho_exec=0x00000001\n' >> "$STATUS_TXT"
printf 'stage67_xnu_link_no_platform_runtime_exec=0x00000001\n' >> "$STATUS_TXT"
printf 'stage67_xnu_link_no_external_mutation=0x%08x\n' "$external_clean" >> "$STATUS_TXT"
printf 'stage67_xnu_link_outputs_ignored=0x00000001\n' >> "$STATUS_TXT"
printf 'stage67_xnu_link_fail_closed=0x00000001\n' >> "$STATUS_TXT"

cat > "$METADATA_TXT" <<EOF_METADATA
stage67-xnu-platform-graph graph_status=0x67000001 bounded-pe-gen=true bounded-arm-pe-bootargs=true bounded-arm-consistent-debug=true no-platform-runtime-exec=true stage67-xnu-link-proof noexec=true public=xnu-2050.22.13 arm-reference=xnu-4570.1.46 commit32=0xcc8a9b0c objects=${linked_object_count} support=${support_object_count} undefined=${undefined_count} elf_bytes=0x$(printf '%08x' "$elf_bytes") elf_sha32=${elf_sha32} text=${text_addr}/0x$(printf '%08x' "$text_size") data=${data_addr}/0x$(printf '%08x' "$data_size") bss=${bss_addr}/0x$(printf '%08x' "$bss_size") no-full-mach-kernel=true no-public-xnu-exec=true no-macho-exec=true
EOF_METADATA

cat > "$HEADER" <<EOF_HEADER
#ifndef MI4IOS6_STAGE67_XNU_LINK_GENERATED_H
#define MI4IOS6_STAGE67_XNU_LINK_GENERATED_H

#define STAGE67_XNU_LINK_HOST_STATUS                 0x$(printf '%08x' "$status")u
#define STAGE67_XNU_LINK_HOST_REQUIRED_MASK          0x0001ffffu
#define STAGE67_XNU_LINK_HOST_SATISFIED_MASK         0x$(printf '%08x' "$satisfied")u
#define STAGE67_XNU_LINK_HOST_FAILURE_MASK           0x$(printf '%08x' "$failure")u
#define STAGE67_XNU_LINK_HOST_LINKED_OBJECT_COUNT    0x$(printf '%08x' "$linked_object_count")u
#define STAGE67_XNU_LINK_HOST_SUPPORT_OBJECT_COUNT   0x$(printf '%08x' "$support_object_count")u
#define STAGE67_XNU_LINK_HOST_UNDEFINED_SYMBOL_COUNT 0x$(printf '%08x' "$undefined_count")u
#define STAGE67_XNU_LINK_HOST_GLOBAL_SYMBOL_COUNT    0x$(printf '%08x' "$global_count")u
#define STAGE67_XNU_LINK_HOST_ELF_BYTES              0x$(printf '%08x' "$elf_bytes")u
#define STAGE67_XNU_LINK_HOST_ELF_SHA32              ${elf_sha32}u
#define STAGE67_XNU_LINK_HOST_TEXT_ADDR              0x$(printf '%08x' "$text_addr")u
#define STAGE67_XNU_LINK_HOST_TEXT_SIZE              0x$(printf '%08x' "$text_size")u
#define STAGE67_XNU_LINK_HOST_DATA_ADDR              0x$(printf '%08x' "$data_addr")u
#define STAGE67_XNU_LINK_HOST_DATA_SIZE              0x$(printf '%08x' "$data_size")u
#define STAGE67_XNU_LINK_HOST_BSS_ADDR               0x$(printf '%08x' "$bss_addr")u
#define STAGE67_XNU_LINK_HOST_BSS_SIZE               0x$(printf '%08x' "$bss_size")u
#define STAGE67_XNU_LINK_HOST_BASELINE_COMMIT32      0xcc8a9b0cu
#define STAGE67_XNU_LINK_HOST_MASTER_VERSION_12_3_0  0x000c0300u
#define STAGE67_XNU_LINK_HOST_PUBLIC_ONLY            1u
#define STAGE67_XNU_LINK_HOST_NO_FULL_XNU_BUILD      1u
#define STAGE67_XNU_LINK_HOST_NO_PUBLIC_XNU_EXEC     1u
#define STAGE67_XNU_LINK_HOST_NO_MACHO_EXEC          1u
#define STAGE67_XNU_LINK_HOST_NO_PLATFORM_RUNTIME_EXEC 1u
#define STAGE67_XNU_LINK_HOST_NO_EXTERNAL_MUTATION   ${external_clean}u
#define STAGE67_XNU_LINK_HOST_OUTPUTS_IGNORED        1u
#define STAGE67_XNU_LINK_HOST_FAIL_CLOSED            1u
