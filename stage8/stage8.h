#ifndef MI4IOS6_STAGE8_H
#define MI4IOS6_STAGE8_H

#include <stdint.h>
#include <stddef.h>

#define STAGE8_BASE          0x00008000u
...
extern struct pe_platform_state PE_state_stage8;
void pe_state_init_from_boot_args(struct boot_args *args);
void pe_state_log(void);
int pe_state_validate(void);

/* MSM8974 GIC skeleton state and controlled Stage8 SGI/IRQ test */
...
extern struct gic_state_snapshot GIC_state_stage8;
extern volatile uint32_t stage8_irq_count;
extern volatile uint32_t stage8_last_iar;
extern volatile uint32_t stage8_last_irq_id;
...
/* stage8_main.c */
void stage8_main(void) __attribute__((noreturn));
void platform_reboot(void) __attribute__((noreturn));
int test_kernel_entry(struct boot_args *args);

extern uint8_t __stage8_image_end[];
extern uint8_t stage8_vectors[];
void trigger_stage8_undef_test(void) __attribute__((noreturn));
void trigger_stage8_data_abort_test(void) __attribute__((noreturn));
