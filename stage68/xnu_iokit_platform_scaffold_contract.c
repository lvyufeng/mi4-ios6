...
static struct stage68_xnu_iokit_platform_scaffold_contract g_stage68_xnu_iokit_platform_scaffold_contract;

static uint32_t stage68_iokit_platform_scaffold_checksum(
    volatile const struct stage68_xnu_iokit_platform_scaffold_contract *contract)
{
    volatile const uint32_t *words = (volatile const uint32_t *)(uintptr_t)contract;
    uint32_t count = (uint32_t)(offsetof(struct stage68_xnu_iokit_platform_scaffold_contract, checksum) /
                                sizeof(uint32_t));
...
static void stage68_iokit_platform_scaffold_log(
    const struct stage68_xnu_iokit_platform_scaffold_contract *contract)
{
    xnu_log_kv32("stage68_xnu_iokit_platform_scaffold_contract_status", contract->status);
    xnu_log_kv32("stage68_xnu_iokit_platform_scaffold_contract_required_mask", contract->required_mask);
    xnu_log_kv32("stage68_xnu_iokit_platform_scaffold_contract_satisfied_mask", contract->satisfied_mask);
    xnu_log_kv32("stage68_xnu_iokit_platform_scaffold_contract_failure_mask", contract->failure_mask);
    xnu_log_kv32("stage68_xnu_iokit_platform_scaffold_contract_checksum", contract->checksum);
    xnu_log_kv32("stage68_xnu_iokit_platform_apple_dt_semantic_mask", contract->apple_dt_semantic_mask);
    xnu_log_kv32("stage68_xnu_iokit_platform_device_tree_props", contract->device_tree_node_prop_count);
    xnu_log_kv32("stage68_xnu_iokit_platform_node_props", contract->iokit_platform_prop_count);
    xnu_log_kv32("stage68_xnu_iokit_platform_driver_props", contract->platform_driver_prop_count);
    xnu_log_kv32("stage68_xnu_iokit_platform_driver_gic_base", contract->platform_driver_gic_base);
    xnu_log_kv32("stage68_xnu_iokit_platform_driver_timer_base", contract->platform_driver_timer_base);
    xnu_log_kv32("stage68_xnu_iokit_platform_driver_timebase_frequency", contract->platform_driver_timebase_frequency);
    xnu_log_kv32("stage68_xnu_iokit_platform_driver_cpu_count", contract->platform_driver_cpu_count);
    xnu_log_kv32("stage68_xnu_iokit_reference_mask", contract->iokit_reference_mask);
    xnu_log_kv32("stage68_xnu_iokit_runtime_blocked_mask", contract->iokit_runtime_blocked_mask);
    xnu_log_kv32("stage68_xnu_iokit_reference_count", contract->iokit_reference_count);
    xnu_log_kv32("stage68_xnu_iokit_public_compile_count", contract->iokit_public_compile_count);
    xnu_log_kv32("stage68_xnu_iokit_public_link_count", contract->iokit_public_link_count);
    xnu_log_kv32("stage68_xnu_iokit_reference_only", contract->iokit_reference_only);
    xnu_log_kv32("stage68_xnu_iokit_no_iokit_runtime_exec", contract->no_iokit_runtime_exec);
    xnu_log_kv32("stage68_xnu_iokit_no_platform_driver_exec", contract->no_platform_driver_exec);
    xnu_log_kv32("stage68_xnu_iokit_no_public_xnu_exec", contract->no_public_xnu_exec);
    xnu_log_kv32("stage68_xnu_iokit_no_live_pmap_tables_installed", contract->no_live_pmap_tables_installed);
    xnu_log_kv32("stage68_xnu_iokit_proposed_workspace_written", contract->proposed_workspace_written);
    xnu_log_kv32("stage68_xnu_iokit_pmap_ttbr_written", contract->pmap_ttbr_written);
    xnu_log_kv32("stage68_xnu_iokit_pmap_tlbs_invalidated", contract->pmap_tlbs_invalidated);
    xnu_log_kv32("stage68_xnu_iokit_caches_changed", contract->caches_changed);
    xnu_log_kv32("stage68_xnu_iokit_persistent_write_attempted", contract->persistent_write_attempted);
    xnu_log_kv32("stage68_xnu_iokit_platform_gap_mask", contract->proposed_platform_gap_mask);
}

int stage68_xnu_iokit_platform_scaffold_contract_selftest(const struct stage68_loader_preflight *preflight)
{
    struct stage68_xnu_iokit_platform_scaffold_contract *contract = &g_stage68_xnu_iokit_platform_scaffold_contract;
    ...
    contract->version = STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_CONTRACT_VERSION;
    contract->size = sizeof(*contract);
    contract->status = STAGE68_STATUS_BASE;
    contract->required_mask = STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_REQUIRED_MASK;

    if (!preflight || !args || !root) {
        contract->failure_mask = STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_SOURCE |
            STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_SAFETY_BOUNDARY;
        contract->checksum = stage68_iokit_platform_scaffold_checksum(contract);
        stage68_iokit_platform_scaffold_log(contract);
        return 0;
    }
    ...
    contract->pexpert_hook_ready = (contract->source_pexpert_status == STAGE68_STATUS_OK &&
                                    contract->source_pexpert_satisfied_mask == STAGE68_XNU_PEXPERT_HOOK_READINESS_REQUIRED_MASK &&
                                    contract->source_pexpert_failure_mask == 0u) ? 1u : 0u;
    contract->pmap_transition_ready = (contract->source_pmap_transition_status == STAGE68_STATUS_OK &&
    ...
    source_ok = (contract->source_pexpert_status == STAGE68_STATUS_OK &&
    ...
    if (source_ok) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_SOURCE_PEXPERT;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_SOURCE;
    }

    if (contract->apple_dt_semantic_mask == STAGE68_DT_READY_REQUIRED) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_APPLE_DT;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_DT;
    }
    ...
    if (contract->device_tree_node_ptr != 0u && contract->device_tree_node_prop_count >= 4u &&
        ...
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_DEVICE_TREE_IMPORT;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_DT;
    }
    ...
    if (iokit_dt_ok) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_IOPLATFORM_NODE;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_IOKIT;
    }

    if (PE_state_stage68.cpuCount == STAGE68_XNU_IOKIT_PLATFORM_EXPECTED_CPU_COUNT &&
        contract->platform_driver_cpu_count == STAGE68_XNU_IOKIT_PLATFORM_EXPECTED_CPU_COUNT) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_IOCPU_TOPOLOGY;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_DRIVER;
    }
    ...
    if (driver_ok) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_PLATFORM_DRIVER_NODE;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_DRIVER;
    }

    if (contract->platform_driver_gic_base == STAGE68_XNU_IOKIT_PLATFORM_EXPECTED_GIC_DIST_BASE &&
        contract->platform_driver_timer_base == STAGE68_XNU_IOKIT_PLATFORM_EXPECTED_TIMER_BASE &&
        contract->platform_driver_timebase_frequency == STAGE68_XNU_IOKIT_PLATFORM_EXPECTED_TIMEBASE_FREQ &&
        contract->platform_driver_cpu_count == STAGE68_XNU_IOKIT_PLATFORM_EXPECTED_CPU_COUNT) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_MSM8974_DRIVER_FACTS;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_DRIVER;
    }

    if (preflight->xnu_pexpert_hook_readiness_contract.gic_dist_base == STAGE68_XNU_IOKIT_PLATFORM_EXPECTED_GIC_DIST_BASE &&
        preflight->xnu_pexpert_hook_readiness_contract.pe_timer_base == STAGE68_XNU_IOKIT_PLATFORM_EXPECTED_TIMER_BASE &&
        preflight->xnu_pexpert_hook_readiness_contract.timebase_freq_hz == STAGE68_XNU_IOKIT_PLATFORM_EXPECTED_TIMEBASE_FREQ &&
        preflight->xnu_pexpert_hook_readiness_contract.proposed_interrupt_ready_mask == STAGE68_LOADER_IRQ_READY_REQUIRED) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_GIC_TIMER_PROVIDER_PLAN;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_DRIVER;
    }
    ...
    if (contract->iokit_reference_mask == STAGE68_XNU_IOKIT_REFERENCE_REQUIRED &&
        ...
        contract->iocpu_reference == 1u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_PUBLIC_IOKIT_REFERENCE;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_IOKIT;
    }
    ...
    if (public_boundary_ok) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_PUBLIC_PEXPERT_BOUNDARY |
            STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_OBJECT_LINK_BOUNDARY;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_PUBLIC_BOUNDARY;
    }

    if (contract->pexpert_hook_ready == 1u && contract->pmap_transition_ready == 1u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_PMAP_TRANSITION_BOUNDARY;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_PMAP_BOUNDARY;
    }

    if (contract->no_iokit_runtime_exec == 1u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_NO_IOKIT_RUNTIME_EXEC;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_SAFETY_BOUNDARY;
    }
    if (contract->no_platform_driver_exec == 1u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_NO_PLATFORM_DRIVER_EXEC;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_SAFETY_BOUNDARY;
    }
    if (contract->no_public_xnu_exec == 1u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_NO_PUBLIC_XNU_EXEC;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_PUBLIC_BOUNDARY;
    }
    if (contract->no_public_pmap_exec == 1u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_NO_PUBLIC_PMAP_EXEC;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_PMAP_BOUNDARY;
    }
    ...
    if (contract->no_live_pmap_tables_installed == 0u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_NO_LIVE_PMAP_INSTALL;
    }
    if (contract->proposed_workspace_written == 0u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_NO_PROPOSED_PMAP_WRITE;
    }
    if (contract->pmap_ttbr_written == 0u && contract->pmap_ttbcr_written == 0u &&
        contract->pmap_dacr_written == 0u && contract->pmap_sctlr_written == 0u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_NO_PMAP_CONTROL_WRITE;
    }
    if (contract->pmap_tlbs_invalidated == 0u && contract->caches_changed == 0u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_NO_TLB_CACHE_CHANGE;
    }
    if (!pmap_boundary_ok) {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_PMAP_BOUNDARY;
    }
    ...
    if (contract->xnu_start_executed == 0u && contract->generated_macho_executed == 0u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_NO_XNU_MACHO_EXEC;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_SAFETY_BOUNDARY;
    }
    if (contract->persistent_write_attempted == 0u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_NO_PERSIST_WRITE;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_SAFETY_BOUNDARY;
    }

    if (contract->proposed_platform_gap_mask == (STAGE68_PLATFORM_GAP_REQUIRED_RECORDED & ~STAGE68_PLATFORM_GAP_IOKIT_STACK) &&
        contract->proposed_pexpert_gap_mask == 0u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_PLATFORM_ROLLUP;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_ROLLUP;
    }

    if (contract->fail_closed == 1u && contract->local_only == 1u) {
        contract->satisfied_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_SAT_FAIL_CLOSED;
    } else {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_SAFETY_BOUNDARY;
    }

    contract->checksum = stage68_iokit_platform_scaffold_checksum(contract);
    if (contract->checksum != stage68_iokit_platform_scaffold_checksum(contract)) {
        contract->failure_mask |= STAGE68_XNU_IOKIT_PLATFORM_SCAFFOLD_FAIL_CHECKSUM;
    }
...
{
    return &g_stage68_xnu_iokit_platform_scaffold_contract;
}
