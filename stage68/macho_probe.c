...
#define STAGE68_PAGE_SIZE 4096u

struct stage68_macho_header_32 {
    uint32_t magic;
    uint32_t cputype;
    uint32_t cpusubtype;
    uint32_t filetype;
...
struct stage68_macho_load_command {
    uint32_t cmd;
    uint32_t cmdsize;
};

struct stage68_macho_segment_command_32 {
    uint32_t cmd;
    uint32_t cmdsize;
    char segname[16];
    uint32_t vmaddr;
...
struct stage68_macho_section_32 {
    char sectname[16];
    char segname[16];
    uint32_t addr;
    uint32_t size;
...
struct stage68_macho_symtab_command {
    uint32_t cmd;
    uint32_t cmdsize;
    uint32_t symoff;
    uint32_t nsyms;
...
static struct stage68_loader_preflight stage68_loader_preflight_block;
static uint8_t stage68_macho_staging_arena[STAGE68_MACHO_STAGING_ARENA_CAP]
    __attribute__((aligned(STAGE68_MACHO_STAGING_ARENA_ALIGN)));
static uint8_t stage68_tte_dryrun_arena[STAGE68_XNU_TTE_WORKSPACE_BYTES]
    __attribute__((aligned(STAGE68_XNU_TTE_ALIGNMENT)));
static uint8_t stage68_safe_table_arena[STAGE68_XNU_TTE_L1_SIZE * 2u]
...
static uint32_t segment_mask_for_name(const char segname[16])
{
    if (fixed16_eq(segname, "__TEXT")) {
        return STAGE68_MACHO_SEG_TEXT;
    }
    if (fixed16_eq(segname, "__DATA")) {
        return STAGE68_MACHO_SEG_DATA;
    }
    if (fixed16_eq(segname, "__LINKEDIT")) {
        return STAGE68_MACHO_SEG_LINKEDIT;
    }
    if (fixed16_eq(segname, "__KLD")) {
        return STAGE68_MACHO_SEG_KLD;
    }
    if (fixed16_eq(segname, "__LAST")) {
        return STAGE68_MACHO_SEG_LAST;
    }
    if (fixed16_eq(segname, "__PRELINK_TEXT")) {
        return STAGE68_MACHO_SEG_PRELINK_TEXT;
    }
    if (fixed16_eq(segname, "__PRELINK_INFO")) {
        return STAGE68_MACHO_SEG_PRELINK_INFO;
    }
    if (fixed16_eq(segname, "__PRELINK_STATE")) {
        return STAGE68_MACHO_SEG_PRELINK_STATE;
    }
    if (fixed16_eq(segname, "__PRELINK")) {
        return STAGE68_MACHO_SEG_PRELINK;
    }

    return 0;
}

static void record_key_segment(struct stage68_macho_probe_result *result,
                               const struct stage68_macho_segment_command_32 *segment,
                               uint32_t seg_bit)
{
    if (seg_bit == STAGE68_MACHO_SEG_TEXT) {
    ...
    } else if (seg_bit == STAGE68_MACHO_SEG_DATA) {
    ...
    } else if (seg_bit == STAGE68_MACHO_SEG_LINKEDIT) {
    ...
    } else if (seg_bit == STAGE68_MACHO_SEG_PRELINK_TEXT) {
    ...
    } else if (seg_bit == STAGE68_MACHO_SEG_PRELINK_INFO) {
    ...
    } else if (seg_bit == STAGE68_MACHO_SEG_PRELINK_STATE) {
...
static void add_load_plan_entry(struct stage68_macho_probe_result *result,
                                const struct stage68_macho_segment_command_32 *segment,
                                uint32_t seg_bit)
{
    struct stage68_macho_load_plan *plan = &result->load_plan;
    struct stage68_macho_load_plan_entry *entry;

    if (segment->vmsize == 0u) {
        return;
    }
    if (plan->entry_count >= STAGE68_MACHO_LOAD_PLAN_MAX) {
        result->failure_mask |= STAGE68_MACHO_FAIL_LOAD_PLAN_FULL;
        return;
    }
    ...
    if (segment->filesize != 0u) {
        entry->flags |= STAGE68_MACHO_LOAD_PLAN_FILE_BACKED;
    }
    if ((seg_bit & STAGE68_MACHO_SEG_PRELINK_MASK) != 0u) {
        entry->flags |= STAGE68_MACHO_LOAD_PLAN_PRELINK;
    }
    if (segment->vmsize > segment->filesize) {
        entry->flags |= STAGE68_MACHO_LOAD_PLAN_ZERO_FILL;
    }
    plan->entry_count++;
}

static void record_section_match(struct stage68_macho_probe_result *result,
                                 const struct stage68_macho_section_32 *section)
{
    uint32_t section_bit = 0;

    if (fixed16_eq(section->segname, "__TEXT") && fixed16_eq(section->sectname, "__text")) {
        section_bit = STAGE68_MACHO_SECT_TEXT_TEXT;
        ...
        section_bit = STAGE68_MACHO_SECT_DATA_CONST;
        ...
        section_bit = STAGE68_MACHO_SECT_PRELINK_TEXT_TEXT;
        ...
        section_bit = STAGE68_MACHO_SECT_PRELINK_INFO_INFO;
                           ...
                           const struct stage68_macho_section_32 *section)
{
    ...
    if (add_overflow_u32(section->addr, section->size, &section_end) ||
        ...
        result->failure_mask |= STAGE68_MACHO_FAIL_SECTION_BOUNDS;
    }
...
static void record_segment(struct stage68_macho_probe_result *result,
                           const struct stage68_macho_segment_command_32 *segment)
{
    uint32_t vm_end = 0;
    uint32_t file_end = 0;
...
static void parse_segment_sections(struct stage68_macho_probe_result *result,
                                   const uint8_t *bytes,
                                   ...
                                   const struct stage68_macho_segment_command_32 *segment)
{
    ...
    if (mul_overflow_u32(segment->nsects, sizeof(struct stage68_macho_section_32), &section_bytes) ||
    ...
    for (uint32_t i = 0; i < segment->nsects; i++) {
        const struct stage68_macho_section_32 *section =
            (const struct stage68_macho_section_32 *)(const void *)(bytes + section_offset);
        record_section(result, segment, section);
        section_offset += sizeof(*section);
    }
}

static void finalize_load_plan(struct stage68_macho_probe_result *result)
{
    struct stage68_macho_load_plan *plan = &result->load_plan;
    ...
    plan->version = STAGE68_MACHO_LOAD_PLAN_VERSION;
    ...
    if (plan->entry_count == 0u || result->min_vmaddr == 0u || result->max_vmaddr <= result->min_vmaddr) {
        result->failure_mask |= STAGE68_MACHO_FAIL_LOAD_PLAN;
        return;
    }

    for (uint32_t i = 0; i < plan->entry_count; i++) {
        struct stage68_macho_load_plan_entry *entry = &plan->entries[i];
        uint32_t vm_delta;
        ...
        if (add_overflow_u32(RAM_PHYS_BASE, vm_delta, &entry->physaddr) ||
            align_up_checked_u32(entry->vmsize, STAGE68_PAGE_SIZE, &aligned_size) ||
            add_overflow_u32(entry->physaddr, aligned_size, &entry_end)) {
            result->failure_mask |= STAGE68_MACHO_FAIL_PHYS_RANGE;
            continue;
        }
    ...
    if ((result->failure_mask & (STAGE68_MACHO_FAIL_LOAD_PLAN | STAGE68_MACHO_FAIL_PHYS_RANGE |
                                 STAGE68_MACHO_FAIL_LOAD_PLAN_FULL)) == 0u) {
        plan->status = STAGE68_STATUS_OK;
        result->load_plan_status = STAGE68_STATUS_OK;
        result->validation_mask |= STAGE68_MACHO_VALID_LOAD_PLAN;
    } else {
        plan->status = STAGE68_STATUS_BASE | result->failure_mask;
...
static void log_load_plan_entry(const char *prefix_vmaddr, const char *prefix_vmsize,
                                ...
                                const struct stage68_macho_load_plan_entry *entry)
{
...
static void log_load_plan_entries(const struct stage68_macho_probe_result *result)
{
    const struct stage68_macho_load_plan *plan = &result->load_plan;
                        ...
                        struct stage68_macho_probe_result *result)
{
    ...
    const struct stage68_macho_header_32 *header;
    ...
    result->version = STAGE68_MACHO_PROBE_VERSION;
    result->artifact_base = (uint32_t)(uintptr_t)artifact;
    result->artifact_size = artifact_size;
    result->segment_required_mask = STAGE68_MACHO_SEG_REQUIRED;
    result->section_required_mask = STAGE68_MACHO_SECT_REQUIRED;
    result->entry_kind_mask = STAGE68_MACHO_ENTRY_NOT_EXECUTED;
    result->status = STAGE68_STATUS_BASE;
    result->load_plan.version = STAGE68_MACHO_LOAD_PLAN_VERSION;

    xnu_log_puts("Stage68 Mach-O probe begin\n");
    xnu_log_kv32("macho_artifact_base", result->artifact_base);
    xnu_log_kv32("macho_artifact_size", result->artifact_size);

    if (!artifact) {
        result->failure_mask |= STAGE68_MACHO_FAIL_NULL;
        goto done;
    }
    if (artifact_size < sizeof(*header)) {
        result->failure_mask |= STAGE68_MACHO_FAIL_SIZE;
        goto done;
    }

    header = (const struct stage68_macho_header_32 *)artifact;
    ...
    if (header->magic == STAGE68_MACHO_MAGIC) {
        result->validation_mask |= STAGE68_MACHO_VALID_MAGIC | STAGE68_MACHO_VALID_ENDIAN;
    } else if (header->magic == STAGE68_MACHO_CIGAM) {
        result->validation_mask |= STAGE68_MACHO_VALID_MAGIC;
        result->failure_mask |= STAGE68_MACHO_FAIL_ENDIAN;
    } else {
        result->failure_mask |= STAGE68_MACHO_FAIL_MAGIC;
    }

    if (header->cputype == STAGE68_MACHO_CPU_TYPE_ARM) {
        result->validation_mask |= STAGE68_MACHO_VALID_CPU;
    } else {
        result->failure_mask |= STAGE68_MACHO_FAIL_CPU;
    }

    if (header->cpusubtype == STAGE68_MACHO_CPU_SUBTYPE_ARM_ALL ||
        header->cpusubtype == STAGE68_MACHO_CPU_SUBTYPE_ARM_V7) {
        result->validation_mask |= STAGE68_MACHO_VALID_SUBTYPE;
    } else {
        result->failure_mask |= STAGE68_MACHO_FAIL_SUBTYPE;
    }

    if (header->filetype == STAGE68_MACHO_FILETYPE_EXECUTE ||
        header->filetype == STAGE68_MACHO_FILETYPE_PRELOAD) {
        result->validation_mask |= STAGE68_MACHO_VALID_FILETYPE;
    } else {
        result->failure_mask |= STAGE68_MACHO_FAIL_FILETYPE;
    }

    if (add_overflow_u32(commands_start, header->sizeofcmds, &commands_end) ||
        commands_end > artifact_size || (header->sizeofcmds & 3u) != 0u) {
        result->failure_mask |= STAGE68_MACHO_FAIL_COMMAND_BOUNDS;
    } else {
        result->validation_mask |= STAGE68_MACHO_VALID_COMMAND_BOUNDS;
    ...
    for (uint32_t i = 0; i < header->ncmds && offset < commands_end; i++) {
        const struct stage68_macho_load_command *load_command;
        ...
        load_command = (const struct stage68_macho_load_command *)(const void *)(bytes + offset);
        if (load_command->cmdsize < sizeof(*load_command) || (load_command->cmdsize & 3u) != 0u ||
            ...
            result->failure_mask |= STAGE68_MACHO_FAIL_COMMAND_SIZE;
            break;
        }

        result->load_command_count_seen++;
        if (load_command->cmd == STAGE68_MACHO_LC_SEGMENT) {
            if (load_command->cmdsize < sizeof(struct stage68_macho_segment_command_32)) {
                result->failure_mask |= STAGE68_MACHO_FAIL_COMMAND_SIZE;
            } else {
                const struct stage68_macho_segment_command_32 *segment =
                    (const struct stage68_macho_segment_command_32 *)(const void *)(bytes + offset);
                result->load_command_seen_mask |= STAGE68_MACHO_CMD_SEGMENT;
                record_segment(result, segment);
                parse_segment_sections(result, bytes, offset, segment);
            }
        } else if (load_command->cmd == STAGE68_MACHO_LC_SYMTAB) {
            result->load_command_seen_mask |= STAGE68_MACHO_CMD_SYMTAB;
            if (load_command->cmdsize >= sizeof(struct stage68_macho_symtab_command)) {
                const struct stage68_macho_symtab_command *symtab =
                    (const struct stage68_macho_symtab_command *)(const void *)(bytes + offset);
                uint32_t str_end = 0;
                if (add_overflow_u32(symtab->stroff, symtab->strsize, &str_end) || str_end > artifact_size) {
                    result->failure_mask |= STAGE68_MACHO_FAIL_SEGMENT_EXTENT;
                }
            }
        } else if (load_command->cmd == STAGE68_MACHO_LC_UNIXTHREAD) {
            result->load_command_seen_mask |= STAGE68_MACHO_CMD_UNIXTHREAD;
            result->entry_kind_mask |= STAGE68_MACHO_ENTRY_UNIXTHREAD;
        ...
        } else if (load_command->cmd == STAGE68_MACHO_LC_MAIN) {
            result->load_command_seen_mask |= STAGE68_MACHO_CMD_MAIN;
            result->entry_kind_mask |= STAGE68_MACHO_ENTRY_MAIN;
        } else {
            result->load_command_seen_mask |= STAGE68_MACHO_CMD_UNKNOWN;
...
after_commands:
    if ((result->segment_seen_mask & STAGE68_MACHO_SEG_REQUIRED) == STAGE68_MACHO_SEG_REQUIRED) {
        result->validation_mask |= STAGE68_MACHO_VALID_REQUIRED_SEGMENTS;
    } else {
        result->failure_mask |= STAGE68_MACHO_FAIL_REQUIRED_SEGMENTS;
    }
    if ((result->section_seen_mask & STAGE68_MACHO_SECT_REQUIRED) == STAGE68_MACHO_SECT_REQUIRED) {
        result->validation_mask |= STAGE68_MACHO_VALID_REQUIRED_SECTIONS;
    } else {
        result->failure_mask |= STAGE68_MACHO_FAIL_REQUIRED_SECTIONS;
    }
    if ((result->failure_mask & STAGE68_MACHO_FAIL_SEGMENT_EXTENT) == 0u) {
        result->validation_mask |= STAGE68_MACHO_VALID_FILE_EXTENTS;
    }
    if ((result->failure_mask & STAGE68_MACHO_FAIL_SECTION_BOUNDS) == 0u) {
        result->validation_mask |= STAGE68_MACHO_VALID_SECTION_BOUNDS;
    }
    if ((result->prelink_segment_seen_mask | result->prelink_section_seen_mask) != 0u) {
        result->validation_mask |= STAGE68_MACHO_VALID_PRELINK_REPORT;
    }
    if ((result->entry_kind_mask & (STAGE68_MACHO_ENTRY_UNIXTHREAD | STAGE68_MACHO_ENTRY_MAIN)) != 0u) {
        result->validation_mask |= STAGE68_MACHO_VALID_ENTRY_NOT_EXECUTED;
    } else {
        result->failure_mask |= STAGE68_MACHO_FAIL_NO_ENTRY_METADATA;
    }
    ...
    {
        const uint32_t parser_required = STAGE68_MACHO_VALID_REQUIRED &
        ...
        if ((result->validation_mask & parser_required) == parser_required &&
            ...
            result->status = STAGE68_STATUS_OK;
            xnu_log_puts("Stage68 Mach-O probe ok; entry metadata observed but not executed\n");
            return 1;
        }
    }

    result->status = STAGE68_STATUS_BASE | result->failure_mask;
    xnu_log_puts("Stage68 Mach-O probe failed\n");
    return 0;
}
...
static void log_stage_entry(const char *copy_src_key, const char *copy_dst_key,
                            ...
                            const struct stage68_macho_staging_plan_entry *entry)
{
...
static void log_staging_entries(const struct stage68_macho_staging_plan *staging)
{
...
static uint32_t stage68_macho_getlastaddr_style(const struct stage68_macho_probe_result *probe)
{
    uint32_t last = 0;

    for (uint32_t i = 0; i < probe->load_plan.entry_count; i++) {
        uint32_t end = 0;
        const struct stage68_macho_load_plan_entry *entry = &probe->load_plan.entries[i];
...
static void stage68_macho_check_arena_aliases(struct stage68_macho_probe_result *result,
                                              uint32_t artifact_base,
                                              uint32_t artifact_size)
{
    struct stage68_macho_staging_plan *staging = &result->staging_plan;
    uint32_t artifact_end = 0;

    if (add_overflow_u32(artifact_base, artifact_size, &artifact_end)) {
        staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_ARENA_ALIAS;
        return;
    }
...
static void stage68_macho_verify_marker(struct stage68_macho_staging_plan *staging,
                                        uint32_t vmaddr,
                                        const char *prefix,
                                        uint32_t bit)
{
    ...
    if (vmaddr < staging->vm_base) {
        staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_MARKERS;
        return;
    }
    ...
    if (bytes_prefix_eq(stage68_macho_staging_arena + offset, prefix)) {
...
static void stage68_macho_verify_zero_fill(struct stage68_macho_staging_plan *staging)
{
    uint32_t required_zero_mask = 0;

    for (uint32_t i = 0; i < staging->entry_count; i++) {
        const struct stage68_macho_staging_plan_entry *entry = &staging->entries[i];
        uint32_t bit = 1u << i;
        ...
        if (entry->arena_offset + entry->copy_size + entry->zero_size > staging->arena_size) {
            staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_ZERO_FILL;
            entry_ok = 0u;
        } else {
            uint32_t zero_offset = entry->arena_offset + entry->copy_size;
            for (uint32_t j = 0; j < entry->zero_size; j++) {
                if (stage68_macho_staging_arena[zero_offset + j] != 0u) {
                    staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_ZERO_FILL;
                    entry_ok = 0u;
                    break;
                }
    ...
    if ((staging->zero_check_mask & required_zero_mask) == required_zero_mask) {
        staging->validation_mask |= STAGE68_MACHO_VALID_ZERO_FILL;
    } else {
        staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_ZERO_FILL;
    }
}

static void stage68_macho_verify_reparse(struct stage68_macho_probe_result *result,
                                         struct stage68_macho_staging_plan *staging)
{
    struct stage68_macho_probe_result materialized;
    uint32_t original_last = stage68_macho_getlastaddr_style(result);
    uint32_t materialized_last;

    if (!stage68_macho_probe(stage68_macho_staging_arena, staging->arena_size, &materialized)) {
        staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_REPARSE;
        return;
    }

    materialized_last = stage68_macho_getlastaddr_style(&materialized);
    ...
    if (materialized.magic == result->magic &&
        ...
        staging->validation_mask |= STAGE68_MACHO_VALID_MATERIALIZED_PARSE;
        xnu_log_puts("Stage68 materialized Mach-O reparse ok\n");
    } else {
        staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_REPARSE;
    }
}

static int stage68_macho_materialize_to_arena(const void *artifact,
                                              uint32_t artifact_size,
                                              struct stage68_macho_probe_result *result)
{
    const uint8_t *source = (const uint8_t *)artifact;
    struct stage68_macho_staging_plan *staging = &result->staging_plan;
    uint32_t arena_base = (uint32_t)(uintptr_t)stage68_macho_staging_arena;
    uint32_t arena_end = 0;
    uint32_t vm_span = 0;
    uint32_t arena_size = 0;

    xnu_log_puts("Stage68 proposed physical load plan is dry-run only; no physical writes performed\n");
    xnu_log_puts("Stage68 Mach-O materialization begin\n");
    memset(stage68_macho_staging_arena, 0, sizeof(stage68_macho_staging_arena));
    memset(staging, 0, sizeof(*staging));
    staging->version = STAGE68_MACHO_STAGING_PLAN_VERSION;
    staging->status = STAGE68_STATUS_BASE;
    staging->arena_base = arena_base;

    if (add_overflow_u32(arena_base, STAGE68_MACHO_STAGING_ARENA_CAP, &arena_end)) {
        staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_SPAN;
        goto finish;
    }
    ...
    if (align_up_checked_u32(vm_span, STAGE68_PAGE_SIZE, &arena_size) ||
        arena_size > STAGE68_MACHO_STAGING_ARENA_CAP) {
        staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_SPAN;
        goto finish;
    }
    ...
    stage68_macho_check_arena_aliases(result, (uint32_t)(uintptr_t)artifact, artifact_size);
    if (staging->failure_mask != 0u) {
        goto finish;
    }

    for (uint32_t i = 0; i < result->load_plan.entry_count; i++) {
        const struct stage68_macho_load_plan_entry *load = &result->load_plan.entries[i];
        struct stage68_macho_staging_plan_entry *entry;
        uint32_t src_end;
        ...
        if (staging->entry_count >= STAGE68_MACHO_STAGING_PLAN_MAX) {
            staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_PLAN_FULL;
            break;
        }
        if (load->vmaddr < staging->vm_base || load->filesize > load->vmsize) {
            staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_DST_BOUNDS;
            continue;
        }
        arena_offset = load->vmaddr - staging->vm_base;
        if (add_overflow_u32(load->fileoff, load->filesize, &src_end) || src_end > artifact_size) {
            staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_SRC_BOUNDS;
            continue;
        }
        ...
        if (load->filesize != 0u) {
            memcpy(stage68_macho_staging_arena + arena_offset, source + load->fileoff, load->filesize);
            staging->file_bytes += load->filesize;
        }
        if (zero_size != 0u) {
            memset(stage68_macho_staging_arena + arena_offset + load->filesize, 0, zero_size);
            staging->zero_bytes += zero_size;
        }
    }

    if (staging->entry_count != result->load_plan.entry_count) {
        staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_COPY;
    }

    if (staging->failure_mask == 0u) {
        staging->validation_mask |= STAGE68_MACHO_VALID_STAGING;
        stage68_macho_verify_reparse(result, staging);
        stage68_macho_verify_marker(staging, result->text_section_vmaddr,
                                    "ST68-TEXT", STAGE68_MACHO_MARKER_TEXT);
        stage68_macho_verify_marker(staging, result->data_const_vmaddr,
                                    "ST68-DATA", STAGE68_MACHO_MARKER_DATA);
        stage68_macho_verify_marker(staging, result->prelink_text_section_vmaddr,
                                    "ST68-PRELINK-TEXT", STAGE68_MACHO_MARKER_PRELINK_TEXT);
        if ((staging->marker_check_mask & STAGE68_MACHO_MARKER_REQUIRED) == STAGE68_MACHO_MARKER_REQUIRED) {
            staging->validation_mask |= STAGE68_MACHO_VALID_MARKERS;
        } else {
            staging->failure_mask |= STAGE68_MACHO_FAIL_STAGE_MARKERS;
        }
        stage68_macho_verify_zero_fill(staging);
    }
...
finish:
    if (staging->failure_mask == 0u &&
        (staging->validation_mask & (STAGE68_MACHO_VALID_STAGING |
                                     ...
                                     STAGE68_MACHO_VALID_MARKERS |
                                     STAGE68_MACHO_VALID_ZERO_FILL)) ==
        (STAGE68_MACHO_VALID_STAGING | STAGE68_MACHO_VALID_MATERIALIZED_PARSE |
         STAGE68_MACHO_VALID_MARKERS | STAGE68_MACHO_VALID_ZERO_FILL)) {
        staging->status = STAGE68_STATUS_OK;
    } else {
        staging->status = STAGE68_STATUS_BASE | staging->failure_mask;
    }
    ...
    return staging->status == STAGE68_STATUS_OK;
}

static uint32_t stage68_tte_dryrun_checksum(volatile const struct stage68_xnu_tte_dryrun *tte)
{
    volatile const uint32_t *words = (volatile const uint32_t *)(uintptr_t)tte;
    uint32_t count = (uint32_t)(offsetof(struct stage68_xnu_tte_dryrun, checksum) / sizeof(uint32_t));
    uint32_t checksum = 0;
...
static void stage68_tte_map_section(uint32_t *l1, uint32_t index, uint32_t desc_flags)
{
...
static void stage68_tte_map_section_range(uint32_t *l1, uint32_t first, uint32_t last, uint32_t desc_flags)
{
    for (uint32_t i = first; i <= last && i < STAGE68_XNU_TTE_L1_ENTRY_COUNT; i++) {
        stage68_tte_map_section(l1, i, desc_flags);
        if (i == UINT32_MAX) {
            break;
        }
    }
...
static uint32_t stage68_tte_l2_and_scratch_zeroed(void)
{
    for (uint32_t i = STAGE68_XNU_TTE_L1_SIZE; i < STAGE68_XNU_TTE_WORKSPACE_BYTES; i++) {
        if (stage68_tte_dryrun_arena[i] != 0u) {
            return 0u;
        }
    }
    return 1u;
...
static uint32_t stage68_tte_section_word(uint32_t index, uint32_t desc_flags)
{
    return (index << 20) | (desc_flags & STAGE68_XNU_TTE_DESC_ATTR_MASK);
}

static uint32_t stage68_tte_descriptor_attrs_ok(uint32_t word, uint32_t expected_flags)
{
    if ((word & STAGE68_XNU_TTE_DESC_TYPE_MASK) != STAGE68_XNU_TTE_DESC_TYPE_SECTION) {
        return 0u;
    }
    return ((word & STAGE68_XNU_TTE_DESC_ATTR_MASK) ==
            (expected_flags & STAGE68_XNU_TTE_DESC_ATTR_MASK)) ? 1u : 0u;
}

static uint32_t stage68_tte_verify_section_slot(const uint32_t *l1,
                                                uint32_t index,
                                                uint32_t desc_flags,
                                                uint32_t *observed_word)
{
    ...
    word = l1[index];
    expected = stage68_tte_section_word(index, desc_flags);
    if (observed_word) {
        *observed_word = word;
    }

    return (word == expected && stage68_tte_descriptor_attrs_ok(word, desc_flags)) ? 1u : 0u;
}

static uint32_t stage68_tte_verify_section_range(const uint32_t *l1,
                                                 uint32_t first,
                                                 uint32_t last,
                                                 uint32_t desc_flags,
                                                 uint32_t *first_word,
                                                 uint32_t *last_word)
{
    ...
    if (first > last || last >= STAGE68_XNU_TTE_L1_ENTRY_COUNT) {
        return 0u;
    }

    for (uint32_t i = first; i <= last; i++) {
        if (!stage68_tte_verify_section_slot(l1, i, desc_flags, &word)) {
            ok = 0u;
        }
...
static uint32_t stage68_safe_table_checksum(const void *base, uint32_t bytes)
{
...
static uint32_t stage68_tte_section_word_pa(uint32_t pa, uint32_t desc_flags)
{
    return (pa & STAGE68_XNU_TTE_DESC_BASE_MASK) |
           (desc_flags & STAGE68_XNU_TTE_DESC_ATTR_MASK);
}

static void stage68_tte_map_section_pa(uint32_t *l1, uint32_t va, uint32_t pa, uint32_t desc_flags)
{
    uint32_t index = section_first_index(va);

    if (index < STAGE68_XNU_TTE_L1_ENTRY_COUNT) {
        l1[index] = stage68_tte_section_word_pa(pa, desc_flags);
    }
}

static void stage68_tte_map_section_range_pa(uint32_t *l1,
                                             uint32_t va_base,
                                             uint32_t pa_base,
                                             uint32_t size,
                                             uint32_t desc_flags)
{
    ...
    va_section = va_base & STAGE68_XNU_TTE_DESC_BASE_MASK;
    pa_section = pa_base & STAGE68_XNU_TTE_DESC_BASE_MASK;

    for (uint32_t i = first; i <= last && i < STAGE68_XNU_TTE_L1_ENTRY_COUNT; i++) {
        uint32_t va = i << 20;
        uint32_t pa = pa_section + (va - va_section);
        stage68_tte_map_section_pa(l1, va, pa, desc_flags);
        if (i == UINT32_MAX) {
            break;
        }
    }
...
static uint32_t stage68_tte_verify_section_slot_pa(const uint32_t *l1,
                                                   uint32_t va,
                                                   ...
                                                   uint32_t *observed_word)
{
    ...
    word = l1[index];
    expected = stage68_tte_section_word_pa(expected_pa, desc_flags);
    if (observed_word) {
        *observed_word = word;
    }
    ...
    if ((word & STAGE68_XNU_TTE_DESC_BASE_MASK) != (expected_pa & STAGE68_XNU_TTE_DESC_BASE_MASK)) {
        return 0u;
    }
    return stage68_tte_descriptor_attrs_ok(word, desc_flags);
}

static uint32_t stage68_tte_expected_section_pa(uint32_t va, uint32_t pa_base)
{
    return (pa_base & STAGE68_XNU_TTE_DESC_BASE_MASK) |
           (va & STAGE68_XNU_TTE_SECTION_OFFSET_MASK);
}

static uint32_t stage68_tte_section_pa_for_va(uint32_t va, uint32_t va_base, uint32_t pa_base)
{
    uint32_t va_section = va & STAGE68_XNU_TTE_DESC_BASE_MASK;
    uint32_t base_section = va_base & STAGE68_XNU_TTE_DESC_BASE_MASK;
    uint32_t pa_section = pa_base & STAGE68_XNU_TTE_DESC_BASE_MASK;

    return pa_section + (va_section - base_section);
}

static uint32_t stage68_tte_translate_l1_section(const uint32_t *l1,
                                                 uint32_t va,
                                                 uint32_t *pa,
                                                 uint32_t *desc_word)
{
    ...
    if ((desc & STAGE68_XNU_TTE_DESC_TYPE_MASK) != STAGE68_XNU_TTE_DESC_TYPE_SECTION) {
        return 0u;
    }
    if (pa) {
        *pa = (desc & STAGE68_XNU_TTE_DESC_BASE_MASK) |
              (va & STAGE68_XNU_TTE_SECTION_OFFSET_MASK);
    }
...
static uint32_t stage68_tte_expect_translation(const uint32_t *l1,
                                               uint32_t va,
                                               uint32_t expected_pa,
                                               uint32_t *observed_pa)
{
    ...
    if (!stage68_tte_translate_l1_section(l1, va, &pa, 0)) {
        return 0u;
    }
    if (observed_pa) {
        *observed_pa = pa;
...
static int stage68_xnu_tte_dryrun_build(const struct stage68_macho_probe_result *probe,
                                        const struct stage68_loader_preflight *preflight,
                                        struct stage68_xnu_tte_dryrun *tte)
{
    ...
    uint32_t embedded_base = (uint32_t)(uintptr_t)stage68_embedded_macho;
    uint32_t embedded_limit = 0;
    uint32_t staging_base = (uint32_t)(uintptr_t)stage68_macho_staging_arena;
    uint32_t staging_limit = 0;
    uint32_t safe_base = (uint32_t)(uintptr_t)stage68_safe_table_arena;
    ...
    uint32_t *l1 = (uint32_t *)(void *)stage68_tte_dryrun_arena;
    uint32_t *safe_identity_l1 = (uint32_t *)(void *)stage68_safe_table_arena;
    uint32_t *safe_highva_l1 = (uint32_t *)(void *)(stage68_safe_table_arena + STAGE68_XNU_TTE_L1_SIZE);

    memset(tte, 0, sizeof(*tte));
    memset(stage68_tte_dryrun_arena, 0, sizeof(stage68_tte_dryrun_arena));
    memset(stage68_safe_table_arena, 0, sizeof(stage68_safe_table_arena));

    tte->version = STAGE68_XNU_TTE_DRYRUN_VERSION;
    tte->size = sizeof(*tte);
    tte->status = STAGE68_STATUS_BASE;
    ...
    tte->workspace_size = STAGE68_XNU_TTE_WORKSPACE_BYTES;
    tte->workspace_alignment = tte->workspace_base & (STAGE68_XNU_TTE_L1_ALIGN - 1u);
    tte->local_arena_base = (uint32_t)(uintptr_t)stage68_tte_dryrun_arena;
    tte->local_arena_size = sizeof(stage68_tte_dryrun_arena);
    tte->l1_table_base = tte->workspace_base;
    tte->l1_table_size = STAGE68_XNU_TTE_L1_SIZE;
    tte->l1_entry_count = STAGE68_XNU_TTE_L1_ENTRY_COUNT;
    tte->l2_table_size = STAGE68_XNU_TTE_L2_SIZE;
    tte->kernel_section_descriptor = STAGE68_XNU_TTE_DESC_SECTION_SO;
    tte->ram_console_section_descriptor = STAGE68_XNU_TTE_DESC_SECTION_SO;
    tte->device_section_descriptor = STAGE68_XNU_TTE_DESC_SECTION_SO;

    if (add_overflow_u32(tte->workspace_base, tte->workspace_size, &workspace_limit)) {
        tte->failure_mask |= STAGE68_TTE_FAIL_WORKSPACE_RANGE;
    } else {
        tte->workspace_limit = workspace_limit;
    }
    if (add_overflow_u32(tte->local_arena_base, tte->local_arena_size, &local_limit)) {
        tte->failure_mask |= STAGE68_TTE_FAIL_LOCAL_ARENA_ALIAS;
    } else {
        tte->local_arena_limit = local_limit;
    }
    if (add_overflow_u32(tte->l1_table_base, tte->l1_table_size, &l1_limit)) {
        tte->failure_mask |= STAGE68_TTE_FAIL_LAYOUT_OVERFLOW;
    } else {
    ...
    expected_workspace_base = align_up_u32(probe->load_phys_end, STAGE68_XNU_TTE_L1_ALIGN);
    if (tte->workspace_limit > tte->workspace_base && tte->workspace_limit < RAM_CONSOLE_BASE &&
        ...
        tte->satisfied_mask |= STAGE68_TTE_SAT_WORKSPACE_RANGE;
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_WORKSPACE_RANGE;
    }
    if (tte->workspace_alignment == 0u && (tte->l1_table_base & (STAGE68_XNU_TTE_L1_ALIGN - 1u)) == 0u &&
        (tte->l2_table_base & (STAGE68_XNU_TTE_L2_ALIGN - 1u)) == 0u) {
        tte->satisfied_mask |= STAGE68_TTE_SAT_WORKSPACE_ALIGNMENT;
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_WORKSPACE_ALIGNMENT;
    }
    if (tte->l1_table_limit <= tte->workspace_limit && tte->l2_table_limit <= tte->workspace_limit &&
        ...
        tte->l1_table_size == STAGE68_XNU_TTE_L1_SIZE && tte->l2_table_size == STAGE68_XNU_TTE_L2_SIZE) {
        tte->satisfied_mask |= STAGE68_TTE_SAT_LAYOUT_FITS;
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_LAYOUT_OVERFLOW;
    }
    ...
    if (probe->load_phys_end > probe->load_phys_base) {
        ...
        if (tte->kernel_l1_last_index < STAGE68_XNU_TTE_L1_ENTRY_COUNT) {
            tte->satisfied_mask |= STAGE68_TTE_SAT_KERNEL_MAPPING;
        } else {
            tte->failure_mask |= STAGE68_TTE_FAIL_KERNEL_MAPPING;
        }
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_KERNEL_MAPPING;
    }
    ...
    if (tte->ram_console_l1_index < STAGE68_XNU_TTE_L1_ENTRY_COUNT) {
        tte->satisfied_mask |= STAGE68_TTE_SAT_RAM_CONSOLE_MAPPING;
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_RAM_CONSOLE_MAPPING;
    }
    ...
    if (tte->gic_l1_first_index <= tte->gic_l1_last_index &&
        tte->gic_l1_last_index < STAGE68_XNU_TTE_L1_ENTRY_COUNT) {
        tte->satisfied_mask |= STAGE68_TTE_SAT_DEVICE_MAPPING;
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_DEVICE_MAPPING;
    }
    ...
    if (!ranges_overlap_u32(probe->load_phys_base, probe->load_phys_end, tte->workspace_base, tte->workspace_limit) &&
        ...
        tte->satisfied_mask |= STAGE68_TTE_SAT_NO_OVERLAP;
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_OVERLAP;
    }

    if (add_overflow_u32(embedded_base, stage68_embedded_macho_size, &embedded_limit) ||
        add_overflow_u32(staging_base, STAGE68_MACHO_STAGING_ARENA_CAP, &staging_limit) ||
    ...
    if ((tte->failure_mask & (STAGE68_TTE_FAIL_KERNEL_MAPPING | STAGE68_TTE_FAIL_DEVICE_MAPPING |
                              STAGE68_TTE_FAIL_RAM_CONSOLE_MAPPING | STAGE68_TTE_FAIL_LAYOUT_OVERFLOW)) == 0u) {
        stage68_tte_map_section_range(l1, tte->lowmem_l1_first_index, tte->lowmem_l1_last_index,
                                      STAGE68_XNU_TTE_DESC_SECTION_SO);
        stage68_tte_map_section_range(l1, tte->kernel_l1_first_index, tte->kernel_l1_last_index,
                                      STAGE68_XNU_TTE_DESC_SECTION_SO);
        stage68_tte_map_section(l1, tte->ram_console_l1_index, STAGE68_XNU_TTE_DESC_SECTION_SO);
        stage68_tte_map_section_range(l1, tte->gic_l1_first_index, tte->gic_l1_last_index,
                                      STAGE68_XNU_TTE_DESC_SECTION_SO);
    }

    tte->descriptor_expected_attr_mask = STAGE68_XNU_TTE_DESC_SECTION_SO &
                                         STAGE68_XNU_TTE_DESC_ATTR_MASK;
    if ((tte->failure_mask & (STAGE68_TTE_FAIL_KERNEL_MAPPING | STAGE68_TTE_FAIL_DEVICE_MAPPING |
                              STAGE68_TTE_FAIL_RAM_CONSOLE_MAPPING | STAGE68_TTE_FAIL_LAYOUT_OVERFLOW)) == 0u) {
        uint32_t attrs_ok = 1u;
        uint32_t identity_desc_required = STAGE68_TTE_DESC_VERIFY_REQUIRED &
            ~(STAGE68_TTE_DESC_VERIFY_HIGHVA_FIRST | STAGE68_TTE_DESC_VERIFY_HIGHVA_LAST);

        if (stage68_tte_verify_section_range(l1, tte->lowmem_l1_first_index, tte->lowmem_l1_last_index,
                                             STAGE68_XNU_TTE_DESC_SECTION_SO,
                                             &tte->descriptor_lowmem_first_word,
                                             &tte->descriptor_lowmem_last_word)) {
            tte->descriptor_verify_mask |= STAGE68_TTE_DESC_VERIFY_LOWMEM_FIRST |
                                           STAGE68_TTE_DESC_VERIFY_LOWMEM_LAST;
        } else {
            tte->descriptor_failure_mask |= STAGE68_TTE_DESC_VERIFY_LOWMEM_FIRST |
                                            STAGE68_TTE_DESC_VERIFY_LOWMEM_LAST;
        }
        if (stage68_tte_verify_section_range(l1, tte->kernel_l1_first_index, tte->kernel_l1_last_index,
                                             STAGE68_XNU_TTE_DESC_SECTION_SO,
                                             &tte->descriptor_kernel_first_word,
                                             &tte->descriptor_kernel_last_word)) {
            tte->descriptor_verify_mask |= STAGE68_TTE_DESC_VERIFY_KERNEL_FIRST |
                                           STAGE68_TTE_DESC_VERIFY_KERNEL_LAST;
        } else {
            tte->descriptor_failure_mask |= STAGE68_TTE_DESC_VERIFY_KERNEL_FIRST |
                                            STAGE68_TTE_DESC_VERIFY_KERNEL_LAST;
        }
        if (stage68_tte_verify_section_slot(l1, tte->ram_console_l1_index,
                                            STAGE68_XNU_TTE_DESC_SECTION_SO,
                                            &tte->descriptor_ram_console_word)) {
            tte->descriptor_verify_mask |= STAGE68_TTE_DESC_VERIFY_RAM_CONSOLE;
        } else {
            tte->descriptor_failure_mask |= STAGE68_TTE_DESC_VERIFY_RAM_CONSOLE;
        }
        if (stage68_tte_verify_section_range(l1, tte->gic_l1_first_index, tte->gic_l1_last_index,
                                             STAGE68_XNU_TTE_DESC_SECTION_SO,
                                             &tte->descriptor_gic_first_word,
                                             &tte->descriptor_gic_last_word)) {
            tte->descriptor_verify_mask |= STAGE68_TTE_DESC_VERIFY_GIC_FIRST |
                                           STAGE68_TTE_DESC_VERIFY_GIC_LAST;
        } else {
            tte->descriptor_failure_mask |= STAGE68_TTE_DESC_VERIFY_GIC_FIRST |
                                            STAGE68_TTE_DESC_VERIFY_GIC_LAST;
        }

        tte->descriptor_type_mask_seen =
            (tte->descriptor_lowmem_first_word & STAGE68_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_lowmem_last_word & STAGE68_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_kernel_first_word & STAGE68_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_kernel_last_word & STAGE68_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_ram_console_word & STAGE68_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_gic_first_word & STAGE68_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_gic_last_word & STAGE68_XNU_TTE_DESC_TYPE_MASK);
        tte->descriptor_attr_mask_seen =
            (tte->descriptor_lowmem_first_word & STAGE68_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_lowmem_last_word & STAGE68_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_kernel_first_word & STAGE68_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_kernel_last_word & STAGE68_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_ram_console_word & STAGE68_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_gic_first_word & STAGE68_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_gic_last_word & STAGE68_XNU_TTE_DESC_ATTR_MASK);

        attrs_ok &= stage68_tte_descriptor_attrs_ok(tte->descriptor_lowmem_first_word,
                                                    STAGE68_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage68_tte_descriptor_attrs_ok(tte->descriptor_lowmem_last_word,
                                                    STAGE68_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage68_tte_descriptor_attrs_ok(tte->descriptor_kernel_first_word,
                                                    STAGE68_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage68_tte_descriptor_attrs_ok(tte->descriptor_kernel_last_word,
                                                    STAGE68_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage68_tte_descriptor_attrs_ok(tte->descriptor_ram_console_word,
                                                    STAGE68_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage68_tte_descriptor_attrs_ok(tte->descriptor_gic_first_word,
                                                    STAGE68_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage68_tte_descriptor_attrs_ok(tte->descriptor_gic_last_word,
                                                    STAGE68_XNU_TTE_DESC_SECTION_SO);

        if ((tte->descriptor_verify_mask & identity_desc_required) == identity_desc_required &&
            tte->descriptor_failure_mask == 0u) {
            tte->satisfied_mask |= STAGE68_TTE_SAT_DESCRIPTOR_READBACK;
        } else {
            tte->failure_mask |= STAGE68_TTE_FAIL_DESCRIPTOR_READBACK;
        }
        if (attrs_ok && tte->descriptor_attr_mask_seen == tte->descriptor_expected_attr_mask &&
            tte->descriptor_type_mask_seen == STAGE68_XNU_TTE_DESC_TYPE_SECTION) {
            tte->satisfied_mask |= STAGE68_TTE_SAT_DESCRIPTOR_ATTRS;
        } else {
            tte->failure_mask |= STAGE68_TTE_FAIL_DESCRIPTOR_ATTRS;
        }
        ...
        if (probe->load_phys_end > probe->load_phys_base) {
            ...
            if (stage68_tte_expect_translation(l1, tte->translation_kernel_text_va,
                                               tte->translation_kernel_text_va,
                                               &tte->translation_kernel_text_pa)) {
                tte->translation_check_mask |= STAGE68_TTE_TRANSLATE_KERNEL_FIRST;
                tte->translation_case_count++;
            } else {
                tte->translation_failure_mask |= STAGE68_TTE_TRANSLATE_KERNEL_FIRST;
            }

            tte->translation_kernel_last_va = kernel_last;
            if (stage68_tte_expect_translation(l1, tte->translation_kernel_last_va,
                                               tte->translation_kernel_last_va,
                                               &tte->translation_kernel_last_pa)) {
                tte->translation_check_mask |= STAGE68_TTE_TRANSLATE_KERNEL_LAST;
                tte->translation_case_count++;
            } else {
                tte->translation_failure_mask |= STAGE68_TTE_TRANSLATE_KERNEL_LAST;
            }
        }

        tte->translation_lowmem_va = RAM_PHYS_BASE;
        if (stage68_tte_expect_translation(l1, tte->translation_lowmem_va,
                                           tte->translation_lowmem_va,
                                           &tte->translation_lowmem_pa)) {
            tte->translation_check_mask |= STAGE68_TTE_TRANSLATE_LOW_RAM;
            tte->translation_case_count++;
        } else {
            tte->translation_failure_mask |= STAGE68_TTE_TRANSLATE_LOW_RAM;
        }

        tte->translation_ram_console_va = RAM_CONSOLE_BASE;
        if (stage68_tte_expect_translation(l1, tte->translation_ram_console_va,
                                           tte->translation_ram_console_va,
                                           &tte->translation_ram_console_pa)) {
            tte->translation_check_mask |= STAGE68_TTE_TRANSLATE_RAM_CONSOLE;
            tte->translation_case_count++;
        } else {
            tte->translation_failure_mask |= STAGE68_TTE_TRANSLATE_RAM_CONSOLE;
        }

        tte->translation_gic_va = 0xf9000000u;
        if (stage68_tte_expect_translation(l1, tte->translation_gic_va,
                                           tte->translation_gic_va,
                                           &tte->translation_gic_pa)) {
            tte->translation_check_mask |= STAGE68_TTE_TRANSLATE_GIC;
            tte->translation_case_count++;
        } else {
            tte->translation_failure_mask |= STAGE68_TTE_TRANSLATE_GIC;
        }

        if ((tte->translation_check_mask & (STAGE68_TTE_TRANSLATE_KERNEL_FIRST |
                                            STAGE68_TTE_TRANSLATE_KERNEL_LAST)) ==
            (STAGE68_TTE_TRANSLATE_KERNEL_FIRST | STAGE68_TTE_TRANSLATE_KERNEL_LAST)) {
            tte->satisfied_mask |= STAGE68_TTE_SAT_TRANSLATION_KERNEL;
        } else {
            tte->failure_mask |= STAGE68_TTE_FAIL_TRANSLATION_KERNEL;
        }
        if ((tte->translation_check_mask & STAGE68_TTE_TRANSLATE_LOW_RAM) != 0u) {
            tte->satisfied_mask |= STAGE68_TTE_SAT_TRANSLATION_LOW_RAM;
        } else {
            tte->failure_mask |= STAGE68_TTE_FAIL_TRANSLATION_LOW_RAM;
        }
        if ((tte->translation_check_mask & STAGE68_TTE_TRANSLATE_RAM_CONSOLE) != 0u) {
            tte->satisfied_mask |= STAGE68_TTE_SAT_TRANSLATION_CONSOLE;
        } else {
            tte->failure_mask |= STAGE68_TTE_FAIL_TRANSLATION_CONSOLE;
        }
        if ((tte->translation_check_mask & STAGE68_TTE_TRANSLATE_GIC) != 0u) {
            tte->satisfied_mask |= STAGE68_TTE_SAT_TRANSLATION_DEVICE;
        } else {
            tte->failure_mask |= STAGE68_TTE_FAIL_TRANSLATION_DEVICE;
        }
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_DESCRIPTOR_READBACK |
                             STAGE68_TTE_FAIL_DESCRIPTOR_ATTRS |
                             STAGE68_TTE_FAIL_TRANSLATION_KERNEL |
                             STAGE68_TTE_FAIL_TRANSLATION_LOW_RAM |
                             STAGE68_TTE_FAIL_TRANSLATION_CONSOLE |
                             STAGE68_TTE_FAIL_TRANSLATION_DEVICE;
    }
    ...
    if (tte->ttbr0_written == 0u && tte->ttbr1_written == 0u) {
        tte->satisfied_mask |= STAGE68_TTE_SAT_NO_TTBR_WRITE;
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_SAFETY;
    }
    if (tte->caches_enabled == 0u) {
        tte->satisfied_mask |= STAGE68_TTE_SAT_CACHES_UNCHANGED;
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_SAFETY;
    }
    if (stage68_tte_l2_and_scratch_zeroed()) {
        tte->local_sim_zeroed = 1u;
        tte->satisfied_mask |= STAGE68_TTE_SAT_LOCAL_SIM_ONLY;
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_SAFETY;
    }
    ...
    tte->safe_table.version = STAGE68_XNU_SAFE_TABLE_VERSION;
    tte->safe_table.size = sizeof(tte->safe_table);
    tte->safe_table.status = STAGE68_STATUS_BASE;
    tte->safe_table.local_base = safe_base;
    if (add_overflow_u32(safe_base, sizeof(stage68_safe_table_arena), &safe_limit)) {
        tte->safe_table.failure_mask |= STAGE68_TTE_FAIL_SAFE_TABLE;
    } else {
        tte->safe_table.local_limit = safe_limit;
    }
    tte->safe_table.identity_l1_base = safe_base;
    tte->safe_table.identity_l1_limit = safe_base + STAGE68_XNU_TTE_L1_SIZE;
    tte->safe_table.highva_l1_base = safe_base + STAGE68_XNU_TTE_L1_SIZE;
    tte->safe_table.highva_l1_limit = safe_base + (STAGE68_XNU_TTE_L1_SIZE * 2u);
    tte->safe_table.l1_entry_count = STAGE68_XNU_SAFE_TABLE_L1_COUNT;
    tte->safe_table.l1_bytes = STAGE68_XNU_SAFE_TABLE_L1_BYTES;
    ...
    if (tte->safe_table.local_base != 0u && tte->safe_table.local_limit > tte->safe_table.local_base &&
        (tte->safe_table.local_base & (STAGE68_XNU_SAFE_TABLE_ALIGNMENT - 1u)) == 0u &&
        tte->safe_table.identity_l1_limit == tte->safe_table.highva_l1_base &&
        tte->safe_table.highva_l1_limit == tte->safe_table.local_limit) {
        memcpy(safe_identity_l1, l1, STAGE68_XNU_TTE_L1_SIZE);
        tte->safe_table.kind_mask |= STAGE68_XNU_SAFE_TABLE_KIND_IDENTITY;
        tte->safe_table.materialized_mask |= STAGE68_XNU_SAFE_TABLE_MAT_IDENTITY;
        tte->safe_table.written_bytes += STAGE68_XNU_TTE_L1_SIZE;
    } else {
        tte->safe_table.failure_mask |= STAGE68_TTE_FAIL_SAFE_TABLE;
    }

    if (probe->load_vm_end > probe->load_vm_base && probe->load_phys_end > probe->load_phys_base) {
        tte->highva_virt_base = probe->load_vm_base & STAGE68_XNU_TTE_DESC_BASE_MASK;
        tte->highva_virt_end = probe->load_vm_end;
        tte->highva_phys_base = probe->load_phys_base & STAGE68_XNU_TTE_DESC_BASE_MASK;
        ...
        highva_first_expected_pa = stage68_tte_expected_section_pa(tte->highva_virt_base,
                                                                   tte->highva_phys_base);
        highva_last_expected_pa = stage68_tte_expected_section_pa(highva_last_va,
                                                                  stage68_tte_section_pa_for_va(highva_last_va,
                                                                                                tte->highva_virt_base,
                                                                                                tte->highva_phys_base));

        if (tte->highva_l1_first_index <= tte->highva_l1_last_index &&
            tte->highva_l1_last_index < STAGE68_XNU_TTE_L1_ENTRY_COUNT && highva_size != 0u) {
            stage68_tte_map_section_range_pa(safe_highva_l1, tte->highva_virt_base,
                                             tte->highva_phys_base, highva_size,
                                             STAGE68_XNU_TTE_DESC_SECTION_SO);
            if (stage68_tte_verify_section_slot_pa(safe_highva_l1, tte->highva_virt_base,
                                                   tte->highva_phys_base,
                                                   STAGE68_XNU_TTE_DESC_SECTION_SO,
                                                   &tte->descriptor_highva_first_word)) {
                tte->descriptor_verify_mask |= STAGE68_TTE_DESC_VERIFY_HIGHVA_FIRST;
            } else {
                tte->descriptor_failure_mask |= STAGE68_TTE_DESC_VERIFY_HIGHVA_FIRST;
            }
            if (stage68_tte_verify_section_slot_pa(safe_highva_l1, highva_last_va,
                                                   stage68_tte_section_pa_for_va(highva_last_va,
                                                                                 tte->highva_virt_base,
                                                                                 tte->highva_phys_base),
                                                   STAGE68_XNU_TTE_DESC_SECTION_SO,
                                                   &tte->descriptor_highva_last_word)) {
                tte->descriptor_verify_mask |= STAGE68_TTE_DESC_VERIFY_HIGHVA_LAST;
            } else {
                tte->descriptor_failure_mask |= STAGE68_TTE_DESC_VERIFY_HIGHVA_LAST;
            }

            tte->descriptor_type_mask_seen |=
                (tte->descriptor_highva_first_word & STAGE68_XNU_TTE_DESC_TYPE_MASK) |
                (tte->descriptor_highva_last_word & STAGE68_XNU_TTE_DESC_TYPE_MASK);
            tte->descriptor_attr_mask_seen |=
                (tte->descriptor_highva_first_word & STAGE68_XNU_TTE_DESC_ATTR_MASK) |
                (tte->descriptor_highva_last_word & STAGE68_XNU_TTE_DESC_ATTR_MASK);

            tte->translation_highva_first_va = tte->highva_virt_base;
            if (stage68_tte_expect_translation(safe_highva_l1, tte->translation_highva_first_va,
                                               highva_first_expected_pa,
                                               &tte->translation_highva_first_pa)) {
                tte->translation_check_mask |= STAGE68_TTE_TRANSLATE_HIGHVA_FIRST;
                tte->translation_case_count++;
            } else {
                tte->translation_failure_mask |= STAGE68_TTE_TRANSLATE_HIGHVA_FIRST;
            }
            tte->translation_highva_last_va = highva_last_va;
            if (stage68_tte_expect_translation(safe_highva_l1, tte->translation_highva_last_va,
                                               highva_last_expected_pa,
                                               &tte->translation_highva_last_pa)) {
                tte->translation_check_mask |= STAGE68_TTE_TRANSLATE_HIGHVA_LAST;
                tte->translation_case_count++;
            } else {
                tte->translation_failure_mask |= STAGE68_TTE_TRANSLATE_HIGHVA_LAST;
            }

            if ((tte->descriptor_verify_mask & (STAGE68_TTE_DESC_VERIFY_HIGHVA_FIRST |
                                                STAGE68_TTE_DESC_VERIFY_HIGHVA_LAST)) ==
                (STAGE68_TTE_DESC_VERIFY_HIGHVA_FIRST | STAGE68_TTE_DESC_VERIFY_HIGHVA_LAST) &&
                (tte->descriptor_failure_mask & (STAGE68_TTE_DESC_VERIFY_HIGHVA_FIRST |
                                                 STAGE68_TTE_DESC_VERIFY_HIGHVA_LAST)) == 0u) {
                tte->satisfied_mask |= STAGE68_TTE_SAT_HIGHVA_MAPPING;
                tte->safe_table.kind_mask |= STAGE68_XNU_SAFE_TABLE_KIND_HIGHVA;
                tte->safe_table.materialized_mask |= STAGE68_XNU_SAFE_TABLE_MAT_HIGHVA;
                tte->safe_table.written_bytes += STAGE68_XNU_TTE_L1_SIZE;
            } else {
                tte->failure_mask |= STAGE68_TTE_FAIL_HIGHVA_MAPPING;
            }
            if ((tte->translation_check_mask & (STAGE68_TTE_TRANSLATE_HIGHVA_FIRST |
                                                STAGE68_TTE_TRANSLATE_HIGHVA_LAST)) ==
                (STAGE68_TTE_TRANSLATE_HIGHVA_FIRST | STAGE68_TTE_TRANSLATE_HIGHVA_LAST) &&
                (tte->translation_failure_mask & (STAGE68_TTE_TRANSLATE_HIGHVA_FIRST |
                                                  STAGE68_TTE_TRANSLATE_HIGHVA_LAST)) == 0u) {
                tte->satisfied_mask |= STAGE68_TTE_SAT_HIGHVA_TRANSLATION;
            } else {
                tte->failure_mask |= STAGE68_TTE_FAIL_HIGHVA_TRANSLATION;
            }
        } else {
            tte->failure_mask |= STAGE68_TTE_FAIL_HIGHVA_MAPPING | STAGE68_TTE_FAIL_HIGHVA_TRANSLATION;
        }
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_HIGHVA_MAPPING | STAGE68_TTE_FAIL_HIGHVA_TRANSLATION;
    }

    if (tte->descriptor_verify_mask == STAGE68_TTE_DESC_VERIFY_REQUIRED &&
        tte->descriptor_failure_mask == 0u) {
        tte->satisfied_mask |= STAGE68_TTE_SAT_DESCRIPTOR_READBACK;
    } else {
        tte->failure_mask |= STAGE68_TTE_FAIL_DESCRIPTOR_READBACK;
    ...
    if (tte->safe_table.kind_mask == STAGE68_XNU_SAFE_TABLE_KIND_REQUIRED &&
        tte->safe_table.materialized_mask == STAGE68_XNU_SAFE_TABLE_MAT_REQUIRED &&
        tte->safe_table.written_bytes == (STAGE68_XNU_TTE_L1_SIZE * 2u) &&
        tte->safe_table.failure_mask == 0u) {
        tte->safe_table.status = STAGE68_STATUS_OK;
        tte->satisfied_mask |= STAGE68_TTE_SAT_SAFE_TABLE;
    } else {
        tte->safe_table.failure_mask |= STAGE68_TTE_FAIL_SAFE_TABLE;
        tte->failure_mask |= STAGE68_TTE_FAIL_SAFE_TABLE;
    }
    ...
    if (add_overflow_u32(embedded_base, stage68_embedded_macho_size, &embedded_limit) ||
        ...
        (probe->load_phys_end != 0u && ranges_overlap_u32(tte->safe_table.local_base, tte->safe_table.local_limit,
                                                          probe->load_phys_base, probe->load_phys_end))) {
        tte->safe_table.failure_mask |= STAGE68_TTE_FAIL_STAGE_OWNED_TABLES;
        tte->failure_mask |= STAGE68_TTE_FAIL_STAGE_OWNED_TABLES;
        ...
        tte->satisfied_mask |= STAGE68_TTE_SAT_STAGE_OWNED_TABLES;
    } else {
        tte->safe_table.failure_mask |= STAGE68_TTE_FAIL_STAGE_OWNED_TABLES;
        tte->failure_mask |= STAGE68_TTE_FAIL_STAGE_OWNED_TABLES;
    }

    tte->safe_table.checksum = stage68_safe_table_checksum(stage68_safe_table_arena,
                                                           sizeof(stage68_safe_table_arena));

    tte->checksum = stage68_tte_dryrun_checksum(tte);
    if (tte->checksum != stage68_tte_dryrun_checksum(tte)) {
        tte->failure_mask |= STAGE68_TTE_FAIL_CHECKSUM;
    }

    if (tte->failure_mask == 0u && tte->satisfied_mask == STAGE68_TTE_SAT_REQUIRED &&
        tte->checksum == stage68_tte_dryrun_checksum(tte)) {
        tte->status = STAGE68_STATUS_OK;
        xnu_log_puts("Stage68 XNU TTE dry-run ok\n");
        return 1;
    }

    tte->status = STAGE68_STATUS_BASE | tte->failure_mask;
    xnu_log_puts("Stage68 XNU TTE dry-run failed\n");
    return 0;
}

static void stage68_xnu_tte_dryrun_log(const struct stage68_xnu_tte_dryrun *tte)
{
    ...
    xnu_log_kv32("xnu_highva_dryrun_status", ((tte->satisfied_mask & (STAGE68_TTE_SAT_HIGHVA_MAPPING | STAGE68_TTE_SAT_HIGHVA_TRANSLATION)) == (STAGE68_TTE_SAT_HIGHVA_MAPPING | STAGE68_TTE_SAT_HIGHVA_TRANSLATION)) ? STAGE68_STATUS_OK : STAGE68_STATUS_BASE);
...
static uint32_t stage68_loader_preflight_checksum(volatile const struct stage68_loader_preflight *preflight)
{
    volatile const uint32_t *words = (volatile const uint32_t *)(uintptr_t)preflight;
    uint32_t count = (uint32_t)(offsetof(struct stage68_loader_preflight, checksum) / sizeof(uint32_t));
    uint32_t checksum = 0;
...
static uint32_t stage68_dt_semantic_mask(struct boot_args *args)
{
    uint32_t mask = 0;
    uint32_t len = 0;
    ...
    const void *platform_personality = find_child(args, root, "stage68-platform-personality");
    const void *interrupt_personality = find_child(args, root, "stage68-interrupt-personality");
    const void *timer_personality = find_child(args, root, "stage68-timer-personality");
    const void *cpu_personality = find_child(args, root, "stage68-cpu-personality");
    const void *rejected_personality = find_child(args, root, "stage68-rejected-personality");

    if (root && args->deviceTreeLength && apple_dt_node_prop_count(root, args->deviceTreeLength, root) != 0u) {
        mask |= STAGE68_DT_READY_BINARY_SELFTEST;
    }
    if (chosen) {
        mask |= STAGE68_DT_READY_CHOSEN;
        if (apple_dt_get_prop(args->deviceTreeP, args->deviceTreeLength, chosen, "boot-args", &len) && len != 0u) {
            mask |= STAGE68_DT_READY_BOOT_ARGS;
        }
        if (apple_dt_get_prop(args->deviceTreeP, args->deviceTreeLength, chosen, "ram-console-reg", &len) && len >= 8u) {
            mask |= STAGE68_DT_READY_RAM_CONSOLE;
        }
    }
    if (memory && apple_dt_get_prop(args->deviceTreeP, args->deviceTreeLength, memory, "reg", &len) && len >= 8u) {
        mask |= STAGE68_DT_READY_MEMORY;
    }
    ...
    if (iokit_platform &&
        ...
        mask |= STAGE68_DT_READY_IOKIT_SCAFFOLD;
    }
    if (platform_driver &&
        ...
        mask |= STAGE68_DT_READY_PLATFORM_DRIVER;
    }
    if (interrupt_service && timer_service && cpu_service &&
        ...
        STAGE68_XNU_IOKIT_REGISTRY_SERVICE_EXPECTED_IRQ_COUNT_MIN &&
        ...
        mask |= STAGE68_DT_READY_IOKIT_REGISTRY_SERVICES;
    }
    if (rejected_driver &&
        ...
        mask |= STAGE68_DT_READY_IOKIT_REJECTED_DRIVER;
    }
    if (catalog_root && platform_personality && interrupt_personality && timer_personality && cpu_personality &&
        ...
        STAGE68_XNU_IOKIT_CATALOG_PROPERTY_CATALOG_VERSION &&
        ...
        mask |= STAGE68_DT_READY_IOKIT_CATALOG_PROPERTY;
    }
...
{
    struct stage68_macho_probe_result probe;
    struct stage68_loader_preflight *preflight = &stage68_loader_preflight_block;
    const struct stage68_ttbr0_roundtrip *ttbr_rt = mmu_stage68_ttbr0_roundtrip_result();
    const struct stage68_xnu_workspace *xnu_ws;
    const struct stage68_xnu_compile_graph *xnu_graph;
    const struct stage68_xnu_object_subset *xnu_obj;
    const struct stage68_xnu_link *xnu_link;
    const struct stage68_xnu_pmap_bootstrap_contract *pmap_contract;
    const struct stage68_xnu_pmap_table_dryrun_contract *pmap_table_contract;
    const struct stage68_xnu_pmap_page_dryrun_contract *pmap_page_contract;
    const struct stage68_xnu_pmap_attr_dryrun_contract *pmap_attr_contract;
    const struct stage68_xnu_pmap_multiwindow_dryrun_contract *pmap_multiwindow_contract;
    const struct stage68_xnu_pmap_transition_dryrun_contract *pmap_transition_contract;
    const struct stage68_xnu_pexpert_hook_readiness_contract *pexpert_hook_contract;
    const struct stage68_xnu_iokit_platform_scaffold_contract *iokit_platform_contract;
    const struct stage68_xnu_iokit_match_dryrun_contract *iokit_match_contract;
    const struct stage68_xnu_iokit_registry_service_dryrun_contract *iokit_registry_contract;
    const struct stage68_xnu_iokit_provider_plane_dryrun_contract *iokit_provider_contract;
    const struct stage68_xnu_iokit_catalog_property_dryrun_contract *iokit_catalog_contract;
    uint32_t workspace_limit;
    uint32_t loaded_end_for_workspace;
    uint32_t macho_ok;

    xnu_log_puts("Stage68 Mach-O/XNU loader preflight begin\n");
    memset(preflight, 0, sizeof(*preflight));
    preflight->version = STAGE68_LOADER_PREFLIGHT_VERSION;
    preflight->size = sizeof(*preflight);
    preflight->xnu_baseline_tag = STAGE68_XNU_BASELINE_2050_22_13;
    preflight->xnu_baseline_commit = STAGE68_XNU_BASELINE_COMMIT_CC8A9B0C;
    preflight->xnu_master_version = STAGE68_XNU_BASELINE_MASTER_12_3_0;
    preflight->xnu_arm_reference = STAGE68_XNU_ARM_REFERENCE_4570_1_46;

    macho_ok = (uint32_t)stage68_macho_probe(stage68_embedded_macho, stage68_embedded_macho_size, &probe);
    if (macho_ok) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_MACHO;
    }
    if (macho_ok && stage68_macho_materialize_to_arena(stage68_embedded_macho, stage68_embedded_macho_size, &probe)) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_MATERIALIZED;
    }
    ...
    preflight->proposed_topOfKernelData = align_up_u32(loaded_end_for_workspace, STAGE68_XNU_TTE_ALIGNMENT);
    preflight->proposed_ttep_workspace_base = preflight->proposed_topOfKernelData;
    preflight->proposed_ttep_workspace_size = STAGE68_XNU_TTE_WORKSPACE_BYTES;
    ...
    preflight->proposed_workspace_alignment = preflight->proposed_topOfKernelData & (STAGE68_XNU_TTE_ALIGNMENT - 1u);
    ...
    preflight->apple_dt_semantic_mask = stage68_dt_semantic_mask(args);
    preflight->pexpert_gap_mask = STAGE68_PLATFORM_GAP_REQUIRED_RECORDED;
    preflight->platform_gap_mask = STAGE68_PLATFORM_GAP_REQUIRED_RECORDED;
    preflight->interrupt_ready_mask = STAGE68_LOADER_IRQ_READY_GIC_DIST |
        STAGE68_LOADER_IRQ_READY_GIC_CPU |
        STAGE68_LOADER_IRQ_READY_TIMEBASE;
    preflight->safety_mask = STAGE68_LOADER_SAFETY_NO_EXECUTE |
    ...
    if (ttbr_rt->status == STAGE68_STATUS_OK &&
        (ttbr_rt->satisfied_mask & STAGE68_TTBR_RT_SAT_STAGE_OWNED_TABLE) != 0u) {
        preflight->safety_mask |= STAGE68_LOADER_SAFETY_STAGE_OWNED_TTBR;
    }
    if (ttbr_rt->status == STAGE68_STATUS_OK &&
        (ttbr_rt->satisfied_mask & STAGE68_TTBR_RT_SAT_ORIGINAL_RESTORED) != 0u) {
        preflight->safety_mask |= STAGE68_LOADER_SAFETY_TTBR_RESTORED;
    }
    if (ttbr_rt->status == STAGE68_STATUS_OK && ttbr_rt->caches_changed == 0u &&
        (ttbr_rt->satisfied_mask & STAGE68_TTBR_RT_SAT_CACHE_BITS_PRESERVED) != 0u) {
        preflight->safety_mask |= STAGE68_LOADER_SAFETY_CACHES_UNCHANGED |
            STAGE68_LOADER_SAFETY_NO_CACHE_CHANGE;
    }

    if (preflight->xnu_baseline_tag == STAGE68_XNU_BASELINE_2050_22_13 &&
        preflight->xnu_baseline_commit == STAGE68_XNU_BASELINE_COMMIT_CC8A9B0C &&
        preflight->xnu_master_version == STAGE68_XNU_BASELINE_MASTER_12_3_0 &&
        preflight->xnu_arm_reference == STAGE68_XNU_ARM_REFERENCE_4570_1_46) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_BASELINE;
    }
    if (preflight->boot_args_rev_ver == 0x00020002u &&
        ...
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_BOOT_ARGS;
    }
    if (preflight->proposed_workspace_range_ok == 1u) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_WORKSPACE;
    }
    if (preflight->apple_dt_semantic_mask == STAGE68_DT_READY_REQUIRED) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_DT;
    }
    if (stage68_xnu_tte_dryrun_build(&probe, preflight, &preflight->tte_dryrun)) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_TTE_DRYRUN;
        if ((preflight->tte_dryrun.satisfied_mask & (STAGE68_TTE_SAT_DESCRIPTOR_READBACK |
                                                     STAGE68_TTE_SAT_DESCRIPTOR_ATTRS)) ==
            (STAGE68_TTE_SAT_DESCRIPTOR_READBACK | STAGE68_TTE_SAT_DESCRIPTOR_ATTRS)) {
            preflight->satisfied_mask |= STAGE68_LOADER_SAT_TTE_VERIFY;
        }
        if ((preflight->tte_dryrun.satisfied_mask & (STAGE68_TTE_SAT_TRANSLATION_KERNEL |
                                                     STAGE68_TTE_SAT_TRANSLATION_LOW_RAM |
                                                     STAGE68_TTE_SAT_TRANSLATION_CONSOLE |
                                                     STAGE68_TTE_SAT_TRANSLATION_DEVICE |
                                                     STAGE68_TTE_SAT_XNU_BOOT_POLICY)) ==
            (STAGE68_TTE_SAT_TRANSLATION_KERNEL |
             STAGE68_TTE_SAT_TRANSLATION_LOW_RAM |
             STAGE68_TTE_SAT_TRANSLATION_CONSOLE |
             STAGE68_TTE_SAT_TRANSLATION_DEVICE |
             STAGE68_TTE_SAT_XNU_BOOT_POLICY)) {
            preflight->satisfied_mask |= STAGE68_LOADER_SAT_VTOP_DRYRUN;
        }
        if ((preflight->tte_dryrun.satisfied_mask & (STAGE68_TTE_SAT_HIGHVA_MAPPING |
                                                     STAGE68_TTE_SAT_HIGHVA_TRANSLATION)) ==
            (STAGE68_TTE_SAT_HIGHVA_MAPPING | STAGE68_TTE_SAT_HIGHVA_TRANSLATION)) {
            preflight->satisfied_mask |= STAGE68_LOADER_SAT_HIGHVA_DRYRUN;
        }
        if ((preflight->tte_dryrun.satisfied_mask & STAGE68_TTE_SAT_SAFE_TABLE) != 0u) {
            preflight->satisfied_mask |= STAGE68_LOADER_SAT_SAFE_TABLE;
        }
        if ((preflight->tte_dryrun.satisfied_mask & STAGE68_TTE_SAT_STAGE_OWNED_TABLES) != 0u) {
            preflight->satisfied_mask |= STAGE68_LOADER_SAT_STAGE_OWNED_TABLES;
        }
    }
    ...
    preflight->tte_verify_status = ((preflight->satisfied_mask & STAGE68_LOADER_SAT_TTE_VERIFY) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;
    preflight->vtop_dryrun_status = ((preflight->satisfied_mask & STAGE68_LOADER_SAT_VTOP_DRYRUN) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;
    preflight->highva_dryrun_status = ((preflight->satisfied_mask & STAGE68_LOADER_SAT_HIGHVA_DRYRUN) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;
    preflight->safe_table_status = ((preflight->satisfied_mask & STAGE68_LOADER_SAT_SAFE_TABLE) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;
    preflight->stage_owned_tables_status = ((preflight->satisfied_mask & STAGE68_LOADER_SAT_STAGE_OWNED_TABLES) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;
    ...
    if (ttbr_rt->status == STAGE68_STATUS_OK) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_TTBR_ROUNDTRIP;
    }
    if (ttbr_rt->status == STAGE68_STATUS_OK &&
        (ttbr_rt->satisfied_mask & STAGE68_TTBR_RT_SAT_STAGE_OWNED_TABLE) != 0u) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_STAGE_OWNED_TTBR;
    }
    if (ttbr_rt->status == STAGE68_STATUS_OK &&
        (ttbr_rt->satisfied_mask & STAGE68_TTBR_RT_SAT_ORIGINAL_RESTORED) != 0u) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_TTBR_RESTORED;
    }
    if (ttbr_rt->status == STAGE68_STATUS_OK && ttbr_rt->caches_changed == 0u &&
        (ttbr_rt->satisfied_mask & STAGE68_TTBR_RT_SAT_CACHE_BITS_PRESERVED) != 0u) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_CACHE_PRESERVED;
    }
    preflight->stage_owned_ttbr_status = ((preflight->satisfied_mask & STAGE68_LOADER_SAT_STAGE_OWNED_TTBR) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;
    preflight->ttbr_restored_status = ((preflight->satisfied_mask & STAGE68_LOADER_SAT_TTBR_RESTORED) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;
    preflight->cache_preserved_status = ((preflight->satisfied_mask & STAGE68_LOADER_SAT_CACHE_PRESERVED) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_workspace_selftest()) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_WORKSPACE;
    }
    xnu_ws = stage68_xnu_workspace_result();
    ...
    if (xnu_ws->status == STAGE68_STATUS_OK && xnu_ws->cancro_target_declared == 1u &&
        xnu_ws->armv7_target_declared == 1u && xnu_ws->msm8974_target_declared == 1u) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_CANCRO_TARGET;
        preflight->cancro_target_status = STAGE68_STATUS_OK;
    } else {
        preflight->cancro_target_status = STAGE68_STATUS_BASE;
    }
    if (xnu_ws->status == STAGE68_STATUS_OK && xnu_ws->stage68_plan_ready == 1u &&
        xnu_ws->public_2050_arm_gap_recorded == 1u) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_STAGE68_PLAN;
        preflight->stage68_plan_status = STAGE68_STATUS_OK;
    } else {
        preflight->stage68_plan_status = STAGE68_STATUS_BASE;
    }

    if (stage68_xnu_compile_graph_selftest()) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_COMPILE_GRAPH;
    }
    xnu_graph = stage68_xnu_compile_graph_result();
    ...
    preflight->xnu_compile_graph_status_rollup =
        ((preflight->satisfied_mask & STAGE68_LOADER_SAT_XNU_COMPILE_GRAPH) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_object_subset_selftest()) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_OBJECT_SUBSET;
    }
    xnu_obj = stage68_xnu_object_subset_result();
    ...
    preflight->xnu_object_subset_status_rollup =
        ((preflight->satisfied_mask & STAGE68_LOADER_SAT_XNU_OBJECT_SUBSET) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_link_selftest()) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_LINK;
    }
    xnu_link = stage68_xnu_link_result();
    ...
    preflight->xnu_link_status_rollup =
        ((preflight->satisfied_mask & STAGE68_LOADER_SAT_XNU_LINK) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (preflight->safety_mask == STAGE68_LOADER_SAFETY_REQUIRED) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_SAFETY;
    }

    if (stage68_xnu_bootstrap_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_BOOTSTRAP_CONTRACT;
    }
    {
        const struct stage68_xnu_bootstrap_contract *contract = stage68_xnu_bootstrap_contract_result();
        ...
        preflight->xnu_bootstrap_contract_status_rollup =
            ((preflight->satisfied_mask & STAGE68_LOADER_SAT_XNU_BOOTSTRAP_CONTRACT) != 0u) ?
            STAGE68_STATUS_OK : STAGE68_STATUS_BASE;
    }

    if (stage68_xnu_pmap_bootstrap_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_PMAP_BOOTSTRAP_CONTRACT;
    }
    pmap_contract = stage68_xnu_pmap_bootstrap_contract_result();
    ...
    preflight->xnu_pmap_bootstrap_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE68_LOADER_SAT_XNU_PMAP_BOOTSTRAP_CONTRACT) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_pmap_table_dryrun_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_PMAP_TABLE_DRYRUN_CONTRACT;
    }
    pmap_table_contract = stage68_xnu_pmap_table_dryrun_contract_result();
    ...
    preflight->xnu_pmap_table_dryrun_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE68_LOADER_SAT_XNU_PMAP_TABLE_DRYRUN_CONTRACT) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_pmap_page_dryrun_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_PMAP_PAGE_DRYRUN_CONTRACT;
    }
    pmap_page_contract = stage68_xnu_pmap_page_dryrun_contract_result();
    ...
    preflight->xnu_pmap_page_dryrun_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE68_LOADER_SAT_XNU_PMAP_PAGE_DRYRUN_CONTRACT) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_pmap_attr_dryrun_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_PMAP_ATTR_DRYRUN_CONTRACT;
    }
    pmap_attr_contract = stage68_xnu_pmap_attr_dryrun_contract_result();
    ...
    preflight->xnu_pmap_attr_dryrun_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE68_LOADER_SAT_XNU_PMAP_ATTR_DRYRUN_CONTRACT) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_pmap_multiwindow_dryrun_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_PMAP_MULTIWINDOW_DRYRUN_CONTRACT;
    }
    pmap_multiwindow_contract = stage68_xnu_pmap_multiwindow_dryrun_contract_result();
    ...
    preflight->xnu_pmap_multiwindow_dryrun_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE68_LOADER_SAT_XNU_PMAP_MULTIWINDOW_DRYRUN_CONTRACT) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_pmap_transition_dryrun_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_XNU_PMAP_TRANSITION_DRYRUN_CONTRACT;
    }
    pmap_transition_contract = stage68_xnu_pmap_transition_dryrun_contract_result();
    ...
    preflight->xnu_pmap_transition_dryrun_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE68_LOADER_SAT_XNU_PMAP_TRANSITION_DRYRUN_CONTRACT) != 0u) ?
        STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_pexpert_hook_readiness_contract_selftest(preflight)) {
        preflight->pexpert_gap_mask = 0u;
        preflight->interrupt_ready_mask = STAGE68_LOADER_IRQ_READY_REQUIRED;
    }
    pexpert_hook_contract = stage68_xnu_pexpert_hook_readiness_contract_result();
    ...
    preflight->xnu_pexpert_hook_readiness_contract_status_rollup =
        (pexpert_hook_contract->status == STAGE68_STATUS_OK) ? STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_iokit_platform_scaffold_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE68_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_platform_contract = stage68_xnu_iokit_platform_scaffold_contract_result();
    ...
    preflight->xnu_iokit_platform_scaffold_contract_status_rollup =
        (iokit_platform_contract->status == STAGE68_STATUS_OK) ? STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_iokit_match_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE68_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_match_contract = stage68_xnu_iokit_match_dryrun_contract_result();
    ...
    preflight->xnu_iokit_match_dryrun_contract_status_rollup =
        (iokit_match_contract->status == STAGE68_STATUS_OK) ? STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_iokit_registry_service_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE68_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_registry_contract = stage68_xnu_iokit_registry_service_dryrun_contract_result();
    ...
    preflight->xnu_iokit_registry_service_dryrun_contract_status_rollup =
        (iokit_registry_contract->status == STAGE68_STATUS_OK) ? STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_iokit_provider_plane_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE68_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_provider_contract = stage68_xnu_iokit_provider_plane_dryrun_contract_result();
    ...
    preflight->xnu_iokit_provider_plane_dryrun_contract_status_rollup =
        (iokit_provider_contract->status == STAGE68_STATUS_OK) ? STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (stage68_xnu_iokit_catalog_property_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE68_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_catalog_contract = stage68_xnu_iokit_catalog_property_dryrun_contract_result();
    ...
    preflight->xnu_iokit_catalog_property_dryrun_contract_status_rollup =
        (iokit_catalog_contract->status == STAGE68_STATUS_OK) ? STAGE68_STATUS_OK : STAGE68_STATUS_BASE;

    if (preflight->platform_gap_mask == (STAGE68_PLATFORM_GAP_REQUIRED_RECORDED & ~STAGE68_PLATFORM_GAP_IOKIT_STACK) &&
        preflight->pexpert_gap_mask == 0u) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_PLATFORM_GAPS;
    }
    if (preflight->interrupt_ready_mask == STAGE68_LOADER_IRQ_READY_REQUIRED) {
        preflight->satisfied_mask |= STAGE68_LOADER_SAT_INTERRUPTS;
    }
    ...
    if (preflight->satisfied_mask != STAGE68_LOADER_SAT_REQUIRED) {
        preflight->failure_mask |= 0x80000000u;
    }
    preflight->checksum = stage68_loader_preflight_checksum(preflight);
    preflight->status = (preflight->satisfied_mask == STAGE68_LOADER_SAT_REQUIRED && preflight->failure_mask == 0u &&
        preflight->checksum == stage68_loader_preflight_checksum(preflight)) ? STAGE68_STATUS_OK : (STAGE68_STATUS_BASE | preflight->failure_mask);
    ...
    stage68_xnu_tte_dryrun_log(&preflight->tte_dryrun);
    ...
    xnu_log_kv32("loader_stage68_plan_status", preflight->stage68_plan_status);
    ...
    xnu_log_puts("Stage68 XNU execution disabled: bootstrap mapping, pmap/bootstrap allocation, local pmap section-table dry-run, local page-granular coarse/L2 dry-run, exact pmap cache/MMU attribute dry-run, local multi-window page-granular dry-run, safe live-table transition prerequisite dry-run, MSM8974 pexpert interrupt/timer hook readiness, and Stage-owned IOKit/platform-driver scaffold, local IOKit service-match dry-run, broader local IOKit registry/service dry-run, local IOKit provider-plane publish/order dry-run, and local IOKit catalog/property/personality dry-run readiness contracts proved from stage-owned PE_state, Apple DT, IOKit registry-plane skeleton, local catalog/personality property facts, local platform-driver match facts, GIC snapshot, SGI/timer IRQ, loader/TTE/highVA/safe-table/TTBR, allocator/workspace, and public-XNU graph/object/link facts; public IOKit IODeviceTreeSupport/IOPlatformExpert/IOCPU references and public arm_vm_init/pmap references are reference-only and compile/link/execute counts are zero; bounded public pe_gen plus ARM pe_bootargs and ARM consistent_debug objects remain compile/link proof inputs only; inert MH_PRELOAD fixture carries metadata only; no full mach_kernel build, no public-XNU execution, no public IOKit execution, no IOKit catalog runtime execution, no IOKit property-plane runtime execution, no IOKit match runtime execution, no IOKit registry/service runtime execution, no IOKit provider-plane runtime execution, no platform driver execution, no platform runtime execution, no Mach-O execution, no proposed physical/workspace writes, no live pmap table install, no control-register writes for proposed pmap, no TLB invalidate for pmap install, no cache change, no persistent writes\n");

    if (preflight->status == STAGE68_STATUS_OK) {
        xnu_log_puts("Stage68 Mach-O/XNU loader preflight ok\n");
        return 1;
    }

    xnu_log_puts("Stage68 Mach-O/XNU loader preflight failed\n");
    return 0;
}
