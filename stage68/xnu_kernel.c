...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage68.gicDistributorBase, PE_state_stage68.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage68_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage68 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage68_loader_preflight_run(args)) {
