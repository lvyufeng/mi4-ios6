...
Stage0 through Stage60 are complete through non-persistent hardware validation. Stage60 validates a Stage-owned exact XNU pmap cache/MMU attribute dry-run contract over the Stage56 bootstrap mapping, Stage57 pmap/bootstrap allocation, Stage58 section-table dry-run, and Stage59 page-granular dry-run prerequisites while keeping the bounded public pexpert object/link set stable. The Stage60 graph consumes `external/xnu-upstream` detached at public `xnu-2050.22.13` (`cc8a9b0c`, `MasterVersion 12.3.0`) read-only, uses `external/xnu-4570.1.46` only as a public ARM reference, classifies twenty candidates, allows only `pexpert/gen/device_tree.c`, `pexpert/gen/bootargs.c`, `pexpert/gen/pe_gen.c`, `pexpert/arm/pe_bootargs.c`, and `pexpert/arm/pe_consistent_debug.c`, treats public ARM `arm_vm_init.c`, `pmap.c`, `pmap.h`, `proc_reg.h`, `osfmk/vm/pmap.h`, and `vm_param.h` as reference-only, links the six-object host-only ARM ELF proof with zero undefined symbols, and records pmap public compile/link/execute counts as zero. The target-side attr dry-run contract proves `stage60_xnu_pmap_attr_dryrun_contract_status=0x60000001`, `stage60_xnu_pmap_attr_dryrun_contract_satisfied_mask=0x00ffffff`, `stage60_xnu_pmap_attr_dryrun_contract_failure_mask=0x00000000`, checksum `0xfd77fdcd`, public cache attribute indices `WRITEBACK=0`, `WRITECOMB=1`, `WRITETHRU=2`, `DISABLE=3`, `INNERWRITEBACK=4`, AP encodings `RWNA=0`, `RWRW=1`, `RONA=2`, `RORO=3`, WIMG values including default/copyback `0x02`, IO `0x07`, posted `0x27`, write-through `0x0b`, write-combine `0x06`, and inner-writeback `0x12`, modeled WIMG-to-PTE bits `0x00000400`, `0x0000000d`, `0x00000005`, `0x00000408`, and `0x00000440`, page templates `0x00000412`, `0x00000413`, `0x00000612`, and `0x00000613`, inherited section/PTE readbacks `0x00010c02` and `0x00000412`, `loader_xnu_pmap_attr_dryrun_contract_status_rollup=0x60000001`, and `loader_status=0x60000001`. Hardware validation recovered 159522 bytes from `/proc/last_kmsg` with 2333 `MI4IOS6_STAGE60` markers and 2307 `MI4IOS6_STAGE60_XNU` markers after non-persistent `fastboot boot`. Stage60 explicitly avoids any full public `mach_kernel` build, public-XNU object execution, public pexpert/platform runtime execution, public ARM VM/pmap runtime execution, generated Mach-O execution, XNU `_start` / `arm_init` jump, proposed physical/TTE/pmap workspace write, live proposed pmap table install, TTBR/TTBCR/DACR/SCTLR write for pmap install, TLB invalidation for pmap install, external checkout mutation, persistent write, or cache change.
...
- Use the Stage49/Stage60 TTBR0 restore and attr-boundary proofs as the live-MMU safety baseline; do not install proposed XNU tables yet.
- Expand from the Stage60 pmap cache/MMU attribute dry-run contract toward broader page-granular pmap windows, MSM8974 pexpert support, XNU interrupt/timer hooks, IOKit/platform-driver scaffolding, or another bounded public-XNU source proof instead of jumping directly to a full kernel.
