...
Stage0 through Stage57 are complete through non-persistent hardware validation. Stage57 validates a Stage-owned XNU pmap/bootstrap allocation contract over the Stage56 bootstrap mapping tuple and Stage-owned allocator/workspace snapshot facts while keeping the bounded public pexpert object/link set stable. The Stage57 graph consumes `external/xnu-upstream` detached at public `xnu-2050.22.13` (`cc8a9b0c`, `MasterVersion 12.3.0`) read-only, uses `external/xnu-4570.1.46` only as a public ARM reference, classifies twenty candidates, allows only `pexpert/gen/device_tree.c`, `pexpert/gen/bootargs.c`, `pexpert/gen/pe_gen.c`, `pexpert/arm/pe_bootargs.c`, and `pexpert/arm/pe_consistent_debug.c`, treats public ARM `arm_vm_init.c`, `pmap.c`, `pmap.h`, `proc_reg.h`, and `vm_param.h` as reference-only, links the six-object host-only ARM ELF proof with zero undefined symbols, and records pmap public compile/link/execute counts as zero. The target-side contract proves `stage57_xnu_pmap_bootstrap_contract_status=0x57000001`, `stage57_xnu_pmap_bootstrap_contract_satisfied_mask=0x7fffffff`, `stage57_xnu_pmap_gVirtBase=0x80008000`, `stage57_xnu_pmap_gPhysBase=0x80000000`, `stage57_xnu_pmap_boot_ttep=0x8000c000`, `stage57_xnu_pmap_cpu_ttep=0x80010000`, `stage57_xnu_pmap_initial_avail_start=0x80016000`, `stage57_xnu_pmap_avail_end=0xde500000`, `stage57_xnu_pmap_vstart=0xc0400000`, `stage57_xnu_pmap_virtual_space_end=0xfffeffff`, `loader_xnu_pmap_bootstrap_contract_status_rollup=0x57000001`, and `loader_status=0x57000001`. Hardware validation recovered 138383 bytes from `/proc/last_kmsg` with 2071 `MI4IOS6_STAGE57` markers and 2045 `MI4IOS6_STAGE57_XNU` markers after non-persistent `fastboot boot`. Stage57 explicitly avoids any full public `mach_kernel` build, public-XNU object execution, public pexpert/platform runtime execution, public ARM VM/pmap runtime execution, generated Mach-O execution, XNU `_start` / `arm_init` jump, proposed physical/TTE workspace write, live proposed pmap table install, external checkout mutation, persistent write, or cache change.

Near-term work should proceed from hardware-validated Stage57 and refine the real-XNU loader/mapping/pmap path while keeping the proven no-execution safety boundary:
...
- Keep descriptor-driven service/phase dispatchers plus registry, policy, manifest, launch-contract, startup-boundary, startup-routine, startup-entry, callout-table, kernel-context, VM-plan, VM-state, allocator, pmap-workspace, object-table, collection-handoff, entry-table, object-graph, dependency-resolution, loader-preflight, load-plan, materialization, TTE dry-run, descriptor verification, high-VA VTOP dry-run, safe-table materialization, TTBR0 restore checks, workspace validation, compile-graph validation, object-subset compile validation, controlled link-proof validation, bootstrap mapping contract, and pmap/bootstrap allocation contract as safety gates.
- Use the Stage49/Stage57 TTBR0 restore proof as the live-MMU safety baseline; do not install proposed XNU tables yet.
- Expand from the Stage57 pmap/bootstrap contract toward deeper page-granular pmap table modeling, exact XNU cache/MMU attributes, MSM8974 pexpert support, XNU interrupt/timer hooks, or another bounded public-XNU source proof instead of jumping directly to a full kernel.
