...
Stage0 through Stage59 are complete through non-persistent hardware validation. Stage59 validates a Stage-owned XNU pmap page-granular dry-run contract over the Stage56/Stage57/Stage58 bootstrap mapping, pmap/bootstrap allocation, and section-table dry-run prerequisites while keeping the bounded public pexpert object/link set stable. The Stage59 graph consumes `external/xnu-upstream` detached at public `xnu-2050.22.13` (`cc8a9b0c`, `MasterVersion 12.3.0`) read-only, uses `external/xnu-4570.1.46` only as a public ARM reference, classifies twenty candidates, allows only `pexpert/gen/device_tree.c`, `pexpert/gen/bootargs.c`, `pexpert/gen/pe_gen.c`, `pexpert/arm/pe_bootargs.c`, and `pexpert/arm/pe_consistent_debug.c`, treats public ARM `arm_vm_init.c`, `pmap.c`, `pmap.h`, `proc_reg.h`, and `vm_param.h` as reference-only, links the six-object host-only ARM ELF proof with zero undefined symbols, and records pmap public compile/link/execute counts as zero. The target-side page dry-run contract proves `stage59_xnu_pmap_page_dryrun_contract_status=0x59000001`, `stage59_xnu_pmap_page_dryrun_contract_satisfied_mask=0x00ffffff`, `stage59_xnu_pmap_page_dryrun_contract_failure_mask=0x00000000`, local L1 dry-run buffer `0x00088000`-`0x0008c000`, local L2 dry-run buffer `0x00085000`-`0x00086000`, four L1 table descriptors, 1024 small-page PTEs, a bounded page-granular window `0x80000000`-`0x80400000`, descriptor/PTE type and attribute masks `0x00000001`/`0x00000412`, four software translations, `loader_xnu_pmap_page_dryrun_contract_status_rollup=0x59000001`, and `loader_status=0x59000001`. Hardware validation recovered 153475 bytes from `/proc/last_kmsg` with 2257 `MI4IOS6_STAGE59` markers and 2231 `MI4IOS6_STAGE59_XNU` markers after non-persistent `fastboot boot`. Stage59 explicitly avoids any full public `mach_kernel` build, public-XNU object execution, public pexpert/platform runtime execution, public ARM VM/pmap runtime execution, generated Mach-O execution, XNU `_start` / `arm_init` jump, proposed physical/TTE/pmap workspace write, live proposed pmap table install, TTBR/TTBCR/DACR/SCTLR write for pmap install, TLB invalidation for pmap install, external checkout mutation, persistent write, or cache change.

Near-term work should proceed from hardware-validated Stage59 and refine the real-XNU loader/mapping/pmap path while keeping the proven no-execution safety boundary:
...
- Keep descriptor-driven service/phase dispatchers plus registry, policy, manifest, launch-contract, startup-boundary, startup-routine, startup-entry, callout-table, kernel-context, VM-plan, VM-state, allocator, pmap-workspace, object-table, collection-handoff, entry-table, object-graph, dependency-resolution, loader-preflight, load-plan, materialization, TTE dry-run, descriptor verification, high-VA VTOP dry-run, safe-table materialization, TTBR0 restore checks, workspace validation, compile-graph validation, object-subset compile validation, controlled link-proof validation, bootstrap mapping contract, pmap/bootstrap allocation contract, pmap section-table dry-run contract, and pmap page-granular dry-run contract as safety gates.
- Use the Stage49/Stage59 TTBR0 restore proof as the live-MMU safety baseline; do not install proposed XNU tables yet.
- Expand from the Stage59 pmap page-granular dry-run contract toward exact XNU cache/MMU attributes, broader page-granular pmap windows, MSM8974 pexpert support, XNU interrupt/timer hooks, or another bounded public-XNU source proof instead of jumping directly to a full kernel.
