...
Stage0 through Stage61 are complete through non-persistent hardware validation. Stage61 validates a Stage-owned XNU pmap multi-window page-granular dry-run contract over the Stage56 bootstrap mapping, Stage57 pmap/bootstrap allocation, Stage58 section-table dry-run, Stage59 page-granular dry-run, and Stage60 cache/MMU attribute dry-run prerequisites while keeping the bounded public pexpert object/link set stable. The Stage61 graph consumes `external/xnu-upstream` detached at public `xnu-2050.22.13` (`cc8a9b0c`, `MasterVersion 12.3.0`) read-only, uses `external/xnu-4570.1.46` only as a public ARM reference, classifies twenty candidates, allows only `pexpert/gen/device_tree.c`, `pexpert/gen/bootargs.c`, `pexpert/gen/pe_gen.c`, `pexpert/arm/pe_bootargs.c`, and `pexpert/arm/pe_consistent_debug.c`, treats public ARM `arm_vm_init.c`, `pmap.c`, `pmap.h`, `proc_reg.h`, `osfmk/vm/pmap.h`, and `vm_param.h` as reference-only, links the six-object host-only ARM ELF proof with zero undefined symbols, and records pmap public compile/link/execute counts as zero. The target-side multi-window dry-run contract proves `stage61_xnu_pmap_multiwindow_dryrun_contract_status=0x61000001`, `stage61_xnu_pmap_multiwindow_dryrun_contract_satisfied_mask=0x1fffffff`, `stage61_xnu_pmap_multiwindow_dryrun_contract_failure_mask=0x00000000`, checksum `0x252caa78`, three local 4 MiB windows, local L1/L2 write counts `0x0000000c`/`0x00000c00`, exact imported PTE templates `0x00000412`, `0x00000413`, and `0x0000001f`, software translations `0x80008000 -> 0x80008000`, `0x80000000 -> 0x80000000`, `0xde500000 -> 0xde500000`, and `0xc0000000 -> 0xf9000000`, `loader_xnu_pmap_multiwindow_dryrun_contract_status_rollup=0x61000001`, and `loader_status=0x61000001`. Hardware validation recovered 159522 bytes from `/proc/last_kmsg` with 2333 `MI4IOS6_STAGE61` markers and 2307 `MI4IOS6_STAGE61_XNU` markers after non-persistent `fastboot boot`. Stage61 explicitly avoids any full public `mach_kernel` build, public-XNU object execution, public pexpert/platform runtime execution, public ARM VM/pmap runtime execution, generated Mach-O execution, XNU `_start` / `arm_init` jump, proposed physical/TTE/pmap workspace write, live proposed pmap table install, TTBR/TTBCR/DACR/SCTLR write for pmap install, TLB invalidation for pmap install, external checkout mutation, persistent write, or cache change.

Near-term work should proceed from hardware-validated Stage60 and refine the real-XNU loader/mapping/pmap path while keeping the proven no-execution safety boundary:
