...
#define STAGE14_HIGH_ALIAS_BASE        0xc0000000u
#define STAGE14_RAM_CONSOLE_ALIAS_BASE 0xc0100000u
#define STAGE14_GIC_ALIAS_BASE         0xc0200000u

static uint32_t stage14_l1_table[L1_SECTION_COUNT] __attribute__((aligned(16384)));
static volatile uint32_t stage14_mmu_probe_word;
static volatile uint32_t stage14_mmu_alias_probe;

struct stage14_alias_state {
    uint32_t magic;
    uint32_t input;
    uint32_t result;
    uint32_t checksum;
...
static struct stage14_alias_state stage14_alias_state_block;

struct stage14_bootstrap_state {
    uint32_t magic;
...
static struct stage14_bootstrap_state stage14_bootstrap_state_block;

static uint32_t stage14_high_alias_target(uint32_t input, volatile struct stage14_alias_state *state)
...
static uint32_t stage14_kernel_bootstrap(struct boot_args *args,
                                         volatile struct pe_platform_state *pe,
                                         volatile struct stage14_bootstrap_state *state)
    __attribute__((noinline));
...
{
    stage14_l1_table[SECTION_INDEX(va)] = (pa & L1_SECTION_MASK) | L1_DESC_SECTION_SO;
}

static uint32_t table_entry_for(uint32_t va)
{
    return stage14_l1_table[SECTION_INDEX(va)];
}

static void build_identity_table(void)
{
    memset(stage14_l1_table, 0, sizeof(stage14_l1_table));
    ...
    map_section(STAGE14_HIGH_ALIAS_BASE, 0x00000000u);
    map_section(STAGE14_RAM_CONSOLE_ALIAS_BASE, RAM_CONSOLE_BASE);
    map_section(STAGE14_GIC_ALIAS_BASE, 0xf9000000u);
...
static void enable_identity_mmu(void)
{
    ...
    dsb_isb();
    write_ttbcr(0);
    write_ttbr0((uint32_t)(uintptr_t)stage14_l1_table);
...
{
    volatile uint32_t *gicd_ctlr = (volatile uint32_t *)(uintptr_t)PE_state_stage14.gicDistributorBase;
    volatile uint32_t *gicc_ctlr = (volatile uint32_t *)(uintptr_t)PE_state_stage14.gicCpuBase;
    ...
    xnu_log_kv32("mmu_l1_table", (uint32_t)(uintptr_t)stage14_l1_table);

    build_identity_table();
    xnu_log_kv32("mmu_entry_low", table_entry_for(0x00008000u));
    xnu_log_kv32("mmu_entry_high_alias", table_entry_for(STAGE14_HIGH_ALIAS_BASE));
    xnu_log_kv32("mmu_entry_high_ram_console", table_entry_for(STAGE14_RAM_CONSOLE_ALIAS_BASE));
    xnu_log_kv32("mmu_entry_high_gic", table_entry_for(STAGE14_GIC_ALIAS_BASE));
    ...
    xnu_log_kv32("mmu_entry_gic", table_entry_for(PE_state_stage14.gicDistributorBase));
    xnu_log_kv32("mmu_entry_timer", table_entry_for(PE_state_stage14.timerBase));
    ...
    stage14_mmu_probe_word = 0x10aa55ffu;
    xnu_log_kv32("mmu_probe_word", stage14_mmu_probe_word);
    ...
    if (stage14_mmu_probe_word != 0x10aa55ffu) {
...
int mmu_high_alias_selftest(void)
{
    ...
    volatile uint32_t *identity_probe = &stage14_mmu_alias_probe;
    volatile uint32_t *alias_vector;
    volatile uint32_t *identity_vector = (volatile uint32_t *)(uintptr_t)stage14_vectors;
    ...
    volatile uint32_t *identity_gicd_ctlr = (volatile uint32_t *)(uintptr_t)PE_state_stage14.gicDistributorBase;
    uint32_t probe_phys = (uint32_t)(uintptr_t)&stage14_mmu_alias_probe;
    uint32_t vector_phys = (uint32_t)(uintptr_t)stage14_vectors;

    xnu_log_puts("mmu high alias selftest begin\n");
    xnu_log_kv32("mmu_alias_base", STAGE14_HIGH_ALIAS_BASE);
    xnu_log_kv32("mmu_alias_entry", table_entry_for(STAGE14_HIGH_ALIAS_BASE));
    ...
    alias_probe = (volatile uint32_t *)(uintptr_t)(STAGE14_HIGH_ALIAS_BASE + probe_phys);
    alias_vector = (volatile uint32_t *)(uintptr_t)(STAGE14_HIGH_ALIAS_BASE + vector_phys);
    alias_ram_console = (volatile uint32_t *)(uintptr_t)STAGE14_RAM_CONSOLE_ALIAS_BASE;
    alias_gicd_ctlr = (volatile uint32_t *)(uintptr_t)(STAGE14_GIC_ALIAS_BASE + (PE_state_stage14.gicDistributorBase - 0xf9000000u));
...
{
    typedef uint32_t (*alias_fn_t)(uint32_t, volatile struct stage14_alias_state *);

    const uint32_t input = 0x55667788u;
    const uint32_t expected = (input ^ 0x12c0ffeeu) + 0x1234u;
    uint32_t fn_phys = (uint32_t)(uintptr_t)stage14_high_alias_target;
    uint32_t state_phys = (uint32_t)(uintptr_t)&stage14_alias_state_block;
    alias_fn_t alias_fn = (alias_fn_t)(uintptr_t)(STAGE14_HIGH_ALIAS_BASE + fn_phys);
    volatile struct stage14_alias_state *alias_state =
        (volatile struct stage14_alias_state *)(uintptr_t)(STAGE14_HIGH_ALIAS_BASE + state_phys);
    uint32_t result;
    ...
    memset(&stage14_alias_state_block, 0, sizeof(stage14_alias_state_block));
    ...
    xnu_log_kv32("mmu_high_call_magic_id", stage14_alias_state_block.magic);
    xnu_log_kv32("mmu_high_call_input_id", stage14_alias_state_block.input);
    xnu_log_kv32("mmu_high_call_result_id", stage14_alias_state_block.result);
    xnu_log_kv32("mmu_high_call_checksum_id", stage14_alias_state_block.checksum);
    xnu_log_kv32("mmu_high_call_magic_alias", alias_state->magic);
    xnu_log_kv32("mmu_high_call_checksum_alias", alias_state->checksum);

    if (result != expected || stage14_alias_state_block.result != expected) {
        xnu_log_puts("mmu high call selftest failed: result mismatch\n");
        return 0;
    }
    if (stage14_alias_state_block.magic != 0x12001200u || stage14_alias_state_block.input != input) {
        xnu_log_puts("mmu high call selftest failed: state mismatch\n");
        return 0;
    }
    if (stage14_alias_state_block.checksum != (stage14_alias_state_block.magic ^ input ^ expected)) {
        xnu_log_puts("mmu high call selftest failed: checksum mismatch\n");
        return 0;
    }
    if (alias_state->checksum != stage14_alias_state_block.checksum) {
...
int mmu_high_bootstrap_selftest(void)
{
    typedef uint32_t (*bootstrap_fn_t)(struct boot_args *,
                                       ...
                                       volatile struct stage14_bootstrap_state *);

    uint32_t fn_phys = (uint32_t)(uintptr_t)stage14_kernel_bootstrap;
    uint32_t args_phys = (uint32_t)(uintptr_t)PE_state_stage14.bootArgs;
    uint32_t pe_phys = (uint32_t)(uintptr_t)&PE_state_stage14;
    uint32_t state_phys = (uint32_t)(uintptr_t)&stage14_bootstrap_state_block;
    bootstrap_fn_t bootstrap_fn = (bootstrap_fn_t)(uintptr_t)(STAGE14_HIGH_ALIAS_BASE + fn_phys);
    struct boot_args *alias_args = (struct boot_args *)(uintptr_t)(STAGE14_HIGH_ALIAS_BASE + args_phys);
    volatile struct pe_platform_state *alias_pe =
        (volatile struct pe_platform_state *)(uintptr_t)(STAGE14_HIGH_ALIAS_BASE + pe_phys);
    volatile struct stage14_bootstrap_state *alias_state =
        (volatile struct stage14_bootstrap_state *)(uintptr_t)(STAGE14_HIGH_ALIAS_BASE + state_phys);
    uint32_t expected;
    uint32_t result;
    ...
    memset(&stage14_bootstrap_state_block, 0, sizeof(stage14_bootstrap_state_block));
    ...
    expected = stage14_bootstrap_state_block.magic ^ stage14_bootstrap_state_block.boot_args_rev_ver ^
        stage14_bootstrap_state_block.machine_type ^ stage14_bootstrap_state_block.device_tree_length ^
        stage14_bootstrap_state_block.pe_memory_base ^ stage14_bootstrap_state_block.pe_memory_size ^
        stage14_bootstrap_state_block.pe_cpu_count ^ stage14_bootstrap_state_block.pe_gic_dist_base ^
        stage14_bootstrap_state_block.pe_gic_cpu_base ^ stage14_bootstrap_state_block.pe_timer_base ^
        stage14_bootstrap_state_block.pe_timer_frequency ^ stage14_bootstrap_state_block.pe_vector_base ^
        stage14_bootstrap_state_block.boot_flags;

    xnu_log_kv32("mmu_high_bootstrap_result", result);
    xnu_log_kv32("mmu_high_bootstrap_expected", expected);
    xnu_log_kv32("mmu_high_bootstrap_magic_id", stage14_bootstrap_state_block.magic);
    xnu_log_kv32("mmu_high_bootstrap_rev_ver_id", stage14_bootstrap_state_block.boot_args_rev_ver);
    xnu_log_kv32("mmu_high_bootstrap_machine_id", stage14_bootstrap_state_block.machine_type);
    xnu_log_kv32("mmu_high_bootstrap_dt_len_id", stage14_bootstrap_state_block.device_tree_length);
    xnu_log_kv32("mmu_high_bootstrap_mem_base_id", stage14_bootstrap_state_block.pe_memory_base);
    xnu_log_kv32("mmu_high_bootstrap_mem_size_id", stage14_bootstrap_state_block.pe_memory_size);
    xnu_log_kv32("mmu_high_bootstrap_cpu_count_id", stage14_bootstrap_state_block.pe_cpu_count);
    xnu_log_kv32("mmu_high_bootstrap_gic_dist_id", stage14_bootstrap_state_block.pe_gic_dist_base);
    xnu_log_kv32("mmu_high_bootstrap_gic_cpu_id", stage14_bootstrap_state_block.pe_gic_cpu_base);
    xnu_log_kv32("mmu_high_bootstrap_timer_base_id", stage14_bootstrap_state_block.pe_timer_base);
    xnu_log_kv32("mmu_high_bootstrap_timer_freq_id", stage14_bootstrap_state_block.pe_timer_frequency);
    xnu_log_kv32("mmu_high_bootstrap_vector_id", stage14_bootstrap_state_block.pe_vector_base);
    xnu_log_kv32("mmu_high_bootstrap_boot_flags_id", stage14_bootstrap_state_block.boot_flags);
    xnu_log_kv32("mmu_high_bootstrap_checksum_id", stage14_bootstrap_state_block.checksum);
    ...
    if (result != expected || stage14_bootstrap_state_block.checksum != expected) {
        xnu_log_puts("mmu high bootstrap selftest failed: checksum mismatch\n");
        return 0;
    }
    if (stage14_bootstrap_state_block.magic != 0x14001400u ||
        stage14_bootstrap_state_block.boot_args_rev_ver != 0x00020002u ||
        stage14_bootstrap_state_block.machine_type != MACHINE_TYPE_MSM8974) {
        xnu_log_puts("mmu high bootstrap selftest failed: boot args mismatch\n");
        return 0;
    }
    if (stage14_bootstrap_state_block.pe_memory_base != PE_state_stage14.memoryBase ||
        stage14_bootstrap_state_block.pe_memory_size != PE_state_stage14.memorySize ||
        stage14_bootstrap_state_block.pe_cpu_count != PE_state_stage14.cpuCount ||
        stage14_bootstrap_state_block.pe_gic_dist_base != PE_state_stage14.gicDistributorBase ||
        stage14_bootstrap_state_block.pe_gic_cpu_base != PE_state_stage14.gicCpuBase ||
        stage14_bootstrap_state_block.pe_timer_base != PE_state_stage14.timerBase ||
        stage14_bootstrap_state_block.pe_timer_frequency != 19200000u ||
        stage14_bootstrap_state_block.pe_vector_base != (uint32_t)(uintptr_t)stage14_vectors) {
        xnu_log_puts("mmu high bootstrap selftest failed: PE state mismatch\n");
        return 0;
    }
    if (alias_state->checksum != stage14_bootstrap_state_block.checksum) {
