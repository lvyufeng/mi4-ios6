...
Stage51 should now begin the first minimal public-XNU object-subset compile from public Apple OSS only. It should start from the Stage50 object manifest, keep external checkouts read-only and ignored, keep outputs under ignored `out/stage51/`, avoid a full public `mach_kernel` build, avoid linking a public-XNU Mach-O, and avoid executing public-XNU code on hardware yet.
