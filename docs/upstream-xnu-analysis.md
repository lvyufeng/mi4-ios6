...
## Stage81 Method-C status

The Stage76-Stage81 Method-C pivot is now exercising the public ARM boot-shape observations above on real `cancro` hardware and Stage-owned boot images without crossing into public XNU runtime:
...
- Stage80, hardware-validated non-persistently on `cancro` (`stage80_xnu_arm_init_post_pe_bootstrap_status=0x80000001`, `loader_status=0x80000001`, `kernel_entry returned success`), keeps the Stage79 PE-init-platform-false shape and invokes `stage80_xnu_arm_init_post_pe_bootstrap_run()` from the Stage-owned arm-init stub before `arm_vm_init`. It models CPU topology/master CPU selection, BootCpuData/CpuDataEntries population, bootstrap ordering, rtclock/timebase callback registration, selected pre-VM boot-arg parsing, and an explicit stop before `arm_vm_init`, while recording unchanged TTBR0/TTBCR/DACR/SCTLR samples and explicit zero counters for public bootstrap routines, public `arm_vm_init`, public pmap/pexpert/IOKit runtime, generated Mach-O execution, live pmap install, control-register writes, TLB invalidation, cache-policy change, and persistent writes.
- Stage81, hardware-validated non-persistently on `cancro`, crosses the Stage80 stop-before-VM boundary with a bounded Stage-owned `arm_vm_init` live-pmap install/verify/restore window. It invokes `stage81_xnu_arm_vm_init_live_pmap_run()` after the post-PE bootstrap result, builds a dedicated 16 KiB-aligned candidate L1 table, writes TTBR0, invalidates TLBs, verifies live high-alias/RAM-console/GIC translations, restores the original TTBR0, and invalidates TLBs again. Local validation confirmed `stage81-qcdt.img` `dt_size=2521088`, zero undefined symbols in both payload and host-only public-XNU link proof, fixed `CommandLine[256]` strings at 249 bytes including NUL, Android boot-image cmdlines at 1389 bytes including NUL, no stale Stage80 tokens under `stage81/`, and disassembly-confirmed calls through `stage81_xnu_arm_vm_init_live_pmap_run`. Hardware validation recovered 275864 bytes from `/tmp/cancro-stage81-arm-vm-init-live-pmap-last_kmsg.txt` and confirmed `stage81_xnu_arm_vm_init_live_pmap_status=0x81000001`, satisfied mask `0x01007fff`, failure mask `0x00000000`, candidate L1 `0x000e4000`, TTBR0 restore path `0x000b0000 -> 0x000e4000 -> 0x000b0000`, two TTBR0 writes, two TLB invalidations, high-alias/RAM-console/GIC verification, `loader_status=0x81000001`, and `kernel_entry returned success`.

This keeps the public XNU `_start`, `arm_init`, public bootstrap routines, public `arm_vm_init`, public pmap runtime, pexpert, `PE_init_platform`, `DTInit`, `pe_identify_machine`, and IOKit sources as analysis/reference material only. Stage81 deliberately mutates only a Stage-owned candidate pmap in a bounded TTBR0/TLB window and restores the original TTBR0 before returning; generated Mach-O execution, public pmap/pexpert/IOKit runtime, cache-policy changes, external checkout mutation, and persistent writes remain blocked. The remaining transition work is stricter repeated/fault-injected validation of the live-pmap boundary, broader high-virtual `virtBase`/`physBase` pmap behavior, eventual safe cache policy, real pexpert interrupt/timer hooks, kernelcache/prelink requirements, IOKit/platform drivers, and userspace/code-signing policy.
...
## Stage69 implementation note

Stage69 keeps the Stage68 IOKit catalog/property/personality dry-run contract stable, then adds a Stage-owned IOKit property-inheritance / registry-entry dry-run contract. It does not compile or execute public IOKit runtime sources, does not execute IOKit matching, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, attach/start, or platform-driver runtime code, does not execute a Stage-owned platform driver, and does not enter XNU `_start` / `arm_init`. Instead, it extends the four selected Stage-owned catalog personality nodes with registry-entry dry-run, property-inheritance, deterministic registry-entry ordinal, `IODeviceTree` registry-plane, and deferred attach/start properties while keeping the rejected personality unpublished and unattached.

The Stage69 property-inheritance / registry-entry dry-run contract records `stage69_xnu_iokit_property_inheritance_dryrun_contract_status=0x69000001`, required/satisfied mask `0x7fffffff`, failure mask `0x00000000`, checksum `0xceb8b00b`, source catalog status `0x69000001`, source catalog property checksum `0xfffffffb`, Apple-DT semantic mask `0x0003ffff`, registry-entry marker mask `0x0000000f`, property-inheritance marker mask `0x0000001f`, registry-entry ordinal mask `0x0000000f`, registry-plane match mask `0x0000000f`, inherited and instance property masks both `0x0000000f`, selected registry-entry mask `0x0000000f`, rejected-unattached mask `0x00000010`, property-inheritance matrix `0x0000001f`, property checksum `0x494f4455`, five personality candidates, four selected registry-entry dry-runs, one rejected entry, four attach/start deferred entries, and one rejected/unattached personality. It also imports the retained Stage69 catalog/property dry-run contract, IOKit provider-plane contract, registry/service dry-run contract, IOKit single-match dry-run contract, IOKit scaffold contract, pmap transition dry-run contract, and pexpert hook readiness roll-up, then records the property-inheritance / registry-entry dry-run as a loader preflight roll-up rather than adding a 33rd top-level loader satisfied bit.

Local Stage69 validation reports `stage69_xnu_compile_graph_status=0x69000001`, candidate count `0x00000017`, public pmap compile/link counts both zero, public IOKit compile/link counts both zero, `stage69_xnu_object_subset_status=0x69000001`, `stage69_xnu_link_status=0x69000001`, no undefined symbols in both the boot payload and host-only XNU link proof, final `out/stage69/stage69-qcdt.img` hash `a54bc63d15284328c3021abd71f2471059feddd9024097f695579fa9d8db94db`, and size `text=317208 data=0 bss=429092`. Hardware validation through non-persistent `fastboot boot` recovered 208904 bytes from `/proc/last_kmsg`, confirmed `stage69_xnu_iokit_property_inheritance_dryrun_contract_status=0x69000001`, `loader_xnu_iokit_property_inheritance_dryrun_contract_status_rollup=0x69000001`, `loader_satisfied_mask=0xffffffff`, `loader_status=0x69000001`, `kernel_entry ok`, and returned to Android.

Stage69 still performs no full public `mach_kernel` build, no public-XNU object execution, no public platform runtime execution, no public IOKit runtime execution, no IOKit match, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, attach/start, or platform-driver runtime execution, no public VM/pmap runtime execution, no generated Mach-O execution, no XNU `_start` / `arm_init` jump, no proposed physical/TTE/pmap workspace writes, no live pmap table install, no control-register writes for proposed pmap install, no TLB invalidation for proposed pmap install, no external checkout mutation, no persistent writes, and no cache changes.

## Stage70 implementation note

Stage70 keeps the Stage69 IOKit property-inheritance / registry-entry dry-run contract stable, then adds a Stage-owned IOKit registry-plane / IODeviceTree topology dry-run contract. It does not compile or execute public IOKit runtime sources, does not execute IOKit matching, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, or platform-driver runtime code, does not execute a Stage-owned platform driver, and does not enter XNU `_start` / `arm_init`. Instead, it extends the four selected Stage-owned registry-entry personality nodes with topology dry-run, parent path, entry path, deterministic parent/sibling ordinals, and provenance properties while keeping the rejected personality unpublished, unattached, and unlinked.

The Stage70 registry-plane / IODeviceTree topology dry-run contract records `stage70_xnu_iokit_registry_topology_dryrun_contract_status=0x70000001`, required/satisfied mask `0x7fffffff`, failure mask `0x00000000`, checksum `0xf8001093`, source property-inheritance status `0x70000001`, source selected registry-entry mask `0x0000000f`, source rejected-unattached mask `0x00000010`, Apple-DT semantic mask including the registry-topology readiness bit, topology marker mask `0x0000000f`, registry-plane match mask `0x0000000f`, parent path match mask `0x0000000f`, entry path match mask `0x0000000f`, parent ordinal match mask `0x0000000f`, sibling order match mask `0x0000000f`, provenance match mask `0x0000000f`, selected topology entry mask `0x0000000f`, rejected-unlinked mask `0x00000010`, topology matrix `0x0000001f`, path checksum `0x494f4454`, topology checksum `0xb6b0bbbb`, five personality candidates, four selected topology dry-runs, and one rejected/unlinked personality. It also imports the retained Stage70 property-inheritance, catalog/property dry-run, IOKit provider-plane, registry/service dry-run, IOKit single-match dry-run, IOKit scaffold, pmap transition dry-run, and pexpert hook readiness roll-ups, then records the registry topology dry-run as a loader preflight roll-up rather than adding a 33rd top-level loader satisfied bit.

Local Stage70 validation reports `stage70_xnu_compile_graph_status=0x70000001`, candidate count `0x00000017`, public pmap compile/link counts both zero, public IOKit compile/link counts both zero, `stage70_xnu_object_subset_status=0x70000001`, `stage70_xnu_link_status=0x70000001`, no undefined symbols in both the boot payload and host-only XNU link proof, final `out/stage70/stage70-qcdt.img` hash `cb0585e36a68ad5400c716ecaeacaa3bb79860a216f9e6f5deb04fba6b81c998`, and size `text=329100 data=0 bss=429532`. Hardware validation through non-persistent `fastboot boot` recovered 215910 bytes from `/proc/last_kmsg`, confirmed `stage70_xnu_iokit_registry_topology_dryrun_contract_status=0x70000001`, `loader_xnu_iokit_registry_topology_dryrun_contract_status_rollup=0x70000001`, `loader_satisfied_mask=0xffffffff`, `loader_status=0x70000001`, `kernel_entry ok`, and returned to Android.

Stage70 still performs no full public `mach_kernel` build, no public-XNU object execution, no public platform runtime execution, no public IOKit runtime execution, no IOKit match, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, or platform-driver runtime execution, no public VM/pmap runtime execution, no generated Mach-O execution, no XNU `_start` / `arm_init` jump, no proposed physical/TTE/pmap workspace writes, no live pmap table install, no control-register writes for proposed pmap install, no TLB invalidation for proposed pmap install, no external checkout mutation, no persistent writes, and no cache changes.

## Stage71 implementation note

Stage71 keeps the Stage70 IOKit registry-plane / IODeviceTree topology dry-run contract stable, then adds a Stage-owned IOKit attach/start readiness dry-run contract. It does not compile or execute public IOKit runtime sources, does not execute IOKit matching, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, or platform-driver runtime code, does not execute a Stage-owned platform driver, and does not enter XNU `_start` / `arm_init`. Instead, it extends the four selected Stage-owned topology personality nodes with attach/start readiness, provider path/ordinal, attach/start order, provider-start dependency, runtime-exec-disabled, and provenance properties while keeping the rejected personality unpublished, unattached, unstarted, and unlinked.

The Stage71 attach/start readiness dry-run contract records `stage71_xnu_iokit_attach_start_readiness_dryrun_contract_status=0x71000001`, required/satisfied mask `0x7fffffff`, failure mask `0x00000000`, contract checksum `0x011f8bea`, source topology status `0x71000001`, source selected topology entry mask `0x0000000f`, source rejected-unlinked mask `0x00000010`, Apple-DT semantic mask including the attach/start readiness bit, attach readiness mask `0x0000000f`, start readiness mask `0x0000000f`, attach/start provider path and ordinal match masks all `0x0000000f`, attach/start order match masks both `0x0000000f`, provider-start required and satisfied masks both `0x0000000e`, attach/start runtime-blocked masks both `0x0000000f`, provenance match mask `0x0000000f`, selected attach/start entry mask `0x0000000f`, rejected attach/start mask `0x00000010`, attach/start matrix `0x0000001f`, attach/start checksum `0xf7e3efe7`, five personality candidates, four selected attach/start readiness dry-runs, and one rejected/unattached/unstarted/unlinked personality. It also imports the retained Stage71 registry topology, property-inheritance, catalog/property dry-run, IOKit provider-plane, registry/service dry-run, IOKit single-match dry-run, IOKit scaffold, pmap transition dry-run, and pexpert hook readiness roll-ups, then records the attach/start readiness dry-run as a loader preflight roll-up rather than adding a 33rd top-level loader satisfied bit.

Local Stage71 validation reports `stage71_xnu_compile_graph_status=0x71000001`, candidate count `0x00000017`, public pmap compile/link counts both zero, public IOKit compile/link counts both zero, `stage71_xnu_object_subset_status=0x71000001`, `stage71_xnu_link_status=0x71000001`, no undefined symbols in both the boot payload and host-only XNU link proof, final `out/stage71/stage71-qcdt.img` hash `1365ddbd1f285d7771ab0cc6e232b5e677b196c3857d0871c904b15658c8f3af`, and size `text=347004 data=0 bss=430028`. Hardware validation through non-persistent `fastboot boot` recovered 222337 bytes from `/proc/last_kmsg`, confirmed `stage71_xnu_iokit_attach_start_readiness_dryrun_contract_status=0x71000001`, `loader_xnu_iokit_attach_start_readiness_dryrun_contract_status_rollup=0x71000001`, `loader_satisfied_mask=0xffffffff`, `loader_status=0x71000001`, `kernel_entry ok`, and returned to Android.

Stage71 still performs no full public `mach_kernel` build, no public-XNU object execution, no public platform runtime execution, no public IOKit runtime execution, no IOKit match, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, or platform-driver runtime execution, no public VM/pmap runtime execution, no generated Mach-O execution, no XNU `_start` / `arm_init` jump, no proposed physical/TTE/pmap workspace writes, no live pmap table install, no control-register writes for proposed pmap install, no TLB invalidation for proposed pmap install, no external checkout mutation, no persistent writes, and no cache changes.

## Stage72 implementation note

Stage72 keeps the Stage71 IOKit attach/start readiness dry-run contract stable, then adds a Stage-owned IOKit lifecycle/register-service readiness dry-run contract. It does not compile or execute public IOKit runtime sources, does not execute IOKit matching, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, register-service, notification, or platform-driver runtime code, does not execute a Stage-owned platform driver, and does not enter XNU `_start` / `arm_init`. Instead, it extends the four selected Stage-owned attach/start-ready personality nodes with local lifecycle, register-service, notification, order, provider-dependency, runtime-exec-disabled, and provenance properties while keeping the rejected personality unpublished, unlinked, unattached, unstarted, unregistered, and non-notifying.

The Stage72 lifecycle/register-service readiness dry-run contract records `stage72_xnu_iokit_lifecycle_register_service_readiness_dryrun_contract_status=0x72000001`, required/satisfied mask `0x7fffffff`, failure mask `0x00000000`, source attach/start status `0x72000001`, source selected attach/start entry mask `0x0000000f`, source rejected attach/start mask `0x00000010`, Apple-DT semantic mask including the lifecycle/register-service readiness bit, matched/provider-published/registry-linked/topology-linked masks all `0x0000000f`, attach/start source masks both `0x0000000f`, register-service readiness mask `0x0000000f`, service-registered mask `0x0000000f`, notification-ready mask `0x0000000f`, dependency-ready mask `0x0000000f`, register/notification runtime-blocked masks both `0x0000000f`, register/notification order masks both `0x0000000f`, provider-dependency match mask `0x0000000f`, register-provider-start required and satisfied masks both `0x0000000e`, selected lifecycle entry mask `0x0000000f`, rejected lifecycle mask `0x00000010`, lifecycle matrix `0x0000001f`, lifecycle checksum `0xbba5bdb5`, five personality candidates, four selected lifecycle/register-service readiness dry-runs, and one rejected/unregistered/non-notifying personality. It also imports the retained Stage72 attach/start readiness, registry topology, property-inheritance, catalog/property dry-run, IOKit provider-plane, registry/service dry-run, IOKit single-match dry-run, IOKit scaffold, pmap transition dry-run, and pexpert hook readiness roll-ups, then records the lifecycle/register-service readiness dry-run as a loader preflight roll-up rather than adding a 33rd top-level loader satisfied bit.

Local Stage72 validation reports `stage72_xnu_compile_graph_status=0x72000001`, candidate count `0x00000017`, public pmap compile/link counts both zero, public IOKit compile/link counts both zero, `stage72_xnu_object_subset_status=0x72000001`, `stage72_xnu_link_status=0x72000001`, no undefined symbols in both the boot payload and host-only XNU link proof, final `out/stage72/stage72-qcdt.img` hash `c9e72d0156cd37e22e5e7ebd7fb13e39309a930266f56903d4930480ab58c770`, and size `text=365056 data=0 bss=446936`. Hardware validation through non-persistent `fastboot boot` recovered 229447 bytes from `/proc/last_kmsg`, confirmed `stage72_xnu_iokit_lifecycle_register_service_readiness_dryrun_contract_status=0x72000001`, `loader_xnu_iokit_lifecycle_register_service_readiness_dryrun_contract_status_rollup=0x72000001`, `loader_satisfied_mask=0xffffffff`, `loader_status=0x72000001`, `kernel_entry ok`, and returned to Android.

Stage72 still performs no full public `mach_kernel` build, no public-XNU object execution, no public platform runtime execution, no public IOKit runtime execution, no IOKit match, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, register-service, or notification runtime execution, no public VM/pmap runtime execution, no generated Mach-O execution, no XNU `_start` / `arm_init` jump, no proposed physical/TTE/pmap workspace writes, no live pmap table install, no control-register writes for proposed pmap install, no TLB invalidation for proposed pmap install, no external checkout mutation, no persistent writes, and no cache changes.

## Stage73 implementation note

Stage73 keeps the Stage72 IOKit lifecycle/register-service readiness dry-run contract stable, then adds a Stage-owned IOKit provider-notification / interest / delivery-readiness dry-run contract. It does not compile or execute public IOKit runtime sources, does not execute IOKit matching, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, register-service, notification, interest, delivery, callback, or platform-driver runtime code, does not execute a Stage-owned platform driver, and does not enter XNU `_start` / `arm_init`. Instead, it deepens Stage72's single generic local `notification-ready` fact by extending the four selected Stage-owned lifecycle/register-service-ready personality nodes (from 53 to 68 properties) with provider-notification readiness, interest readiness, delivery readiness, interest provider path/ordinal, interest type mask, expected and delivered notification masks, interest/delivery order, notify-dependency readiness, runtime-exec-disabled, and provenance properties while keeping the rejected personality (extended from 52 to 67 properties) unpublished, unlinked, unattached, unstarted, unregistered, non-notifying, no-interest, and undelivered.

The Stage73 provider-notification / interest / delivery-readiness dry-run contract records `stage73_xnu_iokit_provider_notification_delivery_readiness_dryrun_contract_status=0x73000001`, required/satisfied mask `0x7fffffff`, failure mask `0x00000000`, source lifecycle/register-service status `0x73000001`, source selected lifecycle entry mask `0x0000000f`, source rejected lifecycle mask `0x00000010`, Apple-DT semantic mask including the provider-notification readiness bit `0x00200000`, provider-notification-ready/interest-ready/delivery-ready masks all `0x0000000f`, interest provider path/ordinal/type match masks all `0x0000000f`, expected and delivered notify-mask match masks both `0x0000000f`, notify-dependency-ready mask `0x0000000f`, interest/delivery runtime-blocked masks both `0x0000000f`, interest/delivery order match masks both `0x0000000f`, notify-provenance match mask `0x0000000f`, selected provider-notification entry mask `0x0000000f`, rejected provider-notification mask `0x00000010`, provider-notification matrix `0x0000001f`, provider-notification checksum `0xebebe9ff`, expected checksum `0xebebe9ff`, five personality candidates, four selected provider-notification / interest / delivery-readiness dry-runs, and one rejected/no-interest/undelivered personality. It also imports the retained Stage73 lifecycle/register-service readiness, attach/start readiness, registry topology, property-inheritance, catalog/property dry-run, IOKit provider-plane, registry/service dry-run, IOKit single-match dry-run, IOKit scaffold, pmap transition dry-run, and pexpert hook readiness roll-ups, then records the provider-notification / interest / delivery-readiness dry-run as a loader preflight roll-up rather than adding a 33rd top-level loader satisfied bit.

Local Stage73 validation reports `stage73_xnu_compile_graph_status=0x73000001`, candidate count `0x00000017`, public pmap compile/link counts both zero, public IOKit compile/link counts both zero, `stage73_xnu_object_subset_status=0x73000001`, `stage73_xnu_link_status=0x73000001`, no undefined symbols in both the boot payload and host-only XNU link proof, six-object host-only ARM ELF proof, root child count 19, four selected personality nodes at 68 properties, one rejected personality node at 67 properties, Apple-DT buffer 32768, no over-long Apple-DT property names, no stale Stage72 markers, final `out/stage73/stage73-qcdt.img` with `dt_size=2521088`, and size `text=382092 data=0 bss=447516`. Hardware validation through non-persistent `fastboot boot` recovered 235817 bytes from `/proc/last_kmsg` with 3177 `MI4IOS6_STAGE73` markers and 3139 `MI4IOS6_STAGE73_XNU` markers, confirmed `stage73_xnu_iokit_provider_notification_delivery_readiness_dryrun_contract_status=0x73000001`, `loader_xnu_iokit_provider_notification_delivery_readiness_dryrun_contract_status_rollup=0x73000001`, `loader_satisfied_mask=0xffffffff`, `loader_status=0x73000001`, `kernel_entry ok`, and returned to Android.

Stage73 still performs no full public `mach_kernel` build, no public-XNU object execution, no public platform runtime execution, no public IOKit runtime execution, no IOKit match, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, register-service, notification, interest, or delivery runtime execution, no public VM/pmap runtime execution, no generated Mach-O execution, no XNU `_start` / `arm_init` jump, no proposed physical/TTE/pmap workspace writes, no live pmap table install, no control-register writes for proposed pmap install, no TLB invalidation for proposed pmap install, no external checkout mutation, no persistent writes, and no cache changes.

## Stage74 implementation note

Stage74 keeps the Stage73 IOKit provider-notification / interest / delivery-readiness dry-run contract stable, then adds a Stage-owned IOKit provider-callback / client-notification readiness dry-run contract. It does not compile or execute public IOKit runtime sources, does not execute IOKit matching, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, register-service, notification, interest, delivery, callback, client-notification, or platform-driver runtime code, does not execute a Stage-owned platform driver, and does not enter XNU `_start` / `arm_init`. Instead, it deepens Stage73's provider-notification / interest / delivery facts by extending the four selected Stage-owned provider-notification-ready personality nodes (from 68 to 83 properties) with provider-callback readiness, client-notification readiness, callback provider path/ordinal, callback type mask, expected and delivered callback masks, client acknowledgement mask, callback/client-notification order, callback-dependency readiness, runtime-exec-disabled, and provenance properties while keeping the rejected personality (extended from 67 to 82 properties) unpublished, unlinked, unattached, unstarted, unregistered, non-notifying, no-interest, undelivered, no-callback, and no-client-notification.

The Stage74 provider-callback / client-notification readiness dry-run contract records `stage74_xnu_iokit_provider_callback_client_notification_readiness_dryrun_contract_status=0x74000001`, required/satisfied mask `0x7fffffff`, failure mask `0x00000000`, source provider-notification status `0x74000001`, source selected provider-notification entry mask `0x0000000f`, source rejected provider-notification mask `0x00000010`, source provider-notification matrix `0x0000001f`, Apple-DT semantic mask including the provider-callback readiness bit `0x00400000`, provider-callback-ready/client-notification-ready masks both `0x0000000f`, callback provider path/ordinal/type match masks all `0x0000000f`, expected and delivered callback-mask match masks both `0x0000000f`, client-ack mask `0x0000000f`, callback-dependency-ready mask `0x0000000f`, callback/client-notification runtime-blocked masks both `0x0000000f`, callback/client-notification order match masks both `0x0000000f`, callback-provenance match mask `0x0000000f`, selected provider-callback entry mask `0x0000000f`, rejected provider-callback mask `0x00000010`, provider-callback matrix `0x0000001f`, provider-callback checksum `0xbba8abbd`, expected checksum `0xbba8abbd`, five personality candidates, four selected provider-callback / client-notification readiness dry-runs, and one rejected/no-callback/no-client-notification personality. It also imports the retained Stage74 provider-notification / interest / delivery-readiness, lifecycle/register-service readiness, attach/start readiness, registry topology, property-inheritance, catalog/property dry-run, IOKit provider-plane, registry/service dry-run, IOKit single-match dry-run, IOKit scaffold, pmap transition dry-run, and pexpert hook readiness roll-ups, then records the provider-callback / client-notification readiness dry-run as a loader preflight roll-up rather than adding a 33rd top-level loader satisfied bit.

Local Stage74 validation reports `stage74_xnu_compile_graph_status=0x74000001`, candidate count `0x00000017`, public pmap compile/link counts both zero, public IOKit compile/link counts both zero, `stage74_xnu_object_subset_status=0x74000001`, `stage74_xnu_link_status=0x74000001`, no undefined symbols in both the boot payload and host-only XNU link proof, six-object host-only ARM ELF proof, root child count 19, four selected personality nodes at 83 properties, one rejected personality node at 82 properties, Apple-DT buffer 32768, no over-long Apple-DT property names, no stale Stage73 runtime markers, fixed public ARM `CommandLine[256]` strings at 252 bytes including NUL, Android boot-image command lines at 1514 bytes including NUL, final `out/stage74/stage74-qcdt.img` with `dt_size=2521088`, SHA256 `3b640d77c11680a100e599396cc114d61e953d291cef6bb5bd7aac12898c7ca0`, and size `text=397032 data=0 bss=448136`.

Stage74 still performs no full public `mach_kernel` build, no public-XNU object execution, no public platform runtime execution, no public IOKit runtime execution, no IOKit match, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, register-service, notification, interest, delivery, callback, or client-notification runtime execution, no public VM/pmap runtime execution, no generated Mach-O execution, no XNU `_start` / `arm_init` jump, no proposed physical/TTE/pmap workspace writes, no live pmap table install, no control-register writes for proposed pmap install, no TLB invalidation for proposed pmap install, no external checkout mutation, no persistent writes, and no cache changes.

## Stage75 implementation note

Stage75 keeps the Stage74 IOKit provider-callback / client-notification readiness proof stable and adds a Stage-owned IOKit client-open / provider-claim / close-readiness dry-run contract. It does not compile or execute public IOKit runtime sources, does not execute IOKit matching, registry/service, provider-plane, catalog, property-plane, property-inheritance, registry-entry, registry-topology, attach/start, register-service, notification, interest, delivery, callback, client-notification, open, claim, close, or platform-driver runtime code, does not execute a Stage-owned platform driver, and does not enter XNU `_start` / `arm_init`. Instead, it deepens Stage74's provider-callback / client-notification facts by extending the four selected Stage-owned provider-callback-ready personality nodes (from 83 to 100 properties) with client-open readiness, provider-claim readiness, client-close readiness, open provider path/ordinal, open type mask, expected/claimed open masks, client-close mask, open/close order, open-dependency readiness, runtime-exec-disabled, and provenance properties while keeping the rejected personality (extended from 82 to 99 properties) unpublished, unlinked, unattached, unstarted, unregistered, non-notifying, no-interest, undelivered, no-callback, no-client-notification, no-open, no-claim, and no-close.

The Stage75 client-open / provider-claim / close-readiness dry-run contract records `stage75_xnu_iokit_client_open_provider_claim_close_readiness_dryrun_contract_status=0x75000001`, required/satisfied mask `0x7fffffff`, failure mask `0x00000000`, source provider-callback status `0x75000001`, source selected provider-callback entry mask `0x0000000f`, source rejected provider-callback mask `0x00000010`, source provider-callback matrix `0x0000001f`, Apple-DT semantic mask including the client-open readiness bit, client-open/provider-claim/client-close readiness masks all `0x0000000f`, open provider path/ordinal/type match masks all `0x0000000f`, expected/claimed open-mask and client-close-mask match masks all `0x0000000f`, open-dependency-ready mask `0x0000000f`, open/claim/close runtime-blocked masks all `0x0000000f`, open/close order match masks both `0x0000000f`, open-provenance match mask `0x0000000f`, selected client-open entry mask `0x0000000f`, rejected client-open mask `0x00000010`, client-open matrix `0x0000001f`, client-open checksum `0xf4f8e8f5`, and expected checksum `0xf4f8e8f5`.

Stage75 preserves the saturated loader contract: no new top-level loader satisfied bit is added. The new contract contributes through loader preflight roll-up and failure-mask aggregation only, with `loader_xnu_iokit_client_open_provider_claim_close_readiness_dryrun_contract_status_rollup=0x75000001`, `loader_satisfied_mask=0xffffffff`, and `loader_status=0x75000001` on validated hardware.

Local validation built `stage75-qcdt.img` with `dt_size=2521088`, zero undefined symbols in both the Stage75 payload and the host-only public-XNU link proof, fixed `CommandLine[256]` strings at 222 bytes including NUL, Android boot-image cmdlines at 1496 bytes including NUL, four selected 100-property personalities, one rejected 99-property personality, no long Apple-DT property names, no stale Stage74 runtime markers, no stale 83/82 exact count gates, ignored `out/stage75` artifacts, and clean external checkouts at `external/xnu-upstream` `cc8a9b0c` and `external/xnu-4570.1.46` `76e12aa`.
...
## Stage80 implementation note

Stage80 continues the Method-C live path by adding the next Stage-owned `arm_init` micro-sequence after the Stage79 `PE_init_platform(FALSE,args)` model and before the public `arm_vm_init(xmaxmem,args)` boundary. After `stage80_xnu_pe_init_platform_false_run()` succeeds, the Stage-owned arm-init stub calls `stage80_xnu_arm_init_post_pe_bootstrap_run(args, result)` and records the post-PE bootstrap status/checksum in the entry-stub result.

The new result ABI requires `0x01ffffff` satisfied bits. It imports the Stage80 PE-init-platform-false result, validates `boot_args` and compact command-line markers, inherits `PE_state` / device-tree / platform facts, models cancro CPU topology (`cpu_count=4`, boot/master CPU `0`, max CPU number `3`), records BootCpuData and CpuDataEntries-shaped local facts, records thread/cpu/rtclock/kernel-early/cpu-init/processor-bootstrap ordering facts without public calls, registers only a Stage-owned timebase callback-shaped pointer/frequency fact at `0x0124f800`, parses selected boot args (`debug=0x144`, absent/default `diag`, `maxmem`, `immediate_NMI`), and records `stop_before_arm_vm_init=1`.

Local Stage80 validation built `stage80-qcdt.img` with `dt_size=2521088`, zero undefined symbols in both the Stage80 payload and host-only public-XNU link proof, fixed `CommandLine[256]` strings at 227 bytes including NUL, chosen Apple-DT boot-args at 141 bytes including NUL, Android boot-image cmdlines at 1372 bytes including NUL, no stale Stage79 tokens under `stage80/`, and no legacy failure-status OR collision sites outside `STAGE80_STATUS_FAIL`. Disassembly confirms real calls to `stage80_xnu_start_stub`, `stage80_arm_init_stub`, `stage80_xnu_early_pmap_platform_init_run`, `stage80_xnu_pe_init_platform_false_run`, and `stage80_xnu_arm_init_post_pe_bootstrap_run`.

Stage80 still does not execute public XNU `_start` / `arm_init`, public `thread_bootstrap`, `cpu_bootstrap`, `rtclock_early_init`, `kernel_early_bootstrap`, `cpu_init`, `processor_bootstrap`, public `arm_vm_init`, public pexpert/pmap/IOKit runtime, or generated Mach-O bytes. It does not install live pmap tables, write TTBR0/TTBCR/DACR/SCTLR for a proposed XNU pmap transition, invalidate TLBs, change cache policy, or write persistent storage. Hardware validation must remain non-persistent `fastboot boot` only.

## Stage81 implementation note

Stage81 continues the Method-C live path by crossing Stage80's stop-before-`arm_vm_init` boundary in a bounded Stage-owned way. After `stage81_xnu_arm_init_post_pe_bootstrap_run()` succeeds, the Stage-owned arm-init stub calls `stage81_xnu_arm_vm_init_live_pmap_run(args, result)` and records the live-pmap status/checksum in the entry-stub result.

The new live-pmap result ABI requires `0x01007fff` satisfied bits. It imports the Stage81 post-PE bootstrap result, validates `boot_args` and compact command-line markers, allocates a dedicated 16 KiB-aligned Stage-owned candidate L1 table (`stage81_live_pmap_candidate_l1[4096]`), populates only the sections required to survive and verify the bounded window (low identity `0x00000000 -> 0x00000000`, high alias `0xc0000000 -> 0x00000000`, RAM-console alias `0xc0100000 -> RAM_CONSOLE_BASE`, GIC alias `0xc0200000 -> 0xf9000000`, MSM IMEM `0x0fa00000 -> 0x0fa00000`, RAM-console identity, GIC identity `0xf9000000 -> 0xf9000000`, PS_HOLD `0xfc400000 -> 0xfc400000`, and candidate L1 self-mapping), saves original TTBR0/TTBCR/DACR/SCTLR, writes TTBR0 to the candidate L1 base (`candidate_l1_base=0x000e4000`), invalidates TLBs with `dsb; isb; mcr p15, 0, %0, c8, c7, 0; dsb; isb`, verifies live TTBR0 base and unchanged TTBCR/DACR/SCTLR, verifies high-alias/RAM-console/GIC translations through the installed candidate table, restores the original TTBR0 (`original_ttbr0=0x000b0000`), invalidates TLBs again, verifies original control-register state restored, and records `live_pmap_installed=1`, `live_pmap_verified=1`, `live_pmap_restored=1`, `ttbr0_write_count=2`, `tlb_invalidate_count=2`.

Local Stage81 validation built `stage81-qcdt.img` with `dt_size=2521088`, zero undefined symbols in both the Stage81 payload and host-only public-XNU link proof, fixed `CommandLine[256]` strings at 249 bytes including NUL, chosen Apple-DT boot-args at 241 bytes including NUL, Android boot-image cmdlines at 1389 bytes including NUL, no stale Stage80 tokens under `stage81/`, and clean `git diff --check`. Disassembly confirms real calls to `stage81_xnu_start_stub`, `stage81_arm_init_stub`, `stage81_xnu_early_pmap_platform_init_run`, `stage81_xnu_pe_init_platform_false_run`, `stage81_xnu_arm_init_post_pe_bootstrap_run`, and `stage81_xnu_arm_vm_init_live_pmap_run`.

Hardware validation through non-persistent `fastboot boot` recovered 275864 bytes from `/tmp/cancro-stage81-arm-vm-init-live-pmap-last_kmsg.txt`, confirmed `stage81_xnu_arm_vm_init_live_pmap_status=0x81000001`, satisfied mask `0x01007fff`, failure mask `0x00000000`, `candidate_l1_base=0x000e4000`, `candidate_l1_checksum=0x2d810c02`, `original_ttbr0=0x000b0000`, `live_ttbr0=0x000e4000`, `restored_ttbr0=0x000b0000`, `ttbr0_write_count=0x00000002`, `tlb_invalidate_count=0x00000002`, `live_pmap_installed=0x00000001`, `live_pmap_verified=0x00000001`, `high_alias_verified=0x00000001`, `ram_console_verified=0x00000001`, `gic_verified=0x00000001`, `live_pmap_restored=0x00000001`, `public_arm_vm_init_executed=0x00000000`, `public_pmap_runtime_executed=0x00000000`, `persistent_write_attempted=0x00000000`, `safety_boundary_preserved=0x00000001`, `stage81_xnu_entry_stub_status=0x81000001`, `stage81_xnu_entry_stub_ttbr0_before=0x000b0000`, `stage81_xnu_entry_stub_ttbr0_after=0x000b0000`, `loader_xnu_arm_vm_init_live_pmap_status_rollup=0x81000001`, `loader_satisfied_mask=0xffffffff`, `loader_status=0x81000001`, and `kernel_entry returned success`.

Stage81 is the first Method-C stage in this branch that intentionally installs a Stage-owned live pmap and invalidates TLBs for that install/verify/restore sequence. It still does not execute public XNU `_start` / `arm_init`, public `arm_vm_init`, public pmap runtime, public pexpert/IOKit runtime, or generated Mach-O bytes. It does not write the proposed public-XNU pmap workspace, does not write TTBCR/DACR/SCTLR for a split-TTBR or cache-policy transition, does not change cache policy, and does not write persistent storage. Hardware validation remains non-persistent `fastboot boot` only.
