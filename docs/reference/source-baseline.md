...
Stage81 is the current hardware-validated focus, building on the hardware-validated Stage78 early pmap/platform-init result (`21fb079`), hardware-validated Stage79 PE-init-platform-false result (`ee6f5b9`), and hardware-validated Stage80 post-PE bootstrap/timebase-registration result. Stage81 keeps the Stage-owned `_start` / `arm_init`-shaped entry path, retains the Stage-owned early pmap/platform-init, `PE_init_platform(FALSE,args)`-shaped, and post-PE_FALSE bootstrap/timebase micro-sequences, and adds a bounded Stage-owned `arm_vm_init`-shaped live-pmap install/verify/restore window. The retained Stage56-Stage62 bootstrap/pmap dry-run chain, Stage63 pexpert interrupt/timer readiness proof, Stage64-Stage75 IOKit dry-run ladder, Stage76 live-probe boundary, Stage77 entry-stub boundary, Stage78 early-init boundary, Stage79 PE-init-platform-false boundary, and Stage80 post-PE bootstrap/timebase boundary remain prerequisites.

The public source baseline remains unchanged: `external/xnu-upstream` stays detached at `xnu-2050.22.13` (`cc8a9b0c`, `MasterVersion 12.3.0`) for Darwin 12/iOS 6-era context, and `external/xnu-4570.1.46` remains a later public ARM/IOKit implementation reference. Stage81 still classifies the same selected public candidates through the fail-closed compile graph, allows only the bounded pexpert object subset (`pexpert/gen/device_tree.c`, `pexpert/gen/bootargs.c`, `pexpert/gen/pe_gen.c`, `pexpert/arm/pe_bootargs.c`, and `pexpert/arm/pe_consistent_debug.c`) for the host-only link proof, treats public ARM VM/pmap and public IOKit sources as reference-only, and keeps public pmap/IOKit compile/link/execute counts at zero. Stage81's live-pmap mutation is Stage-owned only: it writes TTBR0 to a dedicated candidate L1 table, invalidates TLBs, verifies high-alias/RAM-console/GIC translations, restores the original TTBR0, and still avoids public `arm_vm_init`, public pmap runtime, generated Mach-O execution, cache-policy changes, and persistent writes.
...
Stage81 local validation confirmed `stage81-qcdt.img` `dt_size=2521088`, zero undefined symbols in both the payload and host-only public-XNU link proof, fixed `CommandLine[256]` strings at 249 bytes including NUL, the chosen Apple-DT boot-args string at 241 bytes including NUL, Android boot-image command lines at 1389 bytes including NUL, no stale Stage80 tokens under Stage81 code/build files, and real disassembly calls through `bl stage81_xnu_start_stub`, `bl stage81_arm_init_stub`, `bl stage81_xnu_early_pmap_platform_init_run`, `bl stage81_xnu_pe_init_platform_false_run`, `bl stage81_xnu_arm_init_post_pe_bootstrap_run`, and `bl stage81_xnu_arm_vm_init_live_pmap_run`. Stage81 build artifacts include `stage81.elf` 509688 bytes, `stage81.bin` 456236 bytes, `stage81.img` 458752 bytes, `stage81-qcdt.img` 2979840 bytes, and `stage81-qcdt.img` SHA256 `9850d88663607d4519f5a92b20569fa04f1b70781f2c197f11a55d7f7178ccc5`. Stage81 non-persistent hardware validation recovered 275864 bytes from `/tmp/cancro-stage81-arm-vm-init-live-pmap-last_kmsg.txt` and confirmed `stage81_xnu_arm_vm_init_live_pmap_status=0x81000001`, live-pmap satisfied mask `0x01007fff`, failure mask `0x00000000`, candidate L1 base `0x000e4000`, TTBR0 restore path `0x000b0000 -> 0x000e4000 -> 0x000b0000`, two TTBR0 writes, two TLB invalidations, high-alias/RAM-console/GIC verification, `loader_xnu_arm_vm_init_live_pmap_status_rollup=0x81000001`, `loader_status=0x81000001`, and `kernel_entry returned success`.

The current analysis/build pass focuses on the Stage81 Method-C VM/pmap boundary:

1. Start from the hardware-validated Stage78 early-init result, hardware-validated Stage79 PE-init-platform-false result, and hardware-validated Stage80 post-PE bootstrap/timebase result as the live handoff baseline.
...
4. Keep outputs ignored under `out/stage81/` and do not mutate `external/`.
5. Do not attempt a full public `mach_kernel` build, do not enter public XNU `_start` / `arm_init`, do not call public `PE_init_platform`, do not call public `DTInit`, do not call public `pe_identify_machine`, do not execute public-XNU pexpert/pmap/IOKit code, and do not execute generated Mach-O bytes. Stage81 permits only its bounded Stage-owned TTBR0 write/TLB invalidation/install-verify-restore window; it does not execute public `arm_vm_init`/pmap runtime, does not write a proposed public-XNU pmap workspace, does not change cache policy, and does not write persistent storage.
6. Continue mapping the next blockers: stricter repeated/fault-injected restore recovery around pmap transitions, broader high-virtual `virtBase`/`physBase` pmap behavior, eventual safe cache policy, kernelcache/prelink details, and later userspace/code-signing policy.

See also `docs/reference/upstream-xnu-analysis.md`, `docs/experiments/experiment-79-stage76-live-xnu-like-execution-probe.md`, `docs/experiments/experiment-80-stage77-xnu-entry-stub.md`, `docs/experiments/experiment-81-stage78-early-pmap-platform-init.md`, `docs/experiments/experiment-82-stage79-pe-init-platform-false.md`, `docs/experiments/experiment-83-stage80-arm-init-post-pe-bootstrap.md`, and `docs/experiments/experiment-84-stage81-arm-vm-init-live-pmap.md` for the current live-execution pivot, plus the Stage50-Stage81 experiment documents for the preceding public-XNU workspace, object-subset, link-proof, bootstrap, pmap, pexpert, IOKit dry-run, and Method-C prerequisites.
