...
- `host_boot_args_check.sh` — executes the Phase 2 `boot_args` module on the host, 32-bit, supplying its one host-unknowable linker symbol by `--defsym` from the built image. The first execution evidence for that module.
