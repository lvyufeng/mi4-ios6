...
Stage57 is the current validated local focus. It adds a Stage-owned XNU pmap/bootstrap allocation contract on top of the Stage56 bootstrap mapping contract while keeping the Stage55 public object/link set stable. It consumes `external/xnu-upstream` at `xnu-2050.22.13` read-only for Darwin 12/iOS 6-era context, uses `external/xnu-4570.1.46` as a later public ARM reference, classifies twenty selected public candidates through a fail-closed compile graph, allows five graph-approved sources (`pexpert/gen/device_tree.c`, `pexpert/gen/bootargs.c`, `pexpert/gen/pe_gen.c`, bounded public ARM `pexpert/arm/pe_bootargs.c`, and bounded public ARM `pexpert/arm/pe_consistent_debug.c`), explicitly blocks runtime-heavy public ARM pexpert sources (`pe_kprintf.c`, `pe_serial.c`, `pe_identify_machine.c`, and `pe_init.c`), treats public ARM VM/pmap sources (`arm_vm_init.c`, `pmap.c`, `pmap.h`, `proc_reg.h`, and `vm_param.h`) as reference-only, compiles five public objects plus one Stage57-owned shim object, links them into a host-only ARM ELF proof artifact under ignored `out/stage57/xnu-link/`, records zero undefined symbols, and locally/hardware validates the Stage-owned pmap/bootstrap allocation contract path while preserving no public-XNU/platform-runtime/VM-pmap/Mach-O execution markers. Non-persistent Stage57 hardware validation recovered 138383 bytes from `/proc/last_kmsg`, confirmed loader status `0x57000001`, and returned to Android. The current analysis/build pass focuses on Stage57:

1. Start from the Stage56 bootstrap mapping contract, keep the allowed public object subset stable, and refine the real XNU loader/mapping/pmap contract deliberately.
...
6. Continue mapping the remaining real-XNU blockers: page-granular `virtBase`/`physBase` mapping, real pmap/bootstrap table population, exact XNU cache policy, MSM8974 pexpert, timer/interrupt hooks, IOKit/platform drivers, kernelcache/prelink details, and later userspace/code-signing policy.

See also `docs/upstream-xnu-analysis.md`, `docs/experiment-53-stage50-public-xnu-workspace-cancro-scaffold.md`, `docs/experiment-54-stage51-public-xnu-object-subset-compile.md`, `docs/experiment-55-stage52-public-xnu-controlled-link-proof.md`, `docs/experiment-56-stage53-xnu-compile-graph.md`, `docs/experiment-57-stage54-pexpert-platform-compile-graph.md`, `docs/experiment-58-stage55-pexpert-consistent-debug-compile-graph.md`, `docs/experiment-59-stage56-xnu-bootstrap-mapping-contract.md`, and `docs/experiment-60-stage57-xnu-pmap-bootstrap-contract.md` for the Stage50 scaffold result, Stage51 compile result, Stage52 controlled link-proof result, Stage53 compile-graph migration result, Stage54 pexpert/platform graph result, Stage55 consistent-debug proof, Stage56 bootstrap mapping contract proof, and Stage57 pmap/bootstrap allocation contract proof.
