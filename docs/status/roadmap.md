...
- **XNU already contains the alternative.** Defining `__ARM_TIME__` moves the timer to a
  ...
  `ml_get_timebase`, `user_timebase_active`), with nothing obviously half-finished; but it is
  still only reading, and the path must be *built* to confirm it compiles.
