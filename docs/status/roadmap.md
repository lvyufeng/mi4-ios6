...
**THE GENERATED INPUTS EXISTED FOR ONE CONFIGURATION ONLY** (2026-09-18,
[`experiment-166`](../experiments/experiment-166-the-generated-inputs-existed-for-one-configuration.md)).
Four of the six remaining `STAGE90_BOOT` failures were one cause and it is not in XNU:
`bsd/dev/arm/conf.c`, `tty_ptmx.c` and `tty_pty.c` include `<pty.h>`, `bsd_init.c` includes
`<loop.h>`, both are generated by config(8)'s device table — and `out/xnu_device/` held `RELEASE/`
only. The generator had honoured `XNU_KERNEL_CONFIG` from the start; what was missing was the run.
The reason it cost six failures instead of an error is the part worth keeping: **a header missing
from the include path is not an error, it is the host's header.** The target is an ELF triple whose
driver sysroot is `/`, so `conf.c:111`'s `<pty.h>` resolved to `/usr/include/pty.h` and the file died
in glibc's `features-time64.h` — an error naming neither XNU nor the header that was absent, which is
how a missing generated directory stayed invisible while only `RELEASE` was built. The two
directories are now byte-identical (`diff -r` empty), because the values are chosen facts about a
device table and not functions of the option set.

C **420 → 423 of 426** and the failing set **6 → 3**; undefined **102 → 94**; the boot path unchanged
at **9**, since all four files sit behind `bsd_init`. Ten symbols closed — eight of them the BSD
device-layer tables `conf.c` exists to define (`bdevsw`/`cdevsw`, `nblkdev`/`nchrdev`,
`chrtoblk`/`chrtoblk_set`), which is what experiment-132 predicted that file was for — and **two
opened**, `devfs_make_node_clone` and `devfs_mutex`, which are the configuration disagreeing with
itself: `tty_ptmx.c` is `optional ptmx` and calls `devfs_make_node_clone` (`devfs_tree.c:1493`,
`optional devfs`) from `ptmx_init()` with no `#if DEVFS` guard, and this configuration has `ptmx 1`
with `DEVFS 0` where `RELEASE` has `DEVFS 1`. Recorded, not turned on: `DEVFS 1` pulls three files
into a configuration built for a first boot, and `NPTMX 0` has not been measured. The pair is the
same shape as `NBPFILTER` and `GPROF` — a configuration decision with a price not yet taken.

`build_xnu_arm_kernel.sh` now refuses to start without this configuration's device headers, the way
it already refused without its option headers (verified by removing the directory: exit 2, and a
message naming the command), and `gen_device_headers.sh`'s header — which still said "there is
exactly one header in it today" — now records that it is per configuration and why its absence is
silent. **What that does not fix is the class**: the preflight catches *this* generator's output,
because this generator is known, while a header XNU includes and nothing generates still falls
through to the host's. The measurement to make next is whether the include path can be closed
altogether — `-nostdinc` with clang's own builtin headers — which would turn every future instance
into `file not found`.

**Both configurations now fail the same three files, and agree on two.** `subr_prof.c`
(`STATIC`/`GPROF`) and `kperfbsd.c` (`ffs`/`fls`/`copyinstr`, the per-component defines) fail in
both; `STAGE90_BOOT` adds `bsd_init.c`, whose single remaining error is
`bsd/netinet/mptcp_var.h:465: no member named 't_mptcb' in 'struct tcpcb'` (`MPTCP` off here — the
