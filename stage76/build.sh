...
./xnu_workspace_validate.sh ../out/stage76
./xnu_compile_graph_scan.py ../out/stage76
./xnu_object_subset_compile.sh ../out/stage76
./xnu_link_proof.sh ../out/stage76
...
"$PYTHON" "$MACHO_FIXTURE_GEN" \
  ...
  --bin-output ../out/stage76/stage76_fixture.macho \
  --symbol-prefix stage76 \
  --metadata-file ../out/stage76/xnu-link-macho-metadata.txt
...
LDFLAGS=(
  ...
  -Wl,-T,linker.ld
  -Wl,--build-id=none
  -Wl,-Map,../out/stage76/stage76.map
)
...
for src in "${SOURCES[@]}"; do
  obj="../out/stage76/${src%.*}.o"
  OBJECTS+=("$obj")
  $CC "${CFLAGS[@]}" -c "$src" -o "$obj"
done

$CC "${CFLAGS[@]}" "${LDFLAGS[@]}" "${OBJECTS[@]}" -o ../out/stage76/stage76.elf
$OBJCOPY -O binary ../out/stage76/stage76.elf ../out/stage76/stage76.bin
$OBJDUMP -d ../out/stage76/stage76.elf > ../out/stage76/stage76.disasm
$NM -n ../out/stage76/stage76.elf > ../out/stage76/stage76.symbols
$SIZE ../out/stage76/stage76.elf > ../out/stage76/stage76.size

: > ../out/stage76/empty-ramdisk

$MKBOOTIMG \
  --kernel ../out/stage76/stage76.bin \
  --ramdisk ../out/stage76/empty-ramdisk \
  --base 0x00000000 \
  ...
  --pagesize 2048 \
  --cmdline 'stage76 mi4ios6=stage76 c-runtime apple-dt macho-fixture load-plan materialize highva-dryrun safe-table stage-owned-tables ttbr0-roundtrip recovery-table cache-bits-preserved xnu-live-probe xnu-bs-contract xnu-pmap-bs-contract xnu-pmap-table-contract xnu-pmap-page-contract xnu-pmap-attr-contract xnu-pmap-mw-contract xnu-pmap-trans-contract xnu-pexpert-hook xnu-iokit-platform xnu-iokit-match-contract xnu-iokit-regsvc-contract xnu-iokit-provider xnu-iokit-catalog xnu-iokit-propinh xnu-iokit-regtop xnu-iokit-attachstart xnu-iokit-lifecycle xnu-iokit-pn-open pexpert-hook-ready iokit-platform-scaffold iokit-match-local iokit-regsvc-local iokit-provider-local iokit-catalog-local iokit-propinh-local iokit-regtop-local iokit-attachstart-local lifecycle-regsvc-local pn-open-local irq-timer-hook pmap-table-local pmap-page-local pmap-attr-local pmap-mw-local pmap-transition-local pmap-bootstrap-ref public-xnu-workspace public-xnu-compile-graph public-xnu-platform-graph public-xnu-object-subset xnu-controlled-link xnu-bounded-pe-gen xnu-arm-pe-bootargs xnu-arm-consistent-debug stage76-xnu-link-proof inert-macho-fixture cancro-target live-stage-owned-xnu-probe no-full-xnu-build no-xnu-start no-platform-runtime-exec no-iokit-runtime-exec no-iokit-propinh-exec no-regtop-exec no-attachstart-exec no-lifecycle-exec no-register-exec no-notify-exec no-int-del-open-exec no-macho-exec no-phys-write no-tte-write no-pmap-write no-live-pmap-install no-tlb-invalidate no-cache-change no-persist-write no-external-mutation' \
  --header_version 0 \
  --output ../out/stage76/stage76.img
...
if [[ -f "$QCDT_DT" ]]; then
  "$QCDT_PACKER" \
    ...
    --pagesize 2048 \
    --cmdline 'stage76 mi4ios6=stage76 c-runtime apple-dt macho-fixture load-plan materialize highva-dryrun safe-table stage-owned-tables ttbr0-roundtrip recovery-table cache-bits-preserved xnu-live-probe xnu-bs-contract xnu-pmap-bs-contract xnu-pmap-table-contract xnu-pmap-page-contract xnu-pmap-attr-contract xnu-pmap-mw-contract xnu-pmap-trans-contract xnu-pexpert-hook xnu-iokit-platform xnu-iokit-match-contract xnu-iokit-regsvc-contract xnu-iokit-provider xnu-iokit-catalog xnu-iokit-propinh xnu-iokit-regtop xnu-iokit-attachstart xnu-iokit-lifecycle xnu-iokit-pn-open pexpert-hook-ready iokit-platform-scaffold iokit-match-local iokit-regsvc-local iokit-provider-local iokit-catalog-local iokit-propinh-local iokit-regtop-local iokit-attachstart-local lifecycle-regsvc-local pn-open-local irq-timer-hook pmap-table-local pmap-page-local pmap-attr-local pmap-mw-local pmap-transition-local pmap-bootstrap-ref public-xnu-workspace public-xnu-compile-graph public-xnu-platform-graph public-xnu-object-subset xnu-controlled-link xnu-bounded-pe-gen xnu-arm-pe-bootargs xnu-arm-consistent-debug stage76-xnu-link-proof inert-macho-fixture cancro-target live-stage-owned-xnu-probe no-full-xnu-build no-xnu-start no-platform-runtime-exec no-iokit-runtime-exec no-iokit-propinh-exec no-regtop-exec no-attachstart-exec no-lifecycle-exec no-register-exec no-notify-exec no-int-del-open-exec no-macho-exec no-phys-write no-tte-write no-pmap-write no-live-pmap-install no-tlb-invalidate no-cache-change no-persist-write no-external-mutation' \
    --output ../out/stage76/stage76-qcdt.img
else
  echo "warning: QCDT_DT not found: $QCDT_DT; skipping stage76-qcdt.img" >&2
fi

sha256sum ../out/stage76/stage76_fixture.macho ../out/stage76/stage76.elf ../out/stage76/stage76.bin ../out/stage76/stage76.img \
  $(if [[ -f ../out/stage76/stage76-qcdt.img ]]; then printf '%s' ../out/stage76/stage76-qcdt.img; fi) \
  > ../out/stage76/SHA256SUMS.txt

ls -l ../out/stage76/stage76.elf ../out/stage76/stage76.bin ../out/stage76/stage76.img \
  $(if [[ -f ../out/stage76/stage76-qcdt.img ]]; then printf '%s' ../out/stage76/stage76-qcdt.img; fi)
cat ../out/stage76/stage76.size
cat ../out/stage76/SHA256SUMS.txt
