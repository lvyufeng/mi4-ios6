...
void build_boot_args(struct boot_args *args, void *dt, uint32_t dt_len)
{
    ...
    args->virtBase = 0;                  /* Stage76 boot args are still identity-based before MMU handoff. */
    args->physBase = STAGE76_BASE;
    args->memSize = RAM_CONSOLE_BASE - RAM_PHYS_BASE;
    args->topOfKernelData = (uint32_t)(uintptr_t)__stage76_image_end;

    args->machineType = MACHINE_TYPE_MSM8974;
    args->deviceTreeP = dt;
    args->deviceTreeLength = dt_len;

    const char cmd[] = "debug=0x144 serial=0x1 mi4ios6.stage=76 xnu-live-probe xnu-cg xnu-bs xnu-pmap-bs pexpert-hook-ready iokit-platform iokit-match iokit-regsvc iokit-provider iokit-catalog iokit-propinh iokit-regtop iokit-as-lf-pn-open pmap-ref st76dt=0x76";
