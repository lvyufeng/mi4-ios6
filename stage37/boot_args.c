...
void build_boot_args(struct boot_args *args, void *dt, uint32_t dt_len)
{
    ...
    args->virtBase = 0;                  /* Stage37 boot args are still identity-based before MMU handoff. */
    args->physBase = STAGE37_BASE;
    args->memSize = RAM_CONSOLE_BASE - RAM_PHYS_BASE;
    args->topOfKernelData = (uint32_t)(uintptr_t)__stage37_image_end;
