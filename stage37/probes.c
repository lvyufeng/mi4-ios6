...
/*
 * Stage37 read-only hardware probes.
 *
...
static void probe_cp15(void)
{
    ...
    log_puts("MI4IOS6_STAGE37 probe cp15 begin\n");
    ...
    log_puts("MI4IOS6_STAGE37 probe cp15 end\n");
}
...
static void probe_generic_timer(void)
{
    ...
    log_puts("MI4IOS6_STAGE37 probe arch-timer begin\n");
    log_kv32("id_pfr1_generic_timer_field", generic_timer_field);
    if (!generic_timer_field) {
        log_puts("MI4IOS6_STAGE37 arch-timer skipped: ID_PFR1 says absent\n");
        log_puts("MI4IOS6_STAGE37 probe arch-timer end\n");
        return;
    }

    log_puts("MI4IOS6_STAGE37 pre read CNTFRQ\n");
    freq = read_cntfrq();
    log_kv32("cntfrq_hz", freq);

    log_puts("MI4IOS6_STAGE37 pre read CNTPCT sample A\n");
    read_cntpct(&a_lo, &a_hi);
    /* short busy gap, then sample again to show the counter advances */
    for (volatile uint32_t i = 0; i < 200000u; i++) {
        __asm__ volatile ("nop");
    }
    log_puts("MI4IOS6_STAGE37 pre read CNTPCT sample B\n");
    ...
    if (b_lo != a_lo || b_hi != a_hi) {
        log_puts("MI4IOS6_STAGE37 arch-timer counter advancing\n");
    } else {
        log_puts("MI4IOS6_STAGE37 arch-timer counter static\n");
    }
    log_puts("MI4IOS6_STAGE37 probe arch-timer end\n");
}
...
static void probe_gic(void)
{
    ...
    log_puts("MI4IOS6_STAGE37 probe gic begin\n");

    log_puts("MI4IOS6_STAGE37 pre read GICD_CTLR\n");
    log_kv32("gicd_ctlr", mmio_r32(GICD_BASE + GICD_CTLR));

    log_puts("MI4IOS6_STAGE37 pre read GICD_TYPER\n");
    ...
    log_puts("MI4IOS6_STAGE37 pre read GICD_IIDR\n");
    log_kv32("gicd_iidr", mmio_r32(GICD_BASE + GICD_IIDR));

    log_puts("MI4IOS6_STAGE37 pre read GICC_IIDR\n");
    log_kv32("gicc_iidr", mmio_r32(GICC_BASE + GICC_IIDR));

    log_puts("MI4IOS6_STAGE37 probe gic end (read-only; IAR untouched)\n");
}

void run_readonly_hardware_probes(void)
{
    log_puts("MI4IOS6_STAGE37 hardware probes begin (read-only)\n");
    probe_cp15();
    probe_generic_timer();
    probe_gic();
    log_puts("MI4IOS6_STAGE37 hardware probes end\n");
}
