...
static void build_stage66_apple_dt(struct apple_dt_builder *b)
{
    ...
    apple_dt_prop_str(b, "compatible", "qcom,msm8974-xnu-stage66");
    apple_dt_prop_str(b, "model", "Xiaomi Mi 4 cancro Stage66");
    ...
    apple_dt_prop_str(b, "compatible", "qcom,msm8974-cancro-stage66");
    apple_dt_prop_str(b, "IOClass", "MSM8974PlatformExpert");
    apple_dt_prop_str(b, "IOProviderClass", "IOPlatformExpertDevice");
    apple_dt_prop_str(b, "IOMatchCategory", "Stage66LocalPlatformScaffold");
    apple_dt_prop_u32(b, "IOProbeScore", STAGE66_XNU_IOKIT_MATCH_EXPECTED_PROBE_SCORE);
    ...
    apple_dt_prop_str(b, "compatible", "qcom,msm8974-gic-stage66");
    apple_dt_prop_str(b, "IOClass", "MSM8974InterruptController");
    apple_dt_prop_str(b, "IOProviderClass", "IOPlatformExpertDevice");
    apple_dt_prop_str(b, "IOMatchCategory", "Stage66LocalInterruptService");
    apple_dt_prop_u32(b, "IOProbeScore", STAGE66_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_PROBE_SCORE);
    apple_dt_prop_u32_array(b, "reg", gic_reg, ARRAY_SIZE(gic_reg));
    apple_dt_prop_u32(b, "irq-count", STAGE66_XNU_IOKIT_REGISTRY_SERVICE_EXPECTED_IRQ_COUNT_MIN);
    ...
    apple_dt_prop_str(b, "compatible", "qcom,msm8974-timer-stage66");
    apple_dt_prop_str(b, "IOClass", "MSM8974Timer");
    apple_dt_prop_str(b, "IOProviderClass", "IOPlatformExpertDevice");
    apple_dt_prop_str(b, "IOMatchCategory", "Stage66LocalTimerService");
    apple_dt_prop_u32(b, "IOProbeScore", STAGE66_XNU_IOKIT_REGISTRY_SERVICE_TIMER_PROBE_SCORE);
    apple_dt_prop_u32_array(b, "reg", timer_reg, ARRAY_SIZE(timer_reg));
    apple_dt_prop_u32(b, "frequency", 19200000u);
    apple_dt_prop_u32(b, "timer-ppi-mask", STAGE66_XNU_IOKIT_REGISTRY_SERVICE_TIMER_PPI_MASK);
    apple_dt_prop_u32(b, "stage-owned-local-only", 1u);

    /* /msm8974-cpu-service: local-only CPU topology service match facts. */
    apple_dt_node_begin(b, 10, 0);
    apple_dt_prop_str(b, "name", "msm8974-cpu-service");
    apple_dt_prop_str(b, "compatible", "qcom,msm8974-cpu-stage66");
    apple_dt_prop_str(b, "IOClass", "IOCPU");
    apple_dt_prop_str(b, "IOProviderClass", "IOPlatformExpertDevice");
    apple_dt_prop_str(b, "IOMatchCategory", "Stage66LocalCPUService");
    apple_dt_prop_u32(b, "IOProbeScore", STAGE66_XNU_IOKIT_REGISTRY_SERVICE_CPU_PROBE_SCORE);
    ...
    apple_dt_prop_str(b, "compatible", "qcom,msm8974-rejected-stage66");
    apple_dt_prop_str(b, "IOClass", "Stage66RejectedDriver");
    apple_dt_prop_str(b, "IOProviderClass", "IOUnknownProvider");
    apple_dt_prop_str(b, "IOMatchCategory", "Stage66RejectedLocalService");
    ...
    apple_dt_prop_str(b, "boot-args", "debug=0x144 serial=0x1 mi4ios6.stage=66 xnu-bootstrap-contract xnu-pmap-contracts pmap-transition-dryrun pexpert-hook-ready iokit-platform-scaffold iokit-match-dryrun iokit-registry-dryrun pmap-ref-only no-xnu-exec no-persist-write");
...
{
    log_puts("MI4IOS6_STAGE66 handoff -> test_kernel_entry(boot_args*)\n");

    if (!args) {
        log_puts("MI4IOS6_STAGE66 handoff bad: null args\n");
        return 0;
    }
    ...
    if (args->Revision != BOOT_ARGS_REVISION || args->Version != BOOT_ARGS_VERSION) {
        log_puts("MI4IOS6_STAGE66 handoff bad: rev/version\n");
        return 0;
    }
    if (!args->deviceTreeP || !args->deviceTreeLength) {
        log_puts("MI4IOS6_STAGE66 handoff bad: no dt\n");
        return 0;
    }

    if (!apple_dt_selftest_and_log(args->deviceTreeP, args->deviceTreeLength)) {
        log_puts("MI4IOS6_STAGE66 handoff bad: dt selftest\n");
        return 0;
    }

    log_puts("MI4IOS6_STAGE66 handoff ok: boot_args + fuller apple_dt validated\n");
    return 1;
}
...
void platform_reboot(void)
{
    ...
    *restart_reason = RESTART_NORMAL;
    ...
    log_puts("MI4IOS6_STAGE66 attempting MSM8974 PS_HOLD reset\n");
    *ps_hold = 0;
    __asm__ volatile ("dsb sy" ::: "memory");
...
void stage66_main(void)
{
    struct apple_dt_builder b;
    uint32_t dt_len;

    log_init();
    log_puts("MI4IOS6_STAGE66 v1 entered; public-XNU pexpert/platform graph + bounded ARM pe_bootargs link proof scaffold; ram_console live\n");
    log_kv32("stage66_image_end", (uint32_t)(uintptr_t)__stage66_image_end);

    build_stage66_apple_dt(&b);
    dt_len = apple_dt_finish(&b);
    log_kv32("built_apple_dt_len", dt_len);

    if (!dt_len) {
        log_puts("MI4IOS6_STAGE66 apple_dt build failed\n");
        platform_reboot();
    }

    build_boot_args(&g_boot_args, g_apple_dt, dt_len);
    log_puts("MI4IOS6_STAGE66 boot_args built (rev2); fuller apple_dt attached\n");

    log_puts("MI4IOS6_STAGE66 vector base installed at ");
    log_hex32((uint32_t)(uintptr_t)stage66_vectors);
    log_puts("\n");

    log_puts("MI4IOS6_STAGE66 boot-wrapper handoff -> kernel_entry(boot_args*)\n");
    if (kernel_entry(&g_boot_args)) {
        log_puts("MI4IOS6_STAGE66 kernel_entry returned success\n");
    } else {
        log_puts("MI4IOS6_STAGE66 kernel_entry returned failure\n");
        platform_reboot();
    }
