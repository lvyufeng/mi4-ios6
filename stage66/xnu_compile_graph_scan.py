#!/usr/bin/env python3
"""Stage66 public-XNU pexpert/platform compile graph scanner.

This is a host-only classifier for the Stage66 compile migration proof. It
...
STATUS_OK = 0x66000001
STATUS_BASE = 0x66000000
...
def main() -> int:
    ...
    out_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else root / "out" / "stage66"
    ...
    if str(out_dir).endswith("/out/stage66") or "/out/stage66" in str(out_dir):
    ...
    status_lines = [
        "Stage66 public-XNU pexpert/platform compile graph scan",
        "This is a host-only classifier; public-XNU code is not executed.",
        f"stage66_xnu_compile_graph_status=0x{status:08x}",
        f"stage66_xnu_compile_graph_required_mask=0x{REQUIRED_MASK:08x}",
        f"stage66_xnu_compile_graph_satisfied_mask=0x{satisfied:08x}",
        f"stage66_xnu_compile_graph_failure_mask=0x{failure:08x}",
        f"stage66_xnu_compile_graph_candidate_count=0x{candidate_count:08x}",
        f"stage66_xnu_compile_graph_allowed_compile_count=0x{allowed_compile_count:08x}",
        f"stage66_xnu_compile_graph_allowed_link_count=0x{allowed_link_count:08x}",
        f"stage66_xnu_compile_graph_forbidden_count=0x{forbidden_count:08x}",
        f"stage66_xnu_compile_graph_shim_required_count=0x{shim_required_count:08x}",
        f"stage66_xnu_compile_graph_max_risk_class=0x{max_risk:08x}",
        f"stage66_xnu_compile_graph_pe_gen_allowed=0x{pe_gen_allowed:08x}",
        f"stage66_xnu_compile_graph_arm_bootargs_allowed=0x{arm_bootargs_allowed:08x}",
        f"stage66_xnu_compile_graph_arm_consistent_debug_allowed=0x{arm_consistent_debug_allowed:08x}",
        f"stage66_xnu_compile_graph_consistent_debug_layout_recorded=0x{consistent_debug_layout_recorded:08x}",
        f"stage66_xnu_compile_graph_arm_pe_kprintf_blocked=0x{arm_pe_kprintf_blocked:08x}",
        f"stage66_xnu_compile_graph_arm_pe_serial_blocked=0x{arm_pe_serial_blocked:08x}",
        f"stage66_xnu_compile_graph_arm_pe_identify_machine_blocked=0x{arm_pe_identify_machine_blocked:08x}",
        f"stage66_xnu_compile_graph_arm_pe_init_blocked=0x{arm_pe_init_blocked:08x}",
        f"stage66_xnu_compile_graph_bootstrap_contract_selected=0x{bootstrap_contract_selected:08x}",
        f"stage66_xnu_compile_graph_pmap_reference_mask=0x{pmap_reference_mask:08x}",
        f"stage66_xnu_compile_graph_pmap_runtime_blocked_mask=0x{pmap_runtime_blocked_mask:08x}",
        f"stage66_xnu_compile_graph_pmap_reference_count=0x{pmap_reference_count:08x}",
        f"stage66_xnu_compile_graph_pmap_public_compile_count=0x{pmap_public_compile_count:08x}",
        f"stage66_xnu_compile_graph_pmap_public_link_count=0x{pmap_public_link_count:08x}",
        f"stage66_xnu_compile_graph_pmap_reference_only=0x{pmap_reference_only:08x}",
        f"stage66_xnu_compile_graph_iokit_reference_mask=0x{iokit_reference_mask:08x}",
        f"stage66_xnu_compile_graph_iokit_runtime_blocked_mask=0x{iokit_runtime_blocked_mask:08x}",
        f"stage66_xnu_compile_graph_iokit_reference_count=0x{iokit_reference_count:08x}",
        f"stage66_xnu_compile_graph_iokit_public_compile_count=0x{iokit_public_compile_count:08x}",
        f"stage66_xnu_compile_graph_iokit_public_link_count=0x{iokit_public_link_count:08x}",
        f"stage66_xnu_compile_graph_iokit_reference_only=0x{iokit_reference_only:08x}",
        f"stage66_xnu_compile_graph_platform_reference_count=0x{platform_reference_count:08x}",
        f"stage66_xnu_compile_graph_blocked_runtime_count=0x{blocked_runtime_count:08x}",
        f"stage66_xnu_compile_graph_duplicate_symbol_count=0x{duplicate_symbol_count:08x}",
        f"stage66_xnu_compile_graph_pe_state_abi_recorded=0x{pe_state_abi_recorded:08x}",
        f"stage66_xnu_compile_graph_boot_args_arm_layout_recorded=0x{boot_args_arm_layout_recorded:08x}",
        f"stage66_xnu_compile_graph_4570_bounded_reference_policy=0x{ref_only:08x}",
        "stage66_xnu_compile_graph_no_full_xnu_build=0x00000001",
        "stage66_xnu_compile_graph_no_public_xnu_exec=0x00000001",
        "stage66_xnu_compile_graph_no_macho_exec=0x00000001",
        "stage66_xnu_compile_graph_no_platform_runtime_exec=0x00000001",
        f"stage66_xnu_compile_graph_no_external_mutation=0x{1 if external_clean else 0:08x}",
        "stage66_xnu_compile_graph_outputs_ignored=0x00000001",
        "stage66_xnu_compile_graph_fail_closed=0x00000001",
    ]
    ...
    header.write_text(f"""#ifndef MI4IOS6_STAGE66_XNU_COMPILE_GRAPH_GENERATED_H
#define MI4IOS6_STAGE66_XNU_COMPILE_GRAPH_GENERATED_H

#define STAGE66_XNU_GRAPH_HOST_STATUS                 0x{status:08x}u
#define STAGE66_XNU_GRAPH_HOST_REQUIRED_MASK          0x{REQUIRED_MASK:08x}u
#define STAGE66_XNU_GRAPH_HOST_SATISFIED_MASK         0x{satisfied:08x}u
#define STAGE66_XNU_GRAPH_HOST_FAILURE_MASK           0x{failure:08x}u
#define STAGE66_XNU_GRAPH_HOST_CANDIDATE_COUNT        0x{candidate_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_ALLOWED_COMPILE_COUNT  0x{allowed_compile_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_ALLOWED_LINK_COUNT     0x{allowed_link_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_FORBIDDEN_COUNT        0x{forbidden_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_SHIM_REQUIRED_COUNT    0x{shim_required_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_MAX_RISK_CLASS         0x{max_risk:08x}u
#define STAGE66_XNU_GRAPH_HOST_DEVICE_TREE_CLASSIFIED {1 if (satisfied & BIT_DEVICE_TREE_CLASSIFIED) else 0}u
#define STAGE66_XNU_GRAPH_HOST_BOOTARGS_CLASSIFIED    {1 if (satisfied & BIT_BOOTARGS_CLASSIFIED) else 0}u
#define STAGE66_XNU_GRAPH_HOST_PE_GEN_CLASSIFIED      {1 if (satisfied & BIT_PE_GEN_CLASSIFIED) else 0}u
#define STAGE66_XNU_GRAPH_HOST_PE_GEN_ALLOWED         {pe_gen_allowed}u
#define STAGE66_XNU_GRAPH_HOST_ARM_BOOTARGS_CLASSIFIED {1 if (satisfied & BIT_ARM_BOOTARGS_CLASSIFIED) else 0}u
#define STAGE66_XNU_GRAPH_HOST_ARM_BOOTARGS_ALLOWED   {arm_bootargs_allowed}u
#define STAGE66_XNU_GRAPH_HOST_ARM_CONSISTENT_DEBUG_CLASSIFIED {1 if (satisfied & BIT_ARM_CONSISTENT_DEBUG_CLASSIFIED) else 0}u
#define STAGE66_XNU_GRAPH_HOST_ARM_CONSISTENT_DEBUG_ALLOWED {arm_consistent_debug_allowed}u
#define STAGE66_XNU_GRAPH_HOST_CONSISTENT_DEBUG_LAYOUT_RECORDED {consistent_debug_layout_recorded}u
#define STAGE66_XNU_GRAPH_HOST_ARM_PE_KPRINTF_BLOCKED {arm_pe_kprintf_blocked}u
#define STAGE66_XNU_GRAPH_HOST_ARM_PE_SERIAL_BLOCKED {arm_pe_serial_blocked}u
#define STAGE66_XNU_GRAPH_HOST_ARM_PE_IDENTIFY_MACHINE_BLOCKED {arm_pe_identify_machine_blocked}u
#define STAGE66_XNU_GRAPH_HOST_ARM_PE_INIT_BLOCKED {arm_pe_init_blocked}u
#define STAGE66_XNU_GRAPH_HOST_BOOTSTRAP_CONTRACT_SELECTED {bootstrap_contract_selected}u
#define STAGE66_XNU_GRAPH_HOST_PMAP_REFERENCE_MASK 0x{pmap_reference_mask:08x}u
#define STAGE66_XNU_GRAPH_HOST_PMAP_RUNTIME_BLOCKED_MASK 0x{pmap_runtime_blocked_mask:08x}u
#define STAGE66_XNU_GRAPH_HOST_PMAP_REFERENCE_COUNT 0x{pmap_reference_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_PMAP_PUBLIC_COMPILE_COUNT 0x{pmap_public_compile_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_PMAP_PUBLIC_LINK_COUNT 0x{pmap_public_link_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_PMAP_REFERENCE_ONLY {pmap_reference_only}u
#define STAGE66_XNU_GRAPH_HOST_IOKIT_REFERENCE_MASK 0x{iokit_reference_mask:08x}u
#define STAGE66_XNU_GRAPH_HOST_IOKIT_RUNTIME_BLOCKED_MASK 0x{iokit_runtime_blocked_mask:08x}u
#define STAGE66_XNU_GRAPH_HOST_IOKIT_REFERENCE_COUNT 0x{iokit_reference_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_IOKIT_PUBLIC_COMPILE_COUNT 0x{iokit_public_compile_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_IOKIT_PUBLIC_LINK_COUNT 0x{iokit_public_link_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_IOKIT_REFERENCE_ONLY {iokit_reference_only}u
#define STAGE66_XNU_GRAPH_HOST_PLATFORM_REFERENCE_COUNT 0x{platform_reference_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_BLOCKED_RUNTIME_COUNT  0x{blocked_runtime_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_DUPLICATE_SYMBOL_COUNT 0x{duplicate_symbol_count:08x}u
#define STAGE66_XNU_GRAPH_HOST_PE_STATE_ABI_RECORDED  {pe_state_abi_recorded}u
#define STAGE66_XNU_GRAPH_HOST_BOOT_ARGS_ARM_LAYOUT_RECORDED {boot_args_arm_layout_recorded}u
#define STAGE66_XNU_GRAPH_HOST_FORBIDDEN_EXCLUDED     {1 if forbidden_excluded else 0}u
#define STAGE66_XNU_GRAPH_HOST_INCLUDE_DEPS_RECORDED  1u
#define STAGE66_XNU_GRAPH_HOST_SYMBOL_DEPS_RECORDED   1u
#define STAGE66_XNU_GRAPH_HOST_SHIM_NEEDS_RECORDED    1u
#define STAGE66_XNU_GRAPH_HOST_RISK_CLASSES_RECORDED  1u
#define STAGE66_XNU_GRAPH_HOST_BASELINE_COMMIT32      0x{BASELINE_COMMIT32:08x}u
#define STAGE66_XNU_GRAPH_HOST_MASTER_VERSION         0x{BASELINE_MASTER32:08x}u
#define STAGE66_XNU_GRAPH_HOST_PUBLIC_2050_BASELINE   {1 if baseline_ok else 0}u
#define STAGE66_XNU_GRAPH_HOST_4570_BOUNDED_REFERENCE_POLICY    {ref_only}u
#define STAGE66_XNU_GRAPH_HOST_NO_FULL_XNU_BUILD      1u
#define STAGE66_XNU_GRAPH_HOST_NO_PUBLIC_XNU_EXEC     1u
#define STAGE66_XNU_GRAPH_HOST_NO_MACHO_EXEC          1u
#define STAGE66_XNU_GRAPH_HOST_NO_PLATFORM_RUNTIME_EXEC 1u
#define STAGE66_XNU_GRAPH_HOST_NO_EXTERNAL_MUTATION   {1 if external_clean else 0}u
#define STAGE66_XNU_GRAPH_HOST_OUTPUTS_IGNORED        1u
#define STAGE66_XNU_GRAPH_HOST_FAIL_CLOSED            1u
