...
static uint32_t stage87_high_va_target(uint32_t input) __attribute__((noinline));

static uint32_t stage87_high_va_target(uint32_t input)
{
	return (input ^ 0xfeedface) + 0x12345678;
}

static struct stage87_xnu_arm_vm_init_high_va_code_exec_result g_result;

static uint32_t stage87_xnu_arm_vm_init_high_va_code_exec_checksum(
    volatile const struct stage87_xnu_arm_vm_init_high_va_code_exec_result *r)
{
	volatile const uint32_t *words = (volatile const uint32_t *)r;
	uint32_t count = (uint32_t)(offsetof(struct stage87_xnu_arm_vm_init_high_va_code_exec_result, checksum) / sizeof(uint32_t));
	uint32_t chk = 0;
...
static void stage87_xnu_arm_vm_init_high_va_code_exec_log(
    volatile const struct stage87_xnu_arm_vm_init_high_va_code_exec_result *r)
{
	xnu_log_puts("stage87_xnu_arm_vm_init_high_va_code_exec result:\n");
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_version", r->version);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_size", r->size);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_status", r->status);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_required_mask", r->required_mask);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_satisfied_mask", r->satisfied_mask);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_failure_mask", r->failure_mask);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_full_pmap_status", r->full_pmap_status);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_full_pmap_checksum", r->full_pmap_checksum);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_fn_phys", r->fn_phys);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_fn_high_va", r->fn_high_va);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_fn_virt_base", r->fn_virt_base);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_fn_called", r->fn_called);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_fn_input", r->fn_input);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_fn_result", r->fn_result);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_fn_expected", r->fn_expected);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_fn_result_correct", r->fn_result_correct);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_public_xnu_executed", r->public_xnu_executed);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_persistent_write_attempted", r->persistent_write_attempted);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_code_exec_checksum", r->checksum);
}

int stage87_xnu_arm_vm_init_high_va_code_exec_run(
    struct boot_args *args,
    struct stage87_xnu_entry_stub_result *entry_result)
{
	struct stage87_xnu_arm_vm_init_high_va_code_exec_result *r = &g_result;
	const struct stage87_xnu_arm_vm_init_full_pmap_result *pmap_result;
	uint32_t fn_phys = (uint32_t)(uintptr_t)stage87_high_va_target;
	uint32_t fn_high_va;
	...
	/* Initialize result */
	memset(r, 0, sizeof(*r));
	r->version = STAGE87_XNU_ARM_VM_INIT_HIGH_VA_CODE_EXEC_VERSION;
	r->size = sizeof(*r);
	r->required_mask = STAGE87_XNU_ARM_VM_INIT_HIGH_VA_CODE_EXEC_REQUIRED_MASK;
	...
	if (!pmap_result || pmap_result->status != STAGE87_STATUS_OK) {
		r->failure_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_CODE_EXEC_FAIL_FULL_PMAP;
		goto finish;
	}
	r->full_pmap_status = pmap_result->status;
	r->full_pmap_checksum = pmap_result->checksum;
	r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_CODE_EXEC_SAT_FULL_PMAP_OK;
	...
	r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_CODE_EXEC_SAT_FN_ADDR_COMPUTED;
	...
	r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_CODE_EXEC_SAT_FN_CALLED;

	/* Verify result */
	if (result == expected) {
		r->fn_result_correct = 1;
		r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_CODE_EXEC_SAT_FN_RESULT_CORRECT;
	} else {
		r->failure_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_CODE_EXEC_FAIL_FN_RESULT_WRONG;
	}

	/* Validate safety boundaries */
	r->public_xnu_executed = 0;  /* Stage-owned only */
	r->persistent_write_attempted = 0;  /* Stage-owned only */
	r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_CODE_EXEC_SAT_NO_PUBLIC_XNU;
	r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_CODE_EXEC_SAT_NO_PERSISTENT_WRITE;
...
finish:
	...
	/* Compute checksum */
	r->checksum = stage87_xnu_arm_vm_init_high_va_code_exec_checksum(r);

	/* Log result */
	stage87_xnu_arm_vm_init_high_va_code_exec_log(r);

	return r->status == STAGE87_STATUS_OK ? 0 : -1;
}

const struct stage87_xnu_arm_vm_init_high_va_code_exec_result *
stage87_xnu_arm_vm_init_high_va_code_exec_result(void)
{
	return &g_result;
}
