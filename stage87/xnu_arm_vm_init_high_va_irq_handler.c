 ...
 * of the existing stage87_vectors table and verifying a timer IRQ is delivered and
 * handled correctly.
 *
 * Stage85 proved:
...
static struct stage87_xnu_arm_vm_init_high_va_irq_handler_result g_result;

static uint32_t stage87_xnu_arm_vm_init_high_va_irq_handler_checksum(
    volatile const struct stage87_xnu_arm_vm_init_high_va_irq_handler_result *r)
{
	volatile const uint32_t *words = (volatile const uint32_t *)r;
	uint32_t count = (uint32_t)(offsetof(struct stage87_xnu_arm_vm_init_high_va_irq_handler_result, checksum) / sizeof(uint32_t));
	uint32_t chk = 0;
...
static void stage87_xnu_arm_vm_init_high_va_irq_handler_log(
    volatile const struct stage87_xnu_arm_vm_init_high_va_irq_handler_result *r)
{
	xnu_log_puts("stage87_xnu_arm_vm_init_high_va_irq_handler result:\n");
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_version", r->version);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_size", r->size);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_status", r->status);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_required_mask", r->required_mask);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_satisfied_mask", r->satisfied_mask);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_failure_mask", r->failure_mask);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_full_pmap_status", r->full_pmap_status);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_vectors_phys", r->vectors_phys);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_vectors_high_va", r->vectors_high_va);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_original_vbar", r->original_vbar);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_high_va_vbar_set", r->high_va_vbar_set);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_irq_count_before", r->irq_count_before);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_timer_irq_count_before", r->timer_irq_count_before);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_irq_window_opened", r->irq_window_opened);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_irq_count_after", r->irq_count_after);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_timer_irq_count_after", r->timer_irq_count_after);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_irq_delivered", r->irq_delivered);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_vbar_restored", r->vbar_restored);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_restored_vbar", r->restored_vbar);
	xnu_log_kv32("stage87_xnu_arm_vm_init_high_va_irq_handler_checksum", r->checksum);
}

int stage87_xnu_arm_vm_init_high_va_irq_handler_run(
    struct boot_args *args,
    struct stage87_xnu_entry_stub_result *entry_result)
{
	struct stage87_xnu_arm_vm_init_high_va_irq_handler_result *r = &g_result;
	...
	/* Initialize result */
	memset(r, 0, sizeof(*r));
	r->version = STAGE87_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_VERSION;
	r->size = sizeof(*r);
	r->required_mask = STAGE87_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_REQUIRED_MASK;
	...
	if (!pmap_result || pmap_result->status != STAGE87_STATUS_OK) {
		r->failure_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_FAIL_FULL_PMAP;
		goto finish;
	}
	r->full_pmap_status = pmap_result->status;
	r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_FULL_PMAP_OK;
	...
	r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_VECTORS_ADDR_COMPUTED;
	...
	irq_count_before = stage87_irq_count;
	timer_irq_count_before = stage87_timer_irq_count;
	 ...
	 * The GIC distributor base is at 0xf9000000 (from GIC_state_stage87 in gic.c). */
	...
	hvir_enable_irqs();
	timeout = 100000;
	while (stage87_timer_irq_count == timer_irq_count_before && timeout > 0) {
		timeout--;
	}
	hvir_disable_irqs();

	/* Check if timer fired with low-VA VBAR */
	if (stage87_timer_irq_count > timer_irq_count_before) {
		...
		r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_HIGH_VA_VBAR_SET;
		...
		r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_IRQ_WINDOW_OPENED;
	...
	irq_count_after = stage87_irq_count;
	timer_irq_count_after = stage87_timer_irq_count;
	...
	if (timer_irq_count_after > timer_irq_count_before) {
		r->irq_delivered = 1;
		r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_IRQ_DELIVERED;
	} else {
		r->failure_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_FAIL_IRQ_NOT_DELIVERED;
	}
	...
	r->satisfied_mask |= STAGE87_XNU_ARM_VM_INIT_HIGH_VA_IRQ_HANDLER_SAT_VBAR_RESTORED;
...
finish:
	...
	/* Compute checksum */
	r->checksum = stage87_xnu_arm_vm_init_high_va_irq_handler_checksum(r);

	/* Log result */
	stage87_xnu_arm_vm_init_high_va_irq_handler_log(r);

	return r->status == STAGE87_STATUS_OK ? 0 : -1;
}

const struct stage87_xnu_arm_vm_init_high_va_irq_handler_result *
stage87_xnu_arm_vm_init_high_va_irq_handler_result(void)
{
	return &g_result;
}
