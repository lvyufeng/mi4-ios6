...
#define STAGE87_PAGE_SIZE 4096u

struct stage87_macho_header_32 {
    uint32_t magic;
    uint32_t cputype;
    uint32_t cpusubtype;
    uint32_t filetype;
...
struct stage87_macho_load_command {
    uint32_t cmd;
    uint32_t cmdsize;
};

struct stage87_macho_segment_command_32 {
    uint32_t cmd;
    uint32_t cmdsize;
    char segname[16];
    uint32_t vmaddr;
...
struct stage87_macho_section_32 {
    char sectname[16];
    char segname[16];
    uint32_t addr;
    uint32_t size;
...
struct stage87_macho_symtab_command {
    uint32_t cmd;
    uint32_t cmdsize;
    uint32_t symoff;
    uint32_t nsyms;
...
static struct stage87_loader_preflight stage87_loader_preflight_block;
static uint8_t stage87_macho_staging_arena[STAGE87_MACHO_STAGING_ARENA_CAP]
    __attribute__((aligned(STAGE87_MACHO_STAGING_ARENA_ALIGN)));
static uint8_t stage87_tte_dryrun_arena[STAGE87_XNU_TTE_WORKSPACE_BYTES]
    __attribute__((aligned(STAGE87_XNU_TTE_ALIGNMENT)));
static uint8_t stage87_safe_table_arena[STAGE87_XNU_TTE_L1_SIZE * 2u]
...
static uint32_t segment_mask_for_name(const char segname[16])
{
    if (fixed16_eq(segname, "__TEXT")) {
        return STAGE87_MACHO_SEG_TEXT;
    }
    if (fixed16_eq(segname, "__DATA")) {
        return STAGE87_MACHO_SEG_DATA;
    }
    if (fixed16_eq(segname, "__LINKEDIT")) {
        return STAGE87_MACHO_SEG_LINKEDIT;
    }
    if (fixed16_eq(segname, "__KLD")) {
        return STAGE87_MACHO_SEG_KLD;
    }
    if (fixed16_eq(segname, "__LAST")) {
        return STAGE87_MACHO_SEG_LAST;
    }
    if (fixed16_eq(segname, "__PRELINK_TEXT")) {
        return STAGE87_MACHO_SEG_PRELINK_TEXT;
    }
    if (fixed16_eq(segname, "__PRELINK_INFO")) {
        return STAGE87_MACHO_SEG_PRELINK_INFO;
    }
    if (fixed16_eq(segname, "__PRELINK_STATE")) {
        return STAGE87_MACHO_SEG_PRELINK_STATE;
    }
    if (fixed16_eq(segname, "__PRELINK")) {
        return STAGE87_MACHO_SEG_PRELINK;
    }

    return 0;
}

static void record_key_segment(struct stage87_macho_probe_result *result,
                               const struct stage87_macho_segment_command_32 *segment,
                               uint32_t seg_bit)
{
    if (seg_bit == STAGE87_MACHO_SEG_TEXT) {
    ...
    } else if (seg_bit == STAGE87_MACHO_SEG_DATA) {
    ...
    } else if (seg_bit == STAGE87_MACHO_SEG_LINKEDIT) {
    ...
    } else if (seg_bit == STAGE87_MACHO_SEG_PRELINK_TEXT) {
    ...
    } else if (seg_bit == STAGE87_MACHO_SEG_PRELINK_INFO) {
    ...
    } else if (seg_bit == STAGE87_MACHO_SEG_PRELINK_STATE) {
...
static void add_load_plan_entry(struct stage87_macho_probe_result *result,
                                const struct stage87_macho_segment_command_32 *segment,
                                uint32_t seg_bit)
{
    struct stage87_macho_load_plan *plan = &result->load_plan;
    struct stage87_macho_load_plan_entry *entry;

    if (segment->vmsize == 0u) {
        return;
    }
    if (plan->entry_count >= STAGE87_MACHO_LOAD_PLAN_MAX) {
        result->failure_mask |= STAGE87_MACHO_FAIL_LOAD_PLAN_FULL;
        return;
    }
    ...
    if (segment->filesize != 0u) {
        entry->flags |= STAGE87_MACHO_LOAD_PLAN_FILE_BACKED;
    }
    if ((seg_bit & STAGE87_MACHO_SEG_PRELINK_MASK) != 0u) {
        entry->flags |= STAGE87_MACHO_LOAD_PLAN_PRELINK;
    }
    if (segment->vmsize > segment->filesize) {
        entry->flags |= STAGE87_MACHO_LOAD_PLAN_ZERO_FILL;
    }
    plan->entry_count++;
}

static void record_section_match(struct stage87_macho_probe_result *result,
                                 const struct stage87_macho_section_32 *section)
{
    uint32_t section_bit = 0;

    if (fixed16_eq(section->segname, "__TEXT") && fixed16_eq(section->sectname, "__text")) {
        section_bit = STAGE87_MACHO_SECT_TEXT_TEXT;
        ...
        section_bit = STAGE87_MACHO_SECT_DATA_CONST;
        ...
        section_bit = STAGE87_MACHO_SECT_PRELINK_TEXT_TEXT;
        ...
        section_bit = STAGE87_MACHO_SECT_PRELINK_INFO_INFO;
                           ...
                           const struct stage87_macho_section_32 *section)
{
    ...
    if (add_overflow_u32(section->addr, section->size, &section_end) ||
        ...
        result->failure_mask |= STAGE87_MACHO_FAIL_SECTION_BOUNDS;
    }
...
static void record_segment(struct stage87_macho_probe_result *result,
                           const struct stage87_macho_segment_command_32 *segment)
{
    uint32_t vm_end = 0;
    uint32_t file_end = 0;
...
static void parse_segment_sections(struct stage87_macho_probe_result *result,
                                   const uint8_t *bytes,
                                   ...
                                   const struct stage87_macho_segment_command_32 *segment)
{
    ...
    if (mul_overflow_u32(segment->nsects, sizeof(struct stage87_macho_section_32), &section_bytes) ||
    ...
    for (uint32_t i = 0; i < segment->nsects; i++) {
        const struct stage87_macho_section_32 *section =
            (const struct stage87_macho_section_32 *)(const void *)(bytes + section_offset);
        record_section(result, segment, section);
        section_offset += sizeof(*section);
    }
}

static void finalize_load_plan(struct stage87_macho_probe_result *result)
{
    struct stage87_macho_load_plan *plan = &result->load_plan;
    ...
    plan->version = STAGE87_MACHO_LOAD_PLAN_VERSION;
    ...
    if (plan->entry_count == 0u || result->min_vmaddr == 0u || result->max_vmaddr <= result->min_vmaddr) {
        result->failure_mask |= STAGE87_MACHO_FAIL_LOAD_PLAN;
        return;
    }

    for (uint32_t i = 0; i < plan->entry_count; i++) {
        struct stage87_macho_load_plan_entry *entry = &plan->entries[i];
        uint32_t vm_delta;
        ...
        if (add_overflow_u32(RAM_PHYS_BASE, vm_delta, &entry->physaddr) ||
            align_up_checked_u32(entry->vmsize, STAGE87_PAGE_SIZE, &aligned_size) ||
            add_overflow_u32(entry->physaddr, aligned_size, &entry_end)) {
            result->failure_mask |= STAGE87_MACHO_FAIL_PHYS_RANGE;
            continue;
        }
    ...
    if ((result->failure_mask & (STAGE87_MACHO_FAIL_LOAD_PLAN | STAGE87_MACHO_FAIL_PHYS_RANGE |
                                 STAGE87_MACHO_FAIL_LOAD_PLAN_FULL)) == 0u) {
        plan->status = STAGE87_STATUS_OK;
        result->load_plan_status = STAGE87_STATUS_OK;
        result->validation_mask |= STAGE87_MACHO_VALID_LOAD_PLAN;
    } else {
        plan->status = STAGE87_STATUS_FAIL(result->failure_mask);
...
static void log_load_plan_entry(const char *prefix_vmaddr, const char *prefix_vmsize,
                                ...
                                const struct stage87_macho_load_plan_entry *entry)
{
...
static void log_load_plan_entries(const struct stage87_macho_probe_result *result)
{
    const struct stage87_macho_load_plan *plan = &result->load_plan;
                        ...
                        struct stage87_macho_probe_result *result)
{
    ...
    const struct stage87_macho_header_32 *header;
    ...
    result->version = STAGE87_MACHO_PROBE_VERSION;
    result->artifact_base = (uint32_t)(uintptr_t)artifact;
    result->artifact_size = artifact_size;
    result->segment_required_mask = STAGE87_MACHO_SEG_REQUIRED;
    result->section_required_mask = STAGE87_MACHO_SECT_REQUIRED;
    result->entry_kind_mask = STAGE87_MACHO_ENTRY_NOT_EXECUTED;
    result->status = STAGE87_STATUS_BASE;
    result->load_plan.version = STAGE87_MACHO_LOAD_PLAN_VERSION;
    ...
    if (!artifact) {
        result->failure_mask |= STAGE87_MACHO_FAIL_NULL;
        goto done;
    }
    if (artifact_size < sizeof(*header)) {
        result->failure_mask |= STAGE87_MACHO_FAIL_SIZE;
        goto done;
    }

    header = (const struct stage87_macho_header_32 *)artifact;
    ...
    if (header->magic == STAGE87_MACHO_MAGIC) {
        result->validation_mask |= STAGE87_MACHO_VALID_MAGIC | STAGE87_MACHO_VALID_ENDIAN;
    } else if (header->magic == STAGE87_MACHO_CIGAM) {
        result->validation_mask |= STAGE87_MACHO_VALID_MAGIC;
        result->failure_mask |= STAGE87_MACHO_FAIL_ENDIAN;
    } else {
        result->failure_mask |= STAGE87_MACHO_FAIL_MAGIC;
    }

    if (header->cputype == STAGE87_MACHO_CPU_TYPE_ARM) {
        result->validation_mask |= STAGE87_MACHO_VALID_CPU;
    } else {
        result->failure_mask |= STAGE87_MACHO_FAIL_CPU;
    }

    if (header->cpusubtype == STAGE87_MACHO_CPU_SUBTYPE_ARM_ALL ||
        header->cpusubtype == STAGE87_MACHO_CPU_SUBTYPE_ARM_V7) {
        result->validation_mask |= STAGE87_MACHO_VALID_SUBTYPE;
    } else {
        result->failure_mask |= STAGE87_MACHO_FAIL_SUBTYPE;
    }

    if (header->filetype == STAGE87_MACHO_FILETYPE_EXECUTE ||
        header->filetype == STAGE87_MACHO_FILETYPE_PRELOAD) {
        result->validation_mask |= STAGE87_MACHO_VALID_FILETYPE;
    } else {
        result->failure_mask |= STAGE87_MACHO_FAIL_FILETYPE;
    }

    if (add_overflow_u32(commands_start, header->sizeofcmds, &commands_end) ||
        commands_end > artifact_size || (header->sizeofcmds & 3u) != 0u) {
        result->failure_mask |= STAGE87_MACHO_FAIL_COMMAND_BOUNDS;
    } else {
        result->validation_mask |= STAGE87_MACHO_VALID_COMMAND_BOUNDS;
    ...
    for (uint32_t i = 0; i < header->ncmds && offset < commands_end; i++) {
        const struct stage87_macho_load_command *load_command;
        ...
        load_command = (const struct stage87_macho_load_command *)(const void *)(bytes + offset);
        if (load_command->cmdsize < sizeof(*load_command) || (load_command->cmdsize & 3u) != 0u ||
            ...
            result->failure_mask |= STAGE87_MACHO_FAIL_COMMAND_SIZE;
            break;
        }

        result->load_command_count_seen++;
        if (load_command->cmd == STAGE87_MACHO_LC_SEGMENT) {
            if (load_command->cmdsize < sizeof(struct stage87_macho_segment_command_32)) {
                result->failure_mask |= STAGE87_MACHO_FAIL_COMMAND_SIZE;
            } else {
                const struct stage87_macho_segment_command_32 *segment =
                    (const struct stage87_macho_segment_command_32 *)(const void *)(bytes + offset);
                result->load_command_seen_mask |= STAGE87_MACHO_CMD_SEGMENT;
                record_segment(result, segment);
                parse_segment_sections(result, bytes, offset, segment);
            }
        } else if (load_command->cmd == STAGE87_MACHO_LC_SYMTAB) {
            result->load_command_seen_mask |= STAGE87_MACHO_CMD_SYMTAB;
            if (load_command->cmdsize >= sizeof(struct stage87_macho_symtab_command)) {
                const struct stage87_macho_symtab_command *symtab =
                    (const struct stage87_macho_symtab_command *)(const void *)(bytes + offset);
                uint32_t str_end = 0;
                if (add_overflow_u32(symtab->stroff, symtab->strsize, &str_end) || str_end > artifact_size) {
                    result->failure_mask |= STAGE87_MACHO_FAIL_SEGMENT_EXTENT;
                }
            }
        } else if (load_command->cmd == STAGE87_MACHO_LC_UNIXTHREAD) {
            result->load_command_seen_mask |= STAGE87_MACHO_CMD_UNIXTHREAD;
            result->entry_kind_mask |= STAGE87_MACHO_ENTRY_UNIXTHREAD;
        ...
        } else if (load_command->cmd == STAGE87_MACHO_LC_MAIN) {
            result->load_command_seen_mask |= STAGE87_MACHO_CMD_MAIN;
            result->entry_kind_mask |= STAGE87_MACHO_ENTRY_MAIN;
        } else {
            result->load_command_seen_mask |= STAGE87_MACHO_CMD_UNKNOWN;
...
after_commands:
    if ((result->segment_seen_mask & STAGE87_MACHO_SEG_REQUIRED) == STAGE87_MACHO_SEG_REQUIRED) {
        result->validation_mask |= STAGE87_MACHO_VALID_REQUIRED_SEGMENTS;
    } else {
        result->failure_mask |= STAGE87_MACHO_FAIL_REQUIRED_SEGMENTS;
    }
    if ((result->section_seen_mask & STAGE87_MACHO_SECT_REQUIRED) == STAGE87_MACHO_SECT_REQUIRED) {
        result->validation_mask |= STAGE87_MACHO_VALID_REQUIRED_SECTIONS;
    } else {
        result->failure_mask |= STAGE87_MACHO_FAIL_REQUIRED_SECTIONS;
    }
    if ((result->failure_mask & STAGE87_MACHO_FAIL_SEGMENT_EXTENT) == 0u) {
        result->validation_mask |= STAGE87_MACHO_VALID_FILE_EXTENTS;
    }
    if ((result->failure_mask & STAGE87_MACHO_FAIL_SECTION_BOUNDS) == 0u) {
        result->validation_mask |= STAGE87_MACHO_VALID_SECTION_BOUNDS;
    }
    if ((result->prelink_segment_seen_mask | result->prelink_section_seen_mask) != 0u) {
        result->validation_mask |= STAGE87_MACHO_VALID_PRELINK_REPORT;
    }
    if ((result->entry_kind_mask & (STAGE87_MACHO_ENTRY_UNIXTHREAD | STAGE87_MACHO_ENTRY_MAIN)) != 0u) {
        result->validation_mask |= STAGE87_MACHO_VALID_ENTRY_NOT_EXECUTED;
    } else {
        result->failure_mask |= STAGE87_MACHO_FAIL_NO_ENTRY_METADATA;
    }
    ...
    {
        const uint32_t parser_required = STAGE87_MACHO_VALID_REQUIRED &
        ...
        if ((result->validation_mask & parser_required) == parser_required &&
            ...
            result->status = STAGE87_STATUS_OK;
            xnu_log_puts("Stage84 Mach-O probe ok; entry metadata observed but not executed\n");
            return 1;
        }
    }

    result->status = STAGE87_STATUS_FAIL(result->failure_mask);
...
static void log_stage_entry(const char *copy_src_key, const char *copy_dst_key,
                            ...
                            const struct stage87_macho_staging_plan_entry *entry)
{
...
static void log_staging_entries(const struct stage87_macho_staging_plan *staging)
{
...
static uint32_t stage87_macho_getlastaddr_style(const struct stage87_macho_probe_result *probe)
{
    uint32_t last = 0;

    for (uint32_t i = 0; i < probe->load_plan.entry_count; i++) {
        uint32_t end = 0;
        const struct stage87_macho_load_plan_entry *entry = &probe->load_plan.entries[i];
...
static void stage87_macho_check_arena_aliases(struct stage87_macho_probe_result *result,
                                              uint32_t artifact_base,
                                              uint32_t artifact_size)
{
    struct stage87_macho_staging_plan *staging = &result->staging_plan;
    uint32_t artifact_end = 0;

    if (add_overflow_u32(artifact_base, artifact_size, &artifact_end)) {
        staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_ARENA_ALIAS;
        return;
    }
...
static void stage87_macho_verify_marker(struct stage87_macho_staging_plan *staging,
                                        uint32_t vmaddr,
                                        const char *prefix,
                                        uint32_t bit)
{
    ...
    if (vmaddr < staging->vm_base) {
        staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_MARKERS;
        return;
    }
    ...
    if (bytes_prefix_eq(stage87_macho_staging_arena + offset, prefix)) {
...
static void stage87_macho_verify_zero_fill(struct stage87_macho_staging_plan *staging)
{
    uint32_t required_zero_mask = 0;

    for (uint32_t i = 0; i < staging->entry_count; i++) {
        const struct stage87_macho_staging_plan_entry *entry = &staging->entries[i];
        uint32_t bit = 1u << i;
        ...
        if (entry->arena_offset + entry->copy_size + entry->zero_size > staging->arena_size) {
            staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_ZERO_FILL;
            entry_ok = 0u;
        } else {
            uint32_t zero_offset = entry->arena_offset + entry->copy_size;
            for (uint32_t j = 0; j < entry->zero_size; j++) {
                if (stage87_macho_staging_arena[zero_offset + j] != 0u) {
                    staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_ZERO_FILL;
                    entry_ok = 0u;
                    break;
                }
    ...
    if ((staging->zero_check_mask & required_zero_mask) == required_zero_mask) {
        staging->validation_mask |= STAGE87_MACHO_VALID_ZERO_FILL;
    } else {
        staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_ZERO_FILL;
    }
}

static void stage87_macho_verify_reparse(struct stage87_macho_probe_result *result,
                                         struct stage87_macho_staging_plan *staging)
{
    struct stage87_macho_probe_result materialized;
    uint32_t original_last = stage87_macho_getlastaddr_style(result);
    uint32_t materialized_last;

    if (!stage87_macho_probe(stage87_macho_staging_arena, staging->arena_size, &materialized)) {
        staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_REPARSE;
        return;
    }

    materialized_last = stage87_macho_getlastaddr_style(&materialized);
    ...
    if (materialized.magic == result->magic &&
        ...
        staging->validation_mask |= STAGE87_MACHO_VALID_MATERIALIZED_PARSE;
...
static int stage87_macho_materialize_to_arena(const void *artifact,
                                              uint32_t artifact_size,
                                              struct stage87_macho_probe_result *result)
{
    const uint8_t *source = (const uint8_t *)artifact;
    struct stage87_macho_staging_plan *staging = &result->staging_plan;
    uint32_t arena_base = (uint32_t)(uintptr_t)stage87_macho_staging_arena;
    ...
    memset(stage87_macho_staging_arena, 0, sizeof(stage87_macho_staging_arena));
    memset(staging, 0, sizeof(*staging));
    staging->version = STAGE87_MACHO_STAGING_PLAN_VERSION;
    staging->status = STAGE87_STATUS_BASE;
    staging->arena_base = arena_base;

    if (add_overflow_u32(arena_base, STAGE87_MACHO_STAGING_ARENA_CAP, &arena_end)) {
        staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_SPAN;
        goto finish;
    }
    ...
    if (align_up_checked_u32(vm_span, STAGE87_PAGE_SIZE, &arena_size) ||
        arena_size > STAGE87_MACHO_STAGING_ARENA_CAP) {
        staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_SPAN;
        goto finish;
    }
    ...
    stage87_macho_check_arena_aliases(result, (uint32_t)(uintptr_t)artifact, artifact_size);
    if (staging->failure_mask != 0u) {
        goto finish;
    }

    for (uint32_t i = 0; i < result->load_plan.entry_count; i++) {
        const struct stage87_macho_load_plan_entry *load = &result->load_plan.entries[i];
        struct stage87_macho_staging_plan_entry *entry;
        uint32_t src_end;
        ...
        if (staging->entry_count >= STAGE87_MACHO_STAGING_PLAN_MAX) {
            staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_PLAN_FULL;
            break;
        }
        if (load->vmaddr < staging->vm_base || load->filesize > load->vmsize) {
            staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_DST_BOUNDS;
            continue;
        }
        arena_offset = load->vmaddr - staging->vm_base;
        if (add_overflow_u32(load->fileoff, load->filesize, &src_end) || src_end > artifact_size) {
            staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_SRC_BOUNDS;
            continue;
        }
        ...
        if (load->filesize != 0u) {
            memcpy(stage87_macho_staging_arena + arena_offset, source + load->fileoff, load->filesize);
            staging->file_bytes += load->filesize;
        }
        if (zero_size != 0u) {
            memset(stage87_macho_staging_arena + arena_offset + load->filesize, 0, zero_size);
            staging->zero_bytes += zero_size;
        }
    }

    if (staging->entry_count != result->load_plan.entry_count) {
        staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_COPY;
    }

    if (staging->failure_mask == 0u) {
        staging->validation_mask |= STAGE87_MACHO_VALID_STAGING;
        stage87_macho_verify_reparse(result, staging);
        stage87_macho_verify_marker(staging, result->text_section_vmaddr,
                                    "ST87-TEXT", STAGE87_MACHO_MARKER_TEXT);
        stage87_macho_verify_marker(staging, result->data_const_vmaddr,
                                    "ST87-DATA", STAGE87_MACHO_MARKER_DATA);
        stage87_macho_verify_marker(staging, result->prelink_text_section_vmaddr,
                                    "ST87-PRELINK-TEXT", STAGE87_MACHO_MARKER_PRELINK_TEXT);
        if ((staging->marker_check_mask & STAGE87_MACHO_MARKER_REQUIRED) == STAGE87_MACHO_MARKER_REQUIRED) {
            staging->validation_mask |= STAGE87_MACHO_VALID_MARKERS;
        } else {
            staging->failure_mask |= STAGE87_MACHO_FAIL_STAGE_MARKERS;
        }
        stage87_macho_verify_zero_fill(staging);
    }
...
finish:
    if (staging->failure_mask == 0u &&
        (staging->validation_mask & (STAGE87_MACHO_VALID_STAGING |
                                     ...
                                     STAGE87_MACHO_VALID_MARKERS |
                                     STAGE87_MACHO_VALID_ZERO_FILL)) ==
        (STAGE87_MACHO_VALID_STAGING | STAGE87_MACHO_VALID_MATERIALIZED_PARSE |
         STAGE87_MACHO_VALID_MARKERS | STAGE87_MACHO_VALID_ZERO_FILL)) {
        staging->status = STAGE87_STATUS_OK;
    } else {
        staging->status = STAGE87_STATUS_FAIL(staging->failure_mask);
    }
    ...
    return staging->status == STAGE87_STATUS_OK;
}

static uint32_t stage87_tte_dryrun_checksum(volatile const struct stage87_xnu_tte_dryrun *tte)
{
    volatile const uint32_t *words = (volatile const uint32_t *)(uintptr_t)tte;
    uint32_t count = (uint32_t)(offsetof(struct stage87_xnu_tte_dryrun, checksum) / sizeof(uint32_t));
    uint32_t checksum = 0;
...
static void stage87_tte_map_section(uint32_t *l1, uint32_t index, uint32_t desc_flags)
{
...
static void stage87_tte_map_section_range(uint32_t *l1, uint32_t first, uint32_t last, uint32_t desc_flags)
{
    for (uint32_t i = first; i <= last && i < STAGE87_XNU_TTE_L1_ENTRY_COUNT; i++) {
        stage87_tte_map_section(l1, i, desc_flags);
        if (i == UINT32_MAX) {
            break;
        }
    }
...
static uint32_t stage87_tte_l2_and_scratch_zeroed(void)
{
    for (uint32_t i = STAGE87_XNU_TTE_L1_SIZE; i < STAGE87_XNU_TTE_WORKSPACE_BYTES; i++) {
        if (stage87_tte_dryrun_arena[i] != 0u) {
            return 0u;
        }
    }
    return 1u;
...
static uint32_t stage87_tte_section_word(uint32_t index, uint32_t desc_flags)
{
    return (index << 20) | (desc_flags & STAGE87_XNU_TTE_DESC_ATTR_MASK);
}

static uint32_t stage87_tte_descriptor_attrs_ok(uint32_t word, uint32_t expected_flags)
{
    if ((word & STAGE87_XNU_TTE_DESC_TYPE_MASK) != STAGE87_XNU_TTE_DESC_TYPE_SECTION) {
        return 0u;
    }
    return ((word & STAGE87_XNU_TTE_DESC_ATTR_MASK) ==
            (expected_flags & STAGE87_XNU_TTE_DESC_ATTR_MASK)) ? 1u : 0u;
}

static uint32_t stage87_tte_verify_section_slot(const uint32_t *l1,
                                                uint32_t index,
                                                uint32_t desc_flags,
                                                uint32_t *observed_word)
{
    ...
    word = l1[index];
    expected = stage87_tte_section_word(index, desc_flags);
    if (observed_word) {
        *observed_word = word;
    }

    return (word == expected && stage87_tte_descriptor_attrs_ok(word, desc_flags)) ? 1u : 0u;
}

static uint32_t stage87_tte_verify_section_range(const uint32_t *l1,
                                                 uint32_t first,
                                                 uint32_t last,
                                                 uint32_t desc_flags,
                                                 uint32_t *first_word,
                                                 uint32_t *last_word)
{
    ...
    if (first > last || last >= STAGE87_XNU_TTE_L1_ENTRY_COUNT) {
        return 0u;
    }

    for (uint32_t i = first; i <= last; i++) {
        if (!stage87_tte_verify_section_slot(l1, i, desc_flags, &word)) {
            ok = 0u;
        }
...
static uint32_t stage87_safe_table_checksum(const void *base, uint32_t bytes)
{
...
static uint32_t stage87_tte_section_word_pa(uint32_t pa, uint32_t desc_flags)
{
    return (pa & STAGE87_XNU_TTE_DESC_BASE_MASK) |
           (desc_flags & STAGE87_XNU_TTE_DESC_ATTR_MASK);
}

static void stage87_tte_map_section_pa(uint32_t *l1, uint32_t va, uint32_t pa, uint32_t desc_flags)
{
    uint32_t index = section_first_index(va);

    if (index < STAGE87_XNU_TTE_L1_ENTRY_COUNT) {
        l1[index] = stage87_tte_section_word_pa(pa, desc_flags);
    }
}

static void stage87_tte_map_section_range_pa(uint32_t *l1,
                                             uint32_t va_base,
                                             uint32_t pa_base,
                                             uint32_t size,
                                             uint32_t desc_flags)
{
    ...
    va_section = va_base & STAGE87_XNU_TTE_DESC_BASE_MASK;
    pa_section = pa_base & STAGE87_XNU_TTE_DESC_BASE_MASK;

    for (uint32_t i = first; i <= last && i < STAGE87_XNU_TTE_L1_ENTRY_COUNT; i++) {
        uint32_t va = i << 20;
        uint32_t pa = pa_section + (va - va_section);
        stage87_tte_map_section_pa(l1, va, pa, desc_flags);
        if (i == UINT32_MAX) {
            break;
        }
    }
...
static uint32_t stage87_tte_verify_section_slot_pa(const uint32_t *l1,
                                                   uint32_t va,
                                                   ...
                                                   uint32_t *observed_word)
{
    ...
    word = l1[index];
    expected = stage87_tte_section_word_pa(expected_pa, desc_flags);
    if (observed_word) {
        *observed_word = word;
    }
    ...
    if ((word & STAGE87_XNU_TTE_DESC_BASE_MASK) != (expected_pa & STAGE87_XNU_TTE_DESC_BASE_MASK)) {
        return 0u;
    }
    return stage87_tte_descriptor_attrs_ok(word, desc_flags);
}

static uint32_t stage87_tte_expected_section_pa(uint32_t va, uint32_t pa_base)
{
    return (pa_base & STAGE87_XNU_TTE_DESC_BASE_MASK) |
           (va & STAGE87_XNU_TTE_SECTION_OFFSET_MASK);
}

static uint32_t stage87_tte_section_pa_for_va(uint32_t va, uint32_t va_base, uint32_t pa_base)
{
    uint32_t va_section = va & STAGE87_XNU_TTE_DESC_BASE_MASK;
    uint32_t base_section = va_base & STAGE87_XNU_TTE_DESC_BASE_MASK;
    uint32_t pa_section = pa_base & STAGE87_XNU_TTE_DESC_BASE_MASK;

    return pa_section + (va_section - base_section);
}

static uint32_t stage87_tte_translate_l1_section(const uint32_t *l1,
                                                 uint32_t va,
                                                 uint32_t *pa,
                                                 uint32_t *desc_word)
{
    ...
    if ((desc & STAGE87_XNU_TTE_DESC_TYPE_MASK) != STAGE87_XNU_TTE_DESC_TYPE_SECTION) {
        return 0u;
    }
    if (pa) {
        *pa = (desc & STAGE87_XNU_TTE_DESC_BASE_MASK) |
              (va & STAGE87_XNU_TTE_SECTION_OFFSET_MASK);
    }
...
static uint32_t stage87_tte_expect_translation(const uint32_t *l1,
                                               uint32_t va,
                                               uint32_t expected_pa,
                                               uint32_t *observed_pa)
{
    ...
    if (!stage87_tte_translate_l1_section(l1, va, &pa, 0)) {
        return 0u;
    }
    if (observed_pa) {
        *observed_pa = pa;
...
static int stage87_xnu_tte_dryrun_build(const struct stage87_macho_probe_result *probe,
                                        const struct stage87_loader_preflight *preflight,
                                        struct stage87_xnu_tte_dryrun *tte)
{
    ...
    uint32_t embedded_base = (uint32_t)(uintptr_t)stage87_embedded_macho;
    uint32_t embedded_limit = 0;
    uint32_t staging_base = (uint32_t)(uintptr_t)stage87_macho_staging_arena;
    uint32_t staging_limit = 0;
    uint32_t safe_base = (uint32_t)(uintptr_t)stage87_safe_table_arena;
    ...
    uint32_t *l1 = (uint32_t *)(void *)stage87_tte_dryrun_arena;
    uint32_t *safe_identity_l1 = (uint32_t *)(void *)stage87_safe_table_arena;
    uint32_t *safe_highva_l1 = (uint32_t *)(void *)(stage87_safe_table_arena + STAGE87_XNU_TTE_L1_SIZE);

    memset(tte, 0, sizeof(*tte));
    memset(stage87_tte_dryrun_arena, 0, sizeof(stage87_tte_dryrun_arena));
    memset(stage87_safe_table_arena, 0, sizeof(stage87_safe_table_arena));

    tte->version = STAGE87_XNU_TTE_DRYRUN_VERSION;
    tte->size = sizeof(*tte);
    tte->status = STAGE87_STATUS_BASE;
    ...
    tte->workspace_size = STAGE87_XNU_TTE_WORKSPACE_BYTES;
    tte->workspace_alignment = tte->workspace_base & (STAGE87_XNU_TTE_L1_ALIGN - 1u);
    tte->local_arena_base = (uint32_t)(uintptr_t)stage87_tte_dryrun_arena;
    tte->local_arena_size = sizeof(stage87_tte_dryrun_arena);
    tte->l1_table_base = tte->workspace_base;
    tte->l1_table_size = STAGE87_XNU_TTE_L1_SIZE;
    tte->l1_entry_count = STAGE87_XNU_TTE_L1_ENTRY_COUNT;
    tte->l2_table_size = STAGE87_XNU_TTE_L2_SIZE;
    tte->kernel_section_descriptor = STAGE87_XNU_TTE_DESC_SECTION_SO;
    tte->ram_console_section_descriptor = STAGE87_XNU_TTE_DESC_SECTION_SO;
    tte->device_section_descriptor = STAGE87_XNU_TTE_DESC_SECTION_SO;

    if (add_overflow_u32(tte->workspace_base, tte->workspace_size, &workspace_limit)) {
        tte->failure_mask |= STAGE87_TTE_FAIL_WORKSPACE_RANGE;
    } else {
        tte->workspace_limit = workspace_limit;
    }
    if (add_overflow_u32(tte->local_arena_base, tte->local_arena_size, &local_limit)) {
        tte->failure_mask |= STAGE87_TTE_FAIL_LOCAL_ARENA_ALIAS;
    } else {
        tte->local_arena_limit = local_limit;
    }
    if (add_overflow_u32(tte->l1_table_base, tte->l1_table_size, &l1_limit)) {
        tte->failure_mask |= STAGE87_TTE_FAIL_LAYOUT_OVERFLOW;
    } else {
    ...
    expected_workspace_base = align_up_u32(probe->load_phys_end, STAGE87_XNU_TTE_L1_ALIGN);
    if (tte->workspace_limit > tte->workspace_base && tte->workspace_limit < RAM_CONSOLE_BASE &&
        ...
        tte->satisfied_mask |= STAGE87_TTE_SAT_WORKSPACE_RANGE;
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_WORKSPACE_RANGE;
    }
    if (tte->workspace_alignment == 0u && (tte->l1_table_base & (STAGE87_XNU_TTE_L1_ALIGN - 1u)) == 0u &&
        (tte->l2_table_base & (STAGE87_XNU_TTE_L2_ALIGN - 1u)) == 0u) {
        tte->satisfied_mask |= STAGE87_TTE_SAT_WORKSPACE_ALIGNMENT;
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_WORKSPACE_ALIGNMENT;
    }
    if (tte->l1_table_limit <= tte->workspace_limit && tte->l2_table_limit <= tte->workspace_limit &&
        ...
        tte->l1_table_size == STAGE87_XNU_TTE_L1_SIZE && tte->l2_table_size == STAGE87_XNU_TTE_L2_SIZE) {
        tte->satisfied_mask |= STAGE87_TTE_SAT_LAYOUT_FITS;
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_LAYOUT_OVERFLOW;
    }
    ...
    if (probe->load_phys_end > probe->load_phys_base) {
        ...
        if (tte->kernel_l1_last_index < STAGE87_XNU_TTE_L1_ENTRY_COUNT) {
            tte->satisfied_mask |= STAGE87_TTE_SAT_KERNEL_MAPPING;
        } else {
            tte->failure_mask |= STAGE87_TTE_FAIL_KERNEL_MAPPING;
        }
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_KERNEL_MAPPING;
    }
    ...
    if (tte->ram_console_l1_index < STAGE87_XNU_TTE_L1_ENTRY_COUNT) {
        tte->satisfied_mask |= STAGE87_TTE_SAT_RAM_CONSOLE_MAPPING;
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_RAM_CONSOLE_MAPPING;
    }
    ...
    if (tte->gic_l1_first_index <= tte->gic_l1_last_index &&
        tte->gic_l1_last_index < STAGE87_XNU_TTE_L1_ENTRY_COUNT) {
        tte->satisfied_mask |= STAGE87_TTE_SAT_DEVICE_MAPPING;
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_DEVICE_MAPPING;
    }
    ...
    if (!ranges_overlap_u32(probe->load_phys_base, probe->load_phys_end, tte->workspace_base, tte->workspace_limit) &&
        ...
        tte->satisfied_mask |= STAGE87_TTE_SAT_NO_OVERLAP;
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_OVERLAP;
    }

    if (add_overflow_u32(embedded_base, stage87_embedded_macho_size, &embedded_limit) ||
        add_overflow_u32(staging_base, STAGE87_MACHO_STAGING_ARENA_CAP, &staging_limit) ||
    ...
    if ((tte->failure_mask & (STAGE87_TTE_FAIL_KERNEL_MAPPING | STAGE87_TTE_FAIL_DEVICE_MAPPING |
                              STAGE87_TTE_FAIL_RAM_CONSOLE_MAPPING | STAGE87_TTE_FAIL_LAYOUT_OVERFLOW)) == 0u) {
        stage87_tte_map_section_range(l1, tte->lowmem_l1_first_index, tte->lowmem_l1_last_index,
                                      STAGE87_XNU_TTE_DESC_SECTION_SO);
        stage87_tte_map_section_range(l1, tte->kernel_l1_first_index, tte->kernel_l1_last_index,
                                      STAGE87_XNU_TTE_DESC_SECTION_SO);
        stage87_tte_map_section(l1, tte->ram_console_l1_index, STAGE87_XNU_TTE_DESC_SECTION_SO);
        stage87_tte_map_section_range(l1, tte->gic_l1_first_index, tte->gic_l1_last_index,
                                      STAGE87_XNU_TTE_DESC_SECTION_SO);
    }

    tte->descriptor_expected_attr_mask = STAGE87_XNU_TTE_DESC_SECTION_SO &
                                         STAGE87_XNU_TTE_DESC_ATTR_MASK;
    if ((tte->failure_mask & (STAGE87_TTE_FAIL_KERNEL_MAPPING | STAGE87_TTE_FAIL_DEVICE_MAPPING |
                              STAGE87_TTE_FAIL_RAM_CONSOLE_MAPPING | STAGE87_TTE_FAIL_LAYOUT_OVERFLOW)) == 0u) {
        uint32_t attrs_ok = 1u;
        uint32_t identity_desc_required = STAGE87_TTE_DESC_VERIFY_REQUIRED &
            ~(STAGE87_TTE_DESC_VERIFY_HIGHVA_FIRST | STAGE87_TTE_DESC_VERIFY_HIGHVA_LAST);

        if (stage87_tte_verify_section_range(l1, tte->lowmem_l1_first_index, tte->lowmem_l1_last_index,
                                             STAGE87_XNU_TTE_DESC_SECTION_SO,
                                             &tte->descriptor_lowmem_first_word,
                                             &tte->descriptor_lowmem_last_word)) {
            tte->descriptor_verify_mask |= STAGE87_TTE_DESC_VERIFY_LOWMEM_FIRST |
                                           STAGE87_TTE_DESC_VERIFY_LOWMEM_LAST;
        } else {
            tte->descriptor_failure_mask |= STAGE87_TTE_DESC_VERIFY_LOWMEM_FIRST |
                                            STAGE87_TTE_DESC_VERIFY_LOWMEM_LAST;
        }
        if (stage87_tte_verify_section_range(l1, tte->kernel_l1_first_index, tte->kernel_l1_last_index,
                                             STAGE87_XNU_TTE_DESC_SECTION_SO,
                                             &tte->descriptor_kernel_first_word,
                                             &tte->descriptor_kernel_last_word)) {
            tte->descriptor_verify_mask |= STAGE87_TTE_DESC_VERIFY_KERNEL_FIRST |
                                           STAGE87_TTE_DESC_VERIFY_KERNEL_LAST;
        } else {
            tte->descriptor_failure_mask |= STAGE87_TTE_DESC_VERIFY_KERNEL_FIRST |
                                            STAGE87_TTE_DESC_VERIFY_KERNEL_LAST;
        }
        if (stage87_tte_verify_section_slot(l1, tte->ram_console_l1_index,
                                            STAGE87_XNU_TTE_DESC_SECTION_SO,
                                            &tte->descriptor_ram_console_word)) {
            tte->descriptor_verify_mask |= STAGE87_TTE_DESC_VERIFY_RAM_CONSOLE;
        } else {
            tte->descriptor_failure_mask |= STAGE87_TTE_DESC_VERIFY_RAM_CONSOLE;
        }
        if (stage87_tte_verify_section_range(l1, tte->gic_l1_first_index, tte->gic_l1_last_index,
                                             STAGE87_XNU_TTE_DESC_SECTION_SO,
                                             &tte->descriptor_gic_first_word,
                                             &tte->descriptor_gic_last_word)) {
            tte->descriptor_verify_mask |= STAGE87_TTE_DESC_VERIFY_GIC_FIRST |
                                           STAGE87_TTE_DESC_VERIFY_GIC_LAST;
        } else {
            tte->descriptor_failure_mask |= STAGE87_TTE_DESC_VERIFY_GIC_FIRST |
                                            STAGE87_TTE_DESC_VERIFY_GIC_LAST;
        }

        tte->descriptor_type_mask_seen =
            (tte->descriptor_lowmem_first_word & STAGE87_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_lowmem_last_word & STAGE87_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_kernel_first_word & STAGE87_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_kernel_last_word & STAGE87_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_ram_console_word & STAGE87_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_gic_first_word & STAGE87_XNU_TTE_DESC_TYPE_MASK) |
            (tte->descriptor_gic_last_word & STAGE87_XNU_TTE_DESC_TYPE_MASK);
        tte->descriptor_attr_mask_seen =
            (tte->descriptor_lowmem_first_word & STAGE87_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_lowmem_last_word & STAGE87_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_kernel_first_word & STAGE87_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_kernel_last_word & STAGE87_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_ram_console_word & STAGE87_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_gic_first_word & STAGE87_XNU_TTE_DESC_ATTR_MASK) |
            (tte->descriptor_gic_last_word & STAGE87_XNU_TTE_DESC_ATTR_MASK);

        attrs_ok &= stage87_tte_descriptor_attrs_ok(tte->descriptor_lowmem_first_word,
                                                    STAGE87_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage87_tte_descriptor_attrs_ok(tte->descriptor_lowmem_last_word,
                                                    STAGE87_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage87_tte_descriptor_attrs_ok(tte->descriptor_kernel_first_word,
                                                    STAGE87_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage87_tte_descriptor_attrs_ok(tte->descriptor_kernel_last_word,
                                                    STAGE87_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage87_tte_descriptor_attrs_ok(tte->descriptor_ram_console_word,
                                                    STAGE87_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage87_tte_descriptor_attrs_ok(tte->descriptor_gic_first_word,
                                                    STAGE87_XNU_TTE_DESC_SECTION_SO);
        attrs_ok &= stage87_tte_descriptor_attrs_ok(tte->descriptor_gic_last_word,
                                                    STAGE87_XNU_TTE_DESC_SECTION_SO);

        if ((tte->descriptor_verify_mask & identity_desc_required) == identity_desc_required &&
            tte->descriptor_failure_mask == 0u) {
            tte->satisfied_mask |= STAGE87_TTE_SAT_DESCRIPTOR_READBACK;
        } else {
            tte->failure_mask |= STAGE87_TTE_FAIL_DESCRIPTOR_READBACK;
        }
        if (attrs_ok && tte->descriptor_attr_mask_seen == tte->descriptor_expected_attr_mask &&
            tte->descriptor_type_mask_seen == STAGE87_XNU_TTE_DESC_TYPE_SECTION) {
            tte->satisfied_mask |= STAGE87_TTE_SAT_DESCRIPTOR_ATTRS;
        } else {
            tte->failure_mask |= STAGE87_TTE_FAIL_DESCRIPTOR_ATTRS;
        }
        ...
        if (probe->load_phys_end > probe->load_phys_base) {
            ...
            if (stage87_tte_expect_translation(l1, tte->translation_kernel_text_va,
                                               tte->translation_kernel_text_va,
                                               &tte->translation_kernel_text_pa)) {
                tte->translation_check_mask |= STAGE87_TTE_TRANSLATE_KERNEL_FIRST;
                tte->translation_case_count++;
            } else {
                tte->translation_failure_mask |= STAGE87_TTE_TRANSLATE_KERNEL_FIRST;
            }

            tte->translation_kernel_last_va = kernel_last;
            if (stage87_tte_expect_translation(l1, tte->translation_kernel_last_va,
                                               tte->translation_kernel_last_va,
                                               &tte->translation_kernel_last_pa)) {
                tte->translation_check_mask |= STAGE87_TTE_TRANSLATE_KERNEL_LAST;
                tte->translation_case_count++;
            } else {
                tte->translation_failure_mask |= STAGE87_TTE_TRANSLATE_KERNEL_LAST;
            }
        }

        tte->translation_lowmem_va = RAM_PHYS_BASE;
        if (stage87_tte_expect_translation(l1, tte->translation_lowmem_va,
                                           tte->translation_lowmem_va,
                                           &tte->translation_lowmem_pa)) {
            tte->translation_check_mask |= STAGE87_TTE_TRANSLATE_LOW_RAM;
            tte->translation_case_count++;
        } else {
            tte->translation_failure_mask |= STAGE87_TTE_TRANSLATE_LOW_RAM;
        }

        tte->translation_ram_console_va = RAM_CONSOLE_BASE;
        if (stage87_tte_expect_translation(l1, tte->translation_ram_console_va,
                                           tte->translation_ram_console_va,
                                           &tte->translation_ram_console_pa)) {
            tte->translation_check_mask |= STAGE87_TTE_TRANSLATE_RAM_CONSOLE;
            tte->translation_case_count++;
        } else {
            tte->translation_failure_mask |= STAGE87_TTE_TRANSLATE_RAM_CONSOLE;
        }

        tte->translation_gic_va = 0xf9000000u;
        if (stage87_tte_expect_translation(l1, tte->translation_gic_va,
                                           tte->translation_gic_va,
                                           &tte->translation_gic_pa)) {
            tte->translation_check_mask |= STAGE87_TTE_TRANSLATE_GIC;
            tte->translation_case_count++;
        } else {
            tte->translation_failure_mask |= STAGE87_TTE_TRANSLATE_GIC;
        }

        if ((tte->translation_check_mask & (STAGE87_TTE_TRANSLATE_KERNEL_FIRST |
                                            STAGE87_TTE_TRANSLATE_KERNEL_LAST)) ==
            (STAGE87_TTE_TRANSLATE_KERNEL_FIRST | STAGE87_TTE_TRANSLATE_KERNEL_LAST)) {
            tte->satisfied_mask |= STAGE87_TTE_SAT_TRANSLATION_KERNEL;
        } else {
            tte->failure_mask |= STAGE87_TTE_FAIL_TRANSLATION_KERNEL;
        }
        if ((tte->translation_check_mask & STAGE87_TTE_TRANSLATE_LOW_RAM) != 0u) {
            tte->satisfied_mask |= STAGE87_TTE_SAT_TRANSLATION_LOW_RAM;
        } else {
            tte->failure_mask |= STAGE87_TTE_FAIL_TRANSLATION_LOW_RAM;
        }
        if ((tte->translation_check_mask & STAGE87_TTE_TRANSLATE_RAM_CONSOLE) != 0u) {
            tte->satisfied_mask |= STAGE87_TTE_SAT_TRANSLATION_CONSOLE;
        } else {
            tte->failure_mask |= STAGE87_TTE_FAIL_TRANSLATION_CONSOLE;
        }
        if ((tte->translation_check_mask & STAGE87_TTE_TRANSLATE_GIC) != 0u) {
            tte->satisfied_mask |= STAGE87_TTE_SAT_TRANSLATION_DEVICE;
        } else {
            tte->failure_mask |= STAGE87_TTE_FAIL_TRANSLATION_DEVICE;
        }
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_DESCRIPTOR_READBACK |
                             STAGE87_TTE_FAIL_DESCRIPTOR_ATTRS |
                             STAGE87_TTE_FAIL_TRANSLATION_KERNEL |
                             STAGE87_TTE_FAIL_TRANSLATION_LOW_RAM |
                             STAGE87_TTE_FAIL_TRANSLATION_CONSOLE |
                             STAGE87_TTE_FAIL_TRANSLATION_DEVICE;
    }
    ...
    if (tte->ttbr0_written == 0u && tte->ttbr1_written == 0u) {
        tte->satisfied_mask |= STAGE87_TTE_SAT_NO_TTBR_WRITE;
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_SAFETY;
    }
    if (tte->caches_enabled == 0u) {
        tte->satisfied_mask |= STAGE87_TTE_SAT_CACHES_UNCHANGED;
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_SAFETY;
    }
    if (stage87_tte_l2_and_scratch_zeroed()) {
        tte->local_sim_zeroed = 1u;
        tte->satisfied_mask |= STAGE87_TTE_SAT_LOCAL_SIM_ONLY;
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_SAFETY;
    }
    ...
    tte->safe_table.version = STAGE87_XNU_SAFE_TABLE_VERSION;
    tte->safe_table.size = sizeof(tte->safe_table);
    tte->safe_table.status = STAGE87_STATUS_BASE;
    tte->safe_table.local_base = safe_base;
    if (add_overflow_u32(safe_base, sizeof(stage87_safe_table_arena), &safe_limit)) {
        tte->safe_table.failure_mask |= STAGE87_TTE_FAIL_SAFE_TABLE;
    } else {
        tte->safe_table.local_limit = safe_limit;
    }
    tte->safe_table.identity_l1_base = safe_base;
    tte->safe_table.identity_l1_limit = safe_base + STAGE87_XNU_TTE_L1_SIZE;
    tte->safe_table.highva_l1_base = safe_base + STAGE87_XNU_TTE_L1_SIZE;
    tte->safe_table.highva_l1_limit = safe_base + (STAGE87_XNU_TTE_L1_SIZE * 2u);
    tte->safe_table.l1_entry_count = STAGE87_XNU_SAFE_TABLE_L1_COUNT;
    tte->safe_table.l1_bytes = STAGE87_XNU_SAFE_TABLE_L1_BYTES;
    ...
    if (tte->safe_table.local_base != 0u && tte->safe_table.local_limit > tte->safe_table.local_base &&
        (tte->safe_table.local_base & (STAGE87_XNU_SAFE_TABLE_ALIGNMENT - 1u)) == 0u &&
        tte->safe_table.identity_l1_limit == tte->safe_table.highva_l1_base &&
        tte->safe_table.highva_l1_limit == tte->safe_table.local_limit) {
        memcpy(safe_identity_l1, l1, STAGE87_XNU_TTE_L1_SIZE);
        tte->safe_table.kind_mask |= STAGE87_XNU_SAFE_TABLE_KIND_IDENTITY;
        tte->safe_table.materialized_mask |= STAGE87_XNU_SAFE_TABLE_MAT_IDENTITY;
        tte->safe_table.written_bytes += STAGE87_XNU_TTE_L1_SIZE;
    } else {
        tte->safe_table.failure_mask |= STAGE87_TTE_FAIL_SAFE_TABLE;
    }

    if (probe->load_vm_end > probe->load_vm_base && probe->load_phys_end > probe->load_phys_base) {
        tte->highva_virt_base = probe->load_vm_base & STAGE87_XNU_TTE_DESC_BASE_MASK;
        tte->highva_virt_end = probe->load_vm_end;
        tte->highva_phys_base = probe->load_phys_base & STAGE87_XNU_TTE_DESC_BASE_MASK;
        ...
        highva_first_expected_pa = stage87_tte_expected_section_pa(tte->highva_virt_base,
                                                                   tte->highva_phys_base);
        highva_last_expected_pa = stage87_tte_expected_section_pa(highva_last_va,
                                                                  stage87_tte_section_pa_for_va(highva_last_va,
                                                                                                tte->highva_virt_base,
                                                                                                tte->highva_phys_base));

        if (tte->highva_l1_first_index <= tte->highva_l1_last_index &&
            tte->highva_l1_last_index < STAGE87_XNU_TTE_L1_ENTRY_COUNT && highva_size != 0u) {
            stage87_tte_map_section_range_pa(safe_highva_l1, tte->highva_virt_base,
                                             tte->highva_phys_base, highva_size,
                                             STAGE87_XNU_TTE_DESC_SECTION_SO);
            if (stage87_tte_verify_section_slot_pa(safe_highva_l1, tte->highva_virt_base,
                                                   tte->highva_phys_base,
                                                   STAGE87_XNU_TTE_DESC_SECTION_SO,
                                                   &tte->descriptor_highva_first_word)) {
                tte->descriptor_verify_mask |= STAGE87_TTE_DESC_VERIFY_HIGHVA_FIRST;
            } else {
                tte->descriptor_failure_mask |= STAGE87_TTE_DESC_VERIFY_HIGHVA_FIRST;
            }
            if (stage87_tte_verify_section_slot_pa(safe_highva_l1, highva_last_va,
                                                   stage87_tte_section_pa_for_va(highva_last_va,
                                                                                 tte->highva_virt_base,
                                                                                 tte->highva_phys_base),
                                                   STAGE87_XNU_TTE_DESC_SECTION_SO,
                                                   &tte->descriptor_highva_last_word)) {
                tte->descriptor_verify_mask |= STAGE87_TTE_DESC_VERIFY_HIGHVA_LAST;
            } else {
                tte->descriptor_failure_mask |= STAGE87_TTE_DESC_VERIFY_HIGHVA_LAST;
            }

            tte->descriptor_type_mask_seen |=
                (tte->descriptor_highva_first_word & STAGE87_XNU_TTE_DESC_TYPE_MASK) |
                (tte->descriptor_highva_last_word & STAGE87_XNU_TTE_DESC_TYPE_MASK);
            tte->descriptor_attr_mask_seen |=
                (tte->descriptor_highva_first_word & STAGE87_XNU_TTE_DESC_ATTR_MASK) |
                (tte->descriptor_highva_last_word & STAGE87_XNU_TTE_DESC_ATTR_MASK);

            tte->translation_highva_first_va = tte->highva_virt_base;
            if (stage87_tte_expect_translation(safe_highva_l1, tte->translation_highva_first_va,
                                               highva_first_expected_pa,
                                               &tte->translation_highva_first_pa)) {
                tte->translation_check_mask |= STAGE87_TTE_TRANSLATE_HIGHVA_FIRST;
                tte->translation_case_count++;
            } else {
                tte->translation_failure_mask |= STAGE87_TTE_TRANSLATE_HIGHVA_FIRST;
            }
            tte->translation_highva_last_va = highva_last_va;
            if (stage87_tte_expect_translation(safe_highva_l1, tte->translation_highva_last_va,
                                               highva_last_expected_pa,
                                               &tte->translation_highva_last_pa)) {
                tte->translation_check_mask |= STAGE87_TTE_TRANSLATE_HIGHVA_LAST;
                tte->translation_case_count++;
            } else {
                tte->translation_failure_mask |= STAGE87_TTE_TRANSLATE_HIGHVA_LAST;
            }

            if ((tte->descriptor_verify_mask & (STAGE87_TTE_DESC_VERIFY_HIGHVA_FIRST |
                                                STAGE87_TTE_DESC_VERIFY_HIGHVA_LAST)) ==
                (STAGE87_TTE_DESC_VERIFY_HIGHVA_FIRST | STAGE87_TTE_DESC_VERIFY_HIGHVA_LAST) &&
                (tte->descriptor_failure_mask & (STAGE87_TTE_DESC_VERIFY_HIGHVA_FIRST |
                                                 STAGE87_TTE_DESC_VERIFY_HIGHVA_LAST)) == 0u) {
                tte->satisfied_mask |= STAGE87_TTE_SAT_HIGHVA_MAPPING;
                tte->safe_table.kind_mask |= STAGE87_XNU_SAFE_TABLE_KIND_HIGHVA;
                tte->safe_table.materialized_mask |= STAGE87_XNU_SAFE_TABLE_MAT_HIGHVA;
                tte->safe_table.written_bytes += STAGE87_XNU_TTE_L1_SIZE;
            } else {
                tte->failure_mask |= STAGE87_TTE_FAIL_HIGHVA_MAPPING;
            }
            if ((tte->translation_check_mask & (STAGE87_TTE_TRANSLATE_HIGHVA_FIRST |
                                                STAGE87_TTE_TRANSLATE_HIGHVA_LAST)) ==
                (STAGE87_TTE_TRANSLATE_HIGHVA_FIRST | STAGE87_TTE_TRANSLATE_HIGHVA_LAST) &&
                (tte->translation_failure_mask & (STAGE87_TTE_TRANSLATE_HIGHVA_FIRST |
                                                  STAGE87_TTE_TRANSLATE_HIGHVA_LAST)) == 0u) {
                tte->satisfied_mask |= STAGE87_TTE_SAT_HIGHVA_TRANSLATION;
            } else {
                tte->failure_mask |= STAGE87_TTE_FAIL_HIGHVA_TRANSLATION;
            }
        } else {
            tte->failure_mask |= STAGE87_TTE_FAIL_HIGHVA_MAPPING | STAGE87_TTE_FAIL_HIGHVA_TRANSLATION;
        }
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_HIGHVA_MAPPING | STAGE87_TTE_FAIL_HIGHVA_TRANSLATION;
    }

    if (tte->descriptor_verify_mask == STAGE87_TTE_DESC_VERIFY_REQUIRED &&
        tte->descriptor_failure_mask == 0u) {
        tte->satisfied_mask |= STAGE87_TTE_SAT_DESCRIPTOR_READBACK;
    } else {
        tte->failure_mask |= STAGE87_TTE_FAIL_DESCRIPTOR_READBACK;
    ...
    if (tte->safe_table.kind_mask == STAGE87_XNU_SAFE_TABLE_KIND_REQUIRED &&
        tte->safe_table.materialized_mask == STAGE87_XNU_SAFE_TABLE_MAT_REQUIRED &&
        tte->safe_table.written_bytes == (STAGE87_XNU_TTE_L1_SIZE * 2u) &&
        tte->safe_table.failure_mask == 0u) {
        tte->safe_table.status = STAGE87_STATUS_OK;
        tte->satisfied_mask |= STAGE87_TTE_SAT_SAFE_TABLE;
    } else {
        tte->safe_table.failure_mask |= STAGE87_TTE_FAIL_SAFE_TABLE;
        tte->failure_mask |= STAGE87_TTE_FAIL_SAFE_TABLE;
    }
    ...
    if (add_overflow_u32(embedded_base, stage87_embedded_macho_size, &embedded_limit) ||
        ...
        (probe->load_phys_end != 0u && ranges_overlap_u32(tte->safe_table.local_base, tte->safe_table.local_limit,
                                                          probe->load_phys_base, probe->load_phys_end))) {
        tte->safe_table.failure_mask |= STAGE87_TTE_FAIL_STAGE_OWNED_TABLES;
        tte->failure_mask |= STAGE87_TTE_FAIL_STAGE_OWNED_TABLES;
        ...
        tte->satisfied_mask |= STAGE87_TTE_SAT_STAGE_OWNED_TABLES;
    } else {
        tte->safe_table.failure_mask |= STAGE87_TTE_FAIL_STAGE_OWNED_TABLES;
        tte->failure_mask |= STAGE87_TTE_FAIL_STAGE_OWNED_TABLES;
    }

    tte->safe_table.checksum = stage87_safe_table_checksum(stage87_safe_table_arena,
                                                           sizeof(stage87_safe_table_arena));

    tte->checksum = stage87_tte_dryrun_checksum(tte);
    if (tte->checksum != stage87_tte_dryrun_checksum(tte)) {
        tte->failure_mask |= STAGE87_TTE_FAIL_CHECKSUM;
    }

    if (tte->failure_mask == 0u && tte->satisfied_mask == STAGE87_TTE_SAT_REQUIRED &&
        tte->checksum == stage87_tte_dryrun_checksum(tte)) {
        tte->status = STAGE87_STATUS_OK;
        xnu_log_puts("Stage84 XNU TTE dry-run ok\n");
        return 1;
    }

    tte->status = STAGE87_STATUS_FAIL(tte->failure_mask);
    xnu_log_puts("Stage84 XNU TTE dry-run failed\n");
    return 0;
}

static void stage87_xnu_tte_dryrun_log(const struct stage87_xnu_tte_dryrun *tte)
{
    ...
    xnu_log_kv32("xnu_highva_dryrun_status", ((tte->satisfied_mask & (STAGE87_TTE_SAT_HIGHVA_MAPPING | STAGE87_TTE_SAT_HIGHVA_TRANSLATION)) == (STAGE87_TTE_SAT_HIGHVA_MAPPING | STAGE87_TTE_SAT_HIGHVA_TRANSLATION)) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE);
...
static uint32_t stage87_loader_preflight_checksum(volatile const struct stage87_loader_preflight *preflight)
{
    volatile const uint32_t *words = (volatile const uint32_t *)(uintptr_t)preflight;
    uint32_t count = (uint32_t)(offsetof(struct stage87_loader_preflight, checksum) / sizeof(uint32_t));
    uint32_t checksum = 0;
...
static uint32_t stage87_dt_semantic_mask(struct boot_args *args)
{
    uint32_t mask = 0;
    uint32_t len = 0;
    ...
    const void *platform_personality = find_child(args, root, "stage87-platform-personality");
    const void *interrupt_personality = find_child(args, root, "stage87-interrupt-personality");
    const void *timer_personality = find_child(args, root, "stage87-timer-personality");
    const void *cpu_personality = find_child(args, root, "stage87-cpu-personality");
    const void *rejected_personality = find_child(args, root, "stage87-rejected-personality");

    if (root && args->deviceTreeLength && apple_dt_node_prop_count(root, args->deviceTreeLength, root) != 0u) {
        mask |= STAGE87_DT_READY_BINARY_SELFTEST;
    }
    if (chosen) {
        mask |= STAGE87_DT_READY_CHOSEN;
        if (apple_dt_get_prop(args->deviceTreeP, args->deviceTreeLength, chosen, "boot-args", &len) && len != 0u) {
            mask |= STAGE87_DT_READY_BOOT_ARGS;
        }
        if (apple_dt_get_prop(args->deviceTreeP, args->deviceTreeLength, chosen, "ram-console-reg", &len) && len >= 8u) {
            mask |= STAGE87_DT_READY_RAM_CONSOLE;
        }
    }
    if (memory && apple_dt_get_prop(args->deviceTreeP, args->deviceTreeLength, memory, "reg", &len) && len >= 8u) {
        mask |= STAGE87_DT_READY_MEMORY;
    }
    ...
    if (iokit_platform &&
        ...
        mask |= STAGE87_DT_READY_IOKIT_SCAFFOLD;
    }
    if (platform_driver &&
        ...
        mask |= STAGE87_DT_READY_PLATFORM_DRIVER;
    }
    if (interrupt_service && timer_service && cpu_service &&
        ...
        STAGE87_XNU_IOKIT_REGISTRY_SERVICE_EXPECTED_IRQ_COUNT_MIN &&
        ...
        mask |= STAGE87_DT_READY_IOKIT_REGISTRY_SERVICES;
    }
    if (rejected_driver &&
        ...
        mask |= STAGE87_DT_READY_IOKIT_REJECTED_DRIVER;
    }
    if (catalog_root && platform_personality && interrupt_personality && timer_personality && cpu_personality &&
        ...
        STAGE87_XNU_IOKIT_CATALOG_PROPERTY_CATALOG_VERSION &&
        ...
        mask |= STAGE87_DT_READY_IOKIT_CATALOG_PROPERTY;
    }
    if (platform_personality && interrupt_personality && timer_personality && cpu_personality && rejected_personality &&
        ...
        mask |= STAGE87_DT_READY_IOKIT_PROPERTY_INHERITANCE;
    }
    if (platform_personality && interrupt_personality && timer_personality && cpu_personality && rejected_personality &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
        ...
        prop_string_matches(args, interrupt_personality, "registry-entry-path",
                            "IODeviceTree:/stage87-interrupt-personality") &&
        prop_string_matches(args, timer_personality, "registry-entry-path",
                            "IODeviceTree:/stage87-timer-personality") &&
        prop_string_matches(args, cpu_personality, "registry-entry-path",
                            "IODeviceTree:/stage87-cpu-personality") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "registry-parent-ordinal", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "registry-parent-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "registry-parent-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "registry-parent-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "registry-sibling-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "registry-sibling-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "registry-sibling-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "registry-sibling-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        prop_string_matches(args, platform_personality, "registry-topology-provenance",
                            "stage87-property-inheritance") &&
        ...
        prop_string_matches(args, rejected_personality, "registry-entry-path",
                            "IODeviceTree:/stage87-rejected-personality") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "registry-parent-ordinal", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "registry-sibling-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        prop_string_matches(args, rejected_personality, "registry-topology-provenance",
                            "stage87-rejected-unlinked") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
        ...
        mask |= STAGE87_DT_READY_IOKIT_REGISTRY_TOPOLOGY;
    }
    ...
    if (platform_personality && interrupt_personality && timer_personality && cpu_personality && rejected_personality &&
        ...
        prop_string_matches(args, cpu_personality, "start-provider-path",
                            "IODeviceTree:/stage87-platform-personality") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "attach-provider-ordinal", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "start-provider-ordinal", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "attach-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "start-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "attach-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "start-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        ...
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "start-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "attach-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "attach-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "attach-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "attach-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "start-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "start-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "start-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "start-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
        ...
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "provider-start-required", 0u) == 1u &&
        prop_string_matches(args, platform_personality, "attach-start-provenance", "stage87-registry-topology") &&
        prop_string_matches(args, interrupt_personality, "attach-start-provenance", "stage87-registry-topology") &&
        prop_string_matches(args, timer_personality, "attach-start-provenance", "stage87-registry-topology") &&
        prop_string_matches(args, cpu_personality, "attach-start-provenance", "stage87-registry-topology") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
        ...
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "start-provider-ordinal", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "attach-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "start-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "provider-start-required", 1u) == 0u &&
        prop_string_matches(args, rejected_personality, "attach-start-provenance", "stage87-rejected-unattached")) {
        mask |= STAGE87_DT_READY_IOKIT_ATTACH_START_READINESS;
    }
    ...
    if (platform_personality && interrupt_personality && timer_personality && cpu_personality && rejected_personality &&
        ...
        prop_string_matches(args, cpu_personality, "register-provider-path",
                            "IODeviceTree:/stage87-platform-personality") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "register-provider-ordinal", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "register-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "register-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "register-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "register-service-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "register-service-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "register-service-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "register-service-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "notification-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "notification-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "notification-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "notification-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        prop_string_matches(args, platform_personality, "lifecycle-provenance", "stage87-attach-start-readiness") &&
        prop_string_matches(args, interrupt_personality, "lifecycle-provenance", "stage87-attach-start-readiness") &&
        prop_string_matches(args, timer_personality, "lifecycle-provenance", "stage87-attach-start-readiness") &&
        prop_string_matches(args, cpu_personality, "lifecycle-provenance", "stage87-attach-start-readiness") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
        ...
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "register-service-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "notification-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "register-dependency-ready", 1u) == 0u &&
        prop_string_matches(args, rejected_personality, "lifecycle-provenance", "stage87-rejected-unregistered")) {
        mask |= STAGE87_DT_READY_IOKIT_LIFECYCLE_REGISTER_SERVICE;
    }
    ...
    if (platform_personality && interrupt_personality && timer_personality && cpu_personality && rejected_personality &&
        ...
        prop_string_matches(args, cpu_personality, "interest-provider-path",
                            "IODeviceTree:/stage87-platform-personality") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "interest-provider-ordinal", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "interest-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "interest-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "interest-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "interest-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "interest-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "interest-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "interest-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "expected-notify-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "expected-notify-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "expected-notify-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "expected-notify-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "delivered-notify-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "delivered-notify-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "delivered-notify-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "delivered-notify-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "interest-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "interest-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "interest-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "interest-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "delivery-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "delivery-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "delivery-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "delivery-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        prop_string_matches(args, platform_personality, "notify-provenance", "stage87-lifecycle-regsvc") &&
        prop_string_matches(args, interrupt_personality, "notify-provenance", "stage87-lifecycle-regsvc") &&
        prop_string_matches(args, timer_personality, "notify-provenance", "stage87-lifecycle-regsvc") &&
        prop_string_matches(args, cpu_personality, "notify-provenance", "stage87-lifecycle-regsvc") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
        ...
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "interest-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "delivery-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "notify-dependency-ready", 1u) == 0u &&
        prop_string_matches(args, rejected_personality, "notify-provenance", "stage87-rejected-undelivered")) {
        mask |= STAGE87_DT_READY_IOKIT_PROVIDER_NOTIFICATION;
    }
    ...
    if (platform_personality && interrupt_personality && timer_personality && cpu_personality && rejected_personality &&
        ...
        prop_string_matches(args, cpu_personality, "callback-provider-path",
                            "IODeviceTree:/stage87-platform-personality") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "callback-provider-ordinal", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "callback-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "callback-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "callback-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "callback-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "callback-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "callback-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "callback-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "expected-callback-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "expected-callback-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "expected-callback-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "expected-callback-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "delivered-callback-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "delivered-callback-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "delivered-callback-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "delivered-callback-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "client-ack-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "client-ack-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "client-ack-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "client-ack-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "callback-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "callback-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "callback-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "callback-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
                              "client-notify-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality,
                              "client-notify-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality,
                              "client-notify-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "client-notify-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality,
        ...
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality,
                              "callback-dependency-ready", 0u) == 1u &&
        prop_string_matches(args, platform_personality, "callback-provenance", "stage87-provider-notify") &&
        prop_string_matches(args, interrupt_personality, "callback-provenance", "stage87-provider-notify") &&
        prop_string_matches(args, timer_personality, "callback-provenance", "stage87-provider-notify") &&
        prop_string_matches(args, cpu_personality, "callback-provenance", "stage87-provider-notify") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
        ...
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "callback-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "client-notify-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality,
                              "callback-dependency-ready", 1u) == 0u &&
        prop_string_matches(args, rejected_personality, "callback-provenance", "stage87-rejected-nocallback")) {
        mask |= STAGE87_DT_READY_IOKIT_PROVIDER_CALLBACK;
    }
    ...
    if (apple_dt_node_prop_count(args->deviceTreeP, args->deviceTreeLength, platform_personality) == 100u &&
        ...
        prop_string_matches(args, interrupt_personality, "open-provider-path", "IODeviceTree:/stage87-platform-personality") &&
        prop_string_matches(args, timer_personality, "open-provider-path", "IODeviceTree:/stage87-platform-personality") &&
        prop_string_matches(args, cpu_personality, "open-provider-path", "IODeviceTree:/stage87-platform-personality") &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality, "open-provider-ordinal", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality, "open-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality, "open-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality, "open-provider-ordinal", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality, "open-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality, "open-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality, "open-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality, "open-type-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality, "expected-open-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality, "expected-open-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality, "expected-open-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality, "expected-open-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality, "claimed-open-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality, "claimed-open-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality, "claimed-open-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality, "claimed-open-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality, "client-close-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_PLATFORM_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality, "client-close-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_INTERRUPT_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality, "client-close-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_TIMER_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality, "client-close-mask", 0u) == STAGE87_XNU_IOKIT_REGISTRY_SERVICE_CPU_BIT &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality, "open-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality, "open-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality, "open-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality, "open-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, platform_personality, "close-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_PLATFORM_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, interrupt_personality, "close-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_INTERRUPT_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, timer_personality, "close-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_TIMER_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, cpu_personality, "close-order", 0xffffffffu) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_CPU_ORDINAL &&
        ...
        prop_string_matches(args, platform_personality, "open-provenance", "stage87-provider-callback") &&
        prop_string_matches(args, interrupt_personality, "open-provenance", "stage87-provider-callback") &&
        prop_string_matches(args, timer_personality, "open-provenance", "stage87-provider-callback") &&
        prop_string_matches(args, cpu_personality, "open-provenance", "stage87-provider-callback") &&
        ...
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality, "open-provider-ordinal", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        ...
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality, "open-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality, "close-order", 0u) == STAGE87_XNU_IOKIT_REGISTRY_TOPOLOGY_REJECTED_ORDINAL &&
        apple_dt_get_u32_prop(args->deviceTreeP, args->deviceTreeLength, rejected_personality, "open-dependency-ready", 1u) == 0u &&
        prop_string_matches(args, rejected_personality, "open-provenance", "stage87-rejected-noopen")) {
        mask |= STAGE87_DT_READY_IOKIT_CLIENT_OPEN;
    }
...
{
    struct stage87_macho_probe_result probe;
    struct stage87_loader_preflight *preflight = &stage87_loader_preflight_block;
    const struct stage87_ttbr0_roundtrip *ttbr_rt = mmu_stage87_ttbr0_roundtrip_result();
    const struct stage87_xnu_workspace *xnu_ws;
    const struct stage87_xnu_compile_graph *xnu_graph;
    const struct stage87_xnu_object_subset *xnu_obj;
    const struct stage87_xnu_link *xnu_link;
    const struct stage87_xnu_pmap_bootstrap_contract *pmap_contract;
    const struct stage87_xnu_pmap_table_dryrun_contract *pmap_table_contract;
    const struct stage87_xnu_pmap_page_dryrun_contract *pmap_page_contract;
    const struct stage87_xnu_pmap_attr_dryrun_contract *pmap_attr_contract;
    const struct stage87_xnu_pmap_multiwindow_dryrun_contract *pmap_multiwindow_contract;
    const struct stage87_xnu_pmap_transition_dryrun_contract *pmap_transition_contract;
    const struct stage87_xnu_pexpert_hook_readiness_contract *pexpert_hook_contract;
    const struct stage87_xnu_iokit_platform_scaffold_contract *iokit_platform_contract;
    const struct stage87_xnu_iokit_match_dryrun_contract *iokit_match_contract;
    const struct stage87_xnu_iokit_registry_service_dryrun_contract *iokit_registry_contract;
    const struct stage87_xnu_iokit_provider_plane_dryrun_contract *iokit_provider_contract;
    const struct stage87_xnu_iokit_catalog_property_dryrun_contract *iokit_catalog_contract;
    const struct stage87_xnu_iokit_property_inheritance_dryrun_contract *iokit_property_inheritance_contract;
    const struct stage87_xnu_iokit_registry_topology_dryrun_contract *iokit_registry_topology_contract;
    const struct stage87_xnu_iokit_attach_start_readiness_dryrun_contract *iokit_attach_start_contract;
    const struct stage87_xnu_iokit_lifecycle_register_service_readiness_dryrun_contract *iokit_lifecycle_contract;
    const struct stage87_xnu_iokit_provider_notification_delivery_readiness_dryrun_contract *iokit_provider_notification_contract;
    const struct stage87_xnu_iokit_provider_callback_client_notification_readiness_dryrun_contract *iokit_provider_callback_contract;
    const struct stage87_xnu_iokit_client_open_provider_claim_close_readiness_dryrun_contract *iokit_client_open_contract;
    const struct stage87_xnu_entry_stub_result *xnu_entry_stub;
    const struct stage87_xnu_early_pmap_platform_init_result *xnu_early_init;
    const struct stage87_xnu_pe_init_platform_false_result *xnu_pe_init;
    const struct stage87_xnu_arm_init_post_pe_bootstrap_result *xnu_post_pe;
    const struct stage87_xnu_arm_vm_init_full_pmap_result *xnu_live_pmap;
    ...
    preflight->version = STAGE87_LOADER_PREFLIGHT_VERSION;
    preflight->size = sizeof(*preflight);
    preflight->xnu_baseline_tag = STAGE87_XNU_BASELINE_2050_22_13;
    preflight->xnu_baseline_commit = STAGE87_XNU_BASELINE_COMMIT_CC8A9B0C;
    preflight->xnu_master_version = STAGE87_XNU_BASELINE_MASTER_12_3_0;
    preflight->xnu_arm_reference = STAGE87_XNU_ARM_REFERENCE_4570_1_46;

    macho_ok = (uint32_t)stage87_macho_probe(stage87_embedded_macho, stage87_embedded_macho_size, &probe);
    if (macho_ok) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_MACHO;
    }
    if (macho_ok && stage87_macho_materialize_to_arena(stage87_embedded_macho, stage87_embedded_macho_size, &probe)) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_MATERIALIZED;
    }
    ...
    preflight->proposed_topOfKernelData = align_up_u32(loaded_end_for_workspace, STAGE87_XNU_TTE_ALIGNMENT);
    preflight->proposed_ttep_workspace_base = preflight->proposed_topOfKernelData;
    preflight->proposed_ttep_workspace_size = STAGE87_XNU_TTE_WORKSPACE_BYTES;
    ...
    preflight->proposed_workspace_alignment = preflight->proposed_topOfKernelData & (STAGE87_XNU_TTE_ALIGNMENT - 1u);
    ...
    preflight->apple_dt_semantic_mask = stage87_dt_semantic_mask(args);
    preflight->pexpert_gap_mask = STAGE87_PLATFORM_GAP_REQUIRED_RECORDED;
    preflight->platform_gap_mask = STAGE87_PLATFORM_GAP_REQUIRED_RECORDED;
    preflight->interrupt_ready_mask = STAGE87_LOADER_IRQ_READY_GIC_DIST |
        STAGE87_LOADER_IRQ_READY_GIC_CPU |
        STAGE87_LOADER_IRQ_READY_TIMEBASE;
    preflight->safety_mask = STAGE87_LOADER_SAFETY_NO_EXECUTE |
    ...
    if (ttbr_rt->status == STAGE87_STATUS_OK &&
        (ttbr_rt->satisfied_mask & STAGE87_TTBR_RT_SAT_STAGE_OWNED_TABLE) != 0u) {
        preflight->safety_mask |= STAGE87_LOADER_SAFETY_STAGE_OWNED_TTBR;
    }
    if (ttbr_rt->status == STAGE87_STATUS_OK &&
        (ttbr_rt->satisfied_mask & STAGE87_TTBR_RT_SAT_ORIGINAL_RESTORED) != 0u) {
        preflight->safety_mask |= STAGE87_LOADER_SAFETY_TTBR_RESTORED;
    }
    if (ttbr_rt->status == STAGE87_STATUS_OK && ttbr_rt->caches_changed == 0u &&
        (ttbr_rt->satisfied_mask & STAGE87_TTBR_RT_SAT_CACHE_BITS_PRESERVED) != 0u) {
        preflight->safety_mask |= STAGE87_LOADER_SAFETY_CACHES_UNCHANGED |
            STAGE87_LOADER_SAFETY_NO_CACHE_CHANGE;
    }

    if (preflight->xnu_baseline_tag == STAGE87_XNU_BASELINE_2050_22_13 &&
        preflight->xnu_baseline_commit == STAGE87_XNU_BASELINE_COMMIT_CC8A9B0C &&
        preflight->xnu_master_version == STAGE87_XNU_BASELINE_MASTER_12_3_0 &&
        preflight->xnu_arm_reference == STAGE87_XNU_ARM_REFERENCE_4570_1_46) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_BASELINE;
    }
    if (preflight->boot_args_rev_ver == 0x00020002u &&
        ...
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_BOOT_ARGS;
    }
    if (preflight->proposed_workspace_range_ok == 1u) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_WORKSPACE;
    }
    if (preflight->apple_dt_semantic_mask == STAGE87_DT_READY_REQUIRED) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_DT;
    }
    if (stage87_xnu_tte_dryrun_build(&probe, preflight, &preflight->tte_dryrun)) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_TTE_DRYRUN;
        if ((preflight->tte_dryrun.satisfied_mask & (STAGE87_TTE_SAT_DESCRIPTOR_READBACK |
                                                     STAGE87_TTE_SAT_DESCRIPTOR_ATTRS)) ==
            (STAGE87_TTE_SAT_DESCRIPTOR_READBACK | STAGE87_TTE_SAT_DESCRIPTOR_ATTRS)) {
            preflight->satisfied_mask |= STAGE87_LOADER_SAT_TTE_VERIFY;
        }
        if ((preflight->tte_dryrun.satisfied_mask & (STAGE87_TTE_SAT_TRANSLATION_KERNEL |
                                                     STAGE87_TTE_SAT_TRANSLATION_LOW_RAM |
                                                     STAGE87_TTE_SAT_TRANSLATION_CONSOLE |
                                                     STAGE87_TTE_SAT_TRANSLATION_DEVICE |
                                                     STAGE87_TTE_SAT_XNU_BOOT_POLICY)) ==
            (STAGE87_TTE_SAT_TRANSLATION_KERNEL |
             STAGE87_TTE_SAT_TRANSLATION_LOW_RAM |
             STAGE87_TTE_SAT_TRANSLATION_CONSOLE |
             STAGE87_TTE_SAT_TRANSLATION_DEVICE |
             STAGE87_TTE_SAT_XNU_BOOT_POLICY)) {
            preflight->satisfied_mask |= STAGE87_LOADER_SAT_VTOP_DRYRUN;
        }
        if ((preflight->tte_dryrun.satisfied_mask & (STAGE87_TTE_SAT_HIGHVA_MAPPING |
                                                     STAGE87_TTE_SAT_HIGHVA_TRANSLATION)) ==
            (STAGE87_TTE_SAT_HIGHVA_MAPPING | STAGE87_TTE_SAT_HIGHVA_TRANSLATION)) {
            preflight->satisfied_mask |= STAGE87_LOADER_SAT_HIGHVA_DRYRUN;
        }
        if ((preflight->tte_dryrun.satisfied_mask & STAGE87_TTE_SAT_SAFE_TABLE) != 0u) {
            preflight->satisfied_mask |= STAGE87_LOADER_SAT_SAFE_TABLE;
        }
        if ((preflight->tte_dryrun.satisfied_mask & STAGE87_TTE_SAT_STAGE_OWNED_TABLES) != 0u) {
            preflight->satisfied_mask |= STAGE87_LOADER_SAT_STAGE_OWNED_TABLES;
        }
    }
    ...
    preflight->tte_verify_status = ((preflight->satisfied_mask & STAGE87_LOADER_SAT_TTE_VERIFY) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;
    preflight->vtop_dryrun_status = ((preflight->satisfied_mask & STAGE87_LOADER_SAT_VTOP_DRYRUN) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;
    preflight->highva_dryrun_status = ((preflight->satisfied_mask & STAGE87_LOADER_SAT_HIGHVA_DRYRUN) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;
    preflight->safe_table_status = ((preflight->satisfied_mask & STAGE87_LOADER_SAT_SAFE_TABLE) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;
    preflight->stage_owned_tables_status = ((preflight->satisfied_mask & STAGE87_LOADER_SAT_STAGE_OWNED_TABLES) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;
    ...
    if (ttbr_rt->status == STAGE87_STATUS_OK) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_TTBR_ROUNDTRIP;
    }
    if (ttbr_rt->status == STAGE87_STATUS_OK &&
        (ttbr_rt->satisfied_mask & STAGE87_TTBR_RT_SAT_STAGE_OWNED_TABLE) != 0u) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_STAGE_OWNED_TTBR;
    }
    if (ttbr_rt->status == STAGE87_STATUS_OK &&
        (ttbr_rt->satisfied_mask & STAGE87_TTBR_RT_SAT_ORIGINAL_RESTORED) != 0u) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_TTBR_RESTORED;
    }
    if (ttbr_rt->status == STAGE87_STATUS_OK && ttbr_rt->caches_changed == 0u &&
        (ttbr_rt->satisfied_mask & STAGE87_TTBR_RT_SAT_CACHE_BITS_PRESERVED) != 0u) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_CACHE_PRESERVED;
    }
    preflight->stage_owned_ttbr_status = ((preflight->satisfied_mask & STAGE87_LOADER_SAT_STAGE_OWNED_TTBR) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;
    preflight->ttbr_restored_status = ((preflight->satisfied_mask & STAGE87_LOADER_SAT_TTBR_RESTORED) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;
    preflight->cache_preserved_status = ((preflight->satisfied_mask & STAGE87_LOADER_SAT_CACHE_PRESERVED) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_workspace_selftest()) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_WORKSPACE;
    }
    xnu_ws = stage87_xnu_workspace_result();
    ...
    if (xnu_ws->status == STAGE87_STATUS_OK && xnu_ws->cancro_target_declared == 1u &&
        xnu_ws->armv7_target_declared == 1u && xnu_ws->msm8974_target_declared == 1u) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_CANCRO_TARGET;
        preflight->cancro_target_status = STAGE87_STATUS_OK;
    } else {
        preflight->cancro_target_status = STAGE87_STATUS_BASE;
    }
    if (xnu_ws->status == STAGE87_STATUS_OK && xnu_ws->stage87_plan_ready == 1u &&
        xnu_ws->public_2050_arm_gap_recorded == 1u) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_STAGE87_PLAN;
        preflight->stage87_plan_status = STAGE87_STATUS_OK;
    } else {
        preflight->stage87_plan_status = STAGE87_STATUS_BASE;
    }

    if (stage87_xnu_compile_graph_selftest()) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_COMPILE_GRAPH;
    }
    xnu_graph = stage87_xnu_compile_graph_result();
    ...
    preflight->xnu_compile_graph_status_rollup =
        ((preflight->satisfied_mask & STAGE87_LOADER_SAT_XNU_COMPILE_GRAPH) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_object_subset_selftest()) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_OBJECT_SUBSET;
    }
    xnu_obj = stage87_xnu_object_subset_result();
    ...
    preflight->xnu_object_subset_status_rollup =
        ((preflight->satisfied_mask & STAGE87_LOADER_SAT_XNU_OBJECT_SUBSET) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_link_selftest()) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_LINK;
    }
    xnu_link = stage87_xnu_link_result();
    ...
    preflight->xnu_link_status_rollup =
        ((preflight->satisfied_mask & STAGE87_LOADER_SAT_XNU_LINK) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (preflight->safety_mask == STAGE87_LOADER_SAFETY_REQUIRED) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_SAFETY;
    }

    if (stage87_xnu_bootstrap_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_BOOTSTRAP_CONTRACT;
    }
    {
        const struct stage87_xnu_bootstrap_contract *contract = stage87_xnu_bootstrap_contract_result();
        ...
        preflight->xnu_bootstrap_contract_status_rollup =
            ((preflight->satisfied_mask & STAGE87_LOADER_SAT_XNU_BOOTSTRAP_CONTRACT) != 0u) ?
            STAGE87_STATUS_OK : STAGE87_STATUS_BASE;
    }

    if (stage87_xnu_pmap_bootstrap_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_PMAP_BOOTSTRAP_CONTRACT;
    }
    pmap_contract = stage87_xnu_pmap_bootstrap_contract_result();
    ...
    preflight->xnu_pmap_bootstrap_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE87_LOADER_SAT_XNU_PMAP_BOOTSTRAP_CONTRACT) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_pmap_table_dryrun_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_PMAP_TABLE_DRYRUN_CONTRACT;
    }
    pmap_table_contract = stage87_xnu_pmap_table_dryrun_contract_result();
    ...
    preflight->xnu_pmap_table_dryrun_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE87_LOADER_SAT_XNU_PMAP_TABLE_DRYRUN_CONTRACT) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_pmap_page_dryrun_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_PMAP_PAGE_DRYRUN_CONTRACT;
    }
    pmap_page_contract = stage87_xnu_pmap_page_dryrun_contract_result();
    ...
    preflight->xnu_pmap_page_dryrun_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE87_LOADER_SAT_XNU_PMAP_PAGE_DRYRUN_CONTRACT) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_pmap_attr_dryrun_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_PMAP_ATTR_DRYRUN_CONTRACT;
    }
    pmap_attr_contract = stage87_xnu_pmap_attr_dryrun_contract_result();
    ...
    preflight->xnu_pmap_attr_dryrun_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE87_LOADER_SAT_XNU_PMAP_ATTR_DRYRUN_CONTRACT) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_pmap_multiwindow_dryrun_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_PMAP_MULTIWINDOW_DRYRUN_CONTRACT;
    }
    pmap_multiwindow_contract = stage87_xnu_pmap_multiwindow_dryrun_contract_result();
    ...
    preflight->xnu_pmap_multiwindow_dryrun_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE87_LOADER_SAT_XNU_PMAP_MULTIWINDOW_DRYRUN_CONTRACT) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_pmap_transition_dryrun_contract_selftest(preflight)) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_XNU_PMAP_TRANSITION_DRYRUN_CONTRACT;
    }
    pmap_transition_contract = stage87_xnu_pmap_transition_dryrun_contract_result();
    ...
    preflight->xnu_pmap_transition_dryrun_contract_status_rollup =
        ((preflight->satisfied_mask & STAGE87_LOADER_SAT_XNU_PMAP_TRANSITION_DRYRUN_CONTRACT) != 0u) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_pexpert_hook_readiness_contract_selftest(preflight)) {
        preflight->pexpert_gap_mask = 0u;
        preflight->interrupt_ready_mask = STAGE87_LOADER_IRQ_READY_REQUIRED;
    }
    pexpert_hook_contract = stage87_xnu_pexpert_hook_readiness_contract_result();
    ...
    preflight->xnu_pexpert_hook_readiness_contract_status_rollup =
        (pexpert_hook_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_platform_scaffold_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_platform_contract = stage87_xnu_iokit_platform_scaffold_contract_result();
    ...
    preflight->xnu_iokit_platform_scaffold_contract_status_rollup =
        (iokit_platform_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_match_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_match_contract = stage87_xnu_iokit_match_dryrun_contract_result();
    ...
    preflight->xnu_iokit_match_dryrun_contract_status_rollup =
        (iokit_match_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_registry_service_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_registry_contract = stage87_xnu_iokit_registry_service_dryrun_contract_result();
    ...
    preflight->xnu_iokit_registry_service_dryrun_contract_status_rollup =
        (iokit_registry_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_provider_plane_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_provider_contract = stage87_xnu_iokit_provider_plane_dryrun_contract_result();
    ...
    preflight->xnu_iokit_provider_plane_dryrun_contract_status_rollup =
        (iokit_provider_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_catalog_property_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_catalog_contract = stage87_xnu_iokit_catalog_property_dryrun_contract_result();
    ...
    preflight->xnu_iokit_catalog_property_dryrun_contract_status_rollup =
        (iokit_catalog_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_property_inheritance_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_property_inheritance_contract = stage87_xnu_iokit_property_inheritance_dryrun_contract_result();
    ...
    preflight->xnu_iokit_property_inheritance_dryrun_contract_status_rollup =
        (iokit_property_inheritance_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_registry_topology_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_registry_topology_contract = stage87_xnu_iokit_registry_topology_dryrun_contract_result();
    ...
    preflight->xnu_iokit_registry_topology_dryrun_contract_status_rollup =
        (iokit_registry_topology_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_attach_start_readiness_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_attach_start_contract = stage87_xnu_iokit_attach_start_readiness_dryrun_contract_result();
    ...
    preflight->xnu_iokit_attach_start_readiness_dryrun_contract_status_rollup =
        (iokit_attach_start_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_lifecycle_register_service_readiness_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_lifecycle_contract = stage87_xnu_iokit_lifecycle_register_service_readiness_dryrun_contract_result();
    ...
    preflight->xnu_iokit_lifecycle_register_service_readiness_dryrun_contract_status_rollup =
        (iokit_lifecycle_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_provider_notification_delivery_readiness_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_provider_notification_contract =
        stage87_xnu_iokit_provider_notification_delivery_readiness_dryrun_contract_result();
    ...
    preflight->xnu_iokit_provider_notification_delivery_readiness_dryrun_contract_status_rollup =
        (iokit_provider_notification_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_provider_callback_client_notification_readiness_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_provider_callback_contract =
        stage87_xnu_iokit_provider_callback_client_notification_readiness_dryrun_contract_result();
    ...
    preflight->xnu_iokit_provider_callback_client_notification_readiness_dryrun_contract_status_rollup =
        (iokit_provider_callback_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (stage87_xnu_iokit_client_open_provider_claim_close_readiness_dryrun_contract_selftest(preflight)) {
        preflight->platform_gap_mask &= ~STAGE87_PLATFORM_GAP_IOKIT_STACK;
    }
    iokit_client_open_contract =
        stage87_xnu_iokit_client_open_provider_claim_close_readiness_dryrun_contract_result();
    ...
    preflight->xnu_iokit_client_open_provider_claim_close_readiness_dryrun_contract_status_rollup =
        (iokit_client_open_contract->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (iokit_client_open_contract->status == STAGE87_STATUS_OK) {
        (void)stage87_xnu_entry_stub_run(args);
    }
    xnu_entry_stub = stage87_xnu_entry_stub_result();
    ...
    preflight->xnu_entry_stub_status_rollup =
        (xnu_entry_stub->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    xnu_early_init = stage87_xnu_early_pmap_platform_init_result();
    ...
    preflight->xnu_early_pmap_platform_init_status_rollup =
        (xnu_early_init->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    xnu_pe_init = stage87_xnu_pe_init_platform_false_result();
    ...
    preflight->xnu_pe_init_platform_false_status_rollup =
        (xnu_pe_init->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    xnu_post_pe = stage87_xnu_arm_init_post_pe_bootstrap_result();
    ...
    preflight->xnu_arm_init_post_pe_bootstrap_status_rollup =
        (xnu_post_pe->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    xnu_live_pmap = stage87_xnu_arm_vm_init_full_pmap_result();
    ...
    preflight->xnu_arm_vm_init_full_pmap_status_rollup =
        (xnu_live_pmap->status == STAGE87_STATUS_OK) ? STAGE87_STATUS_OK : STAGE87_STATUS_BASE;

    if (preflight->platform_gap_mask == (STAGE87_PLATFORM_GAP_REQUIRED_RECORDED & ~STAGE87_PLATFORM_GAP_IOKIT_STACK) &&
        preflight->pexpert_gap_mask == 0u) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_PLATFORM_GAPS;
    }
    if (preflight->interrupt_ready_mask == STAGE87_LOADER_IRQ_READY_REQUIRED) {
        preflight->satisfied_mask |= STAGE87_LOADER_SAT_INTERRUPTS;
    }
    ...
    if (preflight->satisfied_mask != STAGE87_LOADER_SAT_REQUIRED) {
        preflight->failure_mask |= 0x80000000u;
    }
    preflight->checksum = stage87_loader_preflight_checksum(preflight);
    preflight->status = (preflight->satisfied_mask == STAGE87_LOADER_SAT_REQUIRED && preflight->failure_mask == 0u &&
        preflight->xnu_entry_stub_status_rollup == STAGE87_STATUS_OK &&
        preflight->xnu_early_pmap_platform_init_status_rollup == STAGE87_STATUS_OK &&
        preflight->xnu_pe_init_platform_false_status_rollup == STAGE87_STATUS_OK &&
        preflight->xnu_arm_init_post_pe_bootstrap_status_rollup == STAGE87_STATUS_OK &&
        preflight->xnu_arm_vm_init_full_pmap_status_rollup == STAGE87_STATUS_OK &&
        preflight->checksum == stage87_loader_preflight_checksum(preflight)) ?
        STAGE87_STATUS_OK : STAGE87_STATUS_FAIL(preflight->failure_mask);
    ...
    stage87_xnu_tte_dryrun_log(&preflight->tte_dryrun);
    ...
    xnu_log_kv32("loader_stage87_plan_status", preflight->stage87_plan_status);
    ...
    xnu_log_puts("Stage84 Stage-owned arm_vm_init live-pmap installation window enabled: loader calls stage87_xnu_start_stub with boot_args, the stub branches to stage87_arm_init_stub, the arm-init stub invokes stage87_xnu_early_pmap_platform_init_run, stage87_xnu_pe_init_platform_false_run, stage87_xnu_arm_init_post_pe_bootstrap_run, and stage87_xnu_arm_vm_init_full_pmap_run; the live-pmap window builds a Stage-owned candidate L1, writes TTBR0, invalidates TLBs, verifies high-alias/RAM-console/GIC translations, restores the original TTBR0, and returns with public XNU _start/arm_init/bootstrap/rtclock/arm_vm_init, public pmap/pexpert/IOKit objects, generated Mach-O fixture execution, cache policy change, and persistent writes still blocked\n");

    if (preflight->status == STAGE87_STATUS_OK) {
