...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage63.gicDistributorBase, PE_state_stage63.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage63_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage63 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage63_loader_preflight_run(args)) {
