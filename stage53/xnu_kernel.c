...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage53.gicDistributorBase, PE_state_stage53.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage53_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage53 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage53_loader_preflight_run(args)) {
