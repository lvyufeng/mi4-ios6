#!/usr/bin/env python3
"""Stage53 public-XNU compile graph scanner.

This is a host-only classifier for the Stage53 compile migration proof. It
records why a tiny public-XNU source set is allowed for compile/link proof and
why dependency-heavy ARM/reference sources remain excluded. It does not mutate
external checkouts and it does not execute or link public-XNU code.
"""
...
STATUS_OK = 0x53000001
STATUS_BASE = 0x53000000
...
CANDIDATES = [
    ...
    Candidate(
        "arm_pe_identify_machine_reference",
...
def main() -> int:
    ...
    out_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else root / "out" / "stage53"
    ...
    for c in CANDIDATES:
        ...
        if (c.eligibility in ("reference-only", "excluded-high-risk")) and (c.allow_compile or c.allow_link):
    ...
    if str(out_dir).endswith("/out/stage53") or "/out/stage53" in str(out_dir):
    ...
    forbidden_count = sum(1 for c in CANDIDATES if c.eligibility in ("reference-only", "excluded-high-risk"))
    ...
    status_lines = [
        "Stage53 public-XNU compile graph scan",
        "This is a host-only classifier; public-XNU code is not executed.",
        f"stage53_xnu_compile_graph_status=0x{status:08x}",
        f"stage53_xnu_compile_graph_required_mask=0x{REQUIRED_MASK:08x}",
        f"stage53_xnu_compile_graph_satisfied_mask=0x{satisfied:08x}",
        f"stage53_xnu_compile_graph_failure_mask=0x{failure:08x}",
        f"stage53_xnu_compile_graph_candidate_count=0x{candidate_count:08x}",
        f"stage53_xnu_compile_graph_allowed_compile_count=0x{allowed_compile_count:08x}",
        f"stage53_xnu_compile_graph_allowed_link_count=0x{allowed_link_count:08x}",
        f"stage53_xnu_compile_graph_forbidden_count=0x{forbidden_count:08x}",
        f"stage53_xnu_compile_graph_shim_required_count=0x{shim_required_count:08x}",
        f"stage53_xnu_compile_graph_max_risk_class=0x{max_risk:08x}",
        f"stage53_xnu_compile_graph_pe_gen_allowed=0x{pe_gen_allowed:08x}",
        f"stage53_xnu_compile_graph_4570_reference_only=0x{ref_only:08x}",
        ...
        f"stage53_xnu_compile_graph_no_external_mutation=0x{1 if external_clean else 0:08x}",
    ...
    header.write_text(f"""#ifndef MI4IOS6_STAGE53_XNU_COMPILE_GRAPH_GENERATED_H
#define MI4IOS6_STAGE53_XNU_COMPILE_GRAPH_GENERATED_H

#define STAGE53_XNU_GRAPH_HOST_STATUS                 0x{status:08x}u
#define STAGE53_XNU_GRAPH_HOST_REQUIRED_MASK          0x{REQUIRED_MASK:08x}u
#define STAGE53_XNU_GRAPH_HOST_SATISFIED_MASK         0x{satisfied:08x}u
#define STAGE53_XNU_GRAPH_HOST_FAILURE_MASK           0x{failure:08x}u
#define STAGE53_XNU_GRAPH_HOST_CANDIDATE_COUNT        0x{candidate_count:08x}u
#define STAGE53_XNU_GRAPH_HOST_ALLOWED_COMPILE_COUNT  0x{allowed_compile_count:08x}u
#define STAGE53_XNU_GRAPH_HOST_ALLOWED_LINK_COUNT     0x{allowed_link_count:08x}u
#define STAGE53_XNU_GRAPH_HOST_FORBIDDEN_COUNT        0x{forbidden_count:08x}u
#define STAGE53_XNU_GRAPH_HOST_SHIM_REQUIRED_COUNT    0x{shim_required_count:08x}u
#define STAGE53_XNU_GRAPH_HOST_MAX_RISK_CLASS         0x{max_risk:08x}u
#define STAGE53_XNU_GRAPH_HOST_DEVICE_TREE_CLASSIFIED {1 if (satisfied & BIT_DEVICE_TREE_CLASSIFIED) else 0}u
#define STAGE53_XNU_GRAPH_HOST_BOOTARGS_CLASSIFIED    {1 if (satisfied & BIT_BOOTARGS_CLASSIFIED) else 0}u
#define STAGE53_XNU_GRAPH_HOST_PE_GEN_CLASSIFIED      {1 if (satisfied & BIT_PE_GEN_CLASSIFIED) else 0}u
#define STAGE53_XNU_GRAPH_HOST_PE_GEN_ALLOWED         {pe_gen_allowed}u
#define STAGE53_XNU_GRAPH_HOST_FORBIDDEN_EXCLUDED     {1 if forbidden_excluded else 0}u
#define STAGE53_XNU_GRAPH_HOST_INCLUDE_DEPS_RECORDED  1u
#define STAGE53_XNU_GRAPH_HOST_SYMBOL_DEPS_RECORDED   1u
#define STAGE53_XNU_GRAPH_HOST_SHIM_NEEDS_RECORDED    1u
#define STAGE53_XNU_GRAPH_HOST_RISK_CLASSES_RECORDED  1u
#define STAGE53_XNU_GRAPH_HOST_BASELINE_COMMIT32      0x{BASELINE_COMMIT32:08x}u
#define STAGE53_XNU_GRAPH_HOST_MASTER_VERSION         0x{BASELINE_MASTER32:08x}u
#define STAGE53_XNU_GRAPH_HOST_PUBLIC_2050_BASELINE   {1 if baseline_ok else 0}u
#define STAGE53_XNU_GRAPH_HOST_4570_REFERENCE_ONLY    {ref_only}u
#define STAGE53_XNU_GRAPH_HOST_NO_FULL_XNU_BUILD      1u
#define STAGE53_XNU_GRAPH_HOST_NO_PUBLIC_XNU_EXEC     1u
#define STAGE53_XNU_GRAPH_HOST_NO_MACHO_EXEC          1u
#define STAGE53_XNU_GRAPH_HOST_NO_EXTERNAL_MUTATION   {1 if external_clean else 0}u
#define STAGE53_XNU_GRAPH_HOST_OUTPUTS_IGNORED        1u
#define STAGE53_XNU_GRAPH_HOST_FAIL_CLOSED            1u
