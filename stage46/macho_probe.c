...
#define STAGE46_PAGE_SIZE 4096u

struct stage46_macho_header_32 {
    uint32_t magic;
    uint32_t cputype;
    uint32_t cpusubtype;
    uint32_t filetype;
...
struct stage46_macho_load_command {
    uint32_t cmd;
    uint32_t cmdsize;
};

struct stage46_macho_segment_command_32 {
    uint32_t cmd;
    uint32_t cmdsize;
    char segname[16];
    uint32_t vmaddr;
...
struct stage46_macho_section_32 {
    char sectname[16];
    char segname[16];
    uint32_t addr;
    uint32_t size;
...
struct stage46_macho_symtab_command {
    uint32_t cmd;
    uint32_t cmdsize;
    uint32_t symoff;
    uint32_t nsyms;
...
static struct stage46_loader_preflight stage46_loader_preflight_block;
static uint8_t stage46_macho_staging_arena[STAGE46_MACHO_STAGING_ARENA_CAP]
    __attribute__((aligned(STAGE46_MACHO_STAGING_ARENA_ALIGN)));
static uint8_t stage46_tte_dryrun_arena[STAGE46_XNU_TTE_WORKSPACE_BYTES]
    __attribute__((aligned(STAGE46_XNU_TTE_ALIGNMENT)));
...
static uint32_t segment_mask_for_name(const char segname[16])
{
    if (fixed16_eq(segname, "__TEXT")) {
        return STAGE46_MACHO_SEG_TEXT;
    }
    if (fixed16_eq(segname, "__DATA")) {
        return STAGE46_MACHO_SEG_DATA;
    }
    if (fixed16_eq(segname, "__LINKEDIT")) {
        return STAGE46_MACHO_SEG_LINKEDIT;
    }
    if (fixed16_eq(segname, "__KLD")) {
        return STAGE46_MACHO_SEG_KLD;
    }
    if (fixed16_eq(segname, "__LAST")) {
        return STAGE46_MACHO_SEG_LAST;
    }
    if (fixed16_eq(segname, "__PRELINK_TEXT")) {
        return STAGE46_MACHO_SEG_PRELINK_TEXT;
    }
    if (fixed16_eq(segname, "__PRELINK_INFO")) {
        return STAGE46_MACHO_SEG_PRELINK_INFO;
    }
    if (fixed16_eq(segname, "__PRELINK_STATE")) {
        return STAGE46_MACHO_SEG_PRELINK_STATE;
    }
    if (fixed16_eq(segname, "__PRELINK")) {
        return STAGE46_MACHO_SEG_PRELINK;
    }

    return 0;
}

static void record_key_segment(struct stage46_macho_probe_result *result,
                               const struct stage46_macho_segment_command_32 *segment,
                               uint32_t seg_bit)
{
    if (seg_bit == STAGE46_MACHO_SEG_TEXT) {
    ...
    } else if (seg_bit == STAGE46_MACHO_SEG_DATA) {
    ...
    } else if (seg_bit == STAGE46_MACHO_SEG_LINKEDIT) {
    ...
    } else if (seg_bit == STAGE46_MACHO_SEG_PRELINK_TEXT) {
    ...
    } else if (seg_bit == STAGE46_MACHO_SEG_PRELINK_INFO) {
    ...
    } else if (seg_bit == STAGE46_MACHO_SEG_PRELINK_STATE) {
...
static void add_load_plan_entry(struct stage46_macho_probe_result *result,
                                const struct stage46_macho_segment_command_32 *segment,
                                uint32_t seg_bit)
{
    struct stage46_macho_load_plan *plan = &result->load_plan;
    struct stage46_macho_load_plan_entry *entry;

    if (segment->vmsize == 0u) {
        return;
    }
    if (plan->entry_count >= STAGE46_MACHO_LOAD_PLAN_MAX) {
        result->failure_mask |= STAGE46_MACHO_FAIL_LOAD_PLAN_FULL;
        return;
    }
    ...
    if (segment->filesize != 0u) {
        entry->flags |= STAGE46_MACHO_LOAD_PLAN_FILE_BACKED;
    }
    if ((seg_bit & STAGE46_MACHO_SEG_PRELINK_MASK) != 0u) {
        entry->flags |= STAGE46_MACHO_LOAD_PLAN_PRELINK;
    }
    if (segment->vmsize > segment->filesize) {
        entry->flags |= STAGE46_MACHO_LOAD_PLAN_ZERO_FILL;
    }
    plan->entry_count++;
}

static void record_section_match(struct stage46_macho_probe_result *result,
                                 const struct stage46_macho_section_32 *section)
{
    uint32_t section_bit = 0;

    if (fixed16_eq(section->segname, "__TEXT") && fixed16_eq(section->sectname, "__text")) {
        section_bit = STAGE46_MACHO_SECT_TEXT_TEXT;
        ...
        section_bit = STAGE46_MACHO_SECT_DATA_CONST;
        ...
        section_bit = STAGE46_MACHO_SECT_PRELINK_TEXT_TEXT;
        ...
        section_bit = STAGE46_MACHO_SECT_PRELINK_INFO_INFO;
                           ...
                           const struct stage46_macho_section_32 *section)
{
    ...
    if (add_overflow_u32(section->addr, section->size, &section_end) ||
        ...
        result->failure_mask |= STAGE46_MACHO_FAIL_SECTION_BOUNDS;
    }
...
static void record_segment(struct stage46_macho_probe_result *result,
                           const struct stage46_macho_segment_command_32 *segment)
{
    uint32_t vm_end = 0;
    uint32_t file_end = 0;
...
static void parse_segment_sections(struct stage46_macho_probe_result *result,
                                   const uint8_t *bytes,
                                   ...
                                   const struct stage46_macho_segment_command_32 *segment)
{
    ...
    if (mul_overflow_u32(segment->nsects, sizeof(struct stage46_macho_section_32), &section_bytes) ||
    ...
    for (uint32_t i = 0; i < segment->nsects; i++) {
        const struct stage46_macho_section_32 *section =
            (const struct stage46_macho_section_32 *)(const void *)(bytes + section_offset);
        record_section(result, segment, section);
        section_offset += sizeof(*section);
    }
}

static void finalize_load_plan(struct stage46_macho_probe_result *result)
{
    struct stage46_macho_load_plan *plan = &result->load_plan;
    ...
    plan->version = STAGE46_MACHO_LOAD_PLAN_VERSION;
    ...
    if (plan->entry_count == 0u || result->min_vmaddr == 0u || result->max_vmaddr <= result->min_vmaddr) {
        result->failure_mask |= STAGE46_MACHO_FAIL_LOAD_PLAN;
        return;
    }

    for (uint32_t i = 0; i < plan->entry_count; i++) {
        struct stage46_macho_load_plan_entry *entry = &plan->entries[i];
        uint32_t vm_delta;
        ...
        if (add_overflow_u32(RAM_PHYS_BASE, vm_delta, &entry->physaddr) ||
            align_up_checked_u32(entry->vmsize, STAGE46_PAGE_SIZE, &aligned_size) ||
            add_overflow_u32(entry->physaddr, aligned_size, &entry_end)) {
            result->failure_mask |= STAGE46_MACHO_FAIL_PHYS_RANGE;
            continue;
        }
    ...
    if ((result->failure_mask & (STAGE46_MACHO_FAIL_LOAD_PLAN | STAGE46_MACHO_FAIL_PHYS_RANGE |
                                 STAGE46_MACHO_FAIL_LOAD_PLAN_FULL)) == 0u) {
        plan->status = STAGE46_STATUS_OK;
        result->load_plan_status = STAGE46_STATUS_OK;
        result->validation_mask |= STAGE46_MACHO_VALID_LOAD_PLAN;
    } else {
        plan->status = STAGE46_STATUS_BASE | result->failure_mask;
...
static void log_load_plan_entry(const char *prefix_vmaddr, const char *prefix_vmsize,
                                ...
                                const struct stage46_macho_load_plan_entry *entry)
{
...
static void log_load_plan_entries(const struct stage46_macho_probe_result *result)
{
    const struct stage46_macho_load_plan *plan = &result->load_plan;
...
int stage46_macho_probe(const void *artifact, uint32_t artifact_size,
                        struct stage46_macho_probe_result *result)
{
    const uint8_t *bytes = (const uint8_t *)artifact;
    const struct stage46_macho_header_32 *header;
    ...
    result->version = STAGE46_MACHO_PROBE_VERSION;
    result->artifact_base = (uint32_t)(uintptr_t)artifact;
    result->artifact_size = artifact_size;
    result->segment_required_mask = STAGE46_MACHO_SEG_REQUIRED;
    result->section_required_mask = STAGE46_MACHO_SECT_REQUIRED;
    result->entry_kind_mask = STAGE46_MACHO_ENTRY_NOT_EXECUTED;
    result->status = STAGE46_STATUS_BASE;
    result->load_plan.version = STAGE46_MACHO_LOAD_PLAN_VERSION;

    xnu_log_puts("Stage46 Mach-O probe begin\n");
    xnu_log_kv32("macho_artifact_base", result->artifact_base);
    xnu_log_kv32("macho_artifact_size", result->artifact_size);

    if (!artifact) {
        result->failure_mask |= STAGE46_MACHO_FAIL_NULL;
        goto done;
    }
    if (artifact_size < sizeof(*header)) {
        result->failure_mask |= STAGE46_MACHO_FAIL_SIZE;
        goto done;
    }

    header = (const struct stage46_macho_header_32 *)artifact;
    ...
    if (header->magic == STAGE46_MACHO_MAGIC) {
        result->validation_mask |= STAGE46_MACHO_VALID_MAGIC | STAGE46_MACHO_VALID_ENDIAN;
    } else if (header->magic == STAGE46_MACHO_CIGAM) {
        result->validation_mask |= STAGE46_MACHO_VALID_MAGIC;
        result->failure_mask |= STAGE46_MACHO_FAIL_ENDIAN;
    } else {
        result->failure_mask |= STAGE46_MACHO_FAIL_MAGIC;
    }

    if (header->cputype == STAGE46_MACHO_CPU_TYPE_ARM) {
        result->validation_mask |= STAGE46_MACHO_VALID_CPU;
    } else {
        result->failure_mask |= STAGE46_MACHO_FAIL_CPU;
    }

    if (header->cpusubtype == STAGE46_MACHO_CPU_SUBTYPE_ARM_ALL ||
        header->cpusubtype == STAGE46_MACHO_CPU_SUBTYPE_ARM_V7) {
        result->validation_mask |= STAGE46_MACHO_VALID_SUBTYPE;
    } else {
        result->failure_mask |= STAGE46_MACHO_FAIL_SUBTYPE;
    }

    if (header->filetype == STAGE46_MACHO_FILETYPE_EXECUTE ||
        header->filetype == STAGE46_MACHO_FILETYPE_PRELOAD) {
        result->validation_mask |= STAGE46_MACHO_VALID_FILETYPE;
    } else {
        result->failure_mask |= STAGE46_MACHO_FAIL_FILETYPE;
    }

    if (add_overflow_u32(commands_start, header->sizeofcmds, &commands_end) ||
        commands_end > artifact_size || (header->sizeofcmds & 3u) != 0u) {
        result->failure_mask |= STAGE46_MACHO_FAIL_COMMAND_BOUNDS;
    } else {
        result->validation_mask |= STAGE46_MACHO_VALID_COMMAND_BOUNDS;
    ...
    for (uint32_t i = 0; i < header->ncmds && offset < commands_end; i++) {
        const struct stage46_macho_load_command *load_command;
        ...
        load_command = (const struct stage46_macho_load_command *)(const void *)(bytes + offset);
        if (load_command->cmdsize < sizeof(*load_command) || (load_command->cmdsize & 3u) != 0u ||
            ...
            result->failure_mask |= STAGE46_MACHO_FAIL_COMMAND_SIZE;
            break;
        }

        result->load_command_count_seen++;
        if (load_command->cmd == STAGE46_MACHO_LC_SEGMENT) {
            if (load_command->cmdsize < sizeof(struct stage46_macho_segment_command_32)) {
                result->failure_mask |= STAGE46_MACHO_FAIL_COMMAND_SIZE;
            } else {
                const struct stage46_macho_segment_command_32 *segment =
                    (const struct stage46_macho_segment_command_32 *)(const void *)(bytes + offset);
                result->load_command_seen_mask |= STAGE46_MACHO_CMD_SEGMENT;
                record_segment(result, segment);
                parse_segment_sections(result, bytes, offset, segment);
            }
        } else if (load_command->cmd == STAGE46_MACHO_LC_SYMTAB) {
            result->load_command_seen_mask |= STAGE46_MACHO_CMD_SYMTAB;
            if (load_command->cmdsize >= sizeof(struct stage46_macho_symtab_command)) {
                const struct stage46_macho_symtab_command *symtab =
                    (const struct stage46_macho_symtab_command *)(const void *)(bytes + offset);
                uint32_t str_end = 0;
                if (add_overflow_u32(symtab->stroff, symtab->strsize, &str_end) || str_end > artifact_size) {
                    result->failure_mask |= STAGE46_MACHO_FAIL_SEGMENT_EXTENT;
                }
            }
        } else if (load_command->cmd == STAGE46_MACHO_LC_UNIXTHREAD) {
            result->load_command_seen_mask |= STAGE46_MACHO_CMD_UNIXTHREAD;
            result->entry_kind_mask |= STAGE46_MACHO_ENTRY_UNIXTHREAD;
        ...
        } else if (load_command->cmd == STAGE46_MACHO_LC_MAIN) {
            result->load_command_seen_mask |= STAGE46_MACHO_CMD_MAIN;
            result->entry_kind_mask |= STAGE46_MACHO_ENTRY_MAIN;
        } else {
            result->load_command_seen_mask |= STAGE46_MACHO_CMD_UNKNOWN;
...
after_commands:
    if ((result->segment_seen_mask & STAGE46_MACHO_SEG_REQUIRED) == STAGE46_MACHO_SEG_REQUIRED) {
        result->validation_mask |= STAGE46_MACHO_VALID_REQUIRED_SEGMENTS;
    } else {
        result->failure_mask |= STAGE46_MACHO_FAIL_REQUIRED_SEGMENTS;
    }
    if ((result->section_seen_mask & STAGE46_MACHO_SECT_REQUIRED) == STAGE46_MACHO_SECT_REQUIRED) {
        result->validation_mask |= STAGE46_MACHO_VALID_REQUIRED_SECTIONS;
    } else {
        result->failure_mask |= STAGE46_MACHO_FAIL_REQUIRED_SECTIONS;
    }
    if ((result->failure_mask & STAGE46_MACHO_FAIL_SEGMENT_EXTENT) == 0u) {
        result->validation_mask |= STAGE46_MACHO_VALID_FILE_EXTENTS;
    }
    if ((result->failure_mask & STAGE46_MACHO_FAIL_SECTION_BOUNDS) == 0u) {
        result->validation_mask |= STAGE46_MACHO_VALID_SECTION_BOUNDS;
    }
    if ((result->prelink_segment_seen_mask | result->prelink_section_seen_mask) != 0u) {
        result->validation_mask |= STAGE46_MACHO_VALID_PRELINK_REPORT;
    }
    if ((result->entry_kind_mask & (STAGE46_MACHO_ENTRY_UNIXTHREAD | STAGE46_MACHO_ENTRY_MAIN)) != 0u) {
        result->validation_mask |= STAGE46_MACHO_VALID_ENTRY_NOT_EXECUTED;
    } else {
        result->failure_mask |= STAGE46_MACHO_FAIL_NO_ENTRY_METADATA;
    }
    ...
    {
        const uint32_t parser_required = STAGE46_MACHO_VALID_REQUIRED &
            ~(STAGE46_MACHO_VALID_STAGING |
              STAGE46_MACHO_VALID_MATERIALIZED_PARSE |
              STAGE46_MACHO_VALID_MARKERS |
              STAGE46_MACHO_VALID_ZERO_FILL);

        if ((result->validation_mask & parser_required) == parser_required &&
            result->failure_mask == 0u) {
            result->status = STAGE46_STATUS_OK;
            xnu_log_puts("Stage46 Mach-O probe ok; entry metadata observed but not executed\n");
            return 1;
        }
    }

    result->status = STAGE46_STATUS_BASE | result->failure_mask;
    xnu_log_puts("Stage46 Mach-O probe failed\n");
    return 0;
}
...
static void log_stage_entry(const char *copy_src_key, const char *copy_dst_key,
                            ...
                            const struct stage46_macho_staging_plan_entry *entry)
{
...
static void log_staging_entries(const struct stage46_macho_staging_plan *staging)
{
...
static uint32_t stage46_macho_getlastaddr_style(const struct stage46_macho_probe_result *probe)
{
    uint32_t last = 0;

    for (uint32_t i = 0; i < probe->load_plan.entry_count; i++) {
        uint32_t end = 0;
        const struct stage46_macho_load_plan_entry *entry = &probe->load_plan.entries[i];
...
static void stage46_macho_check_arena_aliases(struct stage46_macho_probe_result *result,
                                              uint32_t artifact_base,
                                              uint32_t artifact_size)
{
    struct stage46_macho_staging_plan *staging = &result->staging_plan;
    uint32_t artifact_end = 0;

    if (add_overflow_u32(artifact_base, artifact_size, &artifact_end)) {
        staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_ARENA_ALIAS;
        return;
    }
...
static void stage46_macho_verify_marker(struct stage46_macho_staging_plan *staging,
                                        uint32_t vmaddr,
                                        const char *prefix,
                                        uint32_t bit)
{
    ...
    if (vmaddr < staging->vm_base) {
        staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_MARKERS;
        return;
    }
    ...
    if (bytes_prefix_eq(stage46_macho_staging_arena + offset, prefix)) {
...
static void stage46_macho_verify_zero_fill(struct stage46_macho_staging_plan *staging)
{
    uint32_t required_zero_mask = 0;

    for (uint32_t i = 0; i < staging->entry_count; i++) {
        const struct stage46_macho_staging_plan_entry *entry = &staging->entries[i];
        uint32_t bit = 1u << i;
        ...
        if (entry->arena_offset + entry->copy_size + entry->zero_size > staging->arena_size) {
            staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_ZERO_FILL;
            entry_ok = 0u;
        } else {
            uint32_t zero_offset = entry->arena_offset + entry->copy_size;
            for (uint32_t j = 0; j < entry->zero_size; j++) {
                if (stage46_macho_staging_arena[zero_offset + j] != 0u) {
                    staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_ZERO_FILL;
                    entry_ok = 0u;
                    break;
                }
    ...
    if ((staging->zero_check_mask & required_zero_mask) == required_zero_mask) {
        staging->validation_mask |= STAGE46_MACHO_VALID_ZERO_FILL;
    } else {
        staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_ZERO_FILL;
    }
}

static void stage46_macho_verify_reparse(struct stage46_macho_probe_result *result,
                                         struct stage46_macho_staging_plan *staging)
{
    struct stage46_macho_probe_result materialized;
    uint32_t original_last = stage46_macho_getlastaddr_style(result);
    uint32_t materialized_last;

    if (!stage46_macho_probe(stage46_macho_staging_arena, staging->arena_size, &materialized)) {
        staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_REPARSE;
        return;
    }

    materialized_last = stage46_macho_getlastaddr_style(&materialized);
    ...
    if (materialized.magic == result->magic &&
        ...
        staging->validation_mask |= STAGE46_MACHO_VALID_MATERIALIZED_PARSE;
        xnu_log_puts("Stage46 materialized Mach-O reparse ok\n");
    } else {
        staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_REPARSE;
    }
}

static int stage46_macho_materialize_to_arena(const void *artifact,
                                              uint32_t artifact_size,
                                              struct stage46_macho_probe_result *result)
{
    const uint8_t *source = (const uint8_t *)artifact;
    struct stage46_macho_staging_plan *staging = &result->staging_plan;
    uint32_t arena_base = (uint32_t)(uintptr_t)stage46_macho_staging_arena;
    uint32_t arena_end = 0;
    uint32_t vm_span = 0;
    uint32_t arena_size = 0;

    xnu_log_puts("Stage46 proposed physical load plan is dry-run only; no physical writes performed\n");
    xnu_log_puts("Stage46 Mach-O materialization begin\n");
    memset(stage46_macho_staging_arena, 0, sizeof(stage46_macho_staging_arena));
    memset(staging, 0, sizeof(*staging));
    staging->version = STAGE46_MACHO_STAGING_PLAN_VERSION;
    staging->status = STAGE46_STATUS_BASE;
    staging->arena_base = arena_base;

    if (add_overflow_u32(arena_base, STAGE46_MACHO_STAGING_ARENA_CAP, &arena_end)) {
        staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_SPAN;
        goto finish;
    }
    ...
    if (align_up_checked_u32(vm_span, STAGE46_PAGE_SIZE, &arena_size) ||
        arena_size > STAGE46_MACHO_STAGING_ARENA_CAP) {
        staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_SPAN;
        goto finish;
    }
    ...
    stage46_macho_check_arena_aliases(result, (uint32_t)(uintptr_t)artifact, artifact_size);
    if (staging->failure_mask != 0u) {
        goto finish;
    }

    for (uint32_t i = 0; i < result->load_plan.entry_count; i++) {
        const struct stage46_macho_load_plan_entry *load = &result->load_plan.entries[i];
        struct stage46_macho_staging_plan_entry *entry;
        uint32_t src_end;
        ...
        if (staging->entry_count >= STAGE46_MACHO_STAGING_PLAN_MAX) {
            staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_PLAN_FULL;
            break;
        }
        if (load->vmaddr < staging->vm_base || load->filesize > load->vmsize) {
            staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_DST_BOUNDS;
            continue;
        }
        arena_offset = load->vmaddr - staging->vm_base;
        if (add_overflow_u32(load->fileoff, load->filesize, &src_end) || src_end > artifact_size) {
            staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_SRC_BOUNDS;
            continue;
        }
        ...
        if (load->filesize != 0u) {
            memcpy(stage46_macho_staging_arena + arena_offset, source + load->fileoff, load->filesize);
            staging->file_bytes += load->filesize;
        }
        if (zero_size != 0u) {
            memset(stage46_macho_staging_arena + arena_offset + load->filesize, 0, zero_size);
            staging->zero_bytes += zero_size;
        }
    }

    if (staging->entry_count != result->load_plan.entry_count) {
        staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_COPY;
    }

    if (staging->failure_mask == 0u) {
        staging->validation_mask |= STAGE46_MACHO_VALID_STAGING;
        stage46_macho_verify_reparse(result, staging);
        stage46_macho_verify_marker(staging, result->text_section_vmaddr,
                                    "ST46-TEXT", STAGE46_MACHO_MARKER_TEXT);
        stage46_macho_verify_marker(staging, result->data_const_vmaddr,
                                    "ST46-DATA", STAGE46_MACHO_MARKER_DATA);
        stage46_macho_verify_marker(staging, result->prelink_text_section_vmaddr,
                                    "ST46-PRELINK-TEXT", STAGE46_MACHO_MARKER_PRELINK_TEXT);
        if ((staging->marker_check_mask & STAGE46_MACHO_MARKER_REQUIRED) == STAGE46_MACHO_MARKER_REQUIRED) {
            staging->validation_mask |= STAGE46_MACHO_VALID_MARKERS;
        } else {
            staging->failure_mask |= STAGE46_MACHO_FAIL_STAGE_MARKERS;
        }
        stage46_macho_verify_zero_fill(staging);
    }
...
finish:
    if (staging->failure_mask == 0u &&
        (staging->validation_mask & (STAGE46_MACHO_VALID_STAGING |
                                     ...
                                     STAGE46_MACHO_VALID_MARKERS |
                                     STAGE46_MACHO_VALID_ZERO_FILL)) ==
        (STAGE46_MACHO_VALID_STAGING | STAGE46_MACHO_VALID_MATERIALIZED_PARSE |
         STAGE46_MACHO_VALID_MARKERS | STAGE46_MACHO_VALID_ZERO_FILL)) {
        staging->status = STAGE46_STATUS_OK;
    } else {
        staging->status = STAGE46_STATUS_BASE | staging->failure_mask;
    }
    ...
    return staging->status == STAGE46_STATUS_OK;
}

static uint32_t stage46_tte_dryrun_checksum(volatile const struct stage46_xnu_tte_dryrun *tte)
{
    volatile const uint32_t *words = (volatile const uint32_t *)(uintptr_t)tte;
    uint32_t count = (uint32_t)(offsetof(struct stage46_xnu_tte_dryrun, checksum) / sizeof(uint32_t));
    uint32_t checksum = 0;
...
static void stage46_tte_map_section(uint32_t *l1, uint32_t index, uint32_t desc_flags)
{
    if (index < STAGE46_XNU_TTE_L1_ENTRY_COUNT) {
        l1[index] = (index << 20) | desc_flags;
    }
}

static void stage46_tte_map_section_range(uint32_t *l1, uint32_t first, uint32_t last, uint32_t desc_flags)
{
    for (uint32_t i = first; i <= last && i < STAGE46_XNU_TTE_L1_ENTRY_COUNT; i++) {
        stage46_tte_map_section(l1, i, desc_flags);
        if (i == UINT32_MAX) {
            break;
        }
    }
...
static uint32_t stage46_tte_l2_and_scratch_zeroed(void)
{
    for (uint32_t i = STAGE46_XNU_TTE_L1_SIZE; i < STAGE46_XNU_TTE_WORKSPACE_BYTES; i++) {
        if (stage46_tte_dryrun_arena[i] != 0u) {
            return 0u;
        }
    }
    return 1u;
...
static int stage46_xnu_tte_dryrun_build(const struct stage46_macho_probe_result *probe,
                                        const struct stage46_loader_preflight *preflight,
                                        struct stage46_xnu_tte_dryrun *tte)
{
    ...
    uint32_t embedded_base = (uint32_t)(uintptr_t)stage46_embedded_macho;
    uint32_t embedded_limit = 0;
    uint32_t staging_base = (uint32_t)(uintptr_t)stage46_macho_staging_arena;
    uint32_t staging_limit = 0;
    uint32_t *l1 = (uint32_t *)(void *)stage46_tte_dryrun_arena;

    memset(tte, 0, sizeof(*tte));
    memset(stage46_tte_dryrun_arena, 0, sizeof(stage46_tte_dryrun_arena));

    tte->version = STAGE46_XNU_TTE_DRYRUN_VERSION;
    tte->size = sizeof(*tte);
    tte->status = STAGE46_STATUS_BASE;
    ...
    tte->workspace_size = STAGE46_XNU_TTE_WORKSPACE_BYTES;
    tte->workspace_alignment = tte->workspace_base & (STAGE46_XNU_TTE_L1_ALIGN - 1u);
    tte->local_arena_base = (uint32_t)(uintptr_t)stage46_tte_dryrun_arena;
    tte->local_arena_size = sizeof(stage46_tte_dryrun_arena);
    tte->l1_table_base = tte->workspace_base;
    tte->l1_table_size = STAGE46_XNU_TTE_L1_SIZE;
    tte->l1_entry_count = STAGE46_XNU_TTE_L1_ENTRY_COUNT;
    tte->l2_table_size = STAGE46_XNU_TTE_L2_SIZE;
    tte->kernel_section_descriptor = STAGE46_XNU_TTE_DESC_SECTION_SO;
    tte->ram_console_section_descriptor = STAGE46_XNU_TTE_DESC_SECTION_SO;
    tte->device_section_descriptor = STAGE46_XNU_TTE_DESC_SECTION_SO;

    if (add_overflow_u32(tte->workspace_base, tte->workspace_size, &workspace_limit)) {
        tte->failure_mask |= STAGE46_TTE_FAIL_WORKSPACE_RANGE;
    } else {
        tte->workspace_limit = workspace_limit;
    }
    if (add_overflow_u32(tte->local_arena_base, tte->local_arena_size, &local_limit)) {
        tte->failure_mask |= STAGE46_TTE_FAIL_LOCAL_ARENA_ALIAS;
    } else {
        tte->local_arena_limit = local_limit;
    }
    if (add_overflow_u32(tte->l1_table_base, tte->l1_table_size, &l1_limit)) {
        tte->failure_mask |= STAGE46_TTE_FAIL_LAYOUT_OVERFLOW;
    } else {
    ...
    expected_workspace_base = align_up_u32(probe->load_phys_end, STAGE46_XNU_TTE_L1_ALIGN);
    if (tte->workspace_limit > tte->workspace_base && tte->workspace_limit < RAM_CONSOLE_BASE &&
        ...
        tte->satisfied_mask |= STAGE46_TTE_SAT_WORKSPACE_RANGE;
    } else {
        tte->failure_mask |= STAGE46_TTE_FAIL_WORKSPACE_RANGE;
    }
    if (tte->workspace_alignment == 0u && (tte->l1_table_base & (STAGE46_XNU_TTE_L1_ALIGN - 1u)) == 0u &&
        (tte->l2_table_base & (STAGE46_XNU_TTE_L2_ALIGN - 1u)) == 0u) {
        tte->satisfied_mask |= STAGE46_TTE_SAT_WORKSPACE_ALIGNMENT;
    } else {
        tte->failure_mask |= STAGE46_TTE_FAIL_WORKSPACE_ALIGNMENT;
    }
    if (tte->l1_table_limit <= tte->workspace_limit && tte->l2_table_limit <= tte->workspace_limit &&
        ...
        tte->l1_table_size == STAGE46_XNU_TTE_L1_SIZE && tte->l2_table_size == STAGE46_XNU_TTE_L2_SIZE) {
        tte->satisfied_mask |= STAGE46_TTE_SAT_LAYOUT_FITS;
    } else {
        tte->failure_mask |= STAGE46_TTE_FAIL_LAYOUT_OVERFLOW;
    }
    ...
    if (probe->load_phys_end > probe->load_phys_base) {
        ...
        if (tte->kernel_l1_last_index < STAGE46_XNU_TTE_L1_ENTRY_COUNT) {
            tte->satisfied_mask |= STAGE46_TTE_SAT_KERNEL_MAPPING;
        } else {
            tte->failure_mask |= STAGE46_TTE_FAIL_KERNEL_MAPPING;
        }
    } else {
        tte->failure_mask |= STAGE46_TTE_FAIL_KERNEL_MAPPING;
    }
    ...
    if (tte->ram_console_l1_index < STAGE46_XNU_TTE_L1_ENTRY_COUNT) {
        tte->satisfied_mask |= STAGE46_TTE_SAT_RAM_CONSOLE_MAPPING;
    } else {
        tte->failure_mask |= STAGE46_TTE_FAIL_RAM_CONSOLE_MAPPING;
    }
    ...
    if (tte->gic_l1_first_index <= tte->gic_l1_last_index &&
        tte->gic_l1_last_index < STAGE46_XNU_TTE_L1_ENTRY_COUNT) {
        tte->satisfied_mask |= STAGE46_TTE_SAT_DEVICE_MAPPING;
    } else {
        tte->failure_mask |= STAGE46_TTE_FAIL_DEVICE_MAPPING;
    }
    ...
    if (!ranges_overlap_u32(probe->load_phys_base, probe->load_phys_end, tte->workspace_base, tte->workspace_limit) &&
        ...
        tte->satisfied_mask |= STAGE46_TTE_SAT_NO_OVERLAP;
    } else {
        tte->failure_mask |= STAGE46_TTE_FAIL_OVERLAP;
    }

    if (add_overflow_u32(embedded_base, stage46_embedded_macho_size, &embedded_limit) ||
        add_overflow_u32(staging_base, STAGE46_MACHO_STAGING_ARENA_CAP, &staging_limit) ||
    ...
    if ((tte->failure_mask & (STAGE46_TTE_FAIL_KERNEL_MAPPING | STAGE46_TTE_FAIL_DEVICE_MAPPING |
                              STAGE46_TTE_FAIL_RAM_CONSOLE_MAPPING | STAGE46_TTE_FAIL_LAYOUT_OVERFLOW)) == 0u) {
        stage46_tte_map_section_range(l1, tte->lowmem_l1_first_index, tte->lowmem_l1_last_index,
                                      STAGE46_XNU_TTE_DESC_SECTION_SO);
        stage46_tte_map_section_range(l1, tte->kernel_l1_first_index, tte->kernel_l1_last_index,
                                      STAGE46_XNU_TTE_DESC_SECTION_SO);
        stage46_tte_map_section(l1, tte->ram_console_l1_index, STAGE46_XNU_TTE_DESC_SECTION_SO);
        stage46_tte_map_section_range(l1, tte->gic_l1_first_index, tte->gic_l1_last_index,
                                      STAGE46_XNU_TTE_DESC_SECTION_SO);
    }
    ...
    if (tte->ttbr0_written == 0u && tte->ttbr1_written == 0u) {
        tte->satisfied_mask |= STAGE46_TTE_SAT_NO_TTBR_WRITE;
    } else {
        tte->failure_mask |= STAGE46_TTE_FAIL_SAFETY;
    }
    if (tte->caches_enabled == 0u) {
        tte->satisfied_mask |= STAGE46_TTE_SAT_CACHES_UNCHANGED;
    } else {
        tte->failure_mask |= STAGE46_TTE_FAIL_SAFETY;
    }
    if (stage46_tte_l2_and_scratch_zeroed()) {
        tte->local_sim_zeroed = 1u;
        tte->satisfied_mask |= STAGE46_TTE_SAT_LOCAL_SIM_ONLY;
    } else {
        tte->failure_mask |= STAGE46_TTE_FAIL_SAFETY;
    }

    tte->checksum = stage46_tte_dryrun_checksum(tte);
    if (tte->checksum != stage46_tte_dryrun_checksum(tte)) {
        tte->failure_mask |= STAGE46_TTE_FAIL_CHECKSUM;
    }

    if (tte->failure_mask == 0u && tte->satisfied_mask == STAGE46_TTE_SAT_REQUIRED &&
        tte->checksum == stage46_tte_dryrun_checksum(tte)) {
        tte->status = STAGE46_STATUS_OK;
        xnu_log_puts("Stage46 XNU TTE dry-run ok\n");
        return 1;
    }

    tte->status = STAGE46_STATUS_BASE | tte->failure_mask;
    xnu_log_puts("Stage46 XNU TTE dry-run failed\n");
    return 0;
}

static void stage46_xnu_tte_dryrun_log(const struct stage46_xnu_tte_dryrun *tte)
{
...
static uint32_t stage46_loader_preflight_checksum(volatile const struct stage46_loader_preflight *preflight)
{
    volatile const uint32_t *words = (volatile const uint32_t *)(uintptr_t)preflight;
    uint32_t count = (uint32_t)(offsetof(struct stage46_loader_preflight, checksum) / sizeof(uint32_t));
    uint32_t checksum = 0;
...
static uint32_t stage46_dt_semantic_mask(struct boot_args *args)
{
    uint32_t mask = 0;
    uint32_t len = 0;
...
int stage46_loader_preflight_run(struct boot_args *args)
{
    struct stage46_macho_probe_result probe;
    struct stage46_loader_preflight *preflight = &stage46_loader_preflight_block;
    uint32_t workspace_limit;
    uint32_t loaded_end_for_workspace;
    uint32_t macho_ok;

    xnu_log_puts("Stage46 Mach-O/XNU loader preflight begin\n");
    memset(preflight, 0, sizeof(*preflight));
    preflight->version = STAGE46_LOADER_PREFLIGHT_VERSION;
    preflight->size = sizeof(*preflight);
    preflight->xnu_baseline_tag = STAGE46_XNU_BASELINE_2050_22_13;
    preflight->xnu_baseline_commit = STAGE46_XNU_BASELINE_COMMIT_CC8A9B0C;
    preflight->xnu_master_version = STAGE46_XNU_BASELINE_MASTER_12_3_0;
    preflight->xnu_arm_reference = STAGE46_XNU_ARM_REFERENCE_4570_1_46;

    macho_ok = (uint32_t)stage46_macho_probe(stage46_embedded_macho, stage46_embedded_macho_size, &probe);
    if (macho_ok) {
        preflight->satisfied_mask |= STAGE46_LOADER_SAT_MACHO;
    }
    if (macho_ok && stage46_macho_materialize_to_arena(stage46_embedded_macho, stage46_embedded_macho_size, &probe)) {
        preflight->satisfied_mask |= STAGE46_LOADER_SAT_MATERIALIZED;
    }
    ...
    preflight->proposed_topOfKernelData = align_up_u32(loaded_end_for_workspace, STAGE46_XNU_TTE_ALIGNMENT);
    preflight->proposed_ttep_workspace_base = preflight->proposed_topOfKernelData;
    preflight->proposed_ttep_workspace_size = STAGE46_XNU_TTE_WORKSPACE_BYTES;
    ...
    preflight->proposed_workspace_alignment = preflight->proposed_topOfKernelData & (STAGE46_XNU_TTE_ALIGNMENT - 1u);
    ...
    preflight->apple_dt_semantic_mask = stage46_dt_semantic_mask(args);
    preflight->pexpert_gap_mask = STAGE46_PLATFORM_GAP_PEXPERT_IMPL;
    preflight->platform_gap_mask = STAGE46_PLATFORM_GAP_REQUIRED_RECORDED;
    preflight->interrupt_ready_mask = STAGE46_LOADER_IRQ_READY_REQUIRED;
    preflight->safety_mask = STAGE46_LOADER_SAFETY_REQUIRED;

    if (preflight->xnu_baseline_tag == STAGE46_XNU_BASELINE_2050_22_13 &&
        preflight->xnu_baseline_commit == STAGE46_XNU_BASELINE_COMMIT_CC8A9B0C &&
        preflight->xnu_master_version == STAGE46_XNU_BASELINE_MASTER_12_3_0 &&
        preflight->xnu_arm_reference == STAGE46_XNU_ARM_REFERENCE_4570_1_46) {
        preflight->satisfied_mask |= STAGE46_LOADER_SAT_BASELINE;
    }
    if (preflight->boot_args_rev_ver == 0x00020002u &&
        ...
        preflight->satisfied_mask |= STAGE46_LOADER_SAT_BOOT_ARGS;
    }
    if (preflight->proposed_workspace_range_ok == 1u) {
        preflight->satisfied_mask |= STAGE46_LOADER_SAT_WORKSPACE;
    }
    if (preflight->apple_dt_semantic_mask == STAGE46_DT_READY_REQUIRED) {
        preflight->satisfied_mask |= STAGE46_LOADER_SAT_DT;
    }
    if (preflight->platform_gap_mask == STAGE46_PLATFORM_GAP_REQUIRED_RECORDED) {
        preflight->satisfied_mask |= STAGE46_LOADER_SAT_PLATFORM_GAPS;
    }
    if (preflight->interrupt_ready_mask == STAGE46_LOADER_IRQ_READY_REQUIRED) {
        preflight->satisfied_mask |= STAGE46_LOADER_SAT_INTERRUPTS;
    }
    if (preflight->safety_mask == STAGE46_LOADER_SAFETY_REQUIRED) {
        preflight->satisfied_mask |= STAGE46_LOADER_SAT_SAFETY;
    }
    if (stage46_xnu_tte_dryrun_build(&probe, preflight, &preflight->tte_dryrun)) {
        preflight->satisfied_mask |= STAGE46_LOADER_SAT_TTE_DRYRUN;
    }
    ...
    if (preflight->satisfied_mask != STAGE46_LOADER_SAT_REQUIRED) {
        preflight->failure_mask |= 0x80000000u;
    }
    preflight->checksum = stage46_loader_preflight_checksum(preflight);
    preflight->status = (preflight->satisfied_mask == STAGE46_LOADER_SAT_REQUIRED && preflight->failure_mask == 0u &&
        preflight->checksum == stage46_loader_preflight_checksum(preflight)) ? STAGE46_STATUS_OK : (STAGE46_STATUS_BASE | preflight->failure_mask);
    ...
    stage46_xnu_tte_dryrun_log(&preflight->tte_dryrun);
    ...
    xnu_log_puts("Stage46 XNU handoff disabled: materialized only into local BSS arena; TTE workspace modeled only in local BSS; no Mach-O entry executed, no physical writes, no TTBR writes, caches unchanged\n");

    if (preflight->status == STAGE46_STATUS_OK) {
        xnu_log_puts("Stage46 Mach-O/XNU loader preflight ok\n");
        return 1;
    }

    xnu_log_puts("Stage46 Mach-O/XNU loader preflight failed\n");
    return 0;
}
