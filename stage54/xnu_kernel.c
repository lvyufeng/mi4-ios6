...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage54.gicDistributorBase, PE_state_stage54.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage54_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage54 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage54_loader_preflight_run(args)) {
