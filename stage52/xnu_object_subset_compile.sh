...
CC=${STAGE52_XNU_OBJ_CC:-arm-none-eabi-gcc}
OBJDUMP=${STAGE52_XNU_OBJ_OBJDUMP:-arm-none-eabi-objdump}
NM=${STAGE52_XNU_OBJ_NM:-arm-none-eabi-nm}
SIZE=${STAGE52_XNU_OBJ_SIZE:-arm-none-eabi-size}
...
printf 'Stage52 public-XNU minimal object subset compile\n' >> "$STATUS_TXT"
...
add_satisfied 0x00001000  # outputs under ignored out/stage52
...
printf 'stage52_xnu_object_subset_status=0x%08x\n' "$status" >> "$STATUS_TXT"
printf 'stage52_xnu_object_subset_required_mask=0x%08x\n' 0x00003fff >> "$STATUS_TXT"
printf 'stage52_xnu_object_subset_satisfied_mask=0x%08x\n' "$satisfied" >> "$STATUS_TXT"
printf 'stage52_xnu_object_subset_failure_mask=0x%08x\n' "$failure" >> "$STATUS_TXT"
printf 'stage52_xnu_object_source_mask=0x%08x\n' 0x00000003 >> "$STATUS_TXT"
printf 'stage52_xnu_object_shim_mask=0x%08x\n' 0x0000001f >> "$STATUS_TXT"
printf 'stage52_xnu_object_count=0x%08x\n' "$object_count" >> "$STATUS_TXT"
printf 'stage52_xnu_object_device_tree_bytes=0x%08x\n' "$device_tree_bytes" >> "$STATUS_TXT"
printf 'stage52_xnu_object_bootargs_bytes=0x%08x\n' "$bootargs_bytes" >> "$STATUS_TXT"
printf 'stage52_xnu_object_device_tree_sha32=%s\n' "$device_tree_sha32" >> "$STATUS_TXT"
printf 'stage52_xnu_object_bootargs_sha32=%s\n' "$bootargs_sha32" >> "$STATUS_TXT"
printf 'stage52_xnu_object_public_only=0x00000001\n' >> "$STATUS_TXT"
printf 'stage52_xnu_object_no_full_xnu_build=0x00000001\n' >> "$STATUS_TXT"
printf 'stage52_xnu_object_no_macho_link=0x00000001\n' >> "$STATUS_TXT"
printf 'stage52_xnu_object_no_public_xnu_exec=0x00000001\n' >> "$STATUS_TXT"
printf 'stage52_xnu_object_no_external_mutation=0x00000001\n' >> "$STATUS_TXT"
printf 'stage52_xnu_object_outputs_ignored=0x00000001\n' >> "$STATUS_TXT"
...
#ifndef MI4IOS6_STAGE52_XNU_OBJECT_SUBSET_GENERATED_H
#define MI4IOS6_STAGE52_XNU_OBJECT_SUBSET_GENERATED_H

#define STAGE52_XNU_OBJ_HOST_STATUS                 0x$(printf '%08x' "$status")u
#define STAGE52_XNU_OBJ_HOST_REQUIRED_MASK          0x00003fffu
#define STAGE52_XNU_OBJ_HOST_SATISFIED_MASK         0x$(printf '%08x' "$satisfied")u
#define STAGE52_XNU_OBJ_HOST_FAILURE_MASK           0x$(printf '%08x' "$failure")u
#define STAGE52_XNU_OBJ_HOST_SOURCE_MASK            0x00000003u
#define STAGE52_XNU_OBJ_HOST_SHIM_MASK              0x0000001fu
#define STAGE52_XNU_OBJ_HOST_OBJECT_COUNT           0x$(printf '%08x' "$object_count")u
#define STAGE52_XNU_OBJ_HOST_BASELINE_COMMIT32      0xcc8a9b0cu
#define STAGE52_XNU_OBJ_HOST_MASTER_VERSION_12_3_0  0x000c0300u
#define STAGE52_XNU_OBJ_HOST_DEVICE_TREE_BYTES      0x$(printf '%08x' "$device_tree_bytes")u
#define STAGE52_XNU_OBJ_HOST_BOOTARGS_BYTES         0x$(printf '%08x' "$bootargs_bytes")u
#define STAGE52_XNU_OBJ_HOST_DEVICE_TREE_SHA32      ${device_tree_sha32}u
#define STAGE52_XNU_OBJ_HOST_BOOTARGS_SHA32         ${bootargs_sha32}u
#define STAGE52_XNU_OBJ_HOST_PUBLIC_2050_BASELINE   1u
#define STAGE52_XNU_OBJ_HOST_PUBLIC_ONLY            1u
#define STAGE52_XNU_OBJ_HOST_NO_FULL_XNU_BUILD      1u
#define STAGE52_XNU_OBJ_HOST_NO_MACHO_LINK          1u
#define STAGE52_XNU_OBJ_HOST_NO_PUBLIC_XNU_EXEC     1u
#define STAGE52_XNU_OBJ_HOST_NO_EXTERNAL_MUTATION   1u
#define STAGE52_XNU_OBJ_HOST_OUTPUTS_IGNORED        1u
#define STAGE52_XNU_OBJ_HOST_FAIL_CLOSED            1u
