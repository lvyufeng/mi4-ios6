...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage52.gicDistributorBase, PE_state_stage52.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage52_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage52 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage52_loader_preflight_run(args)) {
