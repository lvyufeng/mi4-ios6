...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage80.gicDistributorBase, PE_state_stage80.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage80_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage80 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage80_loader_preflight_run(args)) {
