...
void build_boot_args(struct boot_args *args, void *dt, uint32_t dt_len)
{
    ...
    args->virtBase = 0;                  /* Stage80 boot args are still identity-based before MMU handoff. */
    args->physBase = STAGE80_BASE;
    args->memSize = RAM_CONSOLE_BASE - RAM_PHYS_BASE;
    args->topOfKernelData = (uint32_t)(uintptr_t)__stage80_image_end;

    args->machineType = MACHINE_TYPE_MSM8974;
    args->deviceTreeP = dt;
    args->deviceTreeLength = dt_len;

    const char cmd[] = "debug=0x144 serial=0x1 mi4ios6.stage=80 xnu-pe-init-false xnu-postpe cpu-topo bootcpu rtclock no-armvm prevm-pexpert dtinit-facts peid-machine no-pub-peinit no-pub-dtinit no-pub-peid no-pub-thread no-pub-cpuboot no-pub-rtclock";
