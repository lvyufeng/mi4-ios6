# Stage81 — Stage-owned `arm_vm_init` live-pmap installation window

Stage81 is the next Method-C step after the hardware-validated Stage80 post-`PE_init_platform(FALSE,args)` bootstrap/timebase boundary. It keeps the Stage-owned `_start` / `arm_init`-shaped path from Stage77-Stage80, then crosses the VM/pmap line that Stage80 deliberately stopped before: a Stage-owned `arm_vm_init`-shaped live-pmap installation window.

This is still not a public XNU boot. Stage81 does not enter public XNU `_start` or `arm_init`, does not execute public `thread_bootstrap`, `cpu_bootstrap`, `rtclock_early_init`, `kernel_early_bootstrap`, `cpu_init`, `processor_bootstrap`, `arm_vm_init`, or public pmap runtime, does not call public `PE_init_platform`, `DTInit`, or `pe_identify_machine`, does not execute public pexpert/IOKit runtime objects, does not execute the generated Mach-O fixture, does not change cache policy, and does not perform persistent writes. The new live behavior is bounded and Stage-owned: Stage81 builds a candidate 16 KiB L1 table, writes it to TTBR0, invalidates TLBs, verifies translation through the installed table, then restores the original TTBR0 and invalidates TLBs again before returning to the loader.
...
```c
uint32_t stage81_xnu_start_stub(struct boot_args *args,
                                struct stage81_xnu_entry_stub_result *result);
uint32_t stage81_arm_init_stub(struct boot_args *args,
                               struct stage81_xnu_entry_stub_result *result);
...
const struct stage81_xnu_early_pmap_platform_init_result *
stage81_xnu_early_pmap_platform_init_result(void);

int stage81_xnu_pe_init_platform_false_run(
    struct boot_args *args,
    struct stage81_xnu_entry_stub_result *entry_result);
const struct stage81_xnu_pe_init_platform_false_result *
stage81_xnu_pe_init_platform_false_result(void);
...
The Stage81 target path remains:
...
5. The wrapper records TTBR0/SCTLR and calls `stage81_xnu_start_stub(args, &g_stage81_xnu_entry_stub_result)`.
6. The assembly stub marks entry and performs a real branch-and-link to `stage81_arm_init_stub(args, result)`.
7. The C `arm_init`-shaped stub validates `boot_args`, invokes `stage81_xnu_early_pmap_platform_init_run(args, result)`, invokes `stage81_xnu_pe_init_platform_false_run(args, result)`, invokes `stage81_xnu_arm_init_post_pe_bootstrap_run(args, result)`, then invokes `stage81_xnu_arm_vm_init_live_pmap_run(args, result)`.
...
9. The loader copies the entry-stub, early-init, PE-init-platform-false, post-PE bootstrap, and live-pmap result ABIs into `struct stage81_loader_preflight` and requires all roll-ups to be OK.
...
The Stage81 entry-result prefix keeps the Stage77/Stage78/Stage79/Stage80 assembly offsets stable:
...
The Stage81-only summary fields appended before the entry checksum now include:
...
`stage81/xnu_arm_vm_init_live_pmap.c` allocates a dedicated 4096-entry, 16 KiB-aligned L1 table and populates only the sections needed to keep the Stage81 window alive and verifiable:

```text
0x00000000 -> 0x00000000 identity section for Stage81 low code/data/probe
...
Stage81 therefore does not add a new top-level loader satisfied bit. Instead the loader copies the full live-pmap result into `struct stage81_loader_preflight`, logs a dedicated status roll-up, ORs the live-pmap failure mask into the final loader failure mask, and requires:

```text
loader_xnu_entry_stub_status_rollup=0x81000001
loader_xnu_early_init_status_rollup=0x81000001
loader_xnu_pe_init_platform_false_status_rollup=0x81000001
loader_xnu_arm_init_post_pe_bootstrap_status_rollup=0x81000001
loader_xnu_arm_vm_init_live_pmap_status_rollup=0x81000001
```
...
Stage81 executes only Stage-owned entry/arm-init/early-init/PE-init-platform-false/post-PE bootstrap/live-pmap code. It now **does**:
...
Compact fixed `CommandLine[256]` Stage81 markers include:
...
No flash, erase, partition write, or persistent bootloader/storage change is part of Stage81 validation.
