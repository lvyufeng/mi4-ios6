...
int kernel_entry(struct boot_args *args)
{
    ...
    gic_readonly_snapshot(PE_state_stage81.gicDistributorBase, PE_state_stage81.gicCpuBase);
    gic_log_snapshot();
    ...
    if (!mmu_stage81_ttbr0_roundtrip_selftest()) {
        xnu_log_puts("kernel_entry bad: Stage81 TTBR0 roundtrip selftest\n");
        return 0;
    }

    if (!stage81_loader_preflight_run(args)) {
